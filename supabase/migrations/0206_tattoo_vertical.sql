-- ============================================================================
-- 0206 — Tattoo vertical (Phase 1 of 15 new lifestyle categories)
-- ----------------------------------------------------------------------------
-- Mirrors handyman_providers byte-for-byte at the column level, with two
-- semantic tweaks for the tattoo industry:
--   • specialties vocab = artistic styles (fineline, blackwork, etc.) not
--     trades (electrical, plumbing).
--   • theme_color default = '#0A0A0A' (black brand) — tattoo studios run
--     dark aesthetics. button_text_color (new column) = '#FACC15' yellow
--     ink-on-black for the CTA accent.
--
-- The pricing structure (hourly_rate_idr + day_rate_idr) is kept identical
-- so the shared marketplace / profile / dashboard machinery reuses without
-- per-vertical branches. Tattoo studios already price per-session-hour
-- (most common quoted unit) with a day-rate option for sleeves / large
-- pieces — fits the column shape exactly.
-- ============================================================================

create table if not exists public.tattoo_providers (
  ...
  -- src/lib/tattoo/types.ts. CHECK constrains to that vocab.
  specialties text[] not null default '{}'
    check (specialties <@ array[
      ...
      'piercing','mixed','other'
    ]::text[]),

  -- Cap at 3 specialties — same UX rule as handyman.
  constraint tattoo_providers_specialties_max3
    check (coalesce(array_length(specialties, 1), 0) between 1 and 3),

  -- Pricing — hour and/or day (day = 8 hours / full-day session).
  -- CHECK enforces at least one is set so every public card shows pricing.
  ...
  constraint tattoo_providers_at_least_one_price
    check (
      hourly_rate_idr is not null
      or day_rate_idr is not null
    ),

  -- "Own tools" stays as a generic boolean — for tattoo it means "I bring
  -- my own machine/needles to flash conventions / private studio visits".
  -- Most artists work from a fixed studio so this defaults true.
  ...
  -- Per-provider theme accent (mig 0087 equiv). Default black for tattoo.
  theme_color      text default '#0A0A0A'
                   check (theme_color is null or theme_color ~* '^#[0-9A-F]{6}$'),
  -- Tattoo-specific: ink colour for CTA labels / hero overlay accents.
  -- Defaults to yellow so on the default black brand the contrast is
  -- ink-on-skin / sign-painter feel. mig 0202 mirror for tattoo.
  button_text_color text default '#FACC15'
  ...
  constraint tp_mock_no_user check (
    (is_mock = false and user_id is not null)
    or (is_mock = true and user_id is null)
  ),
  constraint tattoo_providers_gallery_max check (
...
create index if not exists idx_tp_listing
  on public.tattoo_providers (status, availability, city)
  where status = 'active';
create index if not exists idx_tp_owner
  on public.tattoo_providers (user_id);
create index if not exists idx_tp_specialties
  on public.tattoo_providers using gin (specialties)
  where status = 'active';

-- updated_at touch trigger (mirrors handyman).
create or replace function public.touch_tattoo_providers()
returns trigger language plpgsql as $$
begin new.updated_at = now(); return new; end $$;
drop trigger if exists trg_touch_tattoo_providers on public.tattoo_providers;
create trigger trg_touch_tattoo_providers
  before update on public.tattoo_providers
  for each row execute function public.touch_tattoo_providers();

-- Hide-mock-on-real-signup trigger (mirrors handyman 0060).
create or replace function public.hide_one_mock_tattoo_provider()
returns trigger language plpgsql security definer as $$
declare victim_id uuid;
begin
  if new.is_mock = false and new.user_id is not null then
    select id into victim_id
      from public.tattoo_providers
     where is_mock = true and mock_hidden_at is null
     order by created_at asc
     limit 1;
    if victim_id is not null then
      update public.tattoo_providers set mock_hidden_at = now() where id = victim_id;
    end if;
  end if;
  return new;
end $$;
drop trigger if exists trg_hide_mock_on_real_tattoo_signup on public.tattoo_providers;
create trigger trg_hide_mock_on_real_tattoo_signup
  after insert on public.tattoo_providers
  for each row execute function public.hide_one_mock_tattoo_provider();

-- RLS — mirrors handyman policies, renamed tp_*.
alter table public.tattoo_providers enable row level security;
drop policy if exists tp_public_select on public.tattoo_providers;
create policy tp_public_select on public.tattoo_providers
  for select to anon, authenticated using (status = 'active');
drop policy if exists tp_owner_select on public.tattoo_providers;
create policy tp_owner_select on public.tattoo_providers
  for select to authenticated using (user_id = auth.uid());
drop policy if exists tp_owner_update on public.tattoo_providers;
create policy tp_owner_update on public.tattoo_providers
  for update to authenticated
  using (user_id = auth.uid()) with check (user_id = auth.uid());

-- Bookings table (mig 0090 equiv).
create table if not exists public.tattoo_bookings (
  id                 uuid primary key default gen_random_uuid(),
  tattoo_id          uuid not null references public.tattoo_providers(id) on delete cascade,
...
create index if not exists tattoo_bookings_by_provider_date
  on public.tattoo_bookings (tattoo_id, requested_date desc);
create index if not exists tattoo_bookings_pending
  on public.tattoo_bookings (tattoo_id, status)
  where status = 'pending';

create or replace function public.touch_tattoo_bookings_updated_at()
returns trigger language plpgsql as $$
begin new.updated_at = now(); return new; end $$;
drop trigger if exists trg_tattoo_bookings_touch on public.tattoo_bookings;
create trigger trg_tattoo_bookings_touch
  before update on public.tattoo_bookings
  for each row execute function public.touch_tattoo_bookings_updated_at();

alter table public.tattoo_bookings enable row level security;
drop policy if exists tattoo_bookings_owner_read on public.tattoo_bookings;
create policy tattoo_bookings_owner_read on public.tattoo_bookings
  for select using (
    exists (
      select 1 from public.tattoo_providers tp
      where tp.id = tattoo_bookings.tattoo_id
        and tp.user_id = auth.uid()
    )
  );
drop policy if exists tattoo_bookings_owner_update on public.tattoo_bookings;
create policy tattoo_bookings_owner_update on public.tattoo_bookings
  for update using (
    exists (
      select 1 from public.tattoo_providers tp
      where tp.id = tattoo_bookings.tattoo_id
...
-- ============================================================================
-- Wire 'tattoo' into the polymorphic constraints / triggers that the rest
-- of the platform relies on. Without these the profile-view tracker,
-- rating recomputer, and contact form would 500 on tattoo rows.
-- ============================================================================

-- reviews_provider_type_check (mig 0072) — allow 'tattoo'
...
-- provider_profile_views CHECK (mig 0072) — allow 'tattoo'
...
-- bump_provider_visitor_count (mig 0072) — add 'tattoo' case
...
-- _recompute_provider_rating (mig 0075) — add 'tattoo' case
...
-- contact_messages provider_type CHECK (mig 0137) — allow 'tattoo'
...
alter table public.contact_messages
  ...
  check (provider_type in (
    ...
    'tour_guide','bike_rental','property','tattoo'
  ));

-- contact_messages_own_select RLS — add 'tattoo' branch
...
-- payment_intents.product CHECK (mig 0068) — add tattoo_monthly + tattoo_yearly
...
-- extend_tattoo_on_payment trigger — mirrors the 5 verticals in mig 0068
create or replace function public.extend_tattoo_on_payment()
returns trigger
language plpgsql
security definer
set search_path = public, pg_temp
...
begin
  ...
  if new.product not in ('tattoo_monthly','tattoo_yearly') then
    return new;
  end if;

  v_plan := case when new.product = 'tattoo_yearly' then 'yearly' else 'monthly' end;
  ...
  update public.tattoo_providers
...
drop trigger if exists pi_extend_tattoo on public.payment_intents;
create trigger pi_extend_tattoo
  after update of status on public.payment_intents
  for each row execute function public.extend_tattoo_on_payment();

-- Seed ONE demo artist — Rama, Yogyakarta. Black-brand theme (#0A0A0A)
-- with yellow ink CTA. Stock Unsplash tattoo image (HEAD-verified
-- image/jpeg before commit).
insert into public.tattoo_providers (
...
values
  (true, null, 'demo-tattoo-rama', 'Rama', 7,
    'Tattoo artist Yogyakarta — fineline, blackwork, dan custom design. Studio steril, jarum sekali pakai, konsultasi gratis lewat WhatsApp.',
    array['fineline','blackwork','minimalist'], 350000, 2500000, true,
    'Yogyakarta', 'Yogya · Sleman · Bantul · walk-in studio', '+62000000501',
    'https://images.unsplash.com/photo-1565058379802-bbe93b2f703a?w=1200&auto=format&fit=crop',
    '#0A0A0A', '#FACC15',
