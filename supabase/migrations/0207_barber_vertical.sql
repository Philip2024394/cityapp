-- ============================================================================
-- 0207 — Barber vertical (Phase 2 of 15 new lifestyle categories)
-- ----------------------------------------------------------------------------
-- Mirrors handyman_providers / tattoo_providers byte-for-byte at the
-- column level, with three semantic tweaks for the barbershop industry:
--   • specialties vocab = chair-side services (classic_cut, fade, beard
--     trim, hot towel shave, kids cut, line-up) not trades or art styles.
--   • theme_color default = '#1E40AF' (classic barber-pole deep blue) —
--     mirrors the dominant signage tone of independent Indonesian shops.
--     button_text_color = '#FFFFFF' for the CTA accent so the cut-price
--     pills read clean white-on-blue, high contrast.
--   • pricing — hourly_rate_idr is reused as the "starting from per-cut
--     flat fee" (Rp 65k haircut) since the barber industry quotes per-
--     service, not per-hour. day_rate_idr becomes the optional combo
--     (cut + beard + hot towel). Keeping the column names matches the
--     shared marketplace renderer; only the dashboard label changes.
-- ============================================================================

create table if not exists public.barber_providers (
  ...
  -- src/lib/barber/types.ts. CHECK constrains to that vocab.
  ...
  -- Cap at 3 specialties — same UX rule as handyman / tattoo.
  constraint barber_providers_specialties_max3
    check (coalesce(array_length(specialties, 1), 0) between 1 and 3),

  -- Pricing — per-cut flat (hourly_rate_idr column kept for renderer
  -- parity; surfaces as "Per cut" on dashboard). Optional combo (day
  -- rate column). CHECK enforces at least one is set so every public
  -- card shows a starting-from number.
  ...
  constraint barber_providers_at_least_one_price
    check (
      hourly_rate_idr is not null
      or day_rate_idr is not null
    ),

  -- "Own tools" stays as a generic boolean — for barber it means "I bring
  -- my own clippers / razor / cape kit". Most barbers work from a fixed
  -- chair so this defaults true.
  ...
  -- Per-provider theme accent (mig 0087 equiv). Default classic
  -- barber-pole deep blue.
  theme_color      text default '#1E40AF'
                   check (theme_color is null or theme_color ~* '^#[0-9A-F]{6}$'),
  -- Barber-specific: CTA label colour. Defaults to white so on the
  -- default deep-blue brand the contrast is white-on-blue, high
  -- contrast for cut-price pills. mig 0202 mirror for barber.
  ...
  -- Use barberp_ prefix to avoid colliding with beautician's bp_*
  -- policy/constraint namespace (the beautician table uses bp_ for the
  -- same set of triggers/policies).
  constraint barberp_mock_no_user check (
    (is_mock = false and user_id is not null)
    or (is_mock = true and user_id is null)
  ),
  constraint barber_providers_gallery_max check (
...
create index if not exists idx_barberp_listing
  on public.barber_providers (status, availability, city)
  where status = 'active';
create index if not exists idx_barberp_owner
  on public.barber_providers (user_id);
create index if not exists idx_barberp_specialties
  on public.barber_providers using gin (specialties)
  where status = 'active';

-- updated_at touch trigger (mirrors handyman / tattoo).
create or replace function public.touch_barber_providers()
returns trigger language plpgsql as $$
begin new.updated_at = now(); return new; end $$;
drop trigger if exists trg_touch_barber_providers on public.barber_providers;
create trigger trg_touch_barber_providers
  before update on public.barber_providers
  for each row execute function public.touch_barber_providers();

-- Hide-mock-on-real-signup trigger (mirrors handyman 0060 / tattoo 0206).
create or replace function public.hide_one_mock_barber_provider()
returns trigger language plpgsql security definer as $$
declare victim_id uuid;
begin
  if new.is_mock = false and new.user_id is not null then
    select id into victim_id
      from public.barber_providers
     where is_mock = true and mock_hidden_at is null
     order by created_at asc
     limit 1;
    if victim_id is not null then
      update public.barber_providers set mock_hidden_at = now() where id = victim_id;
    end if;
  end if;
  return new;
end $$;
drop trigger if exists trg_hide_mock_on_real_barber_signup on public.barber_providers;
create trigger trg_hide_mock_on_real_barber_signup
  after insert on public.barber_providers
  for each row execute function public.hide_one_mock_barber_provider();

-- RLS — mirrors handyman / tattoo policies, renamed barberp_*.
alter table public.barber_providers enable row level security;
drop policy if exists barberp_public_select on public.barber_providers;
create policy barberp_public_select on public.barber_providers
  for select to anon, authenticated using (status = 'active');
drop policy if exists barberp_owner_select on public.barber_providers;
create policy barberp_owner_select on public.barber_providers
  for select to authenticated using (user_id = auth.uid());
drop policy if exists barberp_owner_update on public.barber_providers;
create policy barberp_owner_update on public.barber_providers
...
create table if not exists public.barber_bookings (
  id                 uuid primary key default gen_random_uuid(),
  barber_id          uuid not null references public.barber_providers(id) on delete cascade,
...
create index if not exists barber_bookings_by_provider_date
  on public.barber_bookings (barber_id, requested_date desc);
create index if not exists barber_bookings_pending
  on public.barber_bookings (barber_id, status)
  where status = 'pending';

create or replace function public.touch_barber_bookings_updated_at()
returns trigger language plpgsql as $$
begin new.updated_at = now(); return new; end $$;
drop trigger if exists trg_barber_bookings_touch on public.barber_bookings;
create trigger trg_barber_bookings_touch
  before update on public.barber_bookings
  for each row execute function public.touch_barber_bookings_updated_at();

alter table public.barber_bookings enable row level security;
drop policy if exists barber_bookings_owner_read on public.barber_bookings;
create policy barber_bookings_owner_read on public.barber_bookings
  for select using (
    exists (
      select 1 from public.barber_providers bp
      where bp.id = barber_bookings.barber_id
        and bp.user_id = auth.uid()
    )
  );
drop policy if exists barber_bookings_owner_update on public.barber_bookings;
create policy barber_bookings_owner_update on public.barber_bookings
  for update using (
    exists (
      select 1 from public.barber_providers bp
      where bp.id = barber_bookings.barber_id
...
-- ============================================================================
-- Wire 'barber' into the polymorphic constraints / triggers that the rest
-- of the platform relies on. Without these the profile-view tracker,
-- rating recomputer, and contact form would 500 on barber rows.
-- ============================================================================

-- reviews_provider_type_check (mig 0072 / 0206) — allow 'barber'
...
-- provider_profile_views CHECK (mig 0072 / 0206) — allow 'barber'
...
-- bump_provider_visitor_count (mig 0072 / 0206) — add 'barber' case
...
-- _recompute_provider_rating (mig 0075 / 0206) — add 'barber' case
...
-- contact_messages provider_type CHECK (mig 0137 / 0206) — allow 'barber'
...
alter table public.contact_messages
  ...
  check (provider_type in (
    ...
    'tour_guide','bike_rental','property','tattoo','barber'
  ));

-- contact_messages_own_select RLS — add 'barber' branch
...
-- payment_intents.product CHECK (mig 0068 / 0206) — add barber_monthly + barber_yearly
...
-- extend_barber_on_payment trigger — mirrors the verticals in mig 0068 / 0206
create or replace function public.extend_barber_on_payment()
returns trigger
language plpgsql
security definer
set search_path = public, pg_temp
...
begin
  ...
  if new.product not in ('barber_monthly','barber_yearly') then
    return new;
  end if;

  v_plan := case when new.product = 'barber_yearly' then 'yearly' else 'monthly' end;
  ...
  update public.barber_providers
...
drop trigger if exists pi_extend_barber on public.payment_intents;
create trigger pi_extend_barber
  after update of status on public.payment_intents
  for each row execute function public.extend_barber_on_payment();

-- Seed ONE demo barber — Bagas, Yogyakarta. Classic barber-pole deep
-- blue theme (#1E40AF) with white CTA. Stock Unsplash barbershop image
-- (HEAD-verified image/jpeg before commit). Idempotent: on conflict
-- (slug) do update keeps the demo row in sync with the schema.
insert into public.barber_providers (
...
values
  (true, null, 'demo-barber-bagas', 'Bagas', 6,
    'Barbershop Yogyakarta — classic cut, fade, beard trim, hot towel shave. Chair bersih, handuk panas, walk-in welcome. Booking lewat WhatsApp.',
    array['classic_cut','fade','beard_trim'], 65000, 150000, true,
    'Yogyakarta', 'Yogya · Sleman · walk-in barbershop', '+62000000502',
