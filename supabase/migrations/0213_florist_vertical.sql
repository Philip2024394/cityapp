-- ============================================================================
-- 0213 — Florist vertical (Phase 7 of 15 new lifestyle categories)
-- ----------------------------------------------------------------------------
-- Mirrors cake_providers byte-for-byte at the column level (0212), with
-- four semantic tweaks for the independent Indonesian florist /
-- flower-shop industry:
--   • specialties vocab = arrangement category (hand_bouquet,
--     vase_arrangement, standing_flower, box_arrangement, corsage,
--     sympathy, graduation, wedding, anniversary, valentine,
--     premium_import, mixed, other) — Indonesian indie florists
--     cluster around the hand-bouquet wisuda gift, standing flower
--     for grand openings + funerals, box arrangement (anniversary /
--     surprise), wedding bridal sets, valentine premium, and the
--     premium_import tag for high-ticket rose / lily / hydrangea /
--     tulip jobs. Buyers send a screenshot ("I want one like this")
--     and pick by photo — so the genre vocab here mirrors the photo
--     categories florist IGs are organised around.
--   • theme_color default = '#16A34A' (botanical green) — florist
--     aesthetic. Indonesian florist signage trends garden green (vs
--     cake's bakery pink), reads as fresh / organic / leaf-and-stem
--     and contrasts well with the photo-led bouquet imagery that
--     dominates the IG feeds buyers browse. button_text_color =
--     '#FFFFFF' so the CTA pills read clean white-on-green, high
--     contrast.
--   • pricing — hourly_rate_idr is reused as the per-ARRANGEMENT
--     starter price (per-UNIT, not per-pax), e.g. Rp 150k for a hand
--     bouquet small, Rp 280k medium, Rp 450k large, Rp 850k premium.
--     Standing flower for openings / funerals can hit Rp 2-5jt.
--     Distinct from cake's per-cake because florist arrangements are
--     even MORE "by photo" — clients quote a screenshot then pick a
--     size tier (small / medium / large / premium). day_rate_idr
--     becomes the optional premium / standing-flower bundle (e.g.
--     Standing Flower Grand Opening Rp 1.8jt, Wedding bridal package
--     from Rp 5jt). Keeping the column names matches the shared
--     marketplace renderer; only the dashboard + profile label
--     changes ("Per-arrangement from" / "Premium bundle"). Pricing
--     FLOOR is Rp 150k (hand bouquet small), higher than cake's
--     Rp 95k because even the cheapest florist bouquet uses imported
--     stems + wrap that don't sell below Rp 150k in indie shops.
--   • has_own_tools boolean repurposed as "Delivery dalam kota same-
--     day" (free in-city drop-off before 16:00 cutoff vs pickup-only
--     studio). Defaults true since Indonesian indie florists almost
--     always deliver finished arrangements by ojek / car (standing
--     flower / sympathy especially needs to arrive fresh on the
--     day of the event).
-- ============================================================================

create table if not exists public.florist_providers (
  ...
  -- src/lib/florist/types.ts. CHECK constrains to that vocab.
  specialties text[] not null default '{}'
    check (specialties <@ array[
      ...
      'premium_import','mixed','other'
    ]::text[]),

  -- Cap at 3 specialties — same UX rule as cake / catering / photo / barber / handyman / tattoo / video.
  constraint florist_providers_specialties_max3
    check (coalesce(array_length(specialties, 1), 0) between 1 and 3),

  -- Pricing — per-arrangement starter (hourly_rate_idr column kept for
  -- renderer parity; surfaces as "Per-arrangement from" on dashboard).
  -- Optional premium bundle (day_rate_idr column). CHECK enforces at
  ...
  constraint florist_providers_at_least_one_price
    check (
      hourly_rate_idr is not null
      or day_rate_idr is not null
    ),

  -- "Own tools" stays as a generic boolean — for florist it means
  -- "Delivery dalam kota same-day" (free in-city drop-off before
  -- 16:00 cutoff vs pickup-only studio). Defaults true since
  -- Indonesian indie florists almost always deliver finished
  -- arrangements by ojek / car (standing flower / sympathy
  -- especially needs to arrive fresh on event day).
  ...
  -- Per-provider theme accent (mig 0087 equiv). Default botanical green
  -- so the leaf-and-stem florist imagery pops with shop-tuned signage.
  theme_color      text default '#16A34A'
                   check (theme_color is null or theme_color ~* '^#[0-9A-F]{6}$'),
  -- Florist-specific: CTA label colour. Defaults to white so on the
  -- default green brand the contrast is white-on-green, high contrast
  -- for the CTA pills. mig 0202 mirror for florist.
  ...
  -- Use floristp_ prefix to avoid colliding with cake's cakep_*
  -- policy/constraint namespace (and catering's caterp_* / photo's
  -- photop_* / barber's barberp_* / handyman's hp_* / tattoo's tp_* /
  -- video's videop_*).
  constraint floristp_mock_no_user check (
    (is_mock = false and user_id is not null)
    or (is_mock = true and user_id is null)
  ),
  constraint florist_providers_gallery_max check (
...
create index if not exists idx_floristp_listing
  on public.florist_providers (status, availability, city)
  where status = 'active';
create index if not exists idx_floristp_owner
  on public.florist_providers (user_id);
create index if not exists idx_floristp_specialties
  on public.florist_providers using gin (specialties)
  where status = 'active';

-- updated_at touch trigger (mirrors cake / catering / photo / barber / handyman / tattoo / video).
create or replace function public.touch_florist_providers()
returns trigger language plpgsql as $$
begin new.updated_at = now(); return new; end $$;
drop trigger if exists trg_touch_florist_providers on public.florist_providers;
create trigger trg_touch_florist_providers
  before update on public.florist_providers
  for each row execute function public.touch_florist_providers();

-- Hide-mock-on-real-signup trigger (mirrors cake 0212 / catering 0211 / video 0210 / photo 0208).
create or replace function public.hide_one_mock_florist_provider()
returns trigger language plpgsql security definer as $$
declare victim_id uuid;
begin
  if new.is_mock = false and new.user_id is not null then
    select id into victim_id
      from public.florist_providers
     where is_mock = true and mock_hidden_at is null
     order by created_at asc
     limit 1;
    if victim_id is not null then
      update public.florist_providers set mock_hidden_at = now() where id = victim_id;
    end if;
  end if;
  return new;
end $$;
drop trigger if exists trg_hide_mock_on_real_florist_signup on public.florist_providers;
create trigger trg_hide_mock_on_real_florist_signup
  after insert on public.florist_providers
  for each row execute function public.hide_one_mock_florist_provider();

-- RLS — mirrors cake / catering / photo / barber / handyman / tattoo / video policies, renamed floristp_*.
alter table public.florist_providers enable row level security;
drop policy if exists floristp_public_select on public.florist_providers;
create policy floristp_public_select on public.florist_providers
  for select to anon, authenticated using (status = 'active');
drop policy if exists floristp_owner_select on public.florist_providers;
create policy floristp_owner_select on public.florist_providers
  for select to authenticated using (user_id = auth.uid());
drop policy if exists floristp_owner_update on public.florist_providers;
create policy floristp_owner_update on public.florist_providers
  for update to authenticated
  using (user_id = auth.uid()) with check (user_id = auth.uid());

-- Bookings table (mig 0090 equiv).
create table if not exists public.florist_bookings (
  id                 uuid primary key default gen_random_uuid(),
  florist_id         uuid not null references public.florist_providers(id) on delete cascade,
...
create index if not exists florist_bookings_by_provider_date
  on public.florist_bookings (florist_id, requested_date desc);
create index if not exists florist_bookings_pending
  on public.florist_bookings (florist_id, status)
  where status = 'pending';

create or replace function public.touch_florist_bookings_updated_at()
returns trigger language plpgsql as $$
begin new.updated_at = now(); return new; end $$;
drop trigger if exists trg_florist_bookings_touch on public.florist_bookings;
create trigger trg_florist_bookings_touch
  before update on public.florist_bookings
  for each row execute function public.touch_florist_bookings_updated_at();

alter table public.florist_bookings enable row level security;
drop policy if exists florist_bookings_owner_read on public.florist_bookings;
create policy florist_bookings_owner_read on public.florist_bookings
  for select using (
    exists (
      select 1 from public.florist_providers vp
      where vp.id = florist_bookings.florist_id
        and vp.user_id = auth.uid()
    )
  );
drop policy if exists florist_bookings_owner_update on public.florist_bookings;
create policy florist_bookings_owner_update on public.florist_bookings
  for update using (
    exists (
      select 1 from public.florist_providers vp
      where vp.id = florist_bookings.florist_id
...
-- ============================================================================
-- Wire 'florist' into the polymorphic constraints / triggers that the
-- rest of the platform relies on. Without these the profile-view
-- tracker, rating recomputer, and contact form would 500 on florist
-- rows.
-- ============================================================================

-- reviews_provider_type_check (mig 0072 / 0207 / 0208 / 0210 / 0211 / 0212) — allow 'florist'
...
alter table public.reviews
  ...
  check (provider_type is null or provider_type in (
    ...
    'tattoo', 'barber', 'photo', 'video', 'catering', 'cake', 'florist'
  ));

-- provider_profile_views CHECK (mig 0072 / 0207 / 0208 / 0210 / 0211 / 0212) — allow 'florist'
...
-- bump_provider_visitor_count (mig 0072 / 0207 / 0208 / 0210 / 0211 / 0212) — add 'florist' case
...
-- _recompute_provider_rating (mig 0075 / 0207 / 0208 / 0210 / 0211 / 0212) — add 'florist' case
...
-- contact_messages provider_type CHECK (mig 0137 / 0207 / 0208 / 0210 / 0211 / 0212) — allow 'florist'
...
alter table public.contact_messages
  ...
  check (provider_type in (
    ...
    'tour_guide','bike_rental','property','tattoo','barber','photo','video','catering','cake','florist'
  ));

-- contact_messages_own_select RLS — add 'florist' branch
...
-- payment_intents.product CHECK (mig 0068 / 0207 / 0208 / 0210 / 0211 / 0212) — add florist_monthly + florist_yearly
...
-- extend_florist_on_payment trigger — mirrors the verticals in mig 0068 / 0207 / 0208 / 0210 / 0211 / 0212
create or replace function public.extend_florist_on_payment()
returns trigger
language plpgsql
security definer
set search_path = public, pg_temp
...
begin
  ...
  if new.product not in ('florist_monthly','florist_yearly') then
    return new;
  end if;

  v_plan := case when new.product = 'florist_yearly' then 'yearly' else 'monthly' end;
  ...
  update public.florist_providers
...
drop trigger if exists pi_extend_florist on public.payment_intents;
create trigger pi_extend_florist
  after update of status on public.payment_intents
  for each row execute function public.extend_florist_on_payment();

-- Seed ONE demo florist — Anggrek Florist Yogya, Yogyakarta. Botanical
-- green theme (#16A34A) with white CTA. Stock Unsplash florist /
-- bouquet image (HEAD-verified image/jpeg before commit). Idempotent:
-- on conflict (slug) do update keeps the demo row in sync with the
-- schema.
insert into public.florist_providers (
...
values
  (true, null, 'demo-florist-anggrek', 'Anggrek Florist Yogya', 6,
    'Spesialis hand bouquet wisuda, ulang tahun, dan standing flower grand opening. Ready stock bunga import (mawar, lily, baby breath). Same-day delivery dalam kota sebelum jam 16:00. Kartu ucapan custom gratis, packing premium.',
    array['hand_bouquet','standing_flower','graduation']::text[], 150000, 1800000, true,
    'Yogyakarta', 'Yogya · Sleman · Bantul · pickup di studio Demangan OK', '+62000000507',
...
-- ============================================================================
-- Seed PortfolioCarousel entries for demo-florist-anggrek — mirrors
-- 0212's pattern for the cake demo. Flat JSONB array of {name,
...
-- ============================================================================
update public.florist_providers set
  service_photos = jsonb_build_array(
    jsonb_build_object(
      'url',         'https://images.unsplash.com/photo-1490750967868-88aa4486c946',
      'name',        'Hand Bouquet Mawar Premium',
      'description', '20 stem mawar import, baby breath, packing premium dengan kartu ucapan custom gratis. Cocok untuk ulang tahun / wisuda / surprise pacar. Same-day delivery dalam kota sebelum jam 16:00.',
      'price_idr',   280000
    ),
    jsonb_build_object(
      'url',         'https://images.unsplash.com/photo-1487530811176-3780de880c2d',
      'name',        'Standing Flower Grand Opening',
      'description', 'Standing flower tinggi 1.8m, mix bunga premium (mawar, lily, krisan). Ucapan custom di pita lebar. Same-day delivery dalam kota — pesan sebelum jam 10:00 untuk antar siang hari yang sama.',
      'price_idr',   1800000
    ),
    jsonb_build_object(
      'url',         'https://images.unsplash.com/photo-1508610048659-a06b669e3321',
      'name',        'Box Arrangement Anniversary',
      'description', 'Kotak acrylic premium, mix bunga + green leaves. Tahan 7 hari dengan water tube. Free balloon mini + kartu ucapan custom. Cocok anniversary, ulang tahun, atau gift kantor.',
      'price_idr',   450000
    ),
    jsonb_build_object(
      'url',         'https://images.unsplash.com/photo-1487070183336-b863922373d4',
      'name',        'Bouquet Wisuda Bunga Kering',
      'description', 'Dried flower bouquet — tahan selamanya, custom warna sesuai permintaan. Cocok kado wisuda atau dekor rumah. Packing premium dengan tali rami + kartu ucapan custom.',
      'price_idr',   195000
    )
  ),
  updated_at = now()
where slug = 'demo-florist-anggrek';
