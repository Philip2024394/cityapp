-- ============================================================================
-- 0212 — Cake / Bakery vertical (Phase 6 of 15 new lifestyle categories)
-- ----------------------------------------------------------------------------
-- Mirrors catering_providers byte-for-byte at the column level (0211), with
-- four semantic tweaks for the independent Indonesian cake / bakery
-- industry:
--   • specialties vocab = product categories (birthday, wedding,
--     korean_bento, cupcake, cookies, brownies, pastry, dessert_table,
--     custom_3d, ready_stock, halal, mixed, other) — Indonesian indie
--     cake makers cluster around birthday cake custom, the viral Korean
--     bento (lunchbox-mini) format, wedding tiers, dessert-table set-
--     ups, and box-style hampers (cookies / brownies / cupcake). Halal
--     is a first-class tag because every Lebaran / arisan order screens
--     for it.
--   • theme_color default = '#F472B6' (soft pink) — bakery aesthetic.
--     Pink imagery is appetising for sweets, photographs well on a feed,
--     and matches the pastel buttercream + sprinkle palette that
--     dominates Indonesian cake studio Instagram. button_text_color =
--     '#FFFFFF' so the CTA pills read clean white-on-pink, high
--     contrast.
--   • pricing — hourly_rate_idr is reused as the per-CAKE starter price
--     (per-UNIT, not per-pax), e.g. Rp 350k for a 1kg round birthday
--     cake, Rp 95k for a Korean bento mini, Rp 95k/box for a cookies
--     hamper. Distinct from catering's per-pax: cake quotes per-unit
--     with a minimum-order constraint baked in (e.g. min 5 box for
--     cookies, min 12 cupcakes). day_rate_idr becomes the optional
--     paket bundle (e.g. dessert table for 30 pax @ Rp 1.5jt, custom
--     3-tier wedding from Rp 4.5jt). Keeping the column names matches
--     the shared marketplace renderer; only the dashboard + profile
--     label changes ("Per-cake from" / "Paket bundle"). Pricing FLOOR
--     is Rp 95k (Korean bento mini cake or cookies-box minimum), much
--     lower per-unit than 0211's catering Rp 22k snack box because cake
--     is sold per-unit and a single cupcake (Rp 8-15k) is below the
--     minimum-order floor.
--   • has_own_tools boolean repurposed as "delivery dalam kota" (free
--     in-city drop-off vs pickup-only studio). Defaults true since
--     Indonesian indie bakers almost always deliver finished cakes by
--     ojek / car (and many won't ship interstate because of the
--     fragility / cold-chain).
-- ============================================================================

create table if not exists public.cake_providers (
  ...
  -- src/lib/cake/types.ts. CHECK constrains to that vocab.
  ...
  -- Cap at 3 specialties — same UX rule as catering / photo / barber / handyman / tattoo / video.
  constraint cake_providers_specialties_max3
    check (coalesce(array_length(specialties, 1), 0) between 1 and 3),

  -- Pricing — per-cake starter (hourly_rate_idr column kept for
  -- renderer parity; surfaces as "Per-cake from" on dashboard).
  ...
  constraint cake_providers_at_least_one_price
    check (
      hourly_rate_idr is not null
      or day_rate_idr is not null
    ),

  -- "Own tools" stays as a generic boolean — for cake it means
  -- "delivery dalam kota" (free in-city drop-off vs pickup-only
  -- studio). Defaults true since Indonesian indie bakers almost
  -- always deliver finished cakes by ojek / car.
  ...
  -- Per-provider theme accent (mig 0087 equiv). Default soft pink so the
  -- pastel buttercream / sprinkle imagery pops with bakery-tuned signage.
  theme_color      text default '#F472B6'
                   check (theme_color is null or theme_color ~* '^#[0-9A-F]{6}$'),
  -- Cake-specific: CTA label colour. Defaults to white so on the
  -- default pink brand the contrast is white-on-pink, high contrast for
  -- the CTA pills. mig 0202 mirror for cake.
  ...
  -- Use cakep_ prefix to avoid colliding with catering's caterp_*
  -- policy/constraint namespace (and photo's photop_* / barber's
  -- barberp_* / handyman's hp_* / tattoo's tp_* / video's videop_*).
  constraint cakep_mock_no_user check (
    (is_mock = false and user_id is not null)
    or (is_mock = true and user_id is null)
  ),
  constraint cake_providers_gallery_max check (
...
create index if not exists idx_cakep_listing
  on public.cake_providers (status, availability, city)
  where status = 'active';
create index if not exists idx_cakep_owner
  on public.cake_providers (user_id);
create index if not exists idx_cakep_specialties
  on public.cake_providers using gin (specialties)
  where status = 'active';

-- updated_at touch trigger (mirrors catering / photo / barber / handyman / tattoo / video).
create or replace function public.touch_cake_providers()
returns trigger language plpgsql as $$
begin new.updated_at = now(); return new; end $$;
drop trigger if exists trg_touch_cake_providers on public.cake_providers;
create trigger trg_touch_cake_providers
  before update on public.cake_providers
  for each row execute function public.touch_cake_providers();

-- Hide-mock-on-real-signup trigger (mirrors catering 0211 / video 0210 / photo 0208).
create or replace function public.hide_one_mock_cake_provider()
returns trigger language plpgsql security definer as $$
declare victim_id uuid;
begin
  if new.is_mock = false and new.user_id is not null then
    select id into victim_id
      from public.cake_providers
     where is_mock = true and mock_hidden_at is null
     order by created_at asc
     limit 1;
    if victim_id is not null then
      update public.cake_providers set mock_hidden_at = now() where id = victim_id;
    end if;
  end if;
  return new;
end $$;
drop trigger if exists trg_hide_mock_on_real_cake_signup on public.cake_providers;
create trigger trg_hide_mock_on_real_cake_signup
  after insert on public.cake_providers
  for each row execute function public.hide_one_mock_cake_provider();

-- RLS — mirrors catering / photo / barber / handyman / tattoo / video policies, renamed cakep_*.
alter table public.cake_providers enable row level security;
drop policy if exists cakep_public_select on public.cake_providers;
create policy cakep_public_select on public.cake_providers
  for select to anon, authenticated using (status = 'active');
drop policy if exists cakep_owner_select on public.cake_providers;
create policy cakep_owner_select on public.cake_providers
  for select to authenticated using (user_id = auth.uid());
drop policy if exists cakep_owner_update on public.cake_providers;
create policy cakep_owner_update on public.cake_providers
  for update to authenticated
  using (user_id = auth.uid()) with check (user_id = auth.uid());

-- Bookings table (mig 0090 equiv).
create table if not exists public.cake_bookings (
  id                 uuid primary key default gen_random_uuid(),
  cake_id            uuid not null references public.cake_providers(id) on delete cascade,
...
create index if not exists cake_bookings_by_provider_date
  on public.cake_bookings (cake_id, requested_date desc);
create index if not exists cake_bookings_pending
  on public.cake_bookings (cake_id, status)
  where status = 'pending';

create or replace function public.touch_cake_bookings_updated_at()
returns trigger language plpgsql as $$
begin new.updated_at = now(); return new; end $$;
drop trigger if exists trg_cake_bookings_touch on public.cake_bookings;
create trigger trg_cake_bookings_touch
  before update on public.cake_bookings
  for each row execute function public.touch_cake_bookings_updated_at();

alter table public.cake_bookings enable row level security;
drop policy if exists cake_bookings_owner_read on public.cake_bookings;
create policy cake_bookings_owner_read on public.cake_bookings
  for select using (
    exists (
      select 1 from public.cake_providers vp
      where vp.id = cake_bookings.cake_id
        and vp.user_id = auth.uid()
    )
  );
drop policy if exists cake_bookings_owner_update on public.cake_bookings;
create policy cake_bookings_owner_update on public.cake_bookings
  for update using (
    exists (
      select 1 from public.cake_providers vp
      where vp.id = cake_bookings.cake_id
...
-- ============================================================================
-- Wire 'cake' into the polymorphic constraints / triggers that the rest
-- of the platform relies on. Without these the profile-view tracker,
-- rating recomputer, and contact form would 500 on cake rows.
-- ============================================================================

-- reviews_provider_type_check (mig 0072 / 0207 / 0208 / 0210 / 0211) — allow 'cake'
...
alter table public.reviews
  ...
  check (provider_type is null or provider_type in (
    ...
    'tattoo', 'barber', 'photo', 'video', 'catering', 'cake'
  ));

-- provider_profile_views CHECK (mig 0072 / 0207 / 0208 / 0210 / 0211) — allow 'cake'
...
-- bump_provider_visitor_count (mig 0072 / 0207 / 0208 / 0210 / 0211) — add 'cake' case
...
-- _recompute_provider_rating (mig 0075 / 0207 / 0208 / 0210 / 0211) — add 'cake' case
...
-- contact_messages provider_type CHECK (mig 0137 / 0207 / 0208 / 0210 / 0211) — allow 'cake'
...
alter table public.contact_messages
  ...
  check (provider_type in (
    ...
    'tour_guide','bike_rental','property','tattoo','barber','photo','video','catering','cake'
  ));

-- contact_messages_own_select RLS — add 'cake' branch
...
-- payment_intents.product CHECK (mig 0068 / 0207 / 0208 / 0210 / 0211) — add cake_monthly + cake_yearly
...
-- extend_cake_on_payment trigger — mirrors the verticals in mig 0068 / 0207 / 0208 / 0210 / 0211
create or replace function public.extend_cake_on_payment()
returns trigger
language plpgsql
security definer
set search_path = public, pg_temp
...
begin
  ...
  if new.product not in ('cake_monthly','cake_yearly') then
    return new;
  end if;

  v_plan := case when new.product = 'cake_yearly' then 'yearly' else 'monthly' end;
...
drop trigger if exists pi_extend_cake on public.payment_intents;
create trigger pi_extend_cake
  after update of status on public.payment_intents
  for each row execute function public.extend_cake_on_payment();

-- Seed ONE demo cake maker — Maya Cake & Bakery, Yogyakarta. Soft-pink
-- theme (#F472B6) with white CTA. Stock Unsplash bakery decorator
-- image (HEAD-verified image/jpeg before commit). Idempotent: on
-- conflict (slug) do update keeps the demo row in sync with the
-- schema.
insert into public.cake_providers (
...
values
  (true, null, 'demo-cake-maya', 'Maya Cake & Bakery', 8,
    'Spesialis kue ulang tahun custom, Korean bento cake viral, dan dessert table untuk acara. Halal sertifikat, semua bahan premium. Lead time 3-5 hari untuk custom, 2 hari untuk bento. Antar dalam kota Yogyakarta gratis. DP 50% lock the date.',
    array['birthday','korean_bento','dessert_table']::text[], 350000, 1500000, true,
    'Yogyakarta', 'Yogya · Sleman · Bantul · pickup di studio Demangan OK', '+62000000506',
    'https://images.unsplash.com/photo-1535254973040-607b474cb50d?w=1200&auto=format&fit=crop',
    '#F472B6', '#FFFFFF',
...
-- ============================================================================
-- Seed PortfolioCarousel entries for demo-cake-maya — mirrors 0211's
-- pattern for the catering demo. Flat JSONB array of {name,
...
-- ============================================================================
update public.cake_providers set
  service_photos = jsonb_build_array(
    jsonb_build_object(
      'url',         'https://images.unsplash.com/photo-1558636508-e0db3814bd1d',
      'name',        'Birthday Round Cake 1kg',
      'description', 'Kue ulang tahun round 1kg, hiasan custom sesuai tema (princess, superhero, floral). Pilihan rasa vanilla, chocolate, red velvet, cheese, mocha. Lead time 3 hari, antar dalam kota Yogyakarta gratis. Halal sertifikat.',
      'price_idr',   350000
    ),
    jsonb_build_object(
      'url',         'https://images.unsplash.com/photo-1562440499-64c9a111f713',
      'name',        'Korean Bento Cake (Lunchbox Mini)',
      'description', 'Korean bento cake viral — mini cake dalam kotak lunchbox dengan custom message sesuai pesanan. Lead time 2 hari saja. Pilihan rasa vanilla / chocolate / matcha. Halal. Cocok untuk surprise pacar / kado mini.',
      'price_idr',   95000
    ),
    jsonb_build_object(
      'url',         'https://images.unsplash.com/photo-1486427944299-d1955d23e34d',
      'name',        'Dessert Table 30-pax',
      'description', 'Dessert table siap acara untuk 30 pax — 5 jenis mini dessert (cupcake, macaron, brownies, cookies, mini tart). Termasuk display stand pinjam. Cocok untuk arisan, baby shower, kantor. Lead time 5 hari.',
      'price_idr',   1500000
    ),
    jsonb_build_object(
      'url',         'https://images.unsplash.com/photo-1599785209707-a456fc1337bb',
      'name',        'Cookies Box Premium',
      'description', 'Box premium berisi 12 kue kering assorted (nastar, kastengel, putri salju, choco chips). Kemasan kado premium dengan pita. Minimum 5 box. Cocok hampers Lebaran / souvenir wedding / kantor. Halal sertifikat.',
      'price_idr',   95000
    )
  ),
  updated_at = now()
where slug = 'demo-cake-maya';
