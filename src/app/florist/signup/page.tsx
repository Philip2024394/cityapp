...
import { ALL_SPECIALTIES, SPECIALTY_LABELS, MAX_FLORIST_SPECIALTIES, type FloristSpecialty } from '@/lib/florist/types'

type AuthState =
  | { status: 'loading' } | { status: 'anon' }
  | { status: 'signedIn'; userId: string } | { status: 'alreadyProvider' }

export default function FloristSignupPage() {
    ...
    supabase.auth.getSession().then(async ({ data }) => {
      ...
      try {
        ...
        if (r.ok) {
          ...
          if (j.provider) { setAuth({ status: 'alreadyProvider' }); setTimeout(() => router.replace('/dashboard/florist'), 400); return }
        }
      } catch { /* fall through */ }
  ...
  return (
    <div className="px-4 pt-12 max-w-md mx-auto">
      <div className="text-center mb-8">
        <div className="inline-block bg-brand text-bg text-[11px] font-extrabold uppercase tracking-[0.15em] px-3 py-1 rounded-full mb-4">Florist business</div>
        <h1 className="text-[26px] font-black leading-tight mb-3">List as a florist business</h1>
        <p className="text-[13px] text-ink/70 leading-relaxed">Create an account first — already have one? Login with the same.</p>
      </div>
      <div className="space-y-3">
        <Link href="/signup?intent=florist&next=/florist/signup" className="block w-full rounded-2xl bg-brand text-bg px-6 py-4 text-center text-[14px] font-extrabold uppercase tracking-wider hover:brightness-105">
          Create new account
        </Link>
        <Link href="/login?next=/florist/signup" className="block w-full rounded-2xl bg-white/[0.06] border border-ink/15 text-ink px-6 py-4 text-center text-[14px] font-extrabold uppercase tracking-wider hover:bg-white/10">
...
function Form({ userId }: { userId: string }) {
  ...
  const [f, setF] = useState({
    ...
    years_experience: 0,
    bio: '',
    specialties: [] as FloristSpecialty[],
    hourly_rate: '',
    day_rate: '',
    has_own_tools: true,
    whatsapp_e164: '',
  ...
  function toggleSpecialty(s: FloristSpecialty) {
    setF((p) => {
      if (p.specialties.includes(s)) {
        return { ...p, specialties: p.specialties.filter((x) => x !== s) }
      }
      if (p.specialties.length >= MAX_FLORIST_SPECIALTIES) return p // cap at 3
  ...
  async function submit(e: React.FormEvent) {
    e.preventDefault()
    if (f.specialties.length === 0) { setErr(`Pick min 1, max ${MAX_FLORIST_SPECIALTIES} kategori arrangement.`); return }
    const h = f.hourly_rate.trim()
    const d = f.day_rate.trim()
    if (h === '' && d === '') { setErr('Enter at least one rate — per-arrangement or premium bundle.'); return }
    setSubmitting(true); setErr(null)
    try {
      const r = await fetch('/api/florist/signup', {
      ...
      setDone(true); setTimeout(() => router.push('/dashboard/florist'), 1200)
  ...
  return (
    <div className="px-4 pt-6 pb-20 max-w-md mx-auto">
      ...
      <h1 className="text-[26px] font-black leading-tight mb-2">Florist business signup</h1>
      <p className="text-[13px] text-ink/70 mb-6">Pick max 3 spesialisasi. Set per-arrangement price dan/atau premium bundle. Rp 38,000/month after a 7-day trial.</p>

      <form onSubmit={submit} className="space-y-4">
        <Field label="Florist business name *">
          <input required maxLength={80} value={f.display_name} onChange={(e) => upd('display_name', e.target.value)} placeholder="Anggrek Florist Yogya" className={inputCls} />
        </Field>
        ...
        <Field label="Bio (max 300) *">
          <textarea required maxLength={300} rows={3} value={f.bio} onChange={(e) => upd('bio', e.target.value)} placeholder="Spesialisasi (hand bouquet / standing flower / box arrangement), same-day delivery cutoff, stok bunga import, kartu ucapan custom, uang muka 50% lock the date." className={inputCls + ' resize-none'} />
        </Field>

        <div className="rounded-2xl bg-black/85 border border-white/10 p-4">
          <div className="flex items-baseline justify-between mb-1">
            <div className="text-[13px] font-extrabold uppercase tracking-wider text-ink">Genres *</div>
            <div className="text-[11px] text-ink/55">{f.specialties.length}/{MAX_FLORIST_SPECIALTIES}</div>
          </div>
          <p className="text-[11px] text-ink/55 mb-3">Pick max {MAX_FLORIST_SPECIALTIES} kategori arrangement andalan.</p>
          <div className="flex flex-wrap gap-1.5">
            {ALL_SPECIALTIES.map((s) => {
              const on = f.specialties.includes(s)
              const atLimit = !on && f.specialties.length >= MAX_FLORIST_SPECIALTIES
              return (
                <button
                  key={s} type="button"
                  disabled={atLimit}
        ...
        <div className="rounded-2xl bg-black/85 border border-white/10 p-4">
          ...
          <p className="text-[11px] text-ink/55 mb-3">Enter at least one — per-arrangement price atau premium bundle (standing flower, wedding package).</p>
          <div className="grid grid-cols-2 gap-2">
            <PriceField label="Per-arrangement from" v={f.hourly_rate} set={(v) => upd('hourly_rate', v)} />
            <PriceField label="Premium bundle"       v={f.day_rate}    set={(v) => upd('day_rate', v)} />
          </div>
          <label className="flex items-center gap-2 mt-3 text-[12px] text-ink/85 cursor-pointer">
            ...
            <span>Delivery dalam kota same-day (antar gratis ke alamat client sebelum jam 16:00)</span>
          </label>
        </div>
        ...
        <Field label="Delivery area / catatan jangkauan"><input value={f.service_area_notes} onChange={(e) => upd('service_area_notes', e.target.value)} placeholder="Yogya · Sleman · Bantul · pickup studio OK" className={inputCls} /></Field>
        ...
        <button type="submit" disabled={submitting} className="w-full rounded-full bg-brand text-bg px-6 py-3.5 text-[14px] font-extrabold uppercase tracking-wider disabled:opacity-60">
          {submitting ? 'Registering…' : 'Create florist business profile'}
        </button>
      </form>
    </div>
  )
...
function humaniseError(code?: string): string {
  switch (code) {
    ...
    case 'at_least_one_specialty':return `Pick min 1, max ${MAX_FLORIST_SPECIALTIES} kategori arrangement.`
    case 'at_least_one_price':    return 'Enter at least one rate — per-arrangement or premium bundle.'
    case 'slug_collision':        return 'Name already taken — try a variation.'
    case 'already_registered':    return 'You already have a florist business profile.'
