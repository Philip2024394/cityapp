...
import {
  ...
  type FloristProviderPublic, type FloristSpecialty,
} from '@/lib/florist/types'

// ============================================================================
// /florist — Marketplace ported from the shared cake/catering/photo/barber shell.
...
// Mirrors /cake + /catering + /photo + /barber + /handyman + /tattoo exactly
// so all verticals feel like one product.
// ============================================================================
...
function effectiveAvailability(p: FloristProviderPublic): 'online' | 'busy' | 'offline' {
...
type ChipDef = { id: 'all' | FloristSpecialty; label: string }
const PRIMARY_CHIPS: ChipDef[] = [
  { id: 'all',              label: 'All'           },
  { id: 'hand_bouquet',     label: 'Hand Bouquet'  },
  { id: 'standing_flower',  label: 'Standing'      },
  { id: 'box_arrangement',  label: 'Box'           },
  { id: 'graduation',       label: 'Wisuda'        },
  { id: 'wedding',          label: 'Wedding'       },
]
const PRIMARY_IDS = new Set<FloristSpecialty>(['hand_bouquet','standing_flower','box_arrangement','graduation','wedding'])
const SECONDARY_IDS: FloristSpecialty[] = ALL_SPECIALTIES.filter((s) => !PRIMARY_IDS.has(s))

export default function FloristMarketplacePage() {
  return (
...
function MarketplaceInner() {
  ...
  const t       = useTranslations('verticals.florist')
  const tCommon = useTranslations('verticals.common')
  const haptic    = useHaptic()
  const search    = useSearchParams()
  const cityLabel = search?.get('city')?.trim() || 'Yogyakarta'

  const [providers, setProviders] = useState<FloristProviderPublic[]>([])
  const [query, setQuery]         = useState('')
  const [specialty, setSpecialty] = useState<FloristSpecialty | null>(null)
  ...
  const load = useCallback(async () => {
    ...
    const r = await fetch(`/api/florist/marketplace${qs.toString() ? `?${qs}` : ''}`, { cache: 'no-store' })
    const j = await r.json() as { providers: FloristProviderPublic[] }
          ...
          <div className="flex flex-col p-4 sm:p-5">
            ...
            <div className="mt-3 relative shrink-0">
              <input
                ...
                placeholder="Cari florist — nama, kota, jenis bunga…"
            ...
            <div
              ...
              role="tablist"
              aria-label="Filter florist businesses by category"
            >
                ...
                <div className="py-10 text-center">
                  <p className="text-[13px] font-bold text-[#71717A] leading-snug">
                    {query.trim()
                      ...
                      : 'No florist businesses in this category yet. Try a different filter.'}
                  </p>
                </div>
              )}
            ...
            <p className="mt-3 text-center text-[12px] text-[#52525B] font-bold leading-snug px-2 shrink-0">
              Arrangements worth waiting on the doorstep for — book direct via WhatsApp.
              Kita2u takes no commission, you pay your florist directly.
            </p>
          </div>
        </div>
      </div>
...
// ─────────────────────────────────────────────────────────────────────────────
// FloristRowCard — landscape card, 112px tall to match /cake + /catering + /photo + /barber + /handyman.
// 88px portrait/headshot left, name + primary category + today's hours middle,
// online dot + credibility pills (per-arrangement price / bundle / years) below,
// rating + chevron right.
// ─────────────────────────────────────────────────────────────────────────────
function FloristRowCard({
  provider: p,
  onOpen,
}: {
  provider: FloristProviderPublic
  onOpen: () => void
}) {
  ...
  // Credibility pills — favour per-arrangement starting price (most
  // useful for the florist industry) then premium bundle rate then
  // years of experience.
  const pills: Array<{ key: string; label: string }> = []
  if (p.hourly_rate_idr != null) {
    pills.push({ key: 'pkg', label: `Rp ${p.hourly_rate_idr.toLocaleString('id-ID')}/bouquet` })
  }
  if (p.day_rate_idr != null && pills.length < 2) {
    pills.push({ key: 'day', label: `Rp ${p.day_rate_idr.toLocaleString('id-ID')}/premium` })
  }
...
// ─────────────────────────────────────────────────────────────────────────────
// Shell — wordmark header matching /cake + /catering + /photo + /barber + /handyman + /beautician + /places + /tattoo.
// ─────────────────────────────────────────────────────────────────────────────
