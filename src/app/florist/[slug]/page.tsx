...
import {
  ALL_SPECIALTIES,
  SPECIALTY_LABELS,
  type FloristProviderPublic,
  type FloristSpecialty,
...
// Default theme accent — used when the florist business hasn't picked their
// own theme_color (mig 0087). Florists choose their accent from the
// dashboard color palette; the chosen hex flows through every accent
// surface on this page via the `theme` constant below. Default
// botanical green (#16A34A) so the leaf-and-stem imagery pops with
// florist-tuned signage (distinct from cake's bakery pink).
...
// /florist/[slug] — universal profile flagship build. Mirrors the
// /catering/[slug] page byte-for-byte at the JSX layer; only the data
// source swaps from catering_providers to florist_providers and a few
// vertical-specific adaptations apply (genre chips, per-florist + paket
// bundle pricing block, flat service_photos array with partial-name
// filter).

// Vertical default for the hero. Used when the florist business hasn't set
// their own cover_image_url yet — keeps the page on-brand instead of
// showing the generic yellow gradient on every new signup.
const DEFAULT_FLORIST_HERO =
  'https://images.unsplash.com/photo-1561181286-d3fee7d55364?w=1200&auto=format&fit=crop'

export default function FloristProviderPage() {
  const params = useParams<{ slug: string }>()
  const slug = String(params?.slug || '').toLowerCase()
  const [p, setP] = useState<FloristProviderPublic | null>(null)
  ...
  const [activeService, setActiveService] = useState<FloristSpecialty | null>(null)
  ...
  // available when the florist business has opted into a physical studio).
  ...
  useEffect(() => {
    ...
    const url = `/api/florist/${encodeURIComponent(slug)}/public${
      draftPassword ? `?p=${encodeURIComponent(draftPassword)}` : ''
    }`
    fetch(url, { cache: 'no-store' })
      .then((r) => r.ok ? r.json() : null)
      .then((j: { provider?: FloristProviderPublic; locked?: boolean } | null) => {
  ...
  useProfileViewTracker({ providerType: 'florist', providerId: p?.id })

  // Resolved accent color for every accent surface on this page.
  // p.theme_color (mig 0087) wins; fall back to global default botanical green.
  ...
  useEffect(() => {
    ...
    fetch(`/api/reviews?provider_type=florist&provider_id=${encodeURIComponent(p.id)}`, { cache: 'no-store' })
      .then((r) => r.ok ? r.json() : null)
    ...
    return (
      <Shell>
        <div className="px-4 pt-20 max-w-md mx-auto text-center">
          <h1 className="text-[20px] font-black mb-2">Florist business not found</h1>
          <Link href="/florist" className="rounded-full bg-brand text-bg px-6 py-3 text-[13px] font-extrabold inline-block">Back to marketplace</Link>
        </div>
      </Shell>
    )
  }
    ...
    return (
      <Shell>
        <DraftPasswordGate
          ...
          fetchUrl={(pw) => `/api/florist/${encodeURIComponent(slug)}/public?p=${encodeURIComponent(pw)}`}
          onUnlocked={(pw, prov) => {
            setDraftPassword(pw)
            setLocked(false)
            setP(prov as FloristProviderPublic)
          }}
        />
      </Shell>
    )
  ...
  return (
    <Shell>
      ...
      <div className="relative pb-2">
        ...
        >
          <img
            src={bannerSrc(p.cover_image_url) || DEFAULT_FLORIST_HERO}
            alt=""
          ...
          {(() => {
            ...
            const line1   = ht.line1   || 'Toko Bunga'
            const line2   = ht.line2   || 'Florist business'
            const tagline = ht.tagline || 'Hand Bouquet · Standing · Box · Wisuda'
            ...
            return (
              <div className={`absolute left-4 z-10 select-none leading-none cr-hero-${effect}`} style={{ top: 31 }}>
                ...
                {/* Service locations the florist delivers to — Home and
                    Hotel are the everyday delivery pairs for indie florist
                    studios (birthday at home, wedding tier at hotel
                    venues). A missing or null service_locations array
                    still hides the whole row to avoid implying
                    locations the baker doesn't serve. */}
                {(() => {
        ...
        <div className="px-4 relative z-20" style={{ marginTop: 12 }}>
          ...
          >
            ...
            {/* Top Rated Seller badge — neutral gray pill so the chip
                ...
                florist business shows their own accent. */}
            <div
      ...
      <div className="px-4 pb-6 max-w-2xl mx-auto space-y-3 pt-3">
        ...
        {(() => {
          const offered = (p.specialties ?? []) as FloristSpecialty[]
        ...
        {/* Portfolio carousel — rich cards (image + name + 2-line desc
            ...
            Florist studios sell on finished florist photos (clients pick by
            picture), so the header reads "Arrangement Gallery". */}
        {(() => {
          ...
          return (
            <section className="space-y-2">
              <div className="flex items-center justify-between gap-2">
                <h2 className="text-[13px] font-extrabold uppercase tracking-wider text-black">
                  {activeService
                    ? `${SPECIALTY_LABELS[activeService]} — Arrangement Gallery`
                    : 'Arrangement Gallery'}
                </h2>
                <PortfolioViewToggle
                  view={portfolioView}
              ...
              <p className="text-[11px] text-gray-500 italic -mt-1">
                Please contact for custom arrangement or kategori belum tercantum
              </p>
              <PortfolioCarousel
                photos={photos}
        ...
        <RunningMarquee
          text={p.promo_text || 'Bunga yang menyampaikan apa yang kata-kata tak bisa — same-day delivery sebelum jam 16:00, kartu ucapan custom gratis. Konsultasi via WhatsApp.'}
        />
      ...
      {/* Right-edge "back" bar — tall vertical strip flush against the
          ...
          vertical "BACK" text below. Diverts back to /florist. */}
      <Link
        href="/florist"
        aria-label="Back to Kita2u florist"
      ...
      {/* Contact / booking popup — opened by both the bottom Contact
          ...
          request server-side first (so it shows up on the florist
          business's calendar) and then opens WhatsApp with a matching
          pre-filled message. Skips busy_dates the florist business
          has marked. */}
      {contactOpen && p.whatsapp_e164 && (
        <ContactBookingPopup
          ...
          providerId={p.id}
          intentVertical="florist"
          intentSource="florist_profile"
          themeColor={theme}
          ...
          busyDates={(p.busy_dates ?? []) as string[]}
          bookEndpoint={`/api/florist/${p.slug}/book`}
      ...
      <PoweredByKita2u defaultVertical="florist" />
    </Shell>
  )
}
...
}: { sid: FloristSpecialty; active: boolean; onClick: () => void; theme: string }) {
  return (
    <button
      type="button"
      onClick={onClick}
...
// Cheapest price across per-arrangement + premium bundle + per-photo prices.
// Falls back to a sensible default when nothing is set so the CTA
// never reads empty. Florist floor is hand bouquet small ~Rp 150k
// baseline (per-unit; even the cheapest indie bouquet uses imported
// stems + wrap that don't sell below Rp 150k).
function formatStartFromPrice(p: FloristProviderPublic): string {
  ...
  if (candidates.length === 0) return 'Rp 150k'
  return formatPriceIdr(Math.min(...candidates)) ?? 'Rp 150k'
}

// Normalises one service_photos entry — florist uses a flat array of
...
// Combines florist business photos into the carousel feed. When a genre
...
function buildPortfolioPhotos(
  p: FloristProviderPublic,
  active: FloristSpecialty | null,
): PortfolioPhoto[] {
  ...
  // Partial-match against the genre label — works when the florist
  // named their photos sensibly. If the filter yields nothing, fall
  // back to all photos so the carousel doesn't collapse to empty when
  // chip data and photo data diverge.
