...
import { ALL_SPECIALTIES, SPECIALTY_LABELS, MAX_VIDEO_SPECIALTIES, type VideoSpecialty } from '@/lib/video/types'

type AuthState =
  | { status: 'loading' } | { status: 'anon' }
  | { status: 'signedIn'; userId: string } | { status: 'alreadyProvider' }

export default function VideoSignupPage() {
    ...
    supabase.auth.getSession().then(async ({ data }) => {
      ...
      try {
        ...
        if (r.ok) {
          ...
          if (j.provider) { setAuth({ status: 'alreadyProvider' }); setTimeout(() => router.replace('/dashboard/video'), 400); return }
        }
      } catch { /* fall through */ }
  ...
  return (
    <div className="px-4 pt-12 max-w-md mx-auto">
      <div className="text-center mb-8">
        <div className="inline-block bg-brand text-bg text-[11px] font-extrabold uppercase tracking-[0.15em] px-3 py-1 rounded-full mb-4">Videographer</div>
        <h1 className="text-[26px] font-black leading-tight mb-3">List as a videographer</h1>
        <p className="text-[13px] text-ink/70 leading-relaxed">Create an account first — already have one? Login with the same.</p>
      </div>
      <div className="space-y-3">
        <Link href="/signup?intent=video&next=/video/signup" className="block w-full rounded-2xl bg-brand text-bg px-6 py-4 text-center text-[14px] font-extrabold uppercase tracking-wider hover:brightness-105">
          Create new account
        </Link>
        <Link href="/login?next=/video/signup" className="block w-full rounded-2xl bg-white/[0.06] border border-ink/15 text-ink px-6 py-4 text-center text-[14px] font-extrabold uppercase tracking-wider hover:bg-white/10">
...
function Form({ userId }: { userId: string }) {
  ...
  function toggleSpecialty(s: VideoSpecialty) {
    setF((p) => {
      if (p.specialties.includes(s)) {
        return { ...p, specialties: p.specialties.filter((x) => x !== s) }
      }
      if (p.specialties.length >= MAX_VIDEO_SPECIALTIES) return p // cap at 3
  ...
  async function submit(e: React.FormEvent) {
    e.preventDefault()
    if (f.specialties.length === 0) { setErr(`Pick min 1, max ${MAX_VIDEO_SPECIALTIES} genres.`); return }
    const h = f.hourly_rate.trim()
    const d = f.day_rate.trim()
    if (h === '' && d === '') { setErr('Enter at least one rate — package or full-day.'); return }
    setSubmitting(true); setErr(null)
    try {
      const r = await fetch('/api/video/signup', {
      ...
      setDone(true); setTimeout(() => router.push('/dashboard/video'), 1200)
  ...
  return (
    <div className="px-4 pt-6 pb-20 max-w-md mx-auto">
      ...
      <h1 className="text-[26px] font-black leading-tight mb-2">Videographer signup</h1>
      ...
      <form onSubmit={submit} className="space-y-4">
        <Field label="Videographer / studio name *">
          <input required maxLength={80} value={f.display_name} onChange={(e) => upd('display_name', e.target.value)} placeholder="Niko Films" className={inputCls} />
        </Field>
        ...
        <div className="rounded-2xl bg-black/85 border border-white/10 p-4">
          <div className="flex items-baseline justify-between mb-1">
            ...
            <div className="text-[11px] text-ink/55">{f.specialties.length}/{MAX_VIDEO_SPECIALTIES}</div>
          </div>
          <p className="text-[11px] text-ink/55 mb-3">Pick max {MAX_VIDEO_SPECIALTIES} signature genres.</p>
          <div className="flex flex-wrap gap-1.5">
            {ALL_SPECIALTIES.map((s) => {
              const on = f.specialties.includes(s)
              const atLimit = !on && f.specialties.length >= MAX_VIDEO_SPECIALTIES
              return (
                <button
                  key={s} type="button"
                  disabled={atLimit}
        ...
        <div className="rounded-2xl bg-black/85 border border-white/10 p-4">
          ...
          <label className="flex items-center gap-2 mt-3 text-[12px] text-ink/85 cursor-pointer">
            ...
            <span>Travels for the shoot (on-location, not studio-only)</span>
          </label>
        </div>
        ...
        <button type="submit" disabled={submitting} className="w-full rounded-full bg-brand text-bg px-6 py-3.5 text-[14px] font-extrabold uppercase tracking-wider disabled:opacity-60">
          {submitting ? 'Registering…' : 'Create videographer profile'}
        </button>
      </form>
    </div>
  )
...
function humaniseError(code?: string): string {
  switch (code) {
    ...
    case 'at_least_one_specialty':return `Pick min 1, max ${MAX_VIDEO_SPECIALTIES} genres.`
    ...
    case 'already_registered':    return 'You already have a videographer profile.'
