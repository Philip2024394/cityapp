...
// Server-side SEO + lapsed redirect for /video/[slug].
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  if (!p) return { title: 'Videographer not found', robots: { index: false, follow: false } }
  const pkg = p.hourly_rate_idr ?? null
  const title       = `${p.display_name} · Videographer ${p.city || 'Indonesia'}`
  const description = (p.bio?.trim())
    || `Book ${p.display_name} (videographer) in ${p.city || 'Indonesia'} via WhatsApp.${pkg ? ` Packages from Rp ${pkg.toLocaleString('id-ID')}.` : ''}`
  const canonical   = `${SITE_URL}/video/${p.slug}`
...
export default async function VideoProviderLayout({
  children, params,
  ...
  if (p) {
    ...
    if (lapsed) {
      const qs = new URLSearchParams({ from: 'lapsed_video', slug: p.slug })
      redirect(`/video?${qs.toString()}`)
    }
  }
  const jsonLd = p
    ? {
        '@context': 'https://schema.org', '@type': 'LocalBusiness',
        '@id': `${SITE_URL}/video/${p.slug}`,
        name: p.display_name, description: p.bio || undefined,
        url: `${SITE_URL}/video/${p.slug}`,
