...
import {
  ...
  type VideoProviderPublic, type VideoSpecialty,
} from '@/lib/video/types'

// ============================================================================
// /video — Marketplace ported from the shared photo/barber/handyman shell.
...
// Mirrors /photo + /barber + /handyman + /tattoo exactly so all verticals feel
// like one product.
// ============================================================================
...
function effectiveAvailability(p: VideoProviderPublic): 'online' | 'busy' | 'offline' {
...
type ChipDef = { id: 'all' | VideoSpecialty; label: string }
const PRIMARY_CHIPS: ChipDef[] = [
  { id: 'all',               label: 'All'      },
  { id: 'wedding_cinematic', label: 'Wedding'  },
  { id: 'music_video',       label: 'Music Video' },
  { id: 'brand_commercial',  label: 'Brand'    },
  { id: 'social_reel',       label: 'Reel'     },
  { id: 'drone_aerial',      label: 'Drone'    },
]
const PRIMARY_IDS = new Set<VideoSpecialty>(['wedding_cinematic','music_video','brand_commercial','social_reel','drone_aerial'])
const SECONDARY_IDS: VideoSpecialty[] = ALL_SPECIALTIES.filter((s) => !PRIMARY_IDS.has(s))

export default function VideoMarketplacePage() {
  return (
...
function MarketplaceInner() {
  ...
  const t       = useTranslations('verticals.video')
  const tCommon = useTranslations('verticals.common')
  const haptic    = useHaptic()
  const search    = useSearchParams()
  const cityLabel = search?.get('city')?.trim() || 'Yogyakarta'

  const [providers, setProviders] = useState<VideoProviderPublic[]>([])
  const [query, setQuery]         = useState('')
  const [specialty, setSpecialty] = useState<VideoSpecialty | null>(null)
  ...
  const load = useCallback(async () => {
    ...
    const r = await fetch(`/api/video/marketplace${qs.toString() ? `?${qs}` : ''}`, { cache: 'no-store' })
    const j = await r.json() as { providers: VideoProviderPublic[] }
          ...
          <div className="flex flex-col p-4 sm:p-5">
            ...
            <div className="mt-3 relative shrink-0">
              <input
                ...
                placeholder="Cari videografer — nama, studio, genre…"
            ...
            <div
              ...
              role="tablist"
              aria-label="Filter videographers by genre"
            >
                ...
                <div className="py-10 text-center">
                  <p className="text-[13px] font-bold text-[#71717A] leading-snug">
                    {query.trim()
                      ...
                      : 'No videographers in this genre yet. Try a different filter.'}
                  </p>
                </div>
              )}
            ...
            <p className="mt-3 text-center text-[12px] text-[#52525B] font-bold leading-snug px-2 shrink-0">
              Cinematic stories worth pressing play — book direct via WhatsApp.
              Kita2u takes no commission, you pay your videographer directly.
            </p>
          </div>
        </div>
      </div>
...
// ─────────────────────────────────────────────────────────────────────────────
// VideoRowCard — landscape card, 112px tall to match /photo + /barber + /handyman.
// 88px portrait/headshot left, name + primary genre + today's hours middle,
// online dot + credibility pills (package price / full-day / years) below,
...
function VideoRowCard({
  ...
  // Credibility pills — favour package starting price (most useful for
  // the videography industry) then full-day rate then years of experience.
  const pills: Array<{ key: string; label: string }> = []
  if (p.hourly_rate_idr != null) {
    pills.push({ key: 'pkg', label: `Rp ${p.hourly_rate_idr.toLocaleString('id-ID')}/pkg` })
  }
...
// ─────────────────────────────────────────────────────────────────────────────
// Shell — wordmark header matching /photo + /barber + /handyman + /beautician + /places + /tattoo.
// ─────────────────────────────────────────────────────────────────────────────
