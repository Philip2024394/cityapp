...
import { ALL_SPECIALTIES, SPECIALTY_LABELS, MAX_PARCEL_SPECIALTIES, type ParcelSpecialty } from '@/lib/parcel/types'

type AuthState =
  | { status: 'loading' } | { status: 'anon' }
  | { status: 'signedIn'; userId: string } | { status: 'alreadyProvider' }

export default function ParcelSignupPage() {
    ...
    supabase.auth.getSession().then(async ({ data }) => {
      ...
      try {
        ...
        if (r.ok) {
          ...
          if (j.provider) { setAuth({ status: 'alreadyProvider' }); setTimeout(() => router.replace('/dashboard/parcel'), 400); return }
        }
      } catch { /* fall through */ }
  ...
  return (
    <div className="px-4 pt-12 max-w-md mx-auto">
      <div className="text-center mb-8">
        <div className="inline-block bg-brand text-bg text-[11px] font-extrabold uppercase tracking-[0.15em] px-3 py-1 rounded-full mb-4">Parcel</div>
        <h1 className="text-[26px] font-black leading-tight mb-3">List your parcel courier business</h1>
        <p className="text-[13px] text-ink/70 leading-relaxed">Create an account first — already have one? Login with the same.</p>
      </div>
      <div className="space-y-3">
        <Link href="/signup?intent=parcel&next=/parcel/signup" className="block w-full rounded-2xl bg-brand text-bg px-6 py-4 text-center text-[14px] font-extrabold uppercase tracking-wider hover:brightness-105">
          Create new account
        </Link>
        <Link href="/login?next=/parcel/signup" className="block w-full rounded-2xl bg-white/[0.06] border border-ink/15 text-ink px-6 py-4 text-center text-[14px] font-extrabold uppercase tracking-wider hover:bg-white/10">
...
function Form({ userId }: { userId: string }) {
  ...
  const [f, setF] = useState({
    ...
    years_experience: 0,
    bio: '',
    specialties: [] as ParcelSpecialty[],
    hourly_rate: '',
    day_rate: '',
    has_own_tools: true,
    whatsapp_e164: '',
  ...
  function toggleSpecialty(s: ParcelSpecialty) {
    setF((p) => {
      if (p.specialties.includes(s)) {
        return { ...p, specialties: p.specialties.filter((x) => x !== s) }
      }
      if (p.specialties.length >= MAX_PARCEL_SPECIALTIES) return p // cap at 3
  ...
  async function submit(e: React.FormEvent) {
    e.preventDefault()
    if (f.specialties.length === 0) { setErr(`Pick min 1, max ${MAX_PARCEL_SPECIALTIES} vehicle + service.`); return }
    const h = f.hourly_rate.trim()
    const d = f.day_rate.trim()
    if (h === '' && d === '') { setErr('Enter at least one rate — anchor price atau paket bulk.'); return }
    setSubmitting(true); setErr(null)
    try {
      const r = await fetch('/api/parcel/signup', {
      ...
      setDone(true); setTimeout(() => router.push('/dashboard/parcel'), 1200)
  ...
  return (
    <div className="px-4 pt-6 pb-20 max-w-md mx-auto">
      ...
      <h1 className="text-[26px] font-black leading-tight mb-2">Parcel courier signup</h1>
      <p className="text-[13px] text-ink/70 mb-6">Pick max 3 vehicle/service specialties. Set anchor price (cheapest line, e.g. motor dalam kota Rp 15k typical job) dan/atau paket bulk all-in rate. Rp 38,000/month after a 7-day trial.</p>

      <form onSubmit={submit} className="space-y-4">
        <Field label="Business name *">
          <input required maxLength={80} value={f.display_name} onChange={(e) => upd('display_name', e.target.value)} placeholder="Cepat Kurir Yogya" className={inputCls} />
        </Field>
        ...
        <Field label="Bio (max 300) *">
          <textarea required maxLength={300} rows={3} value={f.bio} onChange={(e) => upd('bio', e.target.value)} placeholder="Armada (motor / pickup / box CDD), coverage area (dalam kota / antar kota / antar provinsi), GPS share real-time, COD tersedia, asuransi paket berharga, instant 60-menit garansi, pengalaman tahun." className={inputCls + ' resize-none'} />
        </Field>

        <div className="rounded-2xl bg-black/85 border border-white/10 p-4">
          <div className="flex items-baseline justify-between mb-1">
            <div className="text-[13px] font-extrabold uppercase tracking-wider text-ink">Vehicle + service specialties *</div>
            <div className="text-[11px] text-ink/55">{f.specialties.length}/{MAX_PARCEL_SPECIALTIES}</div>
          </div>
          <p className="text-[11px] text-ink/55 mb-3">Pick max {MAX_PARCEL_SPECIALTIES} (vehicle + service levels you offer).</p>
          <div className="flex flex-wrap gap-1.5">
            {ALL_SPECIALTIES.map((s) => {
              const on = f.specialties.includes(s)
              const atLimit = !on && f.specialties.length >= MAX_PARCEL_SPECIALTIES
              return (
                <button
                  key={s} type="button"
                  disabled={atLimit}
        ...
        <div className="rounded-2xl bg-black/85 border border-white/10 p-4">
          ...
          <p className="text-[11px] text-ink/55 mb-3">Enter at least one — anchor price (cheapest line, e.g. motor dalam kota Rp 15k typical job) atau paket bulk all-in rate.</p>
          <div className="grid grid-cols-2 gap-2">
            <PriceField label="Mulai dari (anchor)"  v={f.hourly_rate} set={(v) => upd('hourly_rate', v)} />
            <PriceField label="Paket bulk (all-in)"  v={f.day_rate}    set={(v) => upd('day_rate', v)} />
          </div>
          <label className="flex items-center gap-2 mt-3 text-[12px] text-ink/85 cursor-pointer">
            ...
            <span>Pickup di Lokasi (siap pickup di lokasi pengirim)</span>
          </label>
        </div>
        ...
        <Field label="Service area / catatan jangkauan"><input value={f.service_area_notes} onChange={(e) => upd('service_area_notes', e.target.value)} placeholder="Yogya · DIY · Jateng antar kota · pickup di lokasi · GPS share · COD cap 1-2% · asuransi 0.3% nilai" className={inputCls} /></Field>
        ...
        <button type="submit" disabled={submitting} className="w-full rounded-full bg-brand text-bg px-6 py-3.5 text-[14px] font-extrabold uppercase tracking-wider disabled:opacity-60">
          {submitting ? 'Registering…' : 'Create parcel courier profile'}
        </button>
      </form>
    </div>
  )
...
function humaniseError(code?: string): string {
  switch (code) {
    ...
    case 'at_least_one_specialty':return `Pick min 1, max ${MAX_PARCEL_SPECIALTIES} vehicle + service.`
    case 'at_least_one_price':    return 'Enter at least one rate — anchor price atau paket bulk.'
    case 'slug_collision':        return 'Name already taken — try a variation.'
    case 'already_registered':    return 'You already have a parcel courier profile.'
