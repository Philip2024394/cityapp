...
import {
  ...
  type ParcelProviderPublic, type ParcelSpecialty,
} from '@/lib/parcel/types'

// ============================================================================
// /parcel — Marketplace ported from the shared carwash/tailor/florist/cake shell.
...
// Mirrors /florist + /cake + /catering + /photo + /barber + /handyman + /tattoo + /pet + /tailor + /carwash
...
function effectiveAvailability(p: ParcelProviderPublic): 'online' | 'busy' | 'offline' {
...
type ChipDef = { id: 'all' | ParcelSpecialty; label: string }
const PRIMARY_CHIPS: ChipDef[] = [
  { id: 'all',              label: 'All'        },
  { id: 'motor',            label: 'Motor'      },
  { id: 'pickup_van',       label: 'Pickup'     },
  { id: 'box_cdd',          label: 'Box CDD'    },
  { id: 'same_day',         label: 'Same-Day'   },
  { id: 'instant_60min',    label: 'Instant'    },
  { id: 'antar_kota',       label: 'Antar Kota' },
]
const PRIMARY_IDS = new Set<ParcelSpecialty>(['motor','pickup_van','box_cdd','same_day','instant_60min','antar_kota'])
const SECONDARY_IDS: ParcelSpecialty[] = ALL_SPECIALTIES.filter((s) => !PRIMARY_IDS.has(s))

export default function ParcelMarketplacePage() {
  return (
...
function MarketplaceInner() {
  ...
  const t       = useTranslations('verticals.parcel')
  const tCommon = useTranslations('verticals.common')
  const haptic    = useHaptic()
  const search    = useSearchParams()
  const cityLabel = search?.get('city')?.trim() || 'Yogyakarta'

  const [providers, setProviders] = useState<ParcelProviderPublic[]>([])
  const [query, setQuery]         = useState('')
  const [specialty, setSpecialty] = useState<ParcelSpecialty | null>(null)
  ...
  const load = useCallback(async () => {
    ...
    const r = await fetch(`/api/parcel/marketplace${qs.toString() ? `?${qs}` : ''}`, { cache: 'no-store' })
    const j = await r.json() as { providers: ParcelProviderPublic[] }
          ...
          <div className="flex flex-col p-4 sm:p-5">
            ...
            <div className="mt-3 relative shrink-0">
              <input
                ...
                placeholder="Cari kurir antar barang — nama, kota, jenis layanan…"
            ...
            <div
              ...
              role="tablist"
              aria-label="Filter parcel courier by vehicle / service level"
            >
                ...
                <div className="py-10 text-center">
                  <p className="text-[13px] font-bold text-[#71717A] leading-snug">
                    {query.trim()
                      ...
                      : 'No couriers in this category yet. Try a different filter.'}
                  </p>
                </div>
              )}
            ...
            <p className="mt-3 text-center text-[12px] text-[#52525B] font-bold leading-snug px-2 shrink-0">
              Paketmu, sampai tepat waktu. Book direct via WhatsApp.
              Kita2u takes no commission, you pay your courier directly.
            </p>
          </div>
        </div>
      </div>
...
// ─────────────────────────────────────────────────────────────────────────────
// ParcelRowCard — landscape card, 112px tall to match /florist + /cake + /pet + /carwash.
...
// ─────────────────────────────────────────────────────────────────────────────
function ParcelRowCard({
  provider: p,
  onOpen,
}: {
  provider: ParcelProviderPublic
  onOpen: () => void
}) {
  ...
  // the kurir industry) then bulk paket then years of experience.
...
// ─────────────────────────────────────────────────────────────────────────────
// Shell — wordmark header matching /florist + /cake + /catering + /pet + /tailor + /carwash.
// ─────────────────────────────────────────────────────────────────────────────
