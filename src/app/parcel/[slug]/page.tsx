...
import {
  ALL_SPECIALTIES,
  SPECIALTY_LABELS,
  type ParcelProviderPublic,
  type ParcelSpecialty,
...
// Default theme accent — used when the kurir hasn't picked their own
// theme_color (mig 0087). Parcel businesses choose their accent from the
// dashboard color palette; the chosen hex flows through every accent
// surface on this page via the `theme` constant below. Default urgent
// orange (#EA580C) so the fast-handover / urgency energy reads, distinct
...
// /parcel/[slug] — universal profile flagship build. Mirrors the
// /carwash/[slug] page byte-for-byte at the JSX layer; only the data
// source swaps from carwash_providers to parcel_providers and a few
...
export default function ParcelProviderPage() {
  ...
  const [activeService, setActiveService] = useState<ParcelSpecialty | null>(null)
  ...
  // available when the kurir has opted into a physical pool / depo).
  ...
  useEffect(() => {
    ...
    const url = `/api/parcel/${encodeURIComponent(slug)}/public${
      draftPassword ? `?p=${encodeURIComponent(draftPassword)}` : ''
    }`
    fetch(url, { cache: 'no-store' })
      .then((r) => r.ok ? r.json() : null)
      .then((j: { provider?: ParcelProviderPublic; locked?: boolean } | null) => {
  ...
  useProfileViewTracker({ providerType: 'parcel', providerId: p?.id })

  // Resolved accent color for every accent surface on this page.
  // p.theme_color (mig 0087) wins; fall back to global default urgent orange.
  ...
  useEffect(() => {
    ...
    fetch(`/api/reviews?provider_type=parcel&provider_id=${encodeURIComponent(p.id)}`, { cache: 'no-store' })
      .then((r) => r.ok ? r.json() : null)
    ...
    return (
      <Shell>
        <div className="px-4 pt-20 max-w-md mx-auto text-center">
          <h1 className="text-[20px] font-black mb-2">Parcel courier not found</h1>
          <Link href="/parcel" className="rounded-full bg-brand text-bg px-6 py-3 text-[13px] font-extrabold inline-block">Back to marketplace</Link>
        </div>
      </Shell>
    )
  }
    ...
    return (
      <Shell>
        <DraftPasswordGate
          ...
          fetchUrl={(pw) => `/api/parcel/${encodeURIComponent(slug)}/public?p=${encodeURIComponent(pw)}`}
          onUnlocked={(pw, prov) => {
            setDraftPassword(pw)
            setLocked(false)
            setP(prov as ParcelProviderPublic)
          }}
        />
      </Shell>
    )
  ...
  const profileUrl = `${siteOrigin}/parcel/${p.slug}`

  // WhatsApp prefill text for the under-carousel contact button.
  const waText = [
    `Halo ${p.display_name}, saya menemukan profile Anda di Kita2u.`,
    `Halo, saya tertarik jasa kurir antar barang — boleh minta quote dan slot pickup?`,
  ...
  return (
    <Shell>
      ...
      <div className="relative pb-2">
        ...
        >
          <img
            src={bannerSrc(p.cover_image_url) || DEFAULT_PARCEL_HERO}
            alt=""
          ...
          {(() => {
            ...
            const line1   = ht.line1   || 'Kurir Antar Barang'
            const line2   = ht.line2   || 'Same-Day · Instant 60 Menit · Antar Kota · COD'
            const tagline = ht.tagline || 'Paketmu, sampai tepat waktu.'
            ...
            return (
              <div className={`absolute left-4 z-10 select-none leading-none cr-hero-${effect}`} style={{ top: 31 }}>
                ...
                {/* Service locations the kurir delivers to — Pickup at
                    Lokasi and Hotel/Office Drop are the everyday pickup
                    contexts for indie kurir (pickup di lokasi pengirim
                    / drop-off di counter). A missing or null
                    service_locations array still hides the whole row to
                    avoid implying locations the kurir doesn't serve. */}
                {(() => {
                  ...
                  if (locs.has('home'))  items.push({ key: 'home',  icon: Home,  label: 'Pickup' })
      ...
      <div className="px-4 pb-6 max-w-2xl mx-auto space-y-3 pt-3">
        ...
        {/* Vehicles & Services — only renders specialties that
        ...
        {(() => {
          const offered = (p.specialties ?? []) as ParcelSpecialty[]
        ...
        {/* Armada Kurir — rich cards (image + name + 2-line desc
            + start price + View Details). Cards swipe left-to-right.
            Kurir sell on vehicle-tier + service-level (clients pick the
            kurir whose vehicle + speed matches their parcel), so the
            header reads "Armada Kurir" (Courier Fleet). */}
        {(() => {
          ...
          return (
            <section className="space-y-2">
              <div className="flex items-center justify-between gap-2">
                <h2 className="text-[13px] font-extrabold uppercase tracking-wider text-black">
                  {activeService
                    ? `${SPECIALTY_LABELS[activeService]} — Armada Kurir`
                    : 'Armada Kurir'}
                </h2>
                <PortfolioViewToggle
                  view={portfolioView}
        ...
        <RunningMarquee
          text={p.promo_text || 'Paketmu, sampai tepat waktu — motor dalam kota Rp 15k base. Pickup van antar kota Jateng Rp 250k base. Instant 60-menit garansi Rp 35k. Box CDD e-commerce bulk Rp 350k. GPS share real-time, COD tersedia (cap 1-2%), asuransi paket berharga 0.3% nilai.'}
        />
      ...
      {/* Right-edge "back" bar — tall vertical strip flush against the
          ...
          vertical "BACK" text below. Diverts back to /parcel. */}
      <Link
        href="/parcel"
        aria-label="Back to Kita2u parcel"
      ...
      {/* Service Snapshot "View Details" popup — full image + sample
      ...
      {/* Contact / booking popup — opened by both the bottom Contact
          ...
          request server-side first (so it shows up on the kurir's
          calendar) and then opens WhatsApp with a matching pre-filled
          message. Skips busy_dates the kurir has marked. */}
      {contactOpen && p.whatsapp_e164 && (
        <ContactBookingPopup
          ...
          providerId={p.id}
          intentVertical="parcel"
          intentSource="parcel_profile"
          themeColor={theme}
          ...
          busyDates={(p.busy_dates ?? []) as string[]}
          bookEndpoint={`/api/parcel/${p.slug}/book`}
      ...
      <PoweredByKita2u defaultVertical="parcel" />
    </Shell>
  )
}
...
}: { sid: ParcelSpecialty; active: boolean; onClick: () => void; theme: string }) {
  return (
    <button
      type="button"
      onClick={onClick}
...
// empty. Parcel floor is motor dalam kota base ~Rp 15k typical job.
function formatStartFromPrice(p: ParcelProviderPublic): string {
...
// Normalises one service_photos entry — parcel uses a flat array of
...
// Combines kurir photos into the carousel feed. When a type filter is
...
function buildPortfolioPhotos(
  p: ParcelProviderPublic,
  active: ParcelSpecialty | null,
): PortfolioPhoto[] {
  ...
  // Partial-match against the type label — works when the kurir named
