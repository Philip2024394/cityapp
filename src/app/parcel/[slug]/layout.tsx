...
// Server-side SEO + lapsed redirect for /parcel/[slug].
// Mirrors /carwash/[slug] / /tailor/[slug] / /pet/[slug] / /florist/[slug] / /cake/[slug] / etc.
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  if (!p) return { title: 'Parcel courier not found', robots: { index: false, follow: false } }
  const pkg = p.hourly_rate_idr ?? null
  const title       = `${p.display_name} · Parcel ${p.city || 'Indonesia'}`
  const description = (p.bio?.trim())
    || `Book ${p.display_name} (parcel courier) in ${p.city || 'Indonesia'} via WhatsApp.${pkg ? ` Mulai dari Rp ${pkg.toLocaleString('id-ID')}.` : ''}`
  const canonical   = `${SITE_URL}/parcel/${p.slug}`
...
export default async function ParcelProviderLayout({
  children, params,
  ...
  if (p) {
    ...
    if (lapsed) {
      const qs = new URLSearchParams({ from: 'lapsed_parcel', slug: p.slug })
      redirect(`/parcel?${qs.toString()}`)
    }
  }
  const jsonLd = p
    ? {
        '@context': 'https://schema.org', '@type': 'LocalBusiness',
        '@id': `${SITE_URL}/parcel/${p.slug}`,
        name: p.display_name, description: p.bio || undefined,
        url: `${SITE_URL}/parcel/${p.slug}`,
