...
import {
  ...
  type TutoringProviderPublic, type TutoringSpecialty,
} from '@/lib/tutoring/types'

// ============================================================================
// /tutoring — Marketplace ported from the shared florist/cake/catering shell.
...
function effectiveAvailability(p: TutoringProviderPublic): 'online' | 'busy' | 'offline' {
...
type ChipDef = { id: 'all' | TutoringSpecialty; label: string }
const PRIMARY_CHIPS: ChipDef[] = [
  { id: 'all',           label: 'All'        },
  { id: 'math',          label: 'Matematika' },
  { id: 'physics',       label: 'Fisika'     },
  { id: 'english',       label: 'English'    },
  { id: 'utbk_sbmptn',   label: 'UTBK'       },
  { id: 'mengaji',       label: 'Mengaji'    },
]
const PRIMARY_IDS = new Set<TutoringSpecialty>(['math','physics','english','utbk_sbmptn','mengaji'])
const SECONDARY_IDS: TutoringSpecialty[] = ALL_SPECIALTIES.filter((s) => !PRIMARY_IDS.has(s))
...
function MarketplaceInner() {
  ...
  const t       = useTranslations('verticals.tutoring')
  const tCommon = useTranslations('verticals.common')
  const haptic    = useHaptic()
  const search    = useSearchParams()
  const cityLabel = search?.get('city')?.trim() || 'Yogyakarta'

  const [providers, setProviders] = useState<TutoringProviderPublic[]>([])
  const [query, setQuery]         = useState('')
  const [specialty, setSpecialty] = useState<TutoringSpecialty | null>(null)
  ...
  const load = useCallback(async () => {
    ...
    const r = await fetch(`/api/tutoring/marketplace${qs.toString() ? `?${qs}` : ''}`, { cache: 'no-store' })
    const j = await r.json() as { providers: TutoringProviderPublic[] }
          ...
          <div className="flex flex-col p-4 sm:p-5">
            ...
            <div className="mt-3 relative shrink-0">
              <input
                ...
                placeholder="Cari tutor — nama, kota, mata pelajaran…"
                ...
                <div className="py-10 text-center">
                  <p className="text-[13px] font-bold text-[#71717A] leading-snug">
                    {query.trim()
                      ...
                      : 'No tutors in this subject yet. Try a different filter.'}
                  </p>
                </div>
              )}
            ...
            <p className="mt-3 text-center text-[12px] text-[#52525B] font-bold leading-snug px-2 shrink-0">
              Higher grades. Real understanding. Homework done. Book direct via WhatsApp.
              Kita2u takes no commission, you pay your tutor directly.
            </p>
          </div>
        </div>
      </div>
...
// ─────────────────────────────────────────────────────────────────────────────
// TutoringRowCard — landscape card, 112px tall to match /florist + /cake.
...
// ─────────────────────────────────────────────────────────────────────────────
function TutoringRowCard({
  provider: p,
  onOpen,
}: {
  provider: TutoringProviderPublic
  onOpen: () => void
}) {
