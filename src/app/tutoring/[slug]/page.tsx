...
import {
  ALL_SPECIALTIES,
  SPECIALTY_LABELS,
  type TutoringProviderPublic,
  type TutoringSpecialty,
...
// Default theme accent — used when the tutor hasn't picked
...
// /tutoring/[slug] — universal profile flagship build. Mirrors the
// /florist/[slug] page byte-for-byte at the JSX layer; only the data
// source swaps from florist_providers to tutoring_providers and a few
...
// Vertical default for the hero. Used when the tutor hasn't
...
const DEFAULT_TUTORING_HERO =
  'https://images.unsplash.com/photo-1503676260728-1c00da094a0b?w=1200&auto=format&fit=crop'

export default function TutoringProviderPage() {
  const params = useParams<{ slug: string }>()
  const slug = String(params?.slug || '').toLowerCase()
  const [p, setP] = useState<TutoringProviderPublic | null>(null)
  ...
  const [activeService, setActiveService] = useState<TutoringSpecialty | null>(null)
  ...
  // available when the tutor has opted into a physical studio).
  ...
  useEffect(() => {
    ...
    const url = `/api/tutoring/${encodeURIComponent(slug)}/public${
      draftPassword ? `?p=${encodeURIComponent(draftPassword)}` : ''
    }`
    fetch(url, { cache: 'no-store' })
      .then((r) => r.ok ? r.json() : null)
      .then((j: { provider?: TutoringProviderPublic; locked?: boolean } | null) => {
  ...
  useProfileViewTracker({ providerType: 'tutoring', providerId: p?.id })
  ...
  useEffect(() => {
    ...
    fetch(`/api/reviews?provider_type=tutoring&provider_id=${encodeURIComponent(p.id)}`, { cache: 'no-store' })
      .then((r) => r.ok ? r.json() : null)
    ...
    return (
      <Shell>
        <div className="px-4 pt-20 max-w-md mx-auto text-center">
          <h1 className="text-[20px] font-black mb-2">Tutor not found</h1>
          <Link href="/tutoring" className="rounded-full bg-brand text-bg px-6 py-3 text-[13px] font-extrabold inline-block">Back to marketplace</Link>
        </div>
      </Shell>
    )
  }
    ...
    return (
      <Shell>
        <DraftPasswordGate
          ...
          fetchUrl={(pw) => `/api/tutoring/${encodeURIComponent(slug)}/public?p=${encodeURIComponent(pw)}`}
          onUnlocked={(pw, prov) => {
            setDraftPassword(pw)
            setLocked(false)
            setP(prov as TutoringProviderPublic)
          }}
        />
      </Shell>
    )
  ...
  const profileUrl = `${siteOrigin}/tutoring/${p.slug}`
  ...
  return (
    <Shell>
      ...
      <div className="relative pb-2">
        ...
        >
          <img
            src={bannerSrc(p.cover_image_url) || DEFAULT_TUTORING_HERO}
            alt=""
          ...
          {(() => {
            ...
            const line1   = ht.line1   || 'Tutor'
            const line2   = ht.line2   || 'Matematika · Fisika · UTBK · English'
            const tagline = ht.tagline || 'Nilai naik. Konsep paham. Tugas selesai.'
            ...
            return (
              <div className={`absolute left-4 z-10 select-none leading-none cr-hero-${effect}`} style={{ top: 31 }}>
                ...
                {/* Service locations the trainer delivers to — Home and
                    ...
                    tutors (home gym for clients, hotel gym
      ...
      <div className="px-4 pb-6 max-w-2xl mx-auto space-y-3 pt-3">
        ...
        {(() => {
          const offered = (p.specialties ?? []) as TutoringSpecialty[]
        ...
        {/* Belajar Bareng — rich cards (image + name + 2-line desc
            + start price + View Details). Cards swipe left-to-right.
            Tutors sell on subject specialisation + qualifikasi
            (clients pick the tutor whose track record fits target
            grade / exam), so the header reads "Belajar Bareng". */}
        {(() => {
          ...
          return (
            <section className="space-y-2">
              <div className="flex items-center justify-between gap-2">
                <h2 className="text-[13px] font-extrabold uppercase tracking-wider text-black">
                  {activeService
                    ? `${SPECIALTY_LABELS[activeService]} — Belajar Bareng`
                    : 'Belajar Bareng'}
                </h2>
                <PortfolioViewToggle
                  view={portfolioView}
        ...
        <RunningMarquee
          text={p.promo_text || 'Nilai naik. Konsep paham. Tugas selesai. Gratis trial 30 menit. Paket UTBK 12x hemat 15%. English online Rp 150k/sesi. Mengaji anak Rp 80k. Reservasi via WhatsApp.'}
        />
      ...
      {/* Right-edge "back" bar — tall vertical strip flush against the
          ...
          vertical "BACK" text below. Diverts back to /tutoring. */}
      <Link
        href="/tutoring"
        aria-label="Back to Kita2u tutors"
      ...
      {/* Contact / booking popup — opened by both the bottom Contact
          ...
          pre-filled message. Skips busy_dates the tutor
          has marked. */}
      {contactOpen && p.whatsapp_e164 && (
        <ContactBookingPopup
          ...
          providerId={p.id}
          intentVertical="tutoring"
          intentSource="tutoring_profile"
          themeColor={theme}
          ...
          busyDates={(p.busy_dates ?? []) as string[]}
          bookEndpoint={`/api/tutoring/${p.slug}/book`}
      ...
      <PoweredByKita2u defaultVertical="tutoring" />
    </Shell>
  )
}
...
}: { sid: TutoringSpecialty; active: boolean; onClick: () => void; theme: string }) {
  return (
    <button
      type="button"
      onClick={onClick}
...
// Cheapest price across per-pertemuan + package bundle + per-photo prices.
// Falls back to a sensible default when nothing is set so the CTA
// never reads empty. Tutoring floor is mengaji anak SD-SMP at
// ~Rp 80k per pertemuan 45-60 menit (SD math + mengaji are the
// cheapest baseline; SMA math/physics avg Rp 120k, UTBK Rp 200-300k).
function formatStartFromPrice(p: TutoringProviderPublic): string {
  ...
  if (candidates.length === 0) return 'Rp 80k'
  return formatPriceIdr(Math.min(...candidates)) ?? 'Rp 80k'
}

// Normalises one service_photos entry — tutoring uses a flat array of
...
// Combines tutor photos into the carousel feed. When a type
...
function buildPortfolioPhotos(
  p: TutoringProviderPublic,
  active: TutoringSpecialty | null,
): PortfolioPhoto[] {
