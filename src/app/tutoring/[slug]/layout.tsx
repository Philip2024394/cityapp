...
// Server-side SEO + lapsed redirect for /tutoring/[slug].
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  if (!p) return { title: 'Tutor not found', robots: { index: false, follow: false } }
  const pkg = p.hourly_rate_idr ?? null
  const title       = `${p.display_name} · Tutor ${p.city || 'Indonesia'}`
  const description = (p.bio?.trim())
    || `Book ${p.display_name} (tutor) in ${p.city || 'Indonesia'} via WhatsApp.${pkg ? ` Sessions from Rp ${pkg.toLocaleString('id-ID')}.` : ''}`
  const canonical   = `${SITE_URL}/tutoring/${p.slug}`
...
export default async function TutoringProviderLayout({
  children, params,
  ...
  if (p) {
    ...
    if (lapsed) {
      const qs = new URLSearchParams({ from: 'lapsed_tutoring', slug: p.slug })
  ...
  const jsonLd = p
    ? {
        ...
        '@id': `${SITE_URL}/tutoring/${p.slug}`,
        name: p.display_name, description: p.bio || undefined,
        url: `${SITE_URL}/tutoring/${p.slug}`,
