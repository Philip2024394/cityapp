...
import { ALL_SPECIALTIES, SPECIALTY_LABELS, MAX_TUTORING_SPECIALTIES, type TutoringSpecialty } from '@/lib/tutoring/types'

type AuthState =
  | { status: 'loading' } | { status: 'anon' }
  | { status: 'signedIn'; userId: string } | { status: 'alreadyProvider' }

export default function TutoringSignupPage() {
    ...
    supabase.auth.getSession().then(async ({ data }) => {
      ...
      try {
        ...
        if (r.ok) {
          ...
          if (j.provider) { setAuth({ status: 'alreadyProvider' }); setTimeout(() => router.replace('/dashboard/tutoring'), 400); return }
        }
      } catch { /* fall through */ }
  ...
  return (
    <div className="px-4 pt-12 max-w-md mx-auto">
      <div className="text-center mb-8">
        <div className="inline-block bg-brand text-bg text-[11px] font-extrabold uppercase tracking-[0.15em] px-3 py-1 rounded-full mb-4">Tutor</div>
        <h1 className="text-[26px] font-black leading-tight mb-3">List as a tutor</h1>
        <p className="text-[13px] text-ink/70 leading-relaxed">Create an account first — already have one? Login with the same.</p>
      </div>
      <div className="space-y-3">
        <Link href="/signup?intent=tutoring&next=/tutoring/signup" className="block w-full rounded-2xl bg-brand text-bg px-6 py-4 text-center text-[14px] font-extrabold uppercase tracking-wider hover:brightness-105">
          Create new account
        </Link>
        <Link href="/login?next=/tutoring/signup" className="block w-full rounded-2xl bg-white/[0.06] border border-ink/15 text-ink px-6 py-4 text-center text-[14px] font-extrabold uppercase tracking-wider hover:bg-white/10">
...
function Form({ userId }: { userId: string }) {
  ...
  const [f, setF] = useState({
    ...
    years_experience: 0,
    bio: '',
    specialties: [] as TutoringSpecialty[],
    hourly_rate: '',
    day_rate: '',
    has_own_tools: true,
    whatsapp_e164: '',
  ...
  function toggleSpecialty(s: TutoringSpecialty) {
    setF((p) => {
      if (p.specialties.includes(s)) {
        return { ...p, specialties: p.specialties.filter((x) => x !== s) }
      }
      if (p.specialties.length >= MAX_TUTORING_SPECIALTIES) return p // cap at 3
  ...
  async function submit(e: React.FormEvent) {
    e.preventDefault()
    if (f.specialties.length === 0) { setErr(`Pick min 1, max ${MAX_TUTORING_SPECIALTIES} style tutoring.`); return }
    const h = f.hourly_rate.trim()
    const d = f.day_rate.trim()
    if (h === '' && d === '') { setErr('Enter at least one rate — drop-in or monthly unlimited.'); return }
    setSubmitting(true); setErr(null)
    try {
      const r = await fetch('/api/tutoring/signup', {
      ...
      setDone(true); setTimeout(() => router.push('/dashboard/tutoring'), 1200)
  ...
  return (
    <div className="px-4 pt-6 pb-20 max-w-md mx-auto">
      ...
      <h1 className="text-[26px] font-black leading-tight mb-2">Tutor signup</h1>
      <p className="text-[13px] text-ink/70 mb-6">Pick max 3 mata pelajaran. Set per-pertemuan price dan/atau package bundle (8x / 12x / monthly intensive). Rp 38,000/month after a 7-day trial.</p>

      <form onSubmit={submit} className="space-y-4">
        <Field label="Tutor name *">
          <input required maxLength={80} value={f.display_name} onChange={(e) => upd('display_name', e.target.value)} placeholder="Pak Rahmat — Les Privat" className={inputCls} />
        </Field>
        ...
        <Field label="Bio (max 300) *">
          <textarea required maxLength={300} rows={3} value={f.bio} onChange={(e) => upd('bio', e.target.value)} placeholder="Qualifikasi (S1/S2/S3, jurusan, kampus, sertifikat, pengalaman tahun), mata pelajaran andalan (matematika / fisika / UTBK / english / mengaji), grade-level (SD/SMP/SMA/mahasiswa), format (datang ke rumah / studio / online via Zoom), kurikulum (Merdeka / Cambridge / IB), gratis trial 30 menit." className={inputCls + ' resize-none'} />
        </Field>

        <div className="rounded-2xl bg-black/85 border border-white/10 p-4">
          <div className="flex items-baseline justify-between mb-1">
            <div className="text-[13px] font-extrabold uppercase tracking-wider text-ink">Mata pelajaran *</div>
            <div className="text-[11px] text-ink/55">{f.specialties.length}/{MAX_TUTORING_SPECIALTIES}</div>
          </div>
          <p className="text-[11px] text-ink/55 mb-3">Pick max {MAX_TUTORING_SPECIALTIES} mata pelajaran andalan.</p>
          <div className="flex flex-wrap gap-1.5">
            {ALL_SPECIALTIES.map((s) => {
              const on = f.specialties.includes(s)
              const atLimit = !on && f.specialties.length >= MAX_TUTORING_SPECIALTIES
              return (
                <button
                  key={s} type="button"
                  disabled={atLimit}
        ...
        <div className="rounded-2xl bg-black/85 border border-white/10 p-4">
          ...
          <p className="text-[11px] text-ink/55 mb-3">Enter at least one — per-pertemuan single session (60-90 menit) atau package bundle (paket 8x / 12x / monthly intensive).</p>
          <div className="grid grid-cols-2 gap-2">
            <PriceField label="Per pertemuan from"   v={f.hourly_rate} set={(v) => upd('hourly_rate', v)} />
            <PriceField label="Paket bundle"          v={f.day_rate}    set={(v) => upd('day_rate', v)} />
          </div>
          <label className="flex items-center gap-2 mt-3 text-[12px] text-ink/85 cursor-pointer">
            ...
            <span>Datang ke rumah (home-visit available; selain online via Zoom + studio)</span>
          </label>
        </div>
        ...
        <Field label="Service area / catatan jangkauan"><input value={f.service_area_notes} onChange={(e) => upd('service_area_notes', e.target.value)} placeholder="Yogya · Sleman · Bantul · datang ke rumah / studio / online via Zoom" className={inputCls} /></Field>
        ...
        <button type="submit" disabled={submitting} className="w-full rounded-full bg-brand text-bg px-6 py-3.5 text-[14px] font-extrabold uppercase tracking-wider disabled:opacity-60">
          {submitting ? 'Registering…' : 'Create tutor profile'}
        </button>
      </form>
    </div>
  )
...
function humaniseError(code?: string): string {
  switch (code) {
    ...
    case 'at_least_one_specialty':return `Pick min 1, max ${MAX_TUTORING_SPECIALTIES} mata pelajaran.`
    case 'at_least_one_price':    return 'Enter at least one rate — per-pertemuan or package bundle.'
    case 'slug_collision':        return 'Name already taken — try a variation.'
    case 'already_registered':    return 'You already have a tutor profile.'
