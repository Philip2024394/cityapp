...
import {
  PHOTO_BANNER_LIBRARY,
  PHOTO_BANNER_CATEGORIES,
} from '@/lib/photo/banners'
import {
  type PhotoProvider,
  type PhotoSpecialty,
} from '@/lib/photo/types'

// WYSIWYG profile editor — photographer sees a live preview of their
...
// Photo brand defaults to deep charcoal slate — independent Indonesian
// photographers / studios trend dark/charcoal aesthetics so the work
// pops against the chrome. The CTA accent (button_text_color) defaults
// to white.
const DEFAULT_THEME = '#1F2937'
const DEFAULT_HERO = {
  line1:   'Professional',
  line2:   'Photographer',
  tagline: 'Wedding · Prewedding · Product · Editorial',
}
const DEFAULT_HERO_IMAGE = 'https://images.unsplash.com/photo-1542038784456-1ea8e935640e?w=1200&auto=format&fit=crop'

type PhotoHeroEffect = 'none' | 'shimmer' | 'dance' | 'underline'
...
type PhotoHeroText = {
  ...
  effect?:        PhotoHeroEffect
}

type PhotoServicePhotoEntry = {
  url:               string
  name?:             string | null
...
type Extras = {
  ...
  hero_text?:       PhotoHeroText | null
  promo_text?:      string | null
  // service_photos is a FLAT JSONB array on photo_providers (mig 0089),
  // unlike the beautician's keyed object. The photo editor uses
  // BarberServicePhotosEditor for this shape; this page only
  // reads/writes it via the existing profile dashboard for V1.
  service_photos?: PhotoServicePhotoEntry[] | null
  // Reflect the canonical column name even though the picker chips below
  // never edit specialties on this V1 page.
  specialties?: PhotoSpecialty[] | null
}
type FullProvider = PhotoProvider & Extras
...
export default function PhotoEditPage() {
  ...
  const reload = useCallback(async () => {
    ...
    if (!data?.session?.user) { router.replace('/login?next=/dashboard/photo/edit'); return }
    setLoading(true)
    try {
  ...
  async function save(patch: Partial<FullProvider>) {
    const r = await fetch('/api/photo/me/profile', {
      method: 'POST',
    ...
    return (
      <Shell>
        <div className="px-4 pt-20 max-w-md mx-auto text-center text-ink">
          <h1 className="text-[20px] font-black mb-2">Not a photographer yet</h1>
          <Link href="/photo/signup" className="rounded-full bg-brand text-bg px-6 py-3 text-[13px] font-extrabold inline-block">Register</Link>
        </div>
      </Shell>
    )
  }
  ...
  const effect: PhotoHeroEffect = (['none','shimmer','dance','underline'].includes(rawEffect) ? rawEffect : 'none') as PhotoHeroEffect
    ...
    <Shell>
      <div className="max-w-2xl mx-auto pt-3 pb-32 px-4">
        ...
        {/* Inline banner controls — sit RIGHT under the preview so the
            photographer can see what they're editing without opening a
        ...
        <p className="text-[12px] text-black/55 mt-4 leading-snug">
          ...
          {' '}<Link href="/dashboard/photo" className="text-yellow-700 hover:underline font-bold">full dashboard form</Link>{' '}
          for the rest.
        </p>
      </div>
    </Shell>
...
function BannerInlineControls({
  ...
  const [draftEffect,       setDraftEffect]       = useState<PhotoHeroEffect>(((['none','shimmer','dance','underline'].includes(ht.effect ?? 'none')) ? (ht.effect ?? 'none') : 'none') as PhotoHeroEffect)
  const [savedFlash,   setSavedFlash]   = useState(false)
  // "Follow theme" mode: true when the photographer hasn't explicitly
  // overridden the hero text color. In that mode, picking a new theme
  // color auto-recolors the Photographer word. The moment the
  // photographer picks a custom color from the swatches below, this
  ...
  const EFFECTS: Array<{ id: PhotoHeroEffect; label: string; desc: string }> = [
  ...
  return (
    <div className="mt-4 space-y-5">
      ...
      {/* Theme color — drives the entire page's accent (buttons, badges,
          star ratings, "Photographer" overlay default, etc.). Lives FIRST
          so photographers make the brand decision before fine-tuning.
          Picking a theme color ALSO clears any prior hero-text color
          override so the "Photographer" word follows the new theme. The
      ...
      <Section title="Banner image" icon={<ImageIcon size={16} strokeWidth={2.5} />}>
        <BannerLibraryPicker
          ...
          library={PHOTO_BANNER_LIBRARY}
          categories={PHOTO_BANNER_CATEGORIES}
      ...
      <Section title="Banner text" icon={<Type size={16} strokeWidth={2.5} />}>
        <div className="space-y-3">
          ...
          <FieldWithColor
            ...
            placeholder="Wedding · Prewedding · Editorial…"
      ...
      {/* Effect */}
      <Section title="Text effect (Photographer word)" icon={<Sparkles size={16} strokeWidth={2.5} />}>
      ...
      <Section title="QRIS payment" icon={<QrCodeIcon228 size={16} strokeWidth={2.5} />}>
        <QrisUploaderCard apiBase="/api/photo" accentHex={theme} />
      </Section>
      ...
      {/* Explicit Save button — auto-save runs in the background but this
          gives photographers a clear "I'm done" confirmation. Clicking
...
function PromoTextEditor({
  ...
  const display = (draft.trim() || 'Capture the moments that matter — turnaround 7 days, deposit 30%. Konsultasi paket via WhatsApp.') + ' ✦'
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="flex flex-wrap gap-1.5 pt-1">
        {[
          'Mini session Rp 800k — 30 menit, 20 foto edited',
          'Wedding package mulai Rp 4.5jt — booking 2 bulan dimuka',
          'Turnaround 7 hari — sample preview 48 jam',
          'Deposit 30% lock the date — sisanya saat handover',
          'Product shoot Rp 50k/foto — flat lay + lifestyle',
        ].map((s) => (
          <button
            key={s}
            type="button"
