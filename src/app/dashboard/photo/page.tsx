...
import {
  ALL_SPECIALTIES, SPECIALTY_LABELS, MAX_PHOTO_SPECIALTIES,
  type PhotoProvider, type PhotoAvailability, type PhotoSpecialty,
...
export default function PhotoDashboardPage() {
  const [p, setP] = useState<PhotoProvider | null>(null)
  ...
  const reload = useCallback(async () => {
    ...
    try {
      ...
      const j = await r.json() as { provider: PhotoProvider | null }
      setP(j.provider)
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const r = await fetch('/api/photo/me/analytics', { cache: 'no-store' })
        if (!r.ok) return
  ...
  async function setAvailability(next: PhotoAvailability) {
    if (!p) return
    const prev = p.availability
    setP({ ...p, availability: next })
    try {
      const r = await fetch('/api/photo/me/availability', {
  ...
  if (err === 'not_signed_in') return (
    <Shell><div className="px-4 pt-20 max-w-md mx-auto text-center">
      ...
      <Link href="/login?next=/dashboard/photo" className="rounded-full bg-brand text-bg px-6 py-3 text-[13px] font-extrabold inline-block">Sign in</Link>
    </div></Shell>
  )
  if (!p) return (
    <Shell><div className="px-4 pt-20 max-w-md mx-auto text-center">
      <h1 className="text-[20px] font-black mb-2">No photographer profile yet</h1>
      <Link href="/photo/signup" className="rounded-full bg-brand text-bg px-6 py-3 text-[13px] font-extrabold inline-block">Register as photographer</Link>
    </div></Shell>
  )
  ...
  return (
    <Shell>
      <div className="px-4 pt-6 pb-24 max-w-3xl mx-auto space-y-4">
        ...
        <ProviderRenewBanner provider={p} upgradeHref="/photo/upgrade" />
        ...
        {/* Quick-link to the dedicated Packages & portfolio editor. */}
        <Link
        ...
        >
          <div className="flex items-center justify-between gap-3">
            <div className="min-w-0">
              <div className="text-[14px] font-extrabold uppercase tracking-wider text-brand">Packages &amp; portfolio →</div>
              <div className="text-[13px] text-black mt-1">Pick genres, set package + full-day rates, and add portfolio photos that drive the carousel.</div>
            </div>
        ...
        <p className="text-[12px] text-black/60 text-center">
          Public profile: <a href={`/photo/${p.slug}`} target="_blank" rel="noopener" className="text-brand hover:underline">/photo/{p.slug}</a>
        </p>
      </div>
    </Shell>
  )
...
function ReadOnly({ p }: { p: PhotoProvider }) {
  return (
    <div className="space-y-3 text-[13px]">
      <KV k="Years" v={String(p.years_experience)} />
      <KV k="Bio" v={p.bio} multiline />
      <div>
        <div className="text-[11px] uppercase tracking-wider font-bold text-black/55 mb-1">Genres</div>
      ...
      <div className="grid grid-cols-2 gap-2 pt-2">
        {p.hourly_rate_idr != null && <PriceBox label="Package from" v={p.hourly_rate_idr} />}
        {p.day_rate_idr    != null && <PriceBox label="Full-day"     v={p.day_rate_idr}    />}
      </div>
      <KV k="Travels to client" v={p.has_own_tools ? 'Yes' : 'No'} />
...
function EditForm({ p, onSaved }: { p: PhotoProvider; onSaved: () => void }) {
  const pe = p as PhotoProvider & {
  ...
  function toggle(s: PhotoSpecialty) {
    setF((p) => {
      if (p.specialties.includes(s)) return { ...p, specialties: p.specialties.filter((x) => x !== s) }
      if (p.specialties.length >= MAX_PHOTO_SPECIALTIES) return p
    ...
    <form onSubmit={save} className="space-y-3">
      ...
      <div>
        <div className="flex items-baseline justify-between mb-1.5">
          <div className="text-[11px] uppercase tracking-wider font-bold text-black/55">Genres</div>
          <div className="text-[11px] text-black/55">{f.specialties.length}/{MAX_PHOTO_SPECIALTIES}</div>
        </div>
        <div className="flex flex-wrap gap-1.5">
      ...
      <div className="grid grid-cols-2 gap-2">
        <PriceInput label="Package from"   v={f.hourly_rate} set={(v) => upd('hourly_rate', v)} />
        <PriceInput label="Full-day shoot" v={f.day_rate}    set={(v) => upd('day_rate', v)} />
      </div>
      <label className="flex items-center gap-2 text-[12px] text-black/85 cursor-pointer">
        ...
        Travels to client (on-location shoots)
      </label>
