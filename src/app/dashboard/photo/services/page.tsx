...
import {
  ...
  type PhotoProvider, type PhotoSpecialty,
} from '@/lib/photo/types'

// Mirrors /dashboard/barber/services. Single page for everything the
// photographer sells:
//   1. Genres — max 3 they cover (wedding / prewedding / family / product
//      / food / editorial / etc.)
//   2. Pricing  — package starter price + optional full-day rate. The
//      photography industry quotes per-package (or per full-day), not
//      per-hour like most trades.
//   3. Portfolio photos with per-photo metadata (name, description,
//      package price, optional sample-shot pair) — drives the carousel +
//      View Details popup on /photo/[slug].
...
function ServicesForm({ provider, onSaved }: { provider: FullProvider; onSaved: () => void }) {
  ...
  function toggleSpecialty(sid: PhotoSpecialty) {
    const on = f.specialties.includes(sid)
    if (!on && f.specialties.length >= MAX_PHOTO_SPECIALTIES) return
  ...
  return (
    <form onSubmit={save} className="space-y-4">
      {/* 1. Genres */}
      <Card title="Packages I offer" hint={`Pick up to ${MAX_PHOTO_SPECIALTIES} genres — these become the Genres chips on your profile (${f.specialties.length}/${MAX_PHOTO_SPECIALTIES}).`}>
        <div className="flex flex-wrap gap-1.5">
          {ALL_SPECIALTIES.map((sid) => {
            const on    = f.specialties.includes(sid)
            const atCap = !on && f.specialties.length >= MAX_PHOTO_SPECIALTIES
            return (
              <button
                key={sid}
                type="button"
      ...
      {/* 2. Pricing */}
      <Card title="Rates" hint="Shown on the public profile. Type thousands — e.g. 800 = Rp 800k mini session, 4500 = Rp 4.5jt full-day wedding (max 9999).">
        ...
        <label className="flex items-center gap-2 text-[13px] text-gray-700 cursor-pointer mt-2">
          ...
          />
          Travels to client (on-location shoots, not studio-only)
        </label>
      </Card>

      {/* 3. Portfolio */}
      <Card title="Portfolio gallery" hint="Up to 12 finished images per genre with name, description, starting price, and optional sample-shot pair. These power the carousel + View Details popup on your profile.">
        {provider.user_id && (
