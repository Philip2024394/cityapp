...
import {
  CARWASH_BANNER_LIBRARY,
  CARWASH_BANNER_CATEGORIES,
} from '@/lib/carwash/banners'
import {
  type CarwashProvider,
  type CarwashSpecialty,
} from '@/lib/carwash/types'

// WYSIWYG profile editor — car wash sees a live preview of
// their public profile hero/banner section and taps inline controls to
...
// Carwash brand defaults to clean water blue — car wash signage trends
// bright blue + chrome for fresh / hydrophobic / professional energy.
// The CTA accent (button_text_color) defaults to white for high
// contrast on blue.
const DEFAULT_THEME = '#0284C7'
const DEFAULT_HERO = {
  line1:   'Cuci Mobil & Motor',
  line2:   'Cuci Mobil & Motor',
  tagline: 'Body · Detailing · Poles · Coating',
}
const DEFAULT_HERO_IMAGE = 'https://images.unsplash.com/photo-1607860108855-64acf2078ed9?w=1200&auto=format&fit=crop'

type CarwashHeroEffect = 'none' | 'shimmer' | 'dance' | 'underline'

type CarwashHeroText = {
  line1?:         string
  line2?:         string
  tagline?:       string
  color?:         string
  ...
  effect?:        CarwashHeroEffect
}

type CarwashServicePhotoEntry = {
  url:               string
  name?:             string | null
...
type Extras = {
  ...
  hero_text?:       CarwashHeroText | null
  promo_text?:      string | null
  // service_photos is a FLAT JSONB array on carwash_providers (mig 0089-
  // equiv), unlike the beautician's keyed object. This page only
  // reads/writes it via the existing profile dashboard for V1.
  service_photos?: CarwashServicePhotoEntry[] | null
  // Reflect the canonical column name even though the picker chips below
  // never edit specialties on this V1 page.
  specialties?: CarwashSpecialty[] | null
}
type FullProvider = CarwashProvider & Extras

export default function CarwashEditPage() {
  ...
  const reload = useCallback(async () => {
    ...
    if (!data?.session?.user) { router.replace('/login?next=/dashboard/car-wash/edit'); return }
    setLoading(true)
    try {
    ...
    return (
      <Shell>
        <div className="px-4 pt-20 max-w-md mx-auto text-center text-ink">
          <h1 className="text-[20px] font-black mb-2">Not a car wash business yet</h1>
          <Link href="/car-wash/signup" className="rounded-full bg-brand text-bg px-6 py-3 text-[13px] font-extrabold inline-block">Register</Link>
        </div>
      </Shell>
    )
  }
  ...
  const effect: CarwashHeroEffect = (['none','shimmer','dance','underline'].includes(rawEffect) ? rawEffect : 'none') as CarwashHeroEffect
    ...
    <Shell>
      <div className="max-w-2xl mx-auto pt-3 pb-32 px-4">
        ...
        {/* Inline banner controls — sit RIGHT under the preview so the
            car wash can see what they're editing without opening
            a modal. Auto-saves on change. */}
        ...
        <p className="text-[12px] text-black/55 mt-4 leading-snug">
          More inline-edit sections coming soon (catalog · theme · hours). For now use the
          {' '}<Link href="/dashboard/car-wash" className="text-yellow-700 hover:underline font-bold">full dashboard form</Link>{' '}
          for the rest.
        </p>
      </div>
    </Shell>
...
function BannerInlineControls({
  ...
  const [draftEffect,       setDraftEffect]       = useState<CarwashHeroEffect>(((['none','shimmer','dance','underline'].includes(ht.effect ?? 'none')) ? (ht.effect ?? 'none') : 'none') as CarwashHeroEffect)
  const [savedFlash,   setSavedFlash]   = useState(false)
  // "Follow theme" mode: true when the car wash hasn't
  // explicitly overridden the hero text color. In that mode, picking
  // a new theme color auto-recolors the hero main word. The
  // moment the car wash picks a custom color from the swatches below,
  // this flips to false and the color stays put even if the theme
  // changes.
  ...
  const EFFECTS: Array<{ id: CarwashHeroEffect; label: string; desc: string }> = [
    { id: 'none',      label: 'None',          desc: 'Static text' },
    { id: 'shimmer',   label: 'Shimmer',       desc: 'Light sweeps across — premium gloss' },
  ...
  return (
    <div className="mt-4 space-y-5">
      ...
      {/* Theme color — drives the entire page's accent (buttons, badges,
          star ratings, hero main-word default, etc.). Lives FIRST so
          car washes make the brand decision before fine-tuning.
          Picking a theme color ALSO clears any prior hero-text color
          override so the main word follows the new theme.
          The user can lock a custom color below if they want it different. */}
      ...
      <Section title="Banner image" icon={<ImageIcon size={16} strokeWidth={2.5} />}>
        <BannerLibraryPicker
          ...
          library={CARWASH_BANNER_LIBRARY}
          categories={CARWASH_BANNER_CATEGORIES}
      ...
      <Section title="Banner text" icon={<Type size={16} strokeWidth={2.5} />}>
        <div className="space-y-3">
          ...
          <FieldWithColor
            ...
            placeholder="Body · Detailing · Poles · Coating"
      ...
      {/* Effect */}
      <Section title="Text effect (Cuci Mobil & Motor word)" icon={<Sparkles size={16} strokeWidth={2.5} />}>
      ...
      {/* Running marquee text — the yellow ribbon that scrolls under the
          ...
          (carwash API cap, mig 0089-equiv). */}
      <Section title="Running text (marquee under hasil cucian)" icon={<Megaphone size={16} strokeWidth={2.5} />}>
        <PromoTextEditor
      ...
      <Section title="QRIS payment" icon={<QrCodeIcon228 size={16} strokeWidth={2.5} />}>
        <QrisUploaderCard apiBase="/api/carwash" accentHex={theme} />
      </Section>
      ...
      {/* Quick links — catalog manager + public preview. */}
      <Section title="More" icon={<MoreHorizontal size={16} strokeWidth={2.5} />}>
        <div className="grid grid-cols-2 gap-2">
          <Link
            href="/dashboard/car-wash/services"
      ...
      {/* Explicit Save button — auto-save runs in the background but this
          gives car washes a clear "I'm done" confirmation.
          Clicking flushes the pending hero_text payload immediately. */}
      <button
        type="button"
        onClick={async () => {
          const ok = await onSave({
...
function PromoTextEditor({
  ...
  const display = (draft.trim() || 'Mobil bersih, hari menyenangkan. Motor body Rp 15k express 15 menit. Mobil sedang body+dalam Rp 50k. Detailing premium SUV Rp 350k. Coating ceramic 9H garansi 1 tahun. Member card 10x cuci diskon. Panggilan ke rumah dalam kota.') + ' ✦'
  ...
  return (
    <div className="space-y-2">
      ...
      <textarea
        ...
        maxLength={280}
        rows={3}
        placeholder="Write your promo message — appears as scrolling text below your Hasil Cucian gallery."
      ...
      <p className="text-[12px] text-black/60 leading-snug">
        Use these for promos — member card 10x diskon, panggilan ke rumah, weekend express, ceramic coating bundle.
        Tap a suggestion to drop it in.
      </p>
      <div className="flex flex-wrap gap-1.5 pt-1">
        {[
          'Member Card 10x Cuci Diskon — bayar 9x dapat 10x cuci, hemat 10% per kunjungan, valid 6 bulan',
          'Panggilan ke Rumah dalam Kota — antar-jemput dalam kota Yogya, tambah Rp 30k, free untuk member card',
          'Weekend Express Promo — Sabtu/Minggu cuci motor body Rp 12k (turun dari Rp 15k), pagi 7-10 WIB',
          'Ceramic Coating Bundle — coating + 1x detailing + 3x cuci mobil, hemat Rp 500k vs harga terpisah',
          'Detailing Premium SUV — body + dalam + engine bay + poles + wax, 3-4 jam, clay-bar treatment include',
        ].map((s) => (
          <button
            key={s}
            type="button"
