...
import {
  ...
  type CarwashProvider, type CarwashAvailability, type CarwashSpecialty,
...
export default function CarwashDashboardPage() {
  const [p, setP] = useState<CarwashProvider | null>(null)
  ...
  const reload = useCallback(async () => {
    ...
    try {
      ...
      const j = await r.json() as { provider: CarwashProvider | null }
      setP(j.provider)
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const r = await fetch('/api/carwash/me/analytics', { cache: 'no-store' })
        if (!r.ok) return
  ...
  async function setAvailability(next: CarwashAvailability) {
    if (!p) return
    const prev = p.availability
    setP({ ...p, availability: next })
    try {
      const r = await fetch('/api/carwash/me/availability', {
  ...
  if (err === 'not_signed_in') return (
    <Shell><div className="px-4 pt-20 max-w-md mx-auto text-center">
      ...
      <Link href="/login?next=/dashboard/car-wash" className="rounded-full bg-brand text-bg px-6 py-3 text-[13px] font-extrabold inline-block">Sign in</Link>
    </div></Shell>
  )
  if (!p) return (
    <Shell><div className="px-4 pt-20 max-w-md mx-auto text-center">
      <h1 className="text-[20px] font-black mb-2">No car wash profile yet</h1>
      <Link href="/car-wash/signup" className="rounded-full bg-brand text-bg px-6 py-3 text-[13px] font-extrabold inline-block">Register car wash</Link>
    </div></Shell>
  )
  ...
  return (
    <Shell>
      <div className="px-4 pt-6 pb-24 max-w-3xl mx-auto space-y-4">
        ...
        <ProviderRenewBanner provider={p} upgradeHref="/car-wash/upgrade" />
        ...
        {/* Quick-link to the dedicated services catalog & portfolio editor. */}
        <Link
        ...
        >
          <div className="flex items-center justify-between gap-3">
            <div className="min-w-0">
              <div className="text-[14px] font-extrabold uppercase tracking-wider text-brand">Layanan & Tarif →</div>
              <div className="text-[13px] text-black mt-1">Pick vehicle + wash specialties (Motor / Mobil Kecil / Mobil Sedang / SUV / Pickup / MPV / Detailing / Coating), set anchor price + paket detailing/coating rate, and add Hasil Cucian photos that customers browse.</div>
            </div>
        ...
        <p className="text-[12px] text-black/60 text-center">
          Public profile: <a href={`/car-wash/${p.slug}`} target="_blank" rel="noopener" className="text-brand hover:underline">/car-wash/{p.slug}</a>
        </p>
      </div>
    </Shell>
  )
...
function ReadOnly({ p }: { p: CarwashProvider }) {
  return (
    <div className="space-y-3 text-[13px]">
      ...
      <div className="grid grid-cols-2 gap-2 pt-2">
        ...
        {p.day_rate_idr    != null && <PriceBox label="paket detailing/coating" v={p.day_rate_idr}    />}
      </div>
      <KV k="Datang ke Rumah" v={p.has_own_tools ? 'Yes' : 'No'} />
...
function EditForm({ p, onSaved }: { p: CarwashProvider; onSaved: () => void }) {
  const pe = p as CarwashProvider & {
  ...
  function toggle(s: CarwashSpecialty) {
    setF((p) => {
      if (p.specialties.includes(s)) return { ...p, specialties: p.specialties.filter((x) => x !== s) }
      if (p.specialties.length >= MAX_CARWASH_SPECIALTIES) return p
    ...
    <form onSubmit={save} className="space-y-3">
      ...
      <div>
        <div className="flex items-baseline justify-between mb-1.5">
          ...
          <div className="text-[11px] text-black/55">{f.specialties.length}/{MAX_CARWASH_SPECIALTIES}</div>
        </div>
        <div className="flex flex-wrap gap-1.5">
      ...
      <div className="grid grid-cols-2 gap-2">
        ...
        <PriceInput label="paket detailing/coating" v={f.day_rate}    set={(v) => upd('day_rate', v)} />
      </div>
