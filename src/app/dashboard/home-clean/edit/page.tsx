...
import {
  ...
  type HomeCleanProvider,
  type HomeCleanService,
  type HomeCleanServicePhoto,
  type HomeCleanHeroText,
  type HomeCleanHeroEffect,
} from '@/lib/home-clean/types'

// WYSIWYG profile editor — home-clean provider sees a live preview of
// their public profile hero/banner section and taps an Edit pencil to
// open a focused modal for that section. Modal changes auto-preview on
// the underlying hero before saving so they see the result first.
//
...
const DEFAULT_HERO = {
  line1:   'Professional',
  line2:   'Home Clean',
  tagline: 'Sparkling homes, effortless living',
}
const DEFAULT_HERO_IMAGE = 'https://ik.imagekit.io/nepgaxllc/ChatGPT%20Image%20May%2025,%202026,%2006_53_11%20AM.png'

// Home-clean banner library — empty until a curated set is shipped.
// BannerLibraryPicker accepts the empty map and renders the upload tile.
const HOME_CLEAN_BANNER_LIBRARY = {}
...
type FullProvider = HomeCleanProvider & Extras

export default function HomeCleanEditPage() {
  ...
  const reload = useCallback(async () => {
    ...
    if (!data?.session?.user) { router.replace('/login?next=/dashboard/home-clean/edit'); return }
    setLoading(true)
    try {
    ...
    return (
      <Shell>
        <div className="px-4 pt-20 max-w-md mx-auto text-center text-ink">
          <h1 className="text-[20px] font-black mb-2">Not a home-clean provider yet</h1>
          <Link href="/home-clean/signup" className="rounded-full bg-brand text-bg px-6 py-3 text-[13px] font-extrabold inline-block">Register</Link>
        </div>
      </Shell>
    )
  }
  ...
  const effect: HomeCleanHeroEffect = (['none','shimmer','dance','underline'].includes(rawEffect) ? rawEffect : 'none') as HomeCleanHeroEffect
  const cover  = provider.cover_image_url || DEFAULT_HERO_IMAGE

  return (
    <Shell>
      <div className="max-w-2xl mx-auto pt-4 pb-32 px-4">
        {/* Brand header — cyan-tinted strip with sparkle icon, matching the
            hub's polished feel. Auto-save badge lives on the right. */}
        <div className="rounded-3xl border border-cyan-200/70 bg-gradient-to-br from-cyan-50 to-white p-5 shadow-sm mb-4">
          <div className="flex items-center gap-3">
            <div className="w-12 h-12 rounded-2xl bg-cyan-500 text-white flex items-center justify-center shadow-sm shrink-0">
              <Sparkles size={22} strokeWidth={2.5} />
            </div>
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-0.5">
                <h1 className="text-[20px] font-black leading-tight text-black truncate">Design Studio</h1>
                <span className="inline-flex items-center gap-1 text-[10.5px] font-extrabold uppercase tracking-wider text-cyan-700 bg-cyan-100 border border-cyan-200 rounded-full px-2 py-0.5">
                  Live
                </span>
              </div>
        ...
        {/* Inline banner controls — sit RIGHT under the preview so the
            provider can see what they're editing without opening a
        ...
        <p className="text-[12px] text-black/55 mt-4 leading-snug">
          ...
          {' '}<Link href="/dashboard/home-clean" className="text-cyan-600 hover:underline font-bold">full dashboard form</Link>{' '}
          for the rest.
        </p>
      </div>
    </Shell>
...
function BannerInlineControls({
  ...
  const [draftEffect,       setDraftEffect]       = useState<HomeCleanHeroEffect>(((['none','shimmer','glow','underline'].includes(ht.effect ?? 'none')) ? (ht.effect ?? 'none') : 'none') as HomeCleanHeroEffect)
  const [savedFlash,   setSavedFlash]   = useState(false)
  // "Follow theme" mode: true when the provider hasn't explicitly
  // overridden the hero text color. In that mode, picking a new theme
  // color auto-recolors the Home Clean word. The moment the provider
  ...
  const EFFECTS: Array<{ id: HomeCleanHeroEffect; label: string; desc: string }> = [
  ...
  return (
    <div className="mt-4 space-y-4">
      ...
      {/* Theme color — drives the entire page's accent (buttons, badges,
          star ratings, "Home Clean" overlay default, etc.). Lives FIRST
          so providers make the brand decision before fine-tuning.
          Picking a theme color ALSO clears any prior hero-text color
          override so the "Home Clean" word follows the new theme. The
      ...
      <Section title="Banner image" icon={<ImageIcon size={16} strokeWidth={2.5} />}>
        <BannerLibraryPicker
          ...
          library={HOME_CLEAN_BANNER_LIBRARY}
          categories={HOME_CLEAN_SERVICES_OFFERED.map((s) => ({ id: s.id, label: s.label }))}
        ...
        <div className="space-y-3">
          ...
          <FieldWithColor
            ...
            colorNote={colorFollowsTheme
              ...
              : <>Override locked. <button type="button" onClick={() => { setColorFollowsTheme(true); setDraftColor(theme) }} className="text-cyan-600 underline font-bold">Reset to theme</button></>
            }
          />
          <FieldWithColor
            label="Tagline" max={80}
            value={draftTagline} onChange={setDraftTagline}
            placeholder="Sparkling homes, effortless living…"
      ...
      {/* Effect */}
      <Section title="Text effect (Home Clean word)" icon={<Sparkles size={16} strokeWidth={2.5} />}>
            ...
            return (
              <button
                ...
                className={`text-left rounded-xl p-3 border transition active:scale-[0.98] ${
                  on ? 'bg-cyan-500 text-white border-cyan-500 shadow-[0_2px_10px_rgba(6,182,212,0.35)]' : 'bg-gray-50 text-black border-gray-200 hover:bg-gray-100'
                }`}
              >
      ...
      <Section title="More" icon={<MoreHorizontal size={16} strokeWidth={2.5} />}>
        <div className="grid grid-cols-2 gap-2">
          <Link
            href="/dashboard/home-clean/services"
            className="inline-flex items-center justify-center gap-1.5 rounded-xl bg-gray-50 border border-gray-200 text-black px-3 py-3 text-[12px] font-extrabold uppercase tracking-wider hover:bg-gray-100 hover:border-cyan-300 transition min-h-[44px]"
          >
            Services (carousel)
          </Link>
          <a
            href={`/home-clean/${provider.slug}`}
            target="_blank"
      ...
      {/* Explicit Save button — auto-save runs in the background but this
          gives providers a clear "I'm done" confirmation. Clicking
...
function PromoTextEditor({
  ...
  const display = (draft.trim() || 'Message me this week — exclusive promo on professional home cleaning delivered straight to your home, hotel or villa, in the comfort of your stay.') + ' ✦'
  ...
  return (
    <div className="space-y-2">
      ...
      <textarea
        ...
        className="w-full rounded-xl bg-white border border-gray-200 px-3 py-2.5 text-[13px] text-black placeholder:text-black/40 focus:outline-none focus:border-cyan-400 focus:ring-2 focus:ring-cyan-100 resize-none leading-snug"
      />
      <p className="text-[12px] text-black/60 leading-snug">
        Use this for special offers — move-in deep cleans, post-renovation
        specials, or seasonal discounts. Tap a suggestion to drop it in.
      </p>
      <div className="flex flex-wrap gap-1.5 pt-1">
        {[
          'Move-in deep clean — book early for relocation season',
          'Post-renovation cleanup — group of 3+ rooms gets 10% off',
          'Sofa & carpet refresh specials this month',
          'Free window wipe with any deep clean this week',
          'Hotel + villa mobile cleaning — Yogya area',
        ].map((s) => (
          <button
            key={s}
            type="button"
  ...
  return (
    <section className="rounded-3xl bg-white border border-gray-200 p-5 shadow-sm space-y-3">
      <div className="flex items-center gap-2 text-[12px] font-extrabold uppercase tracking-wider text-black/70">
        {icon && (
          <span className="w-7 h-7 rounded-lg bg-cyan-100 text-cyan-700 flex items-center justify-center shrink-0">
            {icon}
          </span>
        )}
        <span>{title}</span>
  ...
  return (
    <div className="space-y-2 rounded-xl bg-gray-50 border border-gray-200 p-3">
      ...
      <input
        ...
        className="w-full rounded-xl bg-white border border-gray-200 px-3 py-2.5 text-[14px] font-bold placeholder:text-black/35 placeholder:font-normal focus:outline-none focus:border-cyan-400 focus:ring-2 focus:ring-cyan-100"
