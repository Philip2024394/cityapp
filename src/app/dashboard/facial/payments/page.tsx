...
// /dashboard/facial/payments — vendor wires up their own Stripe or
...
export default function FacialPaymentsPage() {
  ...
  const load = useCallback(async () => {
    ...
    if (!data?.session?.user) { router.replace('/login?next=/dashboard/facial/payments'); return }
    setLoading(true)
    try {
      const r = await fetch('/api/facial/me/payments', { cache: 'no-store' })
      if (r.ok) {
  ...
  async function save(patch: Record<string, unknown>) {
    setSaving(true)
    try {
      const r = await fetch('/api/facial/me/payments', {
        method:  'POST',
...
function TestConnectionButton({ provider }: { provider: 'stripe' | 'midtrans' }) {
  ...
  async function run() {
    ...
    try {
      const r = await fetch('/api/facial/me/payments/test', {
        method:  'POST',
        headers: { 'Content-Type': 'application/json' },
