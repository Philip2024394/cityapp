...
import { HelpCircle, Check, Plus, X, ChevronUp, ChevronDown, Loader2 } from 'lucide-react'
import { getBrowserSupabase } from '@/lib/supabase/client'
import AppNav from '@/components/layout/AppNav'

// /dashboard/facial/faq — FAQ editor. Backed by faq_items (jsonb)
// + faq_enabled (boolean) on the universal /profile route. Debounced
// auto-save (1s after last edit) for the list; the master toggle saves
// instantly. We're keeping reorder simple with up/down buttons because
// the project does not bundle react-beautiful-dnd.
...
export default function FacialFaqPage() {
  ...
  const load = useCallback(async () => {
    ...
    if (!data?.session?.user) { router.replace('/login?next=/dashboard/facial/faq'); return }
    setLoading(true)
    try {
      const r = await fetch('/api/facial/me', { cache: 'no-store' })
      if (r.ok) {
        const j = await r.json() as { provider: ProviderRow | null }
        const list  = Array.isArray(j.provider?.faq_items) ? j.provider!.faq_items! : []
        const flag  = !!j.provider?.faq_enabled
        setItems(list)
        setEnabled(flag)
  ...
  async function commitItems(next: FAQItem[]) {
    ...
    try {
      const r = await fetch('/api/facial/me/profile', {
        ...
        body:    JSON.stringify({ faq_items: next }),
      })
      const j = await r.json().catch(() => ({}))
      if (!r.ok || !j?.ok) { alert(j?.error || 'Could not save.'); return }
  ...
  async function setEnabledNow(next: boolean) {
    ...
    try {
      const r = await fetch('/api/facial/me/profile', {
        method:  'POST',
        headers: { 'Content-Type': 'application/json' },
        body:    JSON.stringify({ faq_enabled: next }),
      })
      const j = await r.json().catch(() => ({}))
      if (!r.ok || !j?.ok) {
        setEnabled(prev)
      ...
      <div className="max-w-2xl mx-auto px-4 pt-4 pb-32">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-0.5">
                <h1 className="text-[20px] font-black leading-tight text-black truncate">FAQ</h1>
                <SavedBadge flash={savedFlash} saving={saving} />
              </div>
              <p className="text-[12.5px] text-black/70 leading-snug">
                Shown above the contact form on your profile. Helps customers self-serve.
              </p>
            </div>
          </div>
        </div>
        ...
        >
          ...
          <button
            ...
            className="w-full mt-3 inline-flex items-center justify-center gap-1.5 rounded-xl bg-pink-500 hover:bg-pink-600 text-white px-4 py-3 text-[13px] font-extrabold uppercase tracking-wider disabled:opacity-50 disabled:cursor-not-allowed transition min-h-[44px]"
          >
    ...
    <div className="relative rounded-2xl bg-gray-50 border border-gray-200 p-3 space-y-2">
      ...
      <div className="space-y-1">
        ...
        <input
          ...
          placeholder="What time can I book?"
          maxLength={Q_MAX}
          className="w-full rounded-xl bg-white border border-gray-200 px-3 py-2.5 text-[13px] font-bold text-black placeholder:text-black/40 placeholder:font-normal focus:outline-none focus:border-pink-400 focus:ring-2 focus:ring-pink-100 min-h-[44px]"
        />
      </div>
      ...
      <div className="space-y-1">
        ...
        <textarea
          ...
          placeholder="Most appointments available 9am–7pm. Message me on WhatsApp to confirm."
          maxLength={A_MAX}
          rows={3}
          className="w-full rounded-xl bg-white border border-gray-200 px-3 py-2.5 text-[13px] text-black placeholder:text-black/40 focus:outline-none focus:border-pink-400 focus:ring-2 focus:ring-pink-100 leading-snug resize-y"
          style={{ minHeight: 80 }}
        />
      </div>
    </div>
