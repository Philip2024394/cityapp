...
import { Sparkles, Check, Palette, Image as ImageIcon, Type, Megaphone, MoreHorizontal, Zap, UserCircle2 } from 'lucide-react'
...
import { FACIAL_BANNER_LIBRARY, FACIAL_BANNER_CATEGORIES } from '@/lib/facial/banners'
import {
  type FacialProvider,
  type FacialServiceOffered,
} from '@/lib/facial/types'

// WYSIWYG profile editor — facial provider sees a live preview of their
...
type FullProvider = FacialProvider & Extras

export default function FacialEditPage() {
  ...
  const reload = useCallback(async () => {
    ...
    if (!data?.session?.user) { router.replace('/login?next=/dashboard/facial/edit'); return }
    setLoading(true)
    try {
  ...
  async function save(patch: Partial<FullProvider>) {
    const r = await fetch('/api/facial/me/profile', {
      method: 'POST',
    ...
    return (
      <Shell>
        <div className="px-4 pt-20 max-w-md mx-auto text-center text-ink">
          <h1 className="text-[20px] font-black mb-2">Not a facial provider yet</h1>
          <Link href="/facial/signup" className="rounded-full bg-brand text-bg px-6 py-3 text-[13px] font-extrabold inline-block">Register</Link>
        </div>
      </Shell>
    )
  }
    ...
    <Shell>
      <div className="max-w-2xl mx-auto pt-4 pb-32 px-4">
        ...
        {/* Inline banner controls — sit RIGHT under the preview so the
            facial provider can see what they're editing without opening a
        ...
        <p className="text-[12px] text-black/55 mt-4 leading-snug">
          ...
          {' '}<Link href="/dashboard/facial" className="text-pink-600 hover:underline font-bold">full dashboard form</Link>{' '}
          for the rest.
        </p>
      </div>
    </Shell>
...
function BannerInlineControls({
  ...
  // "Follow theme" mode: true when the facial provider hasn't explicitly
  // overridden the hero text color. In that mode, picking a new theme
  // color auto-recolors the Facial word. The moment the provider
  ...
  // the provider taps a card, no debounce.
    ...
    <div className="mt-4 space-y-4">
      ...
      <Section title="Banner image" icon={<ImageIcon size={16} strokeWidth={2.5} />}>
        <BannerLibraryPicker
          ...
          library={FACIAL_BANNER_LIBRARY}
          categories={FACIAL_BANNER_CATEGORIES}
          defaultThemeHex="#EC4899"
          purchaseEndpoint="/api/facial/me/buy-banner"
      ...
      {/* Effect */}
      <Section title="Text effect (Facial word)" icon={<Sparkles size={16} strokeWidth={2.5} />}>
      ...
      {/* Avatar frame — animated ring around the public-profile avatar
          ...
          provider can see the effect without leaving the dashboard. */}
      ...
      <Section title="QRIS payment" icon={<QrCodeIcon228 size={16} strokeWidth={2.5} />}>
        <QrisUploaderCard apiBase="/api/facial" accentHex={theme} />
      </Section>
      ...
      <Section title="More" icon={<MoreHorizontal size={16} strokeWidth={2.5} />}>
        <div className="grid grid-cols-2 gap-2">
          <Link
            href="/dashboard/facial/services"
...
function PromoTextEditor({
  ...
  const display = (draft.trim() || 'Message me this week — exclusive promo on professional facial service delivered straight to your home, hotel or villa, in the comfort of your stay.') + ' ✦'
    ...
    <div className="space-y-2">
      ...
      <div className="flex flex-wrap gap-1.5 pt-1">
        {[
          ...
          'Graduation facial specials this month',
          'Free skin consultation with any facial booking this week',
...
// provider sees in the dashboard matches the live result exactly.
