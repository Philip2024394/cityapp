...
// /dashboard/facial/terms — long-form Terms editor. Backed by the
// universal /api/facial/me/profile validator (legal_terms column).
...
1. Services
${vendorName} provides facial services as described on this profile, by appointment.
...
export default function FacialTermsPage() {
  ...
  const load = useCallback(async () => {
    ...
    if (!data?.session?.user) { router.replace('/login?next=/dashboard/facial/terms'); return }
    setLoading(true)
    try {
