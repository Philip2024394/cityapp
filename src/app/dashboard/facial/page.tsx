...
// Facial dashboard hub — task-card layout for first-time users.
...
export default function FacialDashboardPage() {
  ...
  async function setAvailability(next: FacialAvailability) {
    ...
    try {
      const r = await fetch('/api/facial/me/availability', {
        method: 'POST',
  ...
  // Wire to keep linter happy — exposed via the StatusPulse + WeeklyHours
  // controls below for parity with the beautician dashboard.
    ...
    return (
      <Shell>
        <div className="px-4 pt-20 max-w-md mx-auto text-center">
          ...
          <Link href="/login?next=/dashboard/facial" className="rounded-full bg-pink-500 text-white px-6 py-3 text-[14px] font-extrabold inline-block">Sign in</Link>
        </div>
      </Shell>
    )
  }
  if (!provider) {
    return (
      <Shell>
        <div className="px-4 pt-20 max-w-md mx-auto text-center">
          <h1 className="text-[22px] font-black text-black mb-2">Not a facial provider yet</h1>
          <p className="text-[14px] text-black/70 mb-6">Register to start receiving WhatsApp bookings.</p>
          <Link href="/facial/signup" className="rounded-full bg-pink-500 text-white px-6 py-3 text-[14px] font-extrabold inline-block">Sign up</Link>
        </div>
      </Shell>
    )
  }
  ...
  const publicUrl    = typeof window !== 'undefined'
    ? `${window.location.origin}/facial/${provider.slug}`
    : `/facial/${provider.slug}`
      ...
      <div className="px-4 pt-4 pb-32 max-w-lg mx-auto space-y-4">
        ...
        <div className="space-y-3">
          ...
          <TaskCard
            ...
            description="Facial treatments — add photos and set prices"
            status={servicesDone ? 'done' : 'pending'}
          />
          <TaskCard
            href="/dashboard/facial/edit"
        ...
        <section className="rounded-3xl bg-white border border-gray-200 p-4 space-y-3">
          ...
          <WeeklyHoursEditor
            ...
            onChange={async (next) => {
              ...
              try {
                await fetch('/api/facial/me/profile', {
                  method:  'POST',
                  headers: { 'Content-Type': 'application/json' },
