'use client'
// ============================================================================
// /dashboard/bus — Minibus driver dashboard
// ----------------------------------------------------------------------------
...
//
// Minibus customers care most about SEAT CAPACITY and group/charter pricing
// (per-km AND daily charter — Rp 600K–1M typical Yogya). Bio + Pricing
// sections nudge drivers to publish both.
// ============================================================================
...
const ADMIN_WA_RENEW = `https://wa.me/${ADMIN_WHATSAPP_E164}?text=${encodeURIComponent(
  'Halo admin, saya mau bayar/renew langganan dashboard Minibus driver IndoCity (Rp 38.000/bulan).',
)}`
...
type LoadState =
  ...
  | { kind: 'ready'; row: MinibusDriverRow }
...
// Page
// ============================================================================
export default function MinibusDriverDashboardPage() {
  ...
  const reload = useCallback(async () => {
    ...
    const row = data as unknown as MinibusDriverRow
    if (row.vehicle_type !== 'minibus') {
      ...
      <Shell>
        <div className="px-4 pt-20 max-w-md mx-auto text-center">
          ...
          <p className="text-[13px] text-black/70 mb-6">
            This account is not registered as a Minibus / Bus driver. Sign up at <span className="font-mono">/signup/bus</span> with vehicle type Minibus.
          </p>
          <Link
            href="/signup/bus"
            className="rounded-full bg-brand text-bg px-6 py-3 text-[13px] font-extrabold inline-block min-h-[44px]"
  ...
  if (state.kind === 'wrong_type') {
    return (
      <Shell>
        <div className="px-4 pt-20 max-w-md mx-auto text-center">
          <h1 className="text-[20px] font-black mb-2">Not a minibus driver</h1>
          <p className="text-[13px] text-black/70 mb-6">
            This account is registered as <span className="font-mono font-bold">{state.type}</span>, not <span className="font-mono font-bold">minibus</span>.
            Sign up at <span className="font-mono">/signup/bus</span> with vehicle type Minibus, or visit your existing dashboard.
          </p>
...
// ============================================================================
// Dashboard — only mounts when we have a valid minibus-type drivers row
// ----------------------------------------------------------------------------
...
// ============================================================================
function Dashboard({ row, onReload }: { row: MinibusDriverRow; onReload: () => void }) {
      ...
      <div className="px-4 pt-6 pb-24 max-w-3xl mx-auto space-y-4">
        ...
        <header className="rounded-2xl bg-white border border-gray-200 p-5 shadow-sm">
          ...
          <div className="text-[13px] text-black/60">
            Minibus driver dashboard · IndoCity is software — you set every price below.
          </div>
        </header>
...
// Subscription banner
// ----------------------------------------------------------------------------
// "never" — yellow CTA banner, listing is unlisted from /bus marketplace.
...
function SubscriptionBanner({ sub, onPay }: { sub: SubStatus; onPay: () => void }) {
  ...
  const heading = isExpired
    ...
    : 'Welcome! Your Minibus driver listing is currently unlisted.'
  const body = isExpired
    ? `Renew (Rp ${SUBSCRIPTION_IDR.toLocaleString('id-ID')}/month) to keep your listing live in the public /bus marketplace.`
    : `Subscribe (Rp ${SUBSCRIPTION_IDR.toLocaleString('id-ID')}/month) to appear in the public /bus marketplace.`
...
// Availability
// ============================================================================
function AvailabilitySection({ row, onSaved }: { row: MinibusDriverRow; onSaved: () => void }) {
...
// ============================================================================
// Vehicle — Minibus customers book based on group capacity, so SEATS is the
// primary signal here. We surface it as a wide field with helper text.
// ============================================================================
function VehicleSection({ row, onSaved }: { row: MinibusDriverRow; onSaved: () => void }) {
  const { saving, toast, save } = useSectionSaver(onSaved)
  ...
  return (
    <SectionCard title="Vehicle">
      <p className="text-[13px] text-black/70 leading-snug">
        Group capacity matters most — make sure <strong>Seats</strong> is accurate so customers
        can book the right minibus for their group.
      </p>
      <form onSubmit={onSubmit} className="space-y-3">
        {/* Seats on its own row — primary signal for minibus shoppers. */}
        <label className={labelCls}>
          <span className={labelTextCls}>Seats (passenger capacity)</span>
          <input
            type="number"
            min={1}
            max={60}
            ...
            placeholder="e.g. 12 (HiAce) or 15 (Elf)"
          />
        </label>
        <div className="grid grid-cols-2 gap-2">
          ...
          <label className={labelCls}>
            ...
            <input className={inputCls} value={model} onChange={(e) => setModel(e.target.value)} placeholder="HiAce / Elf" />
          </label>
          <label className={labelCls}>
...
// ============================================================================
function PhotosSection({ row, onSaved }: { row: MinibusDriverRow; onSaved: () => void }) {
  const { saving, toast, save } = useSectionSaver(onSaved)
  ...
  return (
    <SectionCard title="Vehicle photos">
      <p className="text-[13px] text-black/70">
        Paste public image URLs (one per row) — include an interior shot so customers can see seating. File upload is coming soon.
      </p>
      <form onSubmit={onSubmit} className="space-y-3">
...
// ----------------------------------------------------------------------------
// Minibus drivers commonly publish BOTH a per-km rate AND a daily charter
// price (Yogya typical: Rp 600K–1M). We only store per-km here in Phase 1;
// the hint nudges drivers to mention daily charter in their bio.
// ============================================================================
function PricingSection({ row, onSaved }: { row: MinibusDriverRow; onSaved: () => void }) {
  const { saving, toast, save } = useSectionSaver(onSaved)
  ...
  return (
    <SectionCard title="Your published rates">
      ...
      <p className="text-[13px] text-black/70 leading-snug">
        Group / charter rates — drivers commonly publish per-km AND a daily charter price
        (Rp 600K–1M typical Yogya); you can mention daily rates in your bio.
      </p>
      <form onSubmit={onSubmit} className="space-y-3">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
          <label className={labelCls}>
            <span className={labelTextCls}>Price per km (Rp)</span>
            <input type="number" min={0} className={inputCls} value={perKm}   onChange={(e) => setPerKm(e.target.value)}   placeholder="8000" />
          </label>
          <label className={labelCls}>
            <span className={labelTextCls}>Minimum fee (Rp)</span>
            <input type="number" min={0} className={inputCls} value={minFee}  onChange={(e) => setMinFee(e.target.value)}  placeholder="75000" />
          </label>
          <label className={labelCls}>
            <span className={labelTextCls}>Pit stop fee (Rp)</span>
            <input type="number" min={0} className={inputCls} value={pitstop} onChange={(e) => setPitstop(e.target.value)} placeholder="10000" />
          </label>
        </div>
        <div className="flex items-center justify-between gap-3">
          {toast ? <Toast kind={toast.kind}>{toast.msg}</Toast> : <span />}
...
// Payment methods
// ============================================================================
function PaymentMethodsSection({ row, onSaved }: { row: MinibusDriverRow; onSaved: () => void }) {
  const { saving, toast, save } = useSectionSaver(onSaved)
...
// ============================================================================
function BusinessProfileSection({ row, onSaved }: { row: MinibusDriverRow; onSaved: () => void }) {
  const { saving, toast, save } = useSectionSaver(onSaved)
    ...
    <SectionCard title="Business profile">
      <form onSubmit={onSubmit} className="space-y-3">
        <label className={labelCls}>
          ...
          <input className={inputCls} value={name} onChange={(e) => setName(e.target.value)} placeholder="Budi Minibus Yogya" />
        </label>
        <label className={labelCls}>
          <span className={labelTextCls}>Bio (mention daily charter rates if you offer them)</span>
          <textarea
            rows={3}
            maxLength={400}
            className={inputCls + ' resize-none'}
            ...
            placeholder="HiAce 12 kursi, ber-AC. Sewa harian Rp 800K (dalam kota), antar wisatawan keliling Yogya…"
          />
        </label>
        <label className={labelCls}>
      ...
      >
        ...
        <div className="px-5 pt-6 pb-5">
          ...
          <p className="text-[13px] text-black/65 mt-1 leading-snug">
            Pay Rp {SUBSCRIPTION_IDR.toLocaleString('id-ID')} to keep your minibus listing live for 30 days.
          </p>
...
// ============================================================================
// Shell — page chrome (matches /dashboard/car + /dashboard/handyman + /dashboard/massage)
// ============================================================================
