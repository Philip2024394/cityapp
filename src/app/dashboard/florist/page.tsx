...
import {
  ...
  type FloristProvider, type FloristAvailability, type FloristSpecialty,
...
export default function FloristDashboardPage() {
  const [p, setP] = useState<FloristProvider | null>(null)
  ...
  const reload = useCallback(async () => {
    ...
    try {
      ...
      const j = await r.json() as { provider: FloristProvider | null }
      setP(j.provider)
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const r = await fetch('/api/florist/me/analytics', { cache: 'no-store' })
        if (!r.ok) return
  ...
  async function setAvailability(next: FloristAvailability) {
    if (!p) return
    const prev = p.availability
    setP({ ...p, availability: next })
    try {
      const r = await fetch('/api/florist/me/availability', {
  ...
  if (err === 'not_signed_in') return (
    <Shell><div className="px-4 pt-20 max-w-md mx-auto text-center">
      ...
      <Link href="/login?next=/dashboard/florist" className="rounded-full bg-brand text-bg px-6 py-3 text-[13px] font-extrabold inline-block">Sign in</Link>
    </div></Shell>
  )
  if (!p) return (
    <Shell><div className="px-4 pt-20 max-w-md mx-auto text-center">
      <h1 className="text-[20px] font-black mb-2">No florist business profile yet</h1>
      <Link href="/florist/signup" className="rounded-full bg-brand text-bg px-6 py-3 text-[13px] font-extrabold inline-block">Register as florist business</Link>
    </div></Shell>
  )
  ...
  return (
    <Shell>
      <div className="px-4 pt-6 pb-24 max-w-3xl mx-auto space-y-4">
        ...
        <ProviderRenewBanner provider={p} upgradeHref="/florist/upgrade" />
        ...
        {/* Quick-link to the dedicated Arrangement catalog & portfolio editor. */}
        <Link
        ...
        >
          <div className="flex items-center justify-between gap-3">
            <div className="min-w-0">
              <div className="text-[14px] font-extrabold uppercase tracking-wider text-brand">Arrangement catalog &amp; portfolio →</div>
              <div className="text-[13px] text-black mt-1">Pick kategori (hand bouquet / standing flower / box arrangement), set per-arrangement + premium bundle, and add arrangement photos that drive the gallery.</div>
            </div>
        ...
        <p className="text-[12px] text-black/60 text-center">
          Public profile: <a href={`/florist/${p.slug}`} target="_blank" rel="noopener" className="text-brand hover:underline">/florist/{p.slug}</a>
        </p>
      </div>
    </Shell>
  )
...
function ReadOnly({ p }: { p: FloristProvider }) {
  return (
    <div className="space-y-3 text-[13px]">
      <KV k="Years" v={String(p.years_experience)} />
      <KV k="Bio" v={p.bio} multiline />
      <div>
        <div className="text-[11px] uppercase tracking-wider font-bold text-black/55 mb-1">Kategori bunga</div>
      ...
      <div className="grid grid-cols-2 gap-2 pt-2">
        {p.hourly_rate_idr != null && <PriceBox label="per-arrangement from" v={p.hourly_rate_idr} />}
        {p.day_rate_idr    != null && <PriceBox label="premium bundle"  v={p.day_rate_idr}    />}
      </div>
      <KV k="Delivery dalam kota" v={p.has_own_tools ? 'Yes' : 'No'} />
...
function EditForm({ p, onSaved }: { p: FloristProvider; onSaved: () => void }) {
  const pe = p as FloristProvider & {
  ...
  function toggle(s: FloristSpecialty) {
    setF((p) => {
      if (p.specialties.includes(s)) return { ...p, specialties: p.specialties.filter((x) => x !== s) }
      if (p.specialties.length >= MAX_FLORIST_SPECIALTIES) return p
    ...
    <form onSubmit={save} className="space-y-3">
      ...
      <div>
        <div className="flex items-baseline justify-between mb-1.5">
          <div className="text-[11px] uppercase tracking-wider font-bold text-black/55">Kategori bunga</div>
          <div className="text-[11px] text-black/55">{f.specialties.length}/{MAX_FLORIST_SPECIALTIES}</div>
        </div>
        <div className="flex flex-wrap gap-1.5">
      ...
      <div className="grid grid-cols-2 gap-2">
        <PriceInput label="per-arrangement from" v={f.hourly_rate} set={(v) => upd('hourly_rate', v)} />
        <PriceInput label="premium bundle"  v={f.day_rate}    set={(v) => upd('day_rate', v)} />
      </div>
