...
import {
  FLORIST_BANNER_LIBRARY,
  FLORIST_BANNER_CATEGORIES,
} from '@/lib/florist/banners'
import {
  type FloristProvider,
  type FloristSpecialty,
} from '@/lib/florist/types'

// WYSIWYG profile editor — florist business sees a live preview of
...
// Florist brand defaults to botanical green — florist aesthetic.
// Indonesian florist signage trends garden green (vs cake's bakery
// pink), reads as fresh / organic / leaf-and-stem and contrasts well
// with the photo-led bouquet imagery that dominates the IG feeds buyers
// browse. The CTA accent (button_text_color) defaults to white for
// high contrast on green.
const DEFAULT_THEME = '#16A34A'
const DEFAULT_HERO = {
  line1:   'Toko Bunga',
  line2:   'Florist business',
  tagline: 'Hand Bouquet · Standing · Box · Wisuda',
}
const DEFAULT_HERO_IMAGE = 'https://images.unsplash.com/photo-1561181286-d3fee7d55364?w=1200&auto=format&fit=crop'

type FloristHeroEffect = 'none' | 'shimmer' | 'dance' | 'underline'

type FloristHeroText = {
  line1?:         string
  line2?:         string
  tagline?:       string
  color?:         string
  ...
  effect?:        FloristHeroEffect
}

type FloristServicePhotoEntry = {
  url:               string
  name?:             string | null
...
export default function FloristEditPage() {
  ...
  const reload = useCallback(async () => {
    ...
    if (!data?.session?.user) { router.replace('/login?next=/dashboard/florist/edit'); return }
    setLoading(true)
    try {
    ...
    return (
      <Shell>
        <div className="px-4 pt-20 max-w-md mx-auto text-center text-ink">
          <h1 className="text-[20px] font-black mb-2">Not a florist business yet</h1>
          <Link href="/florist/signup" className="rounded-full bg-brand text-bg px-6 py-3 text-[13px] font-extrabold inline-block">Register</Link>
        </div>
      </Shell>
    )
  }
  ...
  const effect: FloristHeroEffect = (['none','shimmer','dance','underline'].includes(rawEffect) ? rawEffect : 'none') as FloristHeroEffect
    ...
    <Shell>
      <div className="max-w-2xl mx-auto pt-3 pb-32 px-4">
        ...
        {/* Inline banner controls — sit RIGHT under the preview so the
            florist business can see what they're editing without opening a
        ...
        <p className="text-[12px] text-black/55 mt-4 leading-snug">
          ...
          {' '}<Link href="/dashboard/florist" className="text-yellow-700 hover:underline font-bold">full dashboard form</Link>{' '}
          for the rest.
        </p>
      </div>
    </Shell>
...
function BannerInlineControls({
  ...
  const [draftEffect,       setDraftEffect]       = useState<FloristHeroEffect>(((['none','shimmer','dance','underline'].includes(ht.effect ?? 'none')) ? (ht.effect ?? 'none') : 'none') as FloristHeroEffect)
  const [savedFlash,   setSavedFlash]   = useState(false)
  // "Follow theme" mode: true when the florist business hasn't explicitly
  // overridden the hero text color. In that mode, picking a new theme
  // color auto-recolors the Florist business word. The moment the florist &
  // florist business picks a custom color from the swatches below, this
  ...
  const EFFECTS: Array<{ id: FloristHeroEffect; label: string; desc: string }> = [
  ...
  return (
    <div className="mt-4 space-y-5">
      ...
      {/* Theme color — drives the entire page's accent (buttons, badges,
          star ratings, "Florist business" overlay default, etc.). Lives FIRST
          so florist businesses make the brand decision before fine-tuning.
          Picking a theme color ALSO clears any prior hero-text color
          override so the "Florist business" word follows the new theme.
      ...
      <Section title="Banner image" icon={<ImageIcon size={16} strokeWidth={2.5} />}>
        <BannerLibraryPicker
          ...
          library={FLORIST_BANNER_LIBRARY}
          categories={FLORIST_BANNER_CATEGORIES}
      ...
      <Section title="Banner text" icon={<Type size={16} strokeWidth={2.5} />}>
        <div className="space-y-3">
          ...
          <FieldWithColor
            ...
            placeholder="Hand Bouquet · Standing · Box · Wisuda"
      ...
      {/* Effect */}
      <Section title="Text effect (Florist business word)" icon={<Sparkles size={16} strokeWidth={2.5} />}>
      ...
      {/* Running marquee text — the yellow ribbon that scrolls under the
          ...
          (florist API cap, mig 0089-equiv). */}
      <Section title="Running text (marquee under arrangement gallery)" icon={<Megaphone size={16} strokeWidth={2.5} />}>
        <PromoTextEditor
      ...
      <Section title="QRIS payment" icon={<QrCodeIcon228 size={16} strokeWidth={2.5} />}>
        <QrisUploaderCard apiBase="/api/florist" accentHex={theme} />
      </Section>
      ...
      <Section title="More" icon={<MoreHorizontal size={16} strokeWidth={2.5} />}>
        <div className="grid grid-cols-2 gap-2">
          <Link
            href="/dashboard/florist/services"
      ...
      {/* Explicit Save button — auto-save runs in the background but this
          gives florist businesses a clear "I'm done" confirmation.
...
function PromoTextEditor({
  ...
  const display = (draft.trim() || 'Bunga yang menyampaikan apa yang kata-kata tak bisa — same-day delivery sebelum jam 16:00, kartu ucapan custom gratis. Konsultasi via WhatsApp.') + ' ✦'
  ...
  return (
    <div className="space-y-2">
      ...
      <textarea
        ...
        maxLength={280}
        rows={3}
        placeholder="Write your promo message — appears as scrolling text below your arrangement gallery."
      ...
      <p className="text-[12px] text-black/60 leading-snug">
        Use this for promos — same-day delivery cutoff, hand bouquet wisuda, standing flower grand opening, valentine premium.
        Tap a suggestion to drop it in.
      </p>
      <div className="flex flex-wrap gap-1.5 pt-1">
        {[
          'Same-day delivery sebelum jam 16:00 — kartu ucapan custom gratis dalam kota',
          'Hand bouquet wisuda Rp 150k — 15 stem mawar import, packing premium',
          'Standing flower grand opening Rp 1.8jt — tinggi 1.8m, ucapan custom',
          'Valentine premium import Rp 850k — 33 mawar merah + box acrylic',
          'Box arrangement anniversary Rp 450k — mix bunga, tahan 7 hari',
        ].map((s) => (
          <button
            key={s}
            type="button"
