...
import {
  ...
  type TattooProvider, type TattooAvailability, type TattooSpecialty,
...
export default function TattooDashboardPage() {
  const [p, setP] = useState<TattooProvider | null>(null)
  ...
  const reload = useCallback(async () => {
    ...
    try {
      ...
      const j = await r.json() as { provider: TattooProvider | null }
      setP(j.provider)
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const r = await fetch('/api/tattoo/me/analytics', { cache: 'no-store' })
        if (!r.ok) return
  ...
  async function setAvailability(next: TattooAvailability) {
    if (!p) return
    const prev = p.availability
    setP({ ...p, availability: next })
    try {
      const r = await fetch('/api/tattoo/me/availability', {
  ...
  if (err === 'not_signed_in') return (
    <Shell><div className="px-4 pt-20 max-w-md mx-auto text-center">
      ...
      <Link href="/login?next=/dashboard/tattoo" className="rounded-full bg-brand text-bg px-6 py-3 text-[13px] font-extrabold inline-block">Sign in</Link>
    </div></Shell>
  )
  if (!p) return (
    <Shell><div className="px-4 pt-20 max-w-md mx-auto text-center">
      <h1 className="text-[20px] font-black mb-2">No tattoo profile yet</h1>
      <Link href="/tattoo/signup" className="rounded-full bg-brand text-bg px-6 py-3 text-[13px] font-extrabold inline-block">Register as tattoo artist</Link>
    </div></Shell>
  )
  ...
  return (
    <Shell>
      <div className="px-4 pt-6 pb-24 max-w-3xl mx-auto space-y-4">
        ...
        <ProviderRenewBanner provider={p} upgradeHref="/tattoo/upgrade" />
        ...
        <p className="text-[12px] text-black/60 text-center">
          Public profile: <a href={`/tattoo/${p.slug}`} target="_blank" rel="noopener" className="text-brand hover:underline">/tattoo/{p.slug}</a>
        </p>
      </div>
    </Shell>
  )
...
function ReadOnly({ p }: { p: TattooProvider }) {
  return (
...
function EditForm({ p, onSaved }: { p: TattooProvider; onSaved: () => void }) {
  const pe = p as TattooProvider & {
  ...
  function toggle(s: TattooSpecialty) {
    setF((p) => {
      if (p.specialties.includes(s)) return { ...p, specialties: p.specialties.filter((x) => x !== s) }
      if (p.specialties.length >= MAX_TATTOO_SPECIALTIES) return p
    ...
    <form onSubmit={save} className="space-y-3">
      ...
      <div>
        <div className="flex items-baseline justify-between mb-1.5">
          ...
          <div className="text-[11px] text-black/55">{f.specialties.length}/{MAX_TATTOO_SPECIALTIES}</div>
        </div>
        <div className="flex flex-wrap gap-1.5">
      ...
      <label className="flex items-center gap-2 text-[12px] text-black/85 cursor-pointer">
        ...
        Own kit (machine, single-use needles, sterilised)
      </label>
