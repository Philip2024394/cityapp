...
import {
  TATTOO_BANNER_LIBRARY,
  TATTOO_BANNER_CATEGORIES,
} from '@/lib/tattoo/banners'
import {
  type TattooProvider,
  type TattooSpecialty,
} from '@/lib/tattoo/types'

// WYSIWYG profile editor — tattoo sees a live preview of their
...
// Specialties, theme, etc. continue to be edited from the main dashboard
// form until they're lifted into this WYSIWYG view in follow-ups.

// Tattoo brand defaults to black — most tattoo studios run a dark
// aesthetic. The CTA accent (button_text_color) defaults to yellow ink.
const DEFAULT_THEME = '#0A0A0A'
const DEFAULT_HERO = {
  line1:   'Custom Ink',
  line2:   'Tattoo Artist',
  tagline: 'Custom designs · clean studio · book a consult',
}
const DEFAULT_HERO_IMAGE = 'https://images.unsplash.com/photo-1565058379802-bbe93b2f703a?w=1200&auto=format&fit=crop'

type TattooHeroEffect = 'none' | 'shimmer' | 'dance' | 'underline'
...
type TattooHeroText = {
  ...
  effect?:        TattooHeroEffect
}

type TattooServicePhotoEntry = {
  url:               string
  name?:             string | null
...
type Extras = {
  ...
  hero_text?:       TattooHeroText | null
  promo_text?:      string | null
  // service_photos is a FLAT JSONB array on tattoo_providers (mig 0089),
  // unlike the beautician's keyed object. The tattoo editor uses
  // TattooServicePhotosEditor for this shape; this page only
  // reads/writes it via the existing profile dashboard for V1.
  service_photos?: TattooServicePhotoEntry[] | null
  // Reflect the canonical column name even though the picker chips below
  // never edit specialties on this V1 page.
  specialties?: TattooSpecialty[] | null
}
type FullProvider = TattooProvider & Extras

export default function TattooEditPage() {
  ...
  const reload = useCallback(async () => {
    ...
    if (!data?.session?.user) { router.replace('/login?next=/dashboard/tattoo/edit'); return }
    setLoading(true)
    try {
    ...
    return (
      <Shell>
        <div className="px-4 pt-20 max-w-md mx-auto text-center text-ink">
          <h1 className="text-[20px] font-black mb-2">Not a tattoo yet</h1>
          <Link href="/tattoo/signup" className="rounded-full bg-brand text-bg px-6 py-3 text-[13px] font-extrabold inline-block">Register</Link>
        </div>
      </Shell>
    )
  }
  ...
  const effect: TattooHeroEffect = (['none','shimmer','dance','underline'].includes(rawEffect) ? rawEffect : 'none') as TattooHeroEffect
    ...
    <Shell>
      <div className="max-w-2xl mx-auto pt-3 pb-32 px-4">
        ...
        {/* Inline banner controls — sit RIGHT under the preview so the
            tattoo can see what they're editing without opening a
        ...
        <p className="text-[12px] text-black/55 mt-4 leading-snug">
          More inline-edit sections coming soon (specialties · theme · hours). For now use the
          {' '}<Link href="/dashboard/tattoo" className="text-yellow-700 hover:underline font-bold">full dashboard form</Link>{' '}
          for the rest.
        </p>
      </div>
    </Shell>
...
function BannerInlineControls({
  ...
  const [draftEffect,       setDraftEffect]       = useState<TattooHeroEffect>(((['none','shimmer','dance','underline'].includes(ht.effect ?? 'none')) ? (ht.effect ?? 'none') : 'none') as TattooHeroEffect)
  const [savedFlash,   setSavedFlash]   = useState(false)
  // "Follow theme" mode: true when the tattoo hasn't explicitly
  // overridden the hero text color. In that mode, picking a new theme
  // color auto-recolors the Tattoo word. The moment the tattoo
  ...
  const EFFECTS: Array<{ id: TattooHeroEffect; label: string; desc: string }> = [
  ...
  return (
    <div className="mt-4 space-y-5">
      ...
      {/* Theme color — drives the entire page's accent (buttons, badges,
          star ratings, "Tattoo" overlay default, etc.). Lives FIRST
          so handymen make the brand decision before fine-tuning.
          Picking a theme color ALSO clears any prior hero-text color
          override so the "Tattoo" word follows the new theme. The
      ...
      <Section title="Banner image" icon={<ImageIcon size={16} strokeWidth={2.5} />}>
        <BannerLibraryPicker
          ...
          library={TATTOO_BANNER_LIBRARY}
          categories={TATTOO_BANNER_CATEGORIES}
      ...
      <Section title="Banner text" icon={<Type size={16} strokeWidth={2.5} />}>
        <div className="space-y-3">
          ...
          <FieldWithColor
            ...
            placeholder="Custom tattoo design · book a consult…"
      ...
      {/* Effect */}
      <Section title="Text effect (Tattoo word)" icon={<Sparkles size={16} strokeWidth={2.5} />}>
      ...
      <Section title="QRIS payment" icon={<QrCodeIcon228 size={16} strokeWidth={2.5} />}>
        <QrisUploaderCard apiBase="/api/tattoo" accentHex={theme} />
      </Section>
      ...
      {/* Quick links — specialties manager (carousel) and public preview. */}
      <Section title="More" icon={<MoreHorizontal size={16} strokeWidth={2.5} />}>
        <div className="grid grid-cols-2 gap-2">
          <Link
            href="/dashboard/tattoo/services"
      ...
      {/* Explicit Save button — auto-save runs in the background but this
          gives handymen a clear "I'm done" confirmation. Clicking
...
function PromoTextEditor({
  ...
  const display = (draft.trim() || 'Booking custom design — clean studio, single-use needles, transparent quote. Message me on WhatsApp to start a consult.') + ' ✦'
  ...
  return (
    <div className="space-y-2">
      ...
      <p className="text-[12px] text-black/60 leading-snug">
        Use this for promos — flash sheet days, walk-in slots, or
        small-piece discounts. Tap a suggestion to drop it in.
      </p>
      <div className="flex flex-wrap gap-1.5 pt-1">
        {[
          'Flash day — small pieces dari 350k, walk-in welcome',
          'Custom design booking dibuka untuk bulan depan',
          'Fineline + minimalist specialist — konsultasi gratis',
          'Cover-up + rework ready — kirim foto dulu',
          'Studio bersih, single-use needles, certified',
        ].map((s) => (
          <button
            key={s}
            type="button"
