...
// WYSIWYG profile editor — yoga teacher sees a live preview of
...
// Yoga brand defaults to sky blue — sport / active energy. Reads as
...
type YogaHeroEffect = 'none' | 'shimmer' | 'dance' | 'underline'
...
type YogaHeroText = {
  ...
  effect?:        YogaHeroEffect
}

type YogaServicePhotoEntry = {
  url:               string
  name?:             string | null
...
export default function YogaEditPage() {
  ...
  const reload = useCallback(async () => {
    ...
    if (!data?.session?.user) { router.replace('/login?next=/dashboard/yoga/edit'); return }
    setLoading(true)
    try {
    ...
    return (
      <Shell>
        <div className="px-4 pt-20 max-w-md mx-auto text-center text-ink">
          <h1 className="text-[20px] font-black mb-2">Not a yoga teacher yet</h1>
          <Link href="/yoga/signup" className="rounded-full bg-brand text-bg px-6 py-3 text-[13px] font-extrabold inline-block">Register</Link>
        </div>
      </Shell>
    )
  }
  ...
  const effect: YogaHeroEffect = (['none','shimmer','dance','underline'].includes(rawEffect) ? rawEffect : 'none') as YogaHeroEffect
    ...
    <Shell>
      <div className="max-w-2xl mx-auto pt-3 pb-32 px-4">
        ...
        {/* Inline banner controls — sit RIGHT under the preview so the
            yoga teacher can see what they're editing without opening
        ...
        <p className="text-[12px] text-black/55 mt-4 leading-snug">
          ...
          {' '}<Link href="/dashboard/yoga" className="text-yellow-700 hover:underline font-bold">full dashboard form</Link>{' '}
          for the rest.
        </p>
      </div>
    </Shell>
...
function BannerInlineControls({
  ...
  const [draftEffect,       setDraftEffect]       = useState<YogaHeroEffect>(((['none','shimmer','dance','underline'].includes(ht.effect ?? 'none')) ? (ht.effect ?? 'none') : 'none') as YogaHeroEffect)
  const [savedFlash,   setSavedFlash]   = useState(false)
  // "Follow theme" mode: true when the yoga teacher hasn't
  // explicitly overridden the hero text color. In that mode, picking
  // a new theme color auto-recolors the Yoga Teacher word. The
  ...
  const EFFECTS: Array<{ id: YogaHeroEffect; label: string; desc: string }> = [
  ...
  return (
    <div className="mt-4 space-y-5">
      ...
      {/* Effect */}
      <Section title="Text effect (Yoga Teacher word)" icon={<Sparkles size={16} strokeWidth={2.5} />}>
      ...
      <Section title="QRIS payment" icon={<QrCodeIcon228 size={16} strokeWidth={2.5} />}>
        <QrisUploaderCard apiBase="/api/yoga" accentHex={theme} />
      </Section>
      ...
      {/* Explicit Save button — auto-save runs in the background but this
          gives yoga teachers a clear "I'm done" confirmation.
...
function PromoTextEditor({
  ...
  const display = (draft.trim() || 'Mat-mu sudah menunggu. Ikutilah. Trial class Rp 50k, 10-pack diskon 17%, monthly unlimited + 1 private session bonus. Mat & props disediakan. Reservasi via WhatsApp.') + ' ✦'
  ...
  return (
    <div className="space-y-2">
      ...
      <textarea
        ...
        maxLength={280}
        rows={3}
        placeholder="Write your promo message — appears as scrolling text below your class vibes."
      ...
      <p className="text-[12px] text-black/60 leading-snug">
        Use this for promos — free trial class, 10-pack discount, monthly unlimited, prenatal series.
        Tap a suggestion to drop it in.
      </p>
      <div className="flex flex-wrap gap-1.5 pt-1">
        {[
          'Free trial class Rp 50k untuk pemula — coba 1 kelas vinyasa atau yin sebelum komit',
          '10-pack studio class Rp 1jt — Rp 100k/kelas (hemat 17%), valid 3 bulan',
          'Monthly unlimited Rp 1.5jt — kelas tak terbatas + 1 private session bonus',
          'Prenatal series 6 kelas Rp 750k — program ibu hamil trimester 2-3, instruktur RYT-200 sertifikasi',
          'Private 1-on-1 di rumah Rp 350k — 60 menit, mat & props disediakan',
        ].map((s) => (
          <button
            key={s}
            type="button"
