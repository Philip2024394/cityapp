...
import {
  PARCEL_BANNER_LIBRARY,
  PARCEL_BANNER_CATEGORIES,
} from '@/lib/parcel/banners'
import {
  type ParcelProvider,
  type ParcelSpecialty,
} from '@/lib/parcel/types'

// WYSIWYG profile editor — kurir sees a live preview of their public
// profile hero/banner section and taps inline controls to edit. Modal
// changes auto-preview on the underlying hero before saving so they see
// the result first.
//
...
// Parcel brand defaults to urgent orange — kurir signage trends bright
// orange (JNE, J&T, GoSend, AnterAja, SiCepat) for fast-handover /
// urgency energy. The CTA accent (button_text_color) defaults to white
// for high contrast on orange.
const DEFAULT_THEME = '#EA580C'
const DEFAULT_HERO = {
  line1:   'Kurir Antar Barang',
  line2:   'Kurir Antar Barang',
  tagline: 'Same-Day · Instant 60 Menit · Antar Kota · COD',
}
const DEFAULT_HERO_IMAGE = 'https://images.unsplash.com/photo-1586528116311-ad8dd3c8310d?w=1200&auto=format&fit=crop'

type ParcelHeroEffect = 'none' | 'shimmer' | 'dance' | 'underline'
...
type ParcelHeroText = {
  ...
  effect?:        ParcelHeroEffect
}

type ParcelServicePhotoEntry = {
  url:               string
  name?:             string | null
...
export default function ParcelEditPage() {
  ...
  const reload = useCallback(async () => {
    ...
    if (!data?.session?.user) { router.replace('/login?next=/dashboard/parcel/edit'); return }
    setLoading(true)
    try {
    ...
    return (
      <Shell>
        <div className="px-4 pt-20 max-w-md mx-auto text-center text-ink">
          <h1 className="text-[20px] font-black mb-2">Not a parcel courier business yet</h1>
          <Link href="/parcel/signup" className="rounded-full bg-brand text-bg px-6 py-3 text-[13px] font-extrabold inline-block">Register</Link>
        </div>
      </Shell>
    )
  }
  ...
  const effect: ParcelHeroEffect = (['none','shimmer','dance','underline'].includes(rawEffect) ? rawEffect : 'none') as ParcelHeroEffect
    ...
    <Shell>
      <div className="max-w-2xl mx-auto pt-3 pb-32 px-4">
        ...
        {/* Inline banner controls — sit RIGHT under the preview so the
            kurir can see what they're editing without opening a modal.
        ...
        <p className="text-[12px] text-black/55 mt-4 leading-snug">
          ...
          {' '}<Link href="/dashboard/parcel" className="text-yellow-700 hover:underline font-bold">full dashboard form</Link>{' '}
          for the rest.
        </p>
      </div>
    </Shell>
...
function BannerInlineControls({
  ...
  const [draftEffect,       setDraftEffect]       = useState<ParcelHeroEffect>(((['none','shimmer','dance','underline'].includes(ht.effect ?? 'none')) ? (ht.effect ?? 'none') : 'none') as ParcelHeroEffect)
  const [savedFlash,   setSavedFlash]   = useState(false)
  // "Follow theme" mode: true when the kurir hasn't explicitly
  // overridden the hero text color. In that mode, picking a new theme
  // color auto-recolors the hero main word. The moment the kurir picks
  // a custom color from the swatches below, this flips to false and
  // the color stays put even if the theme changes.
  ...
  const EFFECTS: Array<{ id: ParcelHeroEffect; label: string; desc: string }> = [
  ...
  return (
    <div className="mt-4 space-y-5">
      ...
      {/* Theme color — drives the entire page's accent (buttons, badges,
          ...
          kurir make the brand decision before fine-tuning.
      ...
      <Section title="Banner image" icon={<ImageIcon size={16} strokeWidth={2.5} />}>
        <BannerLibraryPicker
          ...
          library={PARCEL_BANNER_LIBRARY}
          categories={PARCEL_BANNER_CATEGORIES}
      ...
      <Section title="Banner text" icon={<Type size={16} strokeWidth={2.5} />}>
        <div className="space-y-3">
          <FieldWithColor
            ...
            placeholder="Kurir Antar Barang"
          ...
          <FieldWithColor
            ...
            placeholder="Same-Day · Instant 60 Menit · Antar Kota · COD"
      ...
      {/* Effect */}
      <Section title="Text effect (Kurir Antar Barang word)" icon={<Sparkles size={16} strokeWidth={2.5} />}>
      ...
      {/* Running marquee text — the yellow ribbon that scrolls under the
          armada carousel on the public profile. Max 280 chars
          (parcel API cap, mig 0089-equiv). */}
      <Section title="Running text (marquee under armada kurir)" icon={<Megaphone size={16} strokeWidth={2.5} />}>
        <PromoTextEditor
      ...
      <Section title="QRIS payment" icon={<QrCodeIcon228 size={16} strokeWidth={2.5} />}>
        <QrisUploaderCard apiBase="/api/parcel" accentHex={theme} />
      </Section>
      ...
      <Section title="More" icon={<MoreHorizontal size={16} strokeWidth={2.5} />}>
        <div className="grid grid-cols-2 gap-2">
          <Link
            href="/dashboard/parcel/services"
      ...
      {/* Explicit Save button — auto-save runs in the background but this
          gives kurir a clear "I'm done" confirmation. Clicking flushes
...
function PromoTextEditor({
  ...
  const display = (draft.trim() || 'Paketmu, sampai tepat waktu. Motor dalam kota Rp 15k base. Pickup van antar kota Rp 250k. Instant 60-menit garansi Rp 35k. Box CDD e-commerce bulk Rp 350k. GPS share real-time. COD tersedia (cap 1-2%). Asuransi paket berharga 0.3% nilai.') + ' ✦'
  ...
  return (
    <div className="space-y-2">
      ...
      <textarea
        ...
        maxLength={280}
        rows={3}
        placeholder="Write your promo message — appears as scrolling text below your Armada Kurir gallery."
      ...
      <p className="text-[12px] text-black/60 leading-snug">
        Use these for promos — GPS share real-time, COD tersedia, asuransi paket, instant 60-menit garansi.
        Tap a suggestion to drop it in.
      </p>
      <div className="flex flex-wrap gap-1.5 pt-1">
        {[
          'GPS Share Real-Time — share lokasi kurir live ke pengirim & penerima, tracking sampai paket diterima',
          'COD Tersedia (cap 1-2%) — bayar di tempat untuk paket sampai Rp 5jt, fee 1-2% dari nilai barang',
          'Asuransi Paket Berharga — barang elektronik / dokumen penting, premi 0.3% dari nilai deklarasi',
          'Instant 60 Menit Garansi — pickup + antar dalam 60 menit dalam kota Yogya, lewat refund 50%',
          'E-commerce Return Support — pickup return seller online (Shopee / Tokopedia), batch loading',
        ].map((s) => (
          <button
            key={s}
            type="button"
