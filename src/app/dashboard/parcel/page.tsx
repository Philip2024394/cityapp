...
import {
  ...
  type ParcelProvider, type ParcelAvailability, type ParcelSpecialty,
...
export default function ParcelDashboardPage() {
  const [p, setP] = useState<ParcelProvider | null>(null)
  ...
  const reload = useCallback(async () => {
    ...
    try {
      ...
      const j = await r.json() as { provider: ParcelProvider | null }
      setP(j.provider)
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const r = await fetch('/api/parcel/me/analytics', { cache: 'no-store' })
        if (!r.ok) return
  ...
  async function setAvailability(next: ParcelAvailability) {
    if (!p) return
    const prev = p.availability
    setP({ ...p, availability: next })
    try {
      const r = await fetch('/api/parcel/me/availability', {
  ...
  if (err === 'not_signed_in') return (
    <Shell><div className="px-4 pt-20 max-w-md mx-auto text-center">
      ...
      <Link href="/login?next=/dashboard/parcel" className="rounded-full bg-brand text-bg px-6 py-3 text-[13px] font-extrabold inline-block">Sign in</Link>
    </div></Shell>
  )
  if (!p) return (
    <Shell><div className="px-4 pt-20 max-w-md mx-auto text-center">
      <h1 className="text-[20px] font-black mb-2">No parcel courier profile yet</h1>
      <Link href="/parcel/signup" className="rounded-full bg-brand text-bg px-6 py-3 text-[13px] font-extrabold inline-block">Register kurir</Link>
    </div></Shell>
  )
  ...
  return (
    <Shell>
      <div className="px-4 pt-6 pb-24 max-w-3xl mx-auto space-y-4">
        ...
        <ProviderRenewBanner provider={p} upgradeHref="/parcel/upgrade" />
        ...
        >
          <div className="flex items-center justify-between gap-3">
            <div className="min-w-0">
              <div className="text-[14px] font-extrabold uppercase tracking-wider text-brand">Tarif & Armada →</div>
              <div className="text-[13px] text-black mt-1">Pick vehicle + service specialties (Motor / Pickup Van / Box CDD / Sepeda / Same-Day / Instant 60-min / Antar Kota), set anchor price + paket bulk rate, and add Armada Kurir photos that customers browse.</div>
            </div>
        ...
        <p className="text-[12px] text-black/60 text-center">
          Public profile: <a href={`/parcel/${p.slug}`} target="_blank" rel="noopener" className="text-brand hover:underline">/parcel/{p.slug}</a>
        </p>
      </div>
    </Shell>
  )
...
function ReadOnly({ p }: { p: ParcelProvider }) {
  return (
    <div className="space-y-3 text-[13px]">
      ...
      <div className="grid grid-cols-2 gap-2 pt-2">
        ...
        {p.day_rate_idr    != null && <PriceBox label="paket bulk" v={p.day_rate_idr}    />}
      </div>
      <KV k="Pickup di Lokasi" v={p.has_own_tools ? 'Yes' : 'No'} />
...
function EditForm({ p, onSaved }: { p: ParcelProvider; onSaved: () => void }) {
  const pe = p as ParcelProvider & {
  ...
  function toggle(s: ParcelSpecialty) {
    setF((p) => {
      if (p.specialties.includes(s)) return { ...p, specialties: p.specialties.filter((x) => x !== s) }
      if (p.specialties.length >= MAX_PARCEL_SPECIALTIES) return p
    ...
    <form onSubmit={save} className="space-y-3">
      ...
      <div>
        <div className="flex items-baseline justify-between mb-1.5">
          ...
          <div className="text-[11px] text-black/55">{f.specialties.length}/{MAX_PARCEL_SPECIALTIES}</div>
        </div>
        <div className="flex flex-wrap gap-1.5">
      ...
      <div className="grid grid-cols-2 gap-2">
        ...
        <PriceInput label="paket bulk" v={f.day_rate}    set={(v) => upd('day_rate', v)} />
      </div>
