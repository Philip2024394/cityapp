...
import {
  BARBER_BANNER_LIBRARY,
  BARBER_BANNER_CATEGORIES,
} from '@/lib/barber/banners'
import {
  type BarberProvider,
  type BarberSpecialty,
} from '@/lib/barber/types'

// WYSIWYG profile editor — barber sees a live preview of their
...
// Barber brand defaults to deep blue — most independent Indonesian
// barbershops run a classic barber-pole / blue-and-white aesthetic.
// The CTA accent (button_text_color) defaults to white.
const DEFAULT_THEME = '#1E40AF'
const DEFAULT_HERO = {
  line1:   'Classic',
  line2:   'Barber',
  tagline: 'Classic cut · fade · beard trim · hot towel',
}
const DEFAULT_HERO_IMAGE = 'https://images.unsplash.com/photo-1503951914875-452162b0f3f1?w=1200&auto=format&fit=crop'

type BarberHeroEffect = 'none' | 'shimmer' | 'dance' | 'underline'
...
type BarberHeroText = {
  ...
  effect?:        BarberHeroEffect
}

type BarberServicePhotoEntry = {
  url:               string
  name?:             string | null
...
type Extras = {
  ...
  hero_text?:       BarberHeroText | null
  promo_text?:      string | null
  // service_photos is a FLAT JSONB array on barber_providers (mig 0089),
  // unlike the beautician's keyed object. The barber editor uses
  // BarberServicePhotosEditor for this shape; this page only
  // reads/writes it via the existing profile dashboard for V1.
  service_photos?: BarberServicePhotoEntry[] | null
  // Reflect the canonical column name even though the picker chips below
  // never edit specialties on this V1 page.
  specialties?: BarberSpecialty[] | null
}
...
export default function BarberEditPage() {
  ...
  const reload = useCallback(async () => {
    ...
    if (!data?.session?.user) { router.replace('/login?next=/dashboard/barber/edit'); return }
    setLoading(true)
    try {
    ...
    return (
      <Shell>
        <div className="px-4 pt-20 max-w-md mx-auto text-center text-ink">
          <h1 className="text-[20px] font-black mb-2">Not a barber yet</h1>
          <Link href="/barber/signup" className="rounded-full bg-brand text-bg px-6 py-3 text-[13px] font-extrabold inline-block">Register</Link>
        </div>
      </Shell>
    )
  }
  ...
  const effect: BarberHeroEffect = (['none','shimmer','dance','underline'].includes(rawEffect) ? rawEffect : 'none') as BarberHeroEffect
    ...
    <Shell>
      <div className="max-w-2xl mx-auto pt-3 pb-32 px-4">
        ...
        {/* Inline banner controls — sit RIGHT under the preview so the
            barber can see what they're editing without opening a
        ...
        <p className="text-[12px] text-black/55 mt-4 leading-snug">
          ...
          {' '}<Link href="/dashboard/barber" className="text-yellow-700 hover:underline font-bold">full dashboard form</Link>{' '}
          for the rest.
        </p>
      </div>
    </Shell>
...
function BannerInlineControls({
  ...
  const [draftEffect,       setDraftEffect]       = useState<BarberHeroEffect>(((['none','shimmer','dance','underline'].includes(ht.effect ?? 'none')) ? (ht.effect ?? 'none') : 'none') as BarberHeroEffect)
  const [savedFlash,   setSavedFlash]   = useState(false)
  // "Follow theme" mode: true when the barber hasn't explicitly
  // overridden the hero text color. In that mode, picking a new theme
  // color auto-recolors the Barber word. The moment the barber
  ...
  const EFFECTS: Array<{ id: BarberHeroEffect; label: string; desc: string }> = [
  ...
  return (
    <div className="mt-4 space-y-5">
      ...
      {/* Theme color — drives the entire page's accent (buttons, badges,
          star ratings, "Barber" overlay default, etc.). Lives FIRST so
          barbers make the brand decision before fine-tuning. Picking a
          theme color ALSO clears any prior hero-text color override so
          the "Barber" word follows the new theme. The user can lock a
      ...
      <Section title="Banner image" icon={<ImageIcon size={16} strokeWidth={2.5} />}>
        <BannerLibraryPicker
          ...
          library={BARBER_BANNER_LIBRARY}
          categories={BARBER_BANNER_CATEGORIES}
      ...
      <Section title="Banner text" icon={<Type size={16} strokeWidth={2.5} />}>
        <div className="space-y-3">
          ...
          <FieldWithColor
            ...
            placeholder="Classic cut · fade · beard trim…"
      ...
      {/* Effect */}
      <Section title="Text effect (Barber word)" icon={<Sparkles size={16} strokeWidth={2.5} />}>
      ...
      <Section title="QRIS payment" icon={<QrCodeIcon228 size={16} strokeWidth={2.5} />}>
        <QrisUploaderCard apiBase="/api/barber" accentHex={theme} />
      </Section>
      ...
      {/* Explicit Save button — auto-save runs in the background but this
          gives barbers a clear "I'm done" confirmation. Clicking flushes
...
function PromoTextEditor({
  ...
  const display = (draft.trim() || 'Walk-ins welcome — classic cut, fade, beard trim, hot towel. Book your slot on WhatsApp.') + ' ✦'
  ...
  return (
    <div className="space-y-2">
      ...
      <p className="text-[12px] text-black/60 leading-snug">
        Use this for promos — flash sale, walk-in slots, or combo discounts.
        Tap a suggestion to drop it in.
      </p>
      <div className="flex flex-wrap gap-1.5 pt-1">
        {[
          'Walk-in welcome — Rp 65k classic cut, Rp 100k combo',
          'Booking dibuka untuk minggu depan — slot terbatas',
          'Hot towel + beard trim free dengan combo bulan ini',
          'Kids cut Rp 35k — Senin–Jumat saja',
          'Chair bersih, clippers sanitised setiap customer',
        ].map((s) => (
          <button
            key={s}
            type="button"
