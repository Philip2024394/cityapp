...
import {
  ...
  type BarberProvider, type BarberAvailability, type BarberSpecialty,
...
export default function BarberDashboardPage() {
  const [p, setP] = useState<BarberProvider | null>(null)
  ...
  const reload = useCallback(async () => {
    ...
    try {
      ...
      const j = await r.json() as { provider: BarberProvider | null }
      setP(j.provider)
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const r = await fetch('/api/barber/me/analytics', { cache: 'no-store' })
        if (!r.ok) return
  ...
  async function setAvailability(next: BarberAvailability) {
    if (!p) return
    const prev = p.availability
    setP({ ...p, availability: next })
    try {
      const r = await fetch('/api/barber/me/availability', {
  ...
  if (err === 'not_signed_in') return (
    <Shell><div className="px-4 pt-20 max-w-md mx-auto text-center">
      ...
      <Link href="/login?next=/dashboard/barber" className="rounded-full bg-brand text-bg px-6 py-3 text-[13px] font-extrabold inline-block">Sign in</Link>
    </div></Shell>
  )
  if (!p) return (
    <Shell><div className="px-4 pt-20 max-w-md mx-auto text-center">
      <h1 className="text-[20px] font-black mb-2">No barber profile yet</h1>
      <Link href="/barber/signup" className="rounded-full bg-brand text-bg px-6 py-3 text-[13px] font-extrabold inline-block">Register as barber</Link>
    </div></Shell>
  )
  ...
  return (
    <Shell>
      <div className="px-4 pt-6 pb-24 max-w-3xl mx-auto space-y-4">
        ...
        <ProviderRenewBanner provider={p} upgradeHref="/barber/upgrade" />
        ...
        >
          <div className="flex items-center justify-between gap-3">
            <div className="min-w-0">
              ...
              <div className="text-[13px] text-black mt-1">Pick services, set per-cut + combo rates, and add before/after photos that drive the carousel.</div>
            </div>
        ...
        <p className="text-[12px] text-black/60 text-center">
          Public profile: <a href={`/barber/${p.slug}`} target="_blank" rel="noopener" className="text-brand hover:underline">/barber/{p.slug}</a>
        </p>
      </div>
    </Shell>
  )
...
function ReadOnly({ p }: { p: BarberProvider }) {
  return (
    <div className="space-y-3 text-[13px]">
      <KV k="Years" v={String(p.years_experience)} />
      <KV k="Bio" v={p.bio} multiline />
      <div>
        <div className="text-[11px] uppercase tracking-wider font-bold text-black/55 mb-1">Services</div>
      ...
      <div className="grid grid-cols-2 gap-2 pt-2">
        {p.hourly_rate_idr != null && <PriceBox label="Per cut" v={p.hourly_rate_idr} />}
        {p.day_rate_idr    != null && <PriceBox label="Combo"   v={p.day_rate_idr}    />}
      </div>
      <KV k="Own kit" v={p.has_own_tools ? 'Yes' : 'No'} />
...
function EditForm({ p, onSaved }: { p: BarberProvider; onSaved: () => void }) {
  const pe = p as BarberProvider & {
  ...
  function toggle(s: BarberSpecialty) {
    setF((p) => {
      if (p.specialties.includes(s)) return { ...p, specialties: p.specialties.filter((x) => x !== s) }
      if (p.specialties.length >= MAX_BARBER_SPECIALTIES) return p
    ...
    <form onSubmit={save} className="space-y-3">
      ...
      <div>
        <div className="flex items-baseline justify-between mb-1.5">
          <div className="text-[11px] uppercase tracking-wider font-bold text-black/55">Services</div>
          <div className="text-[11px] text-black/55">{f.specialties.length}/{MAX_BARBER_SPECIALTIES}</div>
        </div>
        <div className="flex flex-wrap gap-1.5">
      ...
      <div className="grid grid-cols-2 gap-2">
        <PriceInput label="Per cut"        v={f.hourly_rate} set={(v) => upd('hourly_rate', v)} />
        <PriceInput label="Combo cut+beard" v={f.day_rate}    set={(v) => upd('day_rate', v)} />
      </div>
      <label className="flex items-center gap-2 text-[12px] text-black/85 cursor-pointer">
        ...
        Own kit (clippers, razor, cape, sanitised)
      </label>
