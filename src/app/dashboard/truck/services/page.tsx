'use client'
// ============================================================================
// /dashboard/truck/services — Services & rates (Phase 1B real implementation)
// ----------------------------------------------------------------------------
// Cloned from /dashboard/car/services. Truck-specific notes:
//   - The HEADLINE section for trucks is RENTAL rates (daily/weekly/monthly).
//     Truck customers transact mostly on day-hire — point-to-point per-km
//     work is the exception, not the rule. So the section order is:
//       1. Service offerings chips
//       2. Rental rates (headline for trucks)
//       3. Per-km transport pricing (still relevant for tour / daily-hire-as-
//          tour scenarios)
//       4. Parcel B2B contracts
//   - The rental block always mounts (no `daily_hire` gating) so trucks see
//     it immediately — it's the primary money flow.
//   - Parcel B2B defaults use defaultsFor('car') for now — the
//     /lib/parcel/defaults.ts module only has 'bike' | 'car' variants.
//     TODO: add a 'truck' variant (likely higher tiers + lower capacity).
//
...
export default function TruckServicesPage() {
  ...
  if (state.kind === 'unauth')      return <FullMsg cta={{ href: '/login?next=/dashboard/truck/services', label: 'Sign in' }}>Sign in to edit your services.</FullMsg>
...
// ----------------------------------------------------------------------------
// Truck section ordering:
//   Offerings → Rental (headline) → Per-km pricing → Parcel B2B
// ============================================================================
...
function OfferingsCard({
  ...
  return (
    <Card
      title="What truck jobs do you take?"
      hint="Tick every job type you do. Customers see these badges on your profile."
      icon={<Layers size={18} />}
    >
...
// ============================================================================
// 2. Truck rental rates — HEADLINE section, always mounted for trucks
// ============================================================================
function RentalCard({
  row, onSave,
}: {
  ...
  const initialType: 'self_drive' | 'with_driver' | 'both' = row.rental_type ?? 'with_driver'
  ...
  useEffect(() => { setRType(row.rental_type ?? 'with_driver') },          [row.rental_type])
  ...
  return (
    <Card
      title="Truck rental rates"
      hint="The headline pricing for trucks. Set your daily, weekly, and monthly rates — customers agree terms directly with you."
      icon={<Truck size={18} />}
    >
      {/* Rental type picker */}
      <div>
...
// ============================================================================
// 3. Per-km transport pricing — secondary for trucks (tour / daily-hire-as-tour
// scenarios). Save on blur.
// ============================================================================
function PricingCard({
  ...
  return (
    <Card
      ...
      hint="Optional — for one-off trips and tour-style hires. Most truck jobs settle on the daily rental above."
...
// enabled, we surface them on /cityriders/parcel under truck drivers and
// display their self-published 5-tier volume rate card + daily capacity +
// service zone + outer-zone surcharge.
//
// TODO: /lib/parcel/defaults.ts currently only ships 'bike' | 'car' default
// presets. We reuse defaultsFor('car') here as the closest analogue until a
// dedicated 'truck' variant is added (likely higher tiers + lower capacity
// because trucks handle larger consolidated loads).
// ============================================================================
function ParcelB2BCard({
  ...
  const carDefaults = defaultsFor('car') // TODO: replace with defaultsFor('truck') once added
