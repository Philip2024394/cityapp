...
//
// URL is hardcoded to /truck/<slug> because this page lives under the
// truck dashboard — vehicle_type on the row is informational only here.
// (Car dashboard's QR page derives the path from vehicle_type because it
// historically served all three; truck has its own slice now.)
//
// Includes a CSS @media print A6 flier so the driver can hit Cmd-P to get
...
function publicPathFor(slug: string): string {
  // Truck dashboard always points at /truck/<slug>. We ignore vehicle_type
  // here because the surface is vertical-specific.
  return `/truck/${slug}`
}

export default function TruckQrPage() {
  ...
  // doesn't carry a localhost origin if the driver previews this on dev.
  ...
  async function downloadPng() {
    ...
    // Header text — driver name + scan-to-book line
    ctx.fillStyle    = INK
    ctx.textAlign    = 'center'
    ctx.textBaseline = 'middle'
    ctx.font         = '800 42px system-ui, -apple-system, "Segoe UI", Roboto, sans-serif'
    ctx.fillText(driver.business_name || 'Cityriders truck driver', SIZE / 2, HEADER_H / 2 - 14)
    ...
    a.href     = dataUrl
    a.download = `${driver.slug || 'truck'}-qr.png`
  ...
  function shareToWhatsApp() {
    ...
    const text = `Hubungi saya untuk pesan truk: ${url}`
  ...
  if (state.kind === 'no_driver')   return <FullPageMessage cta={{ href: '/signup?role=driver&vehicle=truck', label: 'Create driver profile' }}>No driver profile yet.</FullPageMessage>
  if (state.kind === 'no_slug')     return <FullPageMessage cta={{ href: '/dashboard/truck/info', label: 'Set up profile' }}>Set your public slug to generate a QR.</FullPageMessage>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[12.5px] font-bold opacity-80 leading-snug">
                Print on business cards, truck cabs, warehouse desks. Scan = direct to your profile.
              </p>
            </div>
          </div>
        </section>
    ...
    <div className="hidden print:block">
      ...
      <div className="cr-flier">
        <div className="cr-flier-head">
          <div className="cr-flier-brand">Truck driver</div>
          <div className="cr-flier-name">{driver.business_name || 'Cityriders truck driver'}</div>
        ...
        <div className="cr-flier-cta">Scan to book</div>
