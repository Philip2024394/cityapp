'use client'
// ============================================================================
// /dashboard/truck — Truck driver dashboard home (redesigned 2026-05-30)
// ----------------------------------------------------------------------------
// New mobile-first overview surface that replaces the 1,147-line monolith
// previously living at this path. Mirrors the redesigned /dashboard/car
// home so trucks get the same compact, scannable shell with focused
// subpages handling the deep editing.
//
// Design lineage: car-driver overview (Phase 1B). Truck differs in two
// ways: (1) eyebrow + copy adapted for trucks, (2) KPI strip surfaces the
// rental_daily_rate + rental_min_days fields, because truck rental is the
// dominant transaction shape (Pindahan, jasa angkut, distribusi). Per-km
// fares are rare for trucks — those stay one click away on the
// services subpage rather than crowding the home tiles.
//
// Regulatory posture: IndoCity is a software directory (PM 12/2019). Every
// rate shown is self-published by the driver; we never compute fares.
// ============================================================================

import { useCallback, useEffect, useMemo, useState } from 'react'
import Link from 'next/link'
import {
  Loader2, Truck, User, Pencil, Layers, CreditCard, Wallet, Flame,
...
type LoadState =
  ...
  | { kind: 'ready'; row: TruckDriverOverview }
...
export default function TruckDriverDashboardHomePage() {
  ...
  const reload = useCallback(async () => {
    ...
    const { data, error } = await supabase
      ...
      .select(
        ...
        'rental_daily_rate_idr, rental_min_days, availability, paid_until, ' +
        'rating, rating_count, cover_image_url',
      )
      .eq('user_id', user.id)
      .maybeSingle()
    if (error) { setState({ kind: 'error', message: error.message }); return }
    if (!data) { setState({ kind: 'no_driver' }); return }
    const row = data as unknown as TruckDriverOverview
  ...
  if (state.kind === 'unauth')       return <FullPageMessage cta={{ href: '/login?next=/dashboard/truck', label: 'Sign in' }}>Sign in to access the truck driver dashboard.</FullPageMessage>
  if (state.kind === 'no_driver')    return <FullPageMessage cta={{ href: '/signup/truck', label: 'List your truck' }}>No driver profile yet.</FullPageMessage>
  if (state.kind === 'wrong_type')   return <FullPageMessage>This dashboard is for truck drivers. Your profile is {state.type}.</FullPageMessage>
...
function DashboardHome({ row, onReload }: { row: TruckDriverOverview; onReload: () => void }) {
  ...
  return (
    <Shell>
      ...
      {/* KPI strip — quick stats the driver wants to see at a glance.
          Trucks lead with DAILY RATE + MIN DAYS because rental is the
          dominant transaction shape (Pindahan, distribusi, jasa angkut). */}
      <section className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3 mb-4">
        ...
        <Kpi
          label="Daily rate"
          value={(row.rental_daily_rate_idr ?? 0) > 0 ? `Rp ${row.rental_daily_rate_idr!.toLocaleString('id-ID')}` : '—'}
          sub="per day"
        />
        <Kpi
          label="Min days"
      ...
      {/* The Six Pages — founder-prioritised core surfaces. Compact tiles,
          2-col on mobile / 3-col on sm+, each ~60-80px tall, with a
          yellow-accent icon chip + charcoal title. */}
      <section className="mb-4">
        <SectionLabel>Your six pages</SectionLabel>
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 sm:gap-3">
          <SixCard href="/dashboard/truck/info"     icon={<User      className="w-4 h-4" strokeWidth={2.5} />} title="Profile" />
          <SixCard href="/dashboard/truck/vehicle"  icon={<Truck     className="w-4 h-4" strokeWidth={2.5} />} title="Truck details" />
          <SixCard href="/dashboard/truck/services" icon={<Layers    className="w-4 h-4" strokeWidth={2.5} />} title="Services & rates" />
          <SixCard href="/dashboard/truck/social"   icon={<Sparkles  className="w-4 h-4" strokeWidth={2.5} />} title="Social posts" />
          <SixCard href="/dashboard/truck/stats"    icon={<BarChart3 className="w-4 h-4" strokeWidth={2.5} />} title="Stats" />
          <SixCard href="/dashboard/truck/hotspots" icon={<Flame     className="w-4 h-4" strokeWidth={2.5} />} title="City busy areas" />
        </div>
      </section>
      ...
      <section className="mb-4">
        ...
        <div className="grid grid-cols-3 gap-2 sm:gap-3">
          <ActionCard compact href="/dashboard/truck/edit"         icon={<Pencil     className="w-5 h-5" strokeWidth={2.5} />} title="Page design" />
          <ActionCard compact href="/dashboard/truck/payments"     icon={<CreditCard className="w-5 h-5" strokeWidth={2.5} />} title="Payments" />
          <ActionCard compact href="/dashboard/truck/subscription" icon={<Wallet     className="w-5 h-5" strokeWidth={2.5} />} title="Subscription" />
        </div>
      </section>
      ...
      <section className="mb-6">
        ...
        <div className="grid grid-cols-3 gap-2 sm:gap-3">
          <ActionCard compact href="/dashboard/truck/faq"     icon={<HelpCircle  className="w-5 h-5" strokeWidth={2.5} />} title="FAQ" />
          <ActionCard compact href="/dashboard/truck/terms"   icon={<FileText    className="w-5 h-5" strokeWidth={2.5} />} title="Terms" />
          <ActionCard compact href="/dashboard/truck/privacy" icon={<ShieldCheck className="w-5 h-5" strokeWidth={2.5} />} title="Privacy" />
        </div>
      </section>
    </Shell>
  )
...
function SubscriptionBanner({ sub }: { sub: SubStatus }) {
  ...
  const body =
    sub.kind === 'never'   ? 'Your truck stays hidden from customers until you pay Rp 38.000 for the first month.' :
...
function AvailabilityBadge({ value, onChange }: { value: 'online' | 'busy' | 'offline'; onChange: () => void }) {
  ...
  // selector into /dashboard/truck/info (Availability section) — for now,
