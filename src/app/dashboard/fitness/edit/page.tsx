...
import {
  FITNESS_BANNER_LIBRARY,
  FITNESS_BANNER_CATEGORIES,
} from '@/lib/fitness/banners'
import {
  type FitnessProvider,
  type FitnessSpecialty,
} from '@/lib/fitness/types'

// WYSIWYG profile editor — personal trainer sees a live preview of
...
// Fitness brand defaults to sky blue — sport / active energy. Reads as
// clean, energetic, sweat-and-water vs florist's botanical green and
// cake's bakery pink. The CTA accent (button_text_color) defaults to
// white for high contrast on blue.
const DEFAULT_THEME = '#0EA5E9'
const DEFAULT_HERO = {
  line1:   'Personal Trainer',
  line2:   'Personal Trainer',
  tagline: 'Strength · HIIT · Fat Loss · Muscle Gain',
}
const DEFAULT_HERO_IMAGE = 'https://images.unsplash.com/photo-1571019613454-1cb2f99b2d8b?w=1200&auto=format&fit=crop'

type FitnessHeroEffect = 'none' | 'shimmer' | 'dance' | 'underline'

type FitnessHeroText = {
  line1?:         string
  line2?:         string
  tagline?:       string
  color?:         string
  ...
  effect?:        FitnessHeroEffect
}

type FitnessServicePhotoEntry = {
  url:               string
  name?:             string | null
...
type Extras = {
  ...
  hero_text?:       FitnessHeroText | null
  promo_text?:      string | null
  // service_photos is a FLAT JSONB array on fitness_providers (mig 0089-
  // equiv), unlike the beautician's keyed object. This page only
  // reads/writes it via the existing profile dashboard for V1.
  service_photos?: FitnessServicePhotoEntry[] | null
  // Reflect the canonical column name even though the picker chips below
  // never edit specialties on this V1 page.
  specialties?: FitnessSpecialty[] | null
}
type FullProvider = FitnessProvider & Extras

export default function FitnessEditPage() {
  ...
  const reload = useCallback(async () => {
    ...
    if (!data?.session?.user) { router.replace('/login?next=/dashboard/fitness/edit'); return }
    setLoading(true)
    try {
  ...
  async function save(patch: Partial<FullProvider>) {
    const r = await fetch('/api/fitness/me/profile', {
      method: 'POST',
    ...
    return (
      <Shell>
        <div className="px-4 pt-20 max-w-md mx-auto text-center text-ink">
          <h1 className="text-[20px] font-black mb-2">Not a personal trainer yet</h1>
          <Link href="/fitness/signup" className="rounded-full bg-brand text-bg px-6 py-3 text-[13px] font-extrabold inline-block">Register</Link>
        </div>
      </Shell>
    )
  }
  ...
  const effect: FitnessHeroEffect = (['none','shimmer','dance','underline'].includes(rawEffect) ? rawEffect : 'none') as FitnessHeroEffect
    ...
    <Shell>
      <div className="max-w-2xl mx-auto pt-3 pb-32 px-4">
        ...
        {/* Inline banner controls — sit RIGHT under the preview so the
            personal trainer can see what they're editing without opening
        ...
        <p className="text-[12px] text-black/55 mt-4 leading-snug">
          ...
          {' '}<Link href="/dashboard/fitness" className="text-yellow-700 hover:underline font-bold">full dashboard form</Link>{' '}
          for the rest.
        </p>
      </div>
    </Shell>
...
function BannerInlineControls({
  ...
  const [draftEffect,       setDraftEffect]       = useState<FitnessHeroEffect>(((['none','shimmer','dance','underline'].includes(ht.effect ?? 'none')) ? (ht.effect ?? 'none') : 'none') as FitnessHeroEffect)
  const [savedFlash,   setSavedFlash]   = useState(false)
  // "Follow theme" mode: true when the personal trainer hasn't
  // explicitly overridden the hero text color. In that mode, picking
  // a new theme color auto-recolors the Personal Trainer word. The
  // moment the trainer picks a custom color from the swatches below,
  ...
  const EFFECTS: Array<{ id: FitnessHeroEffect; label: string; desc: string }> = [
  ...
  return (
    <div className="mt-4 space-y-5">
      ...
      {/* Theme color — drives the entire page's accent (buttons, badges,
          star ratings, "Personal Trainer" overlay default, etc.). Lives
          FIRST so trainers make the brand decision before fine-tuning.
          Picking a theme color ALSO clears any prior hero-text color
          override so the "Personal Trainer" word follows the new theme.
      ...
      <Section title="Banner image" icon={<ImageIcon size={16} strokeWidth={2.5} />}>
        <BannerLibraryPicker
          ...
          library={FITNESS_BANNER_LIBRARY}
          categories={FITNESS_BANNER_CATEGORIES}
      ...
      <Section title="Banner text" icon={<Type size={16} strokeWidth={2.5} />}>
        <div className="space-y-3">
          ...
          <FieldWithColor
            ...
            placeholder="Strength · HIIT · Fat Loss · Muscle Gain"
      ...
      {/* Effect */}
      <Section title="Text effect (Personal Trainer word)" icon={<Sparkles size={16} strokeWidth={2.5} />}>
      ...
      {/* Running marquee text — the yellow ribbon that scrolls under the
          ...
          (fitness API cap, mig 0089-equiv). */}
      <Section title="Running text (marquee under training snapshots)" icon={<Megaphone size={16} strokeWidth={2.5} />}>
        <PromoTextEditor
      ...
      <Section title="QRIS payment" icon={<QrCodeIcon228 size={16} strokeWidth={2.5} />}>
        <QrisUploaderCard apiBase="/api/fitness" accentHex={theme} />
      </Section>
      ...
      <Section title="More" icon={<MoreHorizontal size={16} strokeWidth={2.5} />}>
        <div className="grid grid-cols-2 gap-2">
          <Link
            href="/dashboard/fitness/services"
      ...
      {/* Explicit Save button — auto-save runs in the background but this
          gives personal trainers a clear "I'm done" confirmation.
...
function PromoTextEditor({
  ...
  const display = (draft.trim() || 'Hasilnya datang dari konsistensi — kami yang jaga. 10-pack diskon 10%, free body assessment di sesi pertama, monthly coaching include meal plan + WA 24/7. Konsultasi via WhatsApp.') + ' ✦'
  ...
  return (
    <div className="space-y-2">
      ...
      <textarea
        ...
        maxLength={280}
        rows={3}
        placeholder="Write your promo message — appears as scrolling text below your training snapshots."
      ...
      <p className="text-[12px] text-black/60 leading-snug">
        Use this for promos — 10-pack discount, free body assessment, monthly + nutrition bundle, outdoor sunrise session.
        Tap a suggestion to drop it in.
      </p>
      <div className="flex flex-wrap gap-1.5 pt-1">
        {[
          '10-pack diskon 10% — Rp 180k/sesi, valid 3 bulan, jadwal fleksibel',
          'Free body assessment + foto progress di sesi pertama (drop-in/pack)',
          'Monthly Rp 2.5jt — 12 sesi + meal plan personal + WA check-in 24/7',
          'Outdoor sunrise session di pantai/track — Rp 200k, view bonus gratis',
          'Pair training 2 orang Rp 300k — motivasi extra dengan partner workout',
        ].map((s) => (
          <button
            key={s}
            type="button"
