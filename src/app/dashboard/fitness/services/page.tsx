...
import {
  ALL_SPECIALTIES, SPECIALTY_LABELS, MAX_FITNESS_SPECIALTIES,
  type FitnessProvider, type FitnessSpecialty,
} from '@/lib/fitness/types'

// Mirrors /dashboard/florist/services. Single page for everything the
// personal trainer sells:
//   1. Training types — max 3 they cover (strength / HIIT / yoga /
//      pilates / boxing / functional / weight loss / muscle gain /
//      postnatal / senior / online / outdoor).
//   2. Pricing  — drop-in starter price + optional monthly coaching
//      bundle. The PT industry quotes per-SESSION with package discount
//      tiers (drop-in / 5-pack / 10-pack / monthly) — clients buy
//      COMMITMENT, not items. Distinct from florist's per-arrangement
//      because there's no shippable unit; the unit is the trainer's
//      time-on-the-floor. Floor is Rp 200k (drop-in single session);
//      monthly coaching with nutrition + WA hits Rp 2-3jt.
//   3. Training snapshots with per-photo metadata (name, description,
//      per-session price, optional before/after pair) — drives the
//      training snapshots gallery + View Details popup on /fitness/[slug].
...
export default function FitnessServicesPage() {
    ...
    return (
      <Shell>
        <div className="px-4 pt-20 max-w-md mx-auto text-center">
          ...
          <Link href="/login?next=/dashboard/fitness/services" className="rounded-full bg-yellow-500 text-black px-6 py-3 text-[14px] font-extrabold inline-block">Sign in</Link>
        </div>
      </Shell>
    )
  }
  if (!provider) {
    return (
      <Shell>
        <div className="px-4 pt-20 max-w-md mx-auto text-center">
          <h1 className="text-[20px] font-black text-[#0A0A0A] mb-2">Not a personal trainer yet</h1>
          <Link href="/fitness/signup" className="rounded-full bg-yellow-500 text-black px-6 py-3 text-[14px] font-extrabold inline-block">Sign up</Link>
        </div>
      </Shell>
    )
  }
    ...
    <Shell>
      <div className="px-4 pt-3 pb-32 max-w-lg mx-auto">
        <header className="mb-5 flex items-center justify-between gap-3">
          <div>
            <Link href="/dashboard/fitness" className="text-[12px] font-bold text-gray-500 hover:text-gray-900 inline-block">← Dashboard</Link>
            <h1 className="text-[24px] font-black text-[#0A0A0A] leading-tight">Package catalog &amp; portfolio</h1>
            <p className="text-[13px] text-gray-600 mt-1">Pick training types, set drop-in + monthly coaching bundle, and add training snapshots that clients browse.</p>
          </div>
        </header>
...
function ServicesForm({ provider, onSaved }: { provider: FullProvider; onSaved: () => void }) {
  ...
  function toggleSpecialty(sid: FitnessSpecialty) {
    const on = f.specialties.includes(sid)
    if (!on && f.specialties.length >= MAX_FITNESS_SPECIALTIES) return
  ...
  return (
    <form onSubmit={save} className="space-y-4">
      {/* 1. Training types */}
      <Card title="Package catalog" hint={`Pick up to ${MAX_FITNESS_SPECIALTIES} training types — these become the chips on your profile (${f.specialties.length}/${MAX_FITNESS_SPECIALTIES}).`}>
        <div className="flex flex-wrap gap-1.5">
          {ALL_SPECIALTIES.map((sid) => {
            const on    = f.specialties.includes(sid)
            const atCap = !on && f.specialties.length >= MAX_FITNESS_SPECIALTIES
            return (
              <button
                key={sid}
                type="button"
      ...
      {/* 2. Pricing */}
      <Card title="Rates" hint="Shown on the public profile. Type thousands — e.g. 200 = Rp 200k drop-in single session, 1800 = Rp 1.8jt for the 10-pack, 2500 = Rp 2.5jt monthly coaching (max 9999).">
        <div className="grid grid-cols-2 gap-2">
          {([
            { k: 'hourly_rate', label: 'Drop-in from'     },
            { k: 'day_rate',    label: 'Monthly coaching' },
        ...
        {/* Trainer brings equipment toggle. Stored on has_own_tools for
            column reuse; the fitness surface label (resistance band,
            kettlebell, jump rope) is what matters to the client. */}
        <label className="flex items-center gap-2 text-[13px] text-gray-700 cursor-pointer mt-2">
          ...
          />
          Trainer bawa equipment (resistance band, kettlebell, jump rope)
        </label>
      </Card>

      {/* 3. Portfolio */}
      <Card title="Training Snapshots" hint="Up to 12 action shots + transformation photos per training type with name, description, per-session price, and optional before/after pair. These power the training snapshots gallery + View Details popup on your profile — clients pilih coach by foto.">
        {provider.user_id && (
