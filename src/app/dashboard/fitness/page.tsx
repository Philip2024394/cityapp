...
import {
  ...
  type FitnessProvider, type FitnessAvailability, type FitnessSpecialty,
...
export default function FitnessDashboardPage() {
  const [p, setP] = useState<FitnessProvider | null>(null)
  ...
  const reload = useCallback(async () => {
    ...
    try {
      ...
      const j = await r.json() as { provider: FitnessProvider | null }
      setP(j.provider)
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const r = await fetch('/api/fitness/me/analytics', { cache: 'no-store' })
        if (!r.ok) return
  ...
  async function setAvailability(next: FitnessAvailability) {
    if (!p) return
    const prev = p.availability
    setP({ ...p, availability: next })
    try {
      const r = await fetch('/api/fitness/me/availability', {
  ...
  if (err === 'not_signed_in') return (
    <Shell><div className="px-4 pt-20 max-w-md mx-auto text-center">
      ...
      <Link href="/login?next=/dashboard/fitness" className="rounded-full bg-brand text-bg px-6 py-3 text-[13px] font-extrabold inline-block">Sign in</Link>
    </div></Shell>
  )
  if (!p) return (
    <Shell><div className="px-4 pt-20 max-w-md mx-auto text-center">
      <h1 className="text-[20px] font-black mb-2">No personal trainer profile yet</h1>
      <Link href="/fitness/signup" className="rounded-full bg-brand text-bg px-6 py-3 text-[13px] font-extrabold inline-block">Register as personal trainer</Link>
    </div></Shell>
  )
  ...
  return (
    <Shell>
      <div className="px-4 pt-6 pb-24 max-w-3xl mx-auto space-y-4">
        ...
        <ProviderRenewBanner provider={p} upgradeHref="/fitness/upgrade" />
        ...
        <p className="text-[12px] text-black/60 text-center">
          Public profile: <a href={`/fitness/${p.slug}`} target="_blank" rel="noopener" className="text-brand hover:underline">/fitness/{p.slug}</a>
        </p>
      </div>
    </Shell>
  )
...
function ReadOnly({ p }: { p: FitnessProvider }) {
  return (
    <div className="space-y-3 text-[13px]">
      ...
      <div className="grid grid-cols-2 gap-2 pt-2">
        ...
        {p.day_rate_idr    != null && <PriceBox label="monthly coaching" v={p.day_rate_idr}    />}
      </div>
      <KV k="Trainer brings equipment" v={p.has_own_tools ? 'Yes' : 'No'} />
...
function EditForm({ p, onSaved }: { p: FitnessProvider; onSaved: () => void }) {
  const pe = p as FitnessProvider & {
  ...
  function toggle(s: FitnessSpecialty) {
    setF((p) => {
      if (p.specialties.includes(s)) return { ...p, specialties: p.specialties.filter((x) => x !== s) }
      if (p.specialties.length >= MAX_FITNESS_SPECIALTIES) return p
    ...
    <form onSubmit={save} className="space-y-3">
      ...
      <div>
        <div className="flex items-baseline justify-between mb-1.5">
          ...
          <div className="text-[11px] text-black/55">{f.specialties.length}/{MAX_FITNESS_SPECIALTIES}</div>
        </div>
        <div className="flex flex-wrap gap-1.5">
      ...
      <div className="grid grid-cols-2 gap-2">
        ...
        <PriceInput label="monthly coaching" v={f.day_rate}    set={(v) => upd('day_rate', v)} />
      </div>
