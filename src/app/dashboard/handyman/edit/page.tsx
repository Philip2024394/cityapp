...
// WYSIWYG profile editor — handyman sees a live preview of their
...
const DEFAULT_HERO = {
  line1:   'Professional',
  line2:   'Handyman',
  tagline: 'Tukang andalan untuk rumah, hotel, dan villa',
}
const DEFAULT_HERO_IMAGE = 'https://ik.imagekit.io/nepgaxllc/ChatGPT%20Image%20May%2026,%202026,%2003_33_32%20PM.png'

type HandymanHeroEffect = 'none' | 'shimmer' | 'dance' | 'underline'

type HandymanHeroText = {
  line1?:         string
  line2?:         string
  tagline?:       string
  color?:         string
  ...
  effect?:        HandymanHeroEffect
}

type HandymanServicePhotoEntry = {
  url:               string
  name?:             string | null
...
type Extras = {
  ...
  service_photos?: HandymanServicePhotoEntry[] | null
  // Reflect the canonical column name even though the picker chips below
  // never edit specialties on this V1 page.
  specialties?: HandymanSpecialty[] | null
}
type FullProvider = HandymanProvider & Extras

export default function HandymanEditPage() {
  ...
  const reload = useCallback(async () => {
    ...
    if (!data?.session?.user) { router.replace('/login?next=/dashboard/handyman/edit'); return }
    setLoading(true)
    try {
    ...
    return (
      <Shell>
        <div className="px-4 pt-20 max-w-md mx-auto text-center text-ink">
          <h1 className="text-[20px] font-black mb-2">Not a handyman yet</h1>
          <Link href="/handyman/signup" className="rounded-full bg-brand text-bg px-6 py-3 text-[13px] font-extrabold inline-block">Register</Link>
        </div>
      </Shell>
    )
  }
  ...
  const effect: HandymanHeroEffect = (['none','shimmer','dance','underline'].includes(rawEffect) ? rawEffect : 'none') as HandymanHeroEffect
    ...
    <Shell>
      <div className="max-w-2xl mx-auto pt-3 pb-32 px-4">
        ...
        {/* Inline banner controls — sit RIGHT under the preview so the
            handyman can see what they're editing without opening a
        ...
        <p className="text-[12px] text-black/55 mt-4 leading-snug">
          ...
          {' '}<Link href="/dashboard/handyman" className="text-yellow-700 hover:underline font-bold">full dashboard form</Link>{' '}
          for the rest.
        </p>
      </div>
    </Shell>
...
function BannerInlineControls({
  ...
  const [draftEffect,       setDraftEffect]       = useState<HandymanHeroEffect>(((['none','shimmer','dance','underline'].includes(ht.effect ?? 'none')) ? (ht.effect ?? 'none') : 'none') as HandymanHeroEffect)
  const [savedFlash,   setSavedFlash]   = useState(false)
  // "Follow theme" mode: true when the handyman hasn't explicitly
  // overridden the hero text color. In that mode, picking a new theme
  // color auto-recolors the Handyman word. The moment the handyman
  ...
  const EFFECTS: Array<{ id: HandymanHeroEffect; label: string; desc: string }> = [
  ...
  return (
    <div className="mt-4 space-y-5">
      ...
      <Section title="Banner text" icon={<Type size={16} strokeWidth={2.5} />}>
        <div className="space-y-3">
          ...
          <FieldWithColor
            ...
            placeholder="Tukang andalan untuk rumah, hotel…"
      ...
      {/* Effect */}
      <Section title="Text effect (Handyman word)" icon={<Sparkles size={16} strokeWidth={2.5} />}>
...
function PromoTextEditor({
  ...
  const display = (draft.trim() || 'Tukang andalan, harga jujur — siap layanan kost, rumah, hotel, dan villa. Pesan via WhatsApp untuk jadwal hari ini.') + ' ✦'
  ...
  return (
    <div className="space-y-2">
      ...
      <p className="text-[12px] text-black/60 leading-snug">
        Use this for special offers — weekend rates, kost discounts, or
        seasonal services. Tap a suggestion to drop it in.
      </p>
      <div className="flex flex-wrap gap-1.5 pt-1">
        {[
          'Promo AC service bulan ini — 150k borongan',
          'Tukang siap hari ini — area Yogya',
          'Service pompa air + waterproofing diskon weekend',
          'Pemasangan CCTV + antena, garansi pasang',
          'Tukang serabutan harian — harga jujur',
        ].map((s) => (
          <button
            key={s}
            type="button"
