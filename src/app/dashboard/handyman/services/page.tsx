...
import {
  ...
  type HandymanProvider, type HandymanSpecialty,
...
//   2. Pricing      — hourly + day rate (handyman doesn't have makeup/nail/hair
...
export default function HandymanServicesPage() {
    ...
    return (
      <Shell>
        <div className="px-4 pt-20 max-w-md mx-auto text-center">
          ...
          <Link href="/login?next=/dashboard/handyman/services" className="rounded-full bg-yellow-500 text-black px-6 py-3 text-[14px] font-extrabold inline-block">Sign in</Link>
        </div>
      </Shell>
    )
  }
  if (!provider) {
    return (
      <Shell>
        <div className="px-4 pt-20 max-w-md mx-auto text-center">
          <h1 className="text-[20px] font-black text-[#0A0A0A] mb-2">Not a tukang yet</h1>
          <Link href="/handyman/signup" className="rounded-full bg-yellow-500 text-black px-6 py-3 text-[14px] font-extrabold inline-block">Sign up</Link>
        </div>
      </Shell>
    )
  }
    ...
    <Shell>
      <div className="px-4 pt-3 pb-32 max-w-lg mx-auto">
        <header className="mb-5 flex items-center justify-between gap-3">
          <div>
            <Link href="/dashboard/handyman" className="text-[12px] font-bold text-gray-500 hover:text-gray-900 inline-block">← Dashboard</Link>
            <h1 className="text-[24px] font-black text-[#0A0A0A] leading-tight">Services &amp; prices</h1>
            <p className="text-[13px] text-gray-600 mt-1">Pick your trades, set hourly + day rates, and add portfolio photos.</p>
          </div>
        </header>
...
function ServicesForm({ provider, onSaved }: { provider: FullProvider; onSaved: () => void }) {
  ...
  function toggleSpecialty(sid: HandymanSpecialty) {
    const on = f.specialties.includes(sid)
    if (!on && f.specialties.length >= MAX_HANDYMAN_SPECIALTIES) return
  ...
  return (
    <form onSubmit={save} className="space-y-4">
      {/* 1. Specialties */}
      <Card title="Trades I cover" hint={`Pick up to ${MAX_HANDYMAN_SPECIALTIES} — these become the Services Provided chips on your profile (${f.specialties.length}/${MAX_HANDYMAN_SPECIALTIES}).`}>
        <div className="flex flex-wrap gap-1.5">
          {ALL_SPECIALTIES.map((sid) => {
            const on    = f.specialties.includes(sid)
            const atCap = !on && f.specialties.length >= MAX_HANDYMAN_SPECIALTIES
            return (
              <button
                key={sid}
                type="button"
      ...
      <Card title="Rates" hint="Shown on the public profile. Type thousands — e.g. 75 = Rp 75k/hr, 600 = Rp 600k/day (max 9999).">
        ...
        <label className="flex items-center gap-2 text-[13px] text-gray-700 cursor-pointer mt-2">
          ...
          />
          I bring my own tools (shown as a green badge on the card)
        </label>
      </Card>

      {/* 3. Portfolio photos */}
      <Card title="Portfolio photos" hint="Up to 12 photos with name, description, starting price, and optional before/after pair. These power the carousel + View Details popup on your profile.">
        {provider.user_id && (
