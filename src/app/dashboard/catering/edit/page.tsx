...
import {
  CATERING_BANNER_LIBRARY,
  CATERING_BANNER_CATEGORIES,
} from '@/lib/catering/banners'
import {
  type CateringProvider,
  type CateringSpecialty,
} from '@/lib/catering/types'

// WYSIWYG profile editor — catering business sees a live preview of their
...
// Catering brand defaults to warm red — Indonesian catering signage
// trends warm reds + golds for appetite (rendang brown, sambal red,
// tumpeng yellow). The CTA accent (button_text_color) defaults to
// white for high contrast on red.
const DEFAULT_THEME = '#DC2626'
const DEFAULT_HERO = {
  line1:   'Professional',
  line2:   'Catering business',
...
type CateringHeroEffect = 'none' | 'shimmer' | 'dance' | 'underline'

type CateringHeroText = {
  line1?:         string
  line2?:         string
  tagline?:       string
  color?:         string
  ...
  effect?:        CateringHeroEffect
}

type CateringServicePhotoEntry = {
  url:               string
  name?:             string | null
...
type Extras = {
  ...
  hero_text?:       CateringHeroText | null
  promo_text?:      string | null
  // service_photos is a FLAT JSONB array on catering_providers (mig 0089-
...
export default function CateringEditPage() {
  ...
  const reload = useCallback(async () => {
    ...
    if (!data?.session?.user) { router.replace('/login?next=/dashboard/catering/edit'); return }
    setLoading(true)
    try {
    ...
    return (
      <Shell>
        <div className="px-4 pt-20 max-w-md mx-auto text-center text-ink">
          <h1 className="text-[20px] font-black mb-2">Not a catering business yet</h1>
          <Link href="/catering/signup" className="rounded-full bg-brand text-bg px-6 py-3 text-[13px] font-extrabold inline-block">Register</Link>
        </div>
      </Shell>
    )
  }
  ...
  const effect: CateringHeroEffect = (['none','shimmer','dance','underline'].includes(rawEffect) ? rawEffect : 'none') as CateringHeroEffect
    ...
    <Shell>
      <div className="max-w-2xl mx-auto pt-3 pb-32 px-4">
        ...
        {/* Inline banner controls — sit RIGHT under the preview so the
            catering business can see what they're editing without opening a
        ...
        <p className="text-[12px] text-black/55 mt-4 leading-snug">
          ...
          {' '}<Link href="/dashboard/catering" className="text-yellow-700 hover:underline font-bold">full dashboard form</Link>{' '}
          for the rest.
        </p>
      </div>
    </Shell>
...
function BannerInlineControls({
  ...
  const [draftEffect,       setDraftEffect]       = useState<CateringHeroEffect>(((['none','shimmer','dance','underline'].includes(ht.effect ?? 'none')) ? (ht.effect ?? 'none') : 'none') as CateringHeroEffect)
  const [savedFlash,   setSavedFlash]   = useState(false)
  // "Follow theme" mode: true when the catering business hasn't explicitly
  // overridden the hero text color. In that mode, picking a new theme
  // color auto-recolors the Catering business word. The moment the
  // catering business picks a custom color from the swatches below, this
  ...
  const EFFECTS: Array<{ id: CateringHeroEffect; label: string; desc: string }> = [
  ...
  return (
    <div className="mt-4 space-y-5">
      ...
      {/* Theme color — drives the entire page's accent (buttons, badges,
          star ratings, "Catering business" overlay default, etc.). Lives FIRST
          so catering businesses make the brand decision before fine-tuning.
          Picking a theme color ALSO clears any prior hero-text color
          override so the "Catering business" word follows the new theme. The
      ...
      <Section title="Banner image" icon={<ImageIcon size={16} strokeWidth={2.5} />}>
        <BannerLibraryPicker
          ...
          library={CATERING_BANNER_LIBRARY}
          categories={CATERING_BANNER_CATEGORIES}
      ...
      {/* Effect */}
      <Section title="Text effect (Catering business word)" icon={<Sparkles size={16} strokeWidth={2.5} />}>
      ...
      {/* Running marquee text — the yellow ribbon that scrolls under the
          ...
          (catering API cap, mig 0089-equiv). */}
      ...
      <Section title="QRIS payment" icon={<QrCodeIcon228 size={16} strokeWidth={2.5} />}>
        <QrisUploaderCard apiBase="/api/catering" accentHex={theme} />
      </Section>
      ...
      <Section title="More" icon={<MoreHorizontal size={16} strokeWidth={2.5} />}>
        <div className="grid grid-cols-2 gap-2">
          <Link
            href="/dashboard/catering/services"
      ...
      {/* Explicit Save button — auto-save runs in the background but this
          gives catering businesses a clear "I'm done" confirmation. Clicking
...
function PromoTextEditor({
  ...
  const display = (draft.trim() || 'Makanan rumahan yang bikin tamumu nambah — pesan H-3, uang muka 50%. Konsultasi menu via WhatsApp.') + ' ✦'
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="flex flex-wrap gap-1.5 pt-1">
        {[
          'Nasi Box Premium Rp 35k/pax — minimum 50 pax, antar kota gratis',
          'Prasmanan Wedding Rp 95k/pax — minimum 100 pax, 5-7 menu utama',
          'Tumpeng Mini Rp 450k — komplit untuk 7-10 orang',
          'Uang muka 50% lock the date — sisanya saat handover',
          'Snack Box Meeting Rp 22k/box — minimum 20 box, halal sertifikat',
        ].map((s) => (
          <button
            key={s}
            type="button"
