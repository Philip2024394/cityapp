'use client'
// ============================================================================
// /dashboard/jeep/vehicle — Vehicle details (Phase 1B real implementation)
// ----------------------------------------------------------------------------
...
import { JEEP_COLOR_OPTIONS, normalizeJeepColor } from '@/data/jeepImages'
...
type LoadState =
  ...
  | { kind: 'ready'; row: JeepVehicleRow }

// ============================================================================
// Page shell
// ============================================================================
export default function JeepVehicleDetailsPage() {
  const [state, setState] = useState<LoadState>({ kind: 'loading' })

  const reload = useCallback(async () => {
    // DEV BYPASS — localhost impersonation via cr-dev-uid cookie.
    const dev = await tryLoadDevDriver()
    if (dev) { setState({ kind: 'ready', row: dev.driver as unknown as JeepVehicleRow }); return }
    ...
    setState({ kind: 'ready', row: data as unknown as JeepVehicleRow })
  }, [])
  ...
  if (state.kind === 'unauth')      return <FullPageMessage cta={{ href: '/login?next=/dashboard/jeep/vehicle', label: 'Sign in' }}>Sign in to edit your vehicle.</FullPageMessage>
...
// Editor
// ============================================================================
function VehicleEditor({ row, onReload }: { row: JeepVehicleRow; onReload: () => void }) {
  ...
  return (
    <Shell>
      ...
      {/* Jeep colour picker — visual catalog. Owner picks one of 8 colours.
          The selected colour drives the jeep photo shown on the public
          /jeep marketplace card + /jeep/[slug] profile hero. */}
      <Card title="Choose your jeep colour" hint="Pick the colour that matches your jeep — this photo will display on your profile and search card." icon={<Car size={18} />}>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
          {JEEP_COLOR_OPTIONS.map((opt) => {
            const isActive = normalizeJeepColor(color) === opt.key
            return (
              <button
                key={opt.key}
                type="button"
                onClick={() => {
                  setColor(opt.label)
                  void save('vehicle_color', { vehicle_color: opt.label }, 'Colour saved')
                }}
                className="group relative rounded-2xl overflow-hidden border-2 transition active:scale-[0.98]"
                style={{
                  borderColor: isActive ? '#FACC15' : 'rgba(0,0,0,0.10)',
                  background:  '#FFFFFF',
                }}
              ...
              >
                ...
                <img
                  ...
                  loading="lazy"
                />
                <div className="flex items-center gap-2 px-2.5 py-1.5 border-t border-black/5">
                  <span
                    aria-hidden
                    className="w-3.5 h-3.5 rounded-full border border-black/10"
                    style={{ background: opt.swatch }}
                  />
                  <span className="text-[12px] font-extrabold text-[#0A0A0A]">{opt.label}</span>
                  {isActive && (
                    <CheckCircle2
                      className="ml-auto w-4 h-4 text-[#0A0A0A]"
                      strokeWidth={2.5}
                      style={{ color: '#FACC15' }}
                    />
                  )}
      ...
      {/* Plate / Seats */}
      <Card title="Plate & seats" hint="The quick details riders check before boarding." icon={<Car size={18} />}>
