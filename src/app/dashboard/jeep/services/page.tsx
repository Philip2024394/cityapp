'use client'
// ============================================================================
// /dashboard/jeep/services — Services & rates (Phase 1B real implementation)
// ----------------------------------------------------------------------------
...
// Page entry
// ============================================================================
export default function JeepServicesPage() {
  ...
  if (state.kind === 'unauth')      return <FullMsg cta={{ href: '/login?next=/dashboard/jeep/services', label: 'Sign in' }}>Sign in to edit your services.</FullMsg>
  if (state.kind === 'no_driver')   return <FullMsg cta={{ href: '/signup?role=driver&vehicle=jeep', label: 'Create driver profile' }}>No driver profile yet.</FullMsg>
...
function PricingCard({
  ...
  const market = getMarketDefault('jeep')
    ...
    >
      <div className="grid grid-cols-2 gap-2">
        <div className="space-y-1">
          <IdrInput
            label="Price per km"
            placeholder={market ? `Charter ref: Rp ${formatIdr(market.price_per_km)}` : '6.000'}
            value={perKm}
            onChange={setPerKm}
        ...
        <div className="space-y-1">
          <IdrInput
            label="Minimum fee"
            placeholder={market ? `Charter ref: Rp ${formatIdr(market.min_fee)}` : '35.000'}
            value={minFee}
            onChange={setMinFee}
