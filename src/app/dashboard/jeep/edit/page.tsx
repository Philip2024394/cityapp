'use client'
// ============================================================================
// /dashboard/jeep/edit — Page design (banner picker)
// ----------------------------------------------------------------------------
// Drivers pick the hero backdrop shown on their public /jeep/[slug]
...
// this page scoped to what the customer actually sees on /jeep/[slug].
// ============================================================================
...
import { JEEP_BANNERS, getDefaultBanner } from '@/lib/drivers/banners'
...
// Page
// ============================================================================
export default function JeepEditPage() {
  ...
  if (state.kind === 'unauth')      return <FullPageMessage cta={{ href: '/login?next=/dashboard/jeep/edit', label: 'Sign in' }}>Sign in to edit your page design.</FullPageMessage>
...
function EditShell({ row, onSaved }: { row: DriverRow; onSaved: () => void }) {
  ...
  const profileHref = row.slug ? `/jeep/${row.slug}` : null
  ...
  return (
    <main className="relative min-h-[100dvh] bg-[#F5F5F4] text-[#0A0A0A]">
      ...
      <div className="max-w-2xl mx-auto px-4 sm:px-5 pt-4 pb-24">
        ...
        {/* Banner picker — curated JEEP_BANNERS + upload + paste URL */}
...
function BannerPickerSection({ row, onSaved }: { row: DriverRow; onSaved: () => void }) {
  ...
  const curatedUrls = useMemo(() => new Set(JEEP_BANNERS.map((b) => b.url)), [])
