'use client'
// ============================================================================
// /dashboard/jeep/info — Profile info (Phase 1B real implementation)
// ----------------------------------------------------------------------------
...
// are computed here — pricing lives on /dashboard/jeep/services. This page
...
type LoadState =
  ...
  | { kind: 'ready'; row: JeepDriverInfoRow }

// ============================================================================
// Page shell
// ============================================================================
export default function JeepDriverInfoPage() {
  const [state, setState] = useState<LoadState>({ kind: 'loading' })

  const reload = useCallback(async () => {
    // DEV BYPASS — localhost impersonation via cr-dev-uid cookie.
    const dev = await tryLoadDevDriver()
    if (dev) { setState({ kind: 'ready', row: dev.driver as unknown as JeepDriverInfoRow }); return }
    ...
    setState({ kind: 'ready', row: data as unknown as JeepDriverInfoRow })
  }, [])
  ...
  if (state.kind === 'unauth')      return <FullPageMessage cta={{ href: '/login?next=/dashboard/jeep/info', label: 'Sign in' }}>Sign in to edit your profile info.</FullPageMessage>
...
// ============================================================================
function InfoEditor({ row, onReload }: { row: JeepDriverInfoRow; onReload: () => void }) {
