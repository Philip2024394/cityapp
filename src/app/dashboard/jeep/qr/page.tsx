'use client'
// ============================================================================
// /dashboard/jeep/qr — Printable profile QR for car/bus/truck drivers
// ----------------------------------------------------------------------------
// Pure client-side: pulls the driver row from public.drivers, mints a
// 360×360 canvas QR for the public profile URL (citydrivers.id/<vehicle>/<slug>),
// and offers a 1080×1080 PNG export with a yellow header bar for printing
// on business cards, car windows, hotel concierge desks, etc.
//
// Public URL is vehicle-type-aware:
//   bus    → /bus/<slug>
//   truck  → /truck/<slug>
//   else   → /jeep/<slug>
//
...
function publicPathFor(vehicleType: string | null, slug: string): string {
  if (vehicleType === 'bus')   return `/bus/${slug}`
  if (vehicleType === 'truck') return `/truck/${slug}`
  return `/jeep/${slug}`
}

export default function JeepQrPage() {
  ...
  const publicPath = useMemo(
    () => driver?.slug ? publicPathFor(driver.vehicle_type, driver.slug) : '',
    [driver],
  )
  ...
  async function downloadPng() {
    ...
    ctx.fillText(driver.business_name || 'Cityriders driver', SIZE / 2, HEADER_H / 2 - 14)
    ...
    a.href     = dataUrl
    a.download = `${driver.slug || 'driver'}-qr.png`
  ...
  function shareToWhatsApp() {
    ...
    const text = `Hubungi saya untuk pesan kendaraan: ${url}`
  ...
  if (state.kind === 'no_slug')     return <FullPageMessage cta={{ href: '/dashboard/jeep/info', label: 'Set up profile' }}>Set your public slug to generate a QR.</FullPageMessage>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[12.5px] font-bold opacity-80 leading-snug">
                Print on business cards, car windows, hotel desks. Scan = direct to your profile.
              </p>
            </div>
          </div>
        </section>
    ...
    <div className="hidden print:block">
      ...
      <div className="cr-flier">
        <div className="cr-flier-head">
          <div className="cr-flier-brand">
            {driver.vehicle_type === 'bus' ? 'Bus driver' :
             driver.vehicle_type === 'truck' ? 'Truck driver' : 'Jeep driver'}
          </div>
          <div className="cr-flier-name">{driver.business_name || 'Cityriders driver'}</div>
