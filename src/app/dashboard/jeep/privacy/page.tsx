...
// /dashboard/jeep/privacy — long-form Privacy Policy editor for car / bus
// / truck drivers. Mirrors beautician/privacy 1:1. Backed by
// legal_privacy (text) on the drivers table — Phase 2 migration adds
// the column; until then the save will fail silently.
...
1. What we collect
When you book or contact us, we collect your name, contact details (phone, WhatsApp, email), and trip details you share (pickup, drop-off, passenger count, special requests). When you pay, payment is processed by our payment provider (Stripe, Midtrans, or your bank) — they receive your card details directly. We never see or store your full card number.

2. Why we collect it
We use your information to confirm trips, deliver the ride, communicate before and during the journey, and process refunds where applicable.

3. Sharing
We do not sell your information. We share information only with: (a) the payment processor handling your transaction, (b) our messaging provider (WhatsApp or email) where you've contacted us, (c) authorities where required by law (e.g. road accident reporting).

4. Retention
We keep trip and payment records for as long as required by local tax law (typically 5–7 years in Indonesia).
...
export default function JeepDriverPrivacyPage() {
  ...
  const load = useCallback(async () => {
    ...
    if (!user) { router.replace('/login?next=/dashboard/jeep/privacy'); return }
    ...
    try {
      ...
      const initial = (row?.legal_privacy ?? '').trim()
        ? (row?.legal_privacy ?? '')
        : privacyTemplate(row?.business_name?.trim() || 'Your driver business')
      setDraft(initial)
  ...
  function reset() {
    ...
    const tpl = privacyTemplate(driver.business_name?.trim() || 'Your driver business')
    setDraft(tpl)
    void commit(tpl)
  }
