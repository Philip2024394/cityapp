'use client'
// ============================================================================
// /dashboard/rider/subscription — CityRiders Rp 38.000/month subscription
// ----------------------------------------------------------------------------
// Mirrors /dashboard/car/subscription with copy adjusted for bike riders.
// Riders self-serve via QRIS in their banking app, then upload the
...
// not custody funds — the QRIS payment is between the rider's bank
...
const ADMIN_WA_RENEW = `https://wa.me/${ADMIN_WHATSAPP_E164}?text=${encodeURIComponent(
  'Halo admin, saya mau bayar/renew langganan dashboard Rider Kita2u (Rp 38.000/bulan).',
)}`
...
export default function RiderSubscriptionPage() {
  ...
  const reload = useCallback(async () => {
    ...
    try {
      ...
      if (Array.isArray(rows)) {
        ...
        // Try signed URLs for the screenshots so the rider can preview
  ...
  if (state.kind === 'unauth')      return <FullPageMessage cta={{ href: '/login?next=/dashboard/rider/subscription', label: 'Sign in' }}>Sign in to manage your subscription.</FullPageMessage>
...
function StatusCard({ sub }: { sub: SubStatus }) {
  ...
  const body = isExpired
    ? 'Renew below to come back online in the public bike marketplace.'
    : 'Pay your first month to publish your listing in the public bike marketplace.'
...
// ----------------------------------------------------------------------------
// Rider scans the QR in their bank/wallet app, pays externally, then
...
// rider's bank/wallet and the founder's merchant QRIS.
// ============================================================================
      ...
      >
        ...
        <div className="px-5 pt-6 pb-5">
          ...
          <p className="text-[13px] text-black/65 mt-1 leading-snug">
            Pay Rp {SUBSCRIPTION_MONTHLY_IDR.toLocaleString('id-ID')} to keep your bike listing live for 30 days.
          </p>
