'use client'
// ============================================================================
// /dashboard/rider/info — Profile info (rider mirror of /dashboard/car/info)
// ----------------------------------------------------------------------------
// Riders edit their business identity here: business name, short bio,
...
// are computed here — pricing lives on /dashboard/rider/services. This
// page is identity + reachability only.
// ============================================================================
...
export default function RiderInfoPage() {
  ...
  const reload = useCallback(async () => {
    ...
    if (dev) { setState({ kind: 'ready', row: dev.driver as unknown as RiderInfoRow }); return }
    ...
    setState({ kind: 'ready', row: data as unknown as RiderInfoRow })
  }, [])
  ...
  if (state.kind === 'unauth')      return <FullPageMessage cta={{ href: '/login?next=/dashboard/rider/info', label: 'Sign in' }}>Sign in to edit your profile info.</FullPageMessage>
...
// ============================================================================
function InfoEditor({ row, onReload }: { row: RiderInfoRow; onReload: () => void }) {
  ...
  // missing 'id' still renders correctly.
      ...
      <Card title="Your name &mp; bio" hint="Name or company only — don't add vehicle type or city. Those already appear on your profile hero." icon={<User size={18} />}>
        ...
        >
          <input
            ...
            maxLength={32}
            placeholder="e.g. Budi Bike Yogya"
            className={inputCls}
          />
        </Field>
        <Field
