'use client'
// ============================================================================
// /dashboard/rider/edit — Page design (banner picker, bike variant)
// ----------------------------------------------------------------------------
// Riders pick the hero backdrop shown on their public /r/[slug] profile.
// We persist to drivers.cover_image_url; null falls back to
// DEFAULT_BIKE_HERO in DriverProfileShell.
//
// Out of scope for now: hero text, per-line colors, theme color. Rider
// public pages don't need that copy-customisation surface yet — keep
// this page scoped to what the customer actually sees on /r/[slug].
// ============================================================================
...
import { BIKE_BANNERS, getDefaultBanner } from '@/lib/drivers/banners'
...
export default function RiderEditPage() {
  ...
  if (state.kind === 'unauth')      return <FullPageMessage cta={{ href: '/login?next=/dashboard/rider/edit', label: 'Sign in' }}>Sign in to edit your page design.</FullPageMessage>
  ...
  return (
    <main className="relative min-h-[100dvh] bg-[#F5F5F4] text-[#0A0A0A]">
      ...
      <div className="max-w-2xl mx-auto px-4 sm:px-5 pt-4 pb-24">
        ...
        {/* Banner picker — curated BIKE_BANNERS + upload + paste URL.
            Future curated bike banners get added to BIKE_BANNERS in
            src/lib/drivers/banners.ts. */}
...
function BannerPickerSection({ row, onSaved }: { row: DriverRow; onSaved: () => void }) {
  ...
  const curatedUrls = useMemo(() => new Set(BIKE_BANNERS.map((b) => b.url)), [])
  const customUrl = selected && !curatedUrls.has(selected) ? selected : null

  // Save-on-change: commits the new URL immediately when the rider
