...
// /dashboard/rider/terms — long-form Terms editor for bike riders
// (ojek). Mirrors beautician/terms 1:1: seeds an editable template
// when legal_terms is null so riders don't stare at an empty box,
// auto-saves on blur. Backed by legal_terms (text) on the drivers
// table — migration 0144 added the column.

const TODAY = new Date().toISOString().slice(0, 10) // YYYY-MM-DD

function termsTemplate(riderName: string): string {
  return `TERMS OF SERVICE — ${riderName}

1. Services
${riderName} provides bike rider / ojek transport services as described on this profile, by appointment.
...
4. Conduct
We reserve the right to refuse service to any passenger whose behaviour is abusive, unsafe, or inconsistent with a safe riding environment. Smoking, open alcohol, and illegal substances are not permitted on the bike.
...
export default function RiderTermsPage() {
  ...
  const load = useCallback(async () => {
    ...
    if (!user) { router.replace('/login?next=/dashboard/rider/terms'); return }
    ...
    try {
      ...
      const initial = (row?.legal_terms ?? '').trim()
        ? (row?.legal_terms ?? '')
        : termsTemplate(row?.business_name?.trim() || 'Your rider business')
      setDraft(initial)
  ...
  function reset() {
    ...
    const tpl = termsTemplate(driver.business_name?.trim() || 'Your rider business')
    setDraft(tpl)
    void commit(tpl)
  }
