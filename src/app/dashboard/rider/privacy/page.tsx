...
// /dashboard/rider/privacy — long-form Privacy Policy editor for bike
// riders (ojek). Mirrors beautician/privacy 1:1. Backed by
// legal_privacy (text) on the drivers table — migration 0144 added
// the column.

const TODAY = new Date().toISOString().slice(0, 10)

function privacyTemplate(riderName: string): string {
  return `PRIVACY POLICY — ${riderName}
...
export default function RiderPrivacyPage() {
  ...
  const load = useCallback(async () => {
    ...
    if (!user) { router.replace('/login?next=/dashboard/rider/privacy'); return }
    ...
    try {
      ...
      const initial = (row?.legal_privacy ?? '').trim()
        ? (row?.legal_privacy ?? '')
        : privacyTemplate(row?.business_name?.trim() || 'Your rider business')
      setDraft(initial)
  ...
  function reset() {
    ...
    const tpl = privacyTemplate(driver.business_name?.trim() || 'Your rider business')
    setDraft(tpl)
    void commit(tpl)
  }
