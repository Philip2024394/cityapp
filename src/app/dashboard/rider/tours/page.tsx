'use client'
// ============================================================================
// /dashboard/rider/tours — Tour packages editor (bike riders)
// ----------------------------------------------------------------------------
// Mirror of /dashboard/car/tours, scoped to vehicle_type='bike'. Bike
// tours are personal-pillion experiences (sunset city loop, street-food
// ride, Bukit Panguk sunrise). Max 1 pax across all templates.
// ============================================================================
...
const VEHICLE: TourVehicleKind = 'bike'
const BACK_HREF = '/dashboard/rider'
...
export default function RiderToursPage() {
  ...
  if (state.kind === 'unauth')         return <FullPageMessage cta={{ href: '/login?next=/dashboard/rider/tours', label: 'Sign in' }}>Sign in to manage your tours.</FullPageMessage>
...
function ToursEditor({
  ...
  return (
    <Shell>
      ...
      {tours.length === 0 ? (
        <div className="rounded-3xl bg-white border border-black/10 p-6 text-center shadow-sm">
          <p className="text-[13px] font-extrabold text-[#0A0A0A]">No tours yet.</p>
          <p className="text-[13px] text-black/55 leading-snug mt-1">
            Tap &quot;Add from template&quot; above to clone one of the canonical Yogya bike tours.
          </p>
        </div>
      ) : (
    ...
    <section className="rounded-3xl bg-white border border-black/10 p-5 shadow-sm space-y-3">
      ...
      <Field label="Includes (comma-separated)" saving={busyField === 'includes'}>
        <input
          ...
          className={inputCls}
          placeholder="rider, fuel, helmet"
        />
      </Field>
      {tour.includes.length > 0 && <ChipList chips={tour.includes} tone="positive" />}
      ...
      {tour.excludes.length > 0 && <ChipList chips={tour.excludes} tone="muted" />}
