...
// /dashboard/skincare/orders — the vendor's cart-order inbox. Lists every
// vendor_orders row owned by this skincare provider, with filter chips, expand-to-
// see-detail rows, and Accept / Fulfill / Cancel actions that POST to
// /api/skincare/me/orders/[id].
//
// Stays close to /dashboard/skincare/promos for visual parity — same
...
export default function SkincareOrdersPage() {
  ...
  const reload = useCallback(async (isRefresh = false) => {
    ...
    try {
      const [pRes, oRes] = await Promise.all([
        fetch('/api/skincare/me', { cache: 'no-store' }),
        fetch('/api/skincare/me/orders?status=all', { cache: 'no-store' }),
      ])
    ...
    return (
      <Shell>
        <div className="px-4 pt-20 max-w-md mx-auto text-center">
          ...
          <Link href="/login?next=/dashboard/skincare/orders" className="rounded-full bg-pink-500 text-white px-6 py-3 text-[14px] font-extrabold inline-block">Sign in</Link>
        </div>
      </Shell>
    )
  }
  if (!provider) return <Shell><div className="px-4 pt-20 text-center text-black/70">No skincare profile yet.</div></Shell>
...
function OrderRow({
  ...
  async function setStatus(next: 'accepted' | 'fulfilled' | 'cancelled', kind: 'accept' | 'fulfill' | 'cancel') {
    ...
    try {
      const r = await fetch(`/api/skincare/me/orders/${order.id}`, {
        method:  'POST',
