...
import {
  SKINCARE_BANNER_LIBRARY,
  SKINCARE_BANNER_CATEGORIES,
} from '@/lib/skincare/banners'
import type {
...
// WYSIWYG profile editor — skincare provider sees a live preview of their
...
const DEFAULT_HERO = {
  line1:   'Professional',
  line2:   'Skincare',
  tagline: 'Healthy skin, expert care, every day',
}
...
export default function SkincareEditPage() {
  ...
  const reload = useCallback(async () => {
    ...
    if (!data?.session?.user) { router.replace('/login?next=/dashboard/skincare/edit'); return }
    setLoading(true)
    try {
    ...
    return (
      <Shell>
        <div className="px-4 pt-20 max-w-md mx-auto text-center text-ink">
          <h1 className="text-[20px] font-black mb-2">Not a skincare provider yet</h1>
          <Link href="/skincare/signup" className="rounded-full bg-brand text-bg px-6 py-3 text-[13px] font-extrabold inline-block">Register</Link>
        </div>
      </Shell>
    )
  }
    ...
    <Shell>
      <div className="max-w-2xl mx-auto pt-4 pb-32 px-4">
        ...
        <p className="text-[12px] text-black/55 mt-4 leading-snug">
          ...
          {' '}<Link href="/dashboard/skincare" className="text-pink-600 hover:underline font-bold">full dashboard form</Link>{' '}
          for the rest.
        </p>
      </div>
    </Shell>
...
function BannerInlineControls({
  ...
  // color auto-recolors the Skincare word. The moment the provider
    ...
    <div className="mt-4 space-y-4">
      ...
      <Section title="Banner image" icon={<ImageIcon size={16} strokeWidth={2.5} />}>
        <BannerLibraryPicker
          ...
          library={SKINCARE_BANNER_LIBRARY}
          categories={SKINCARE_BANNER_CATEGORIES}
          defaultThemeHex="#EC4899"
          purchaseEndpoint="/api/skincare/me/buy-banner"
      ...
      <Section title="Banner text" icon={<Type size={16} strokeWidth={2.5} />}>
        <div className="space-y-3">
          ...
          <FieldWithColor
            ...
            placeholder="Healthy skin, expert care…"
      ...
      {/* Effect */}
      <Section title="Text effect (Skincare word)" icon={<Sparkles size={16} strokeWidth={2.5} />}>
...
function PromoTextEditor({
  ...
  const display = (draft.trim() || 'Message me this week — exclusive promo on professional skincare service delivered straight to your home, hotel or villa, in the comfort of your stay.') + ' ✦'
  ...
  return (
    <div className="space-y-2">
      ...
      <p className="text-[12px] text-black/60 leading-snug">
        Use this for special offers — consultations, routines, treatments,
        or seasonal discounts. Tap a suggestion to drop it in.
      </p>
      <div className="flex flex-wrap gap-1.5 pt-1">
        {[
          'New patient consultation — first session 20% off',
          'Acne program — book a 4-week routine and save',
          'K-Beauty routine design — limited slots this month',
          'Free skin analysis with any treatment booking this week',
