...
// /dashboard/skincare/terms — long-form Terms editor. Backed by the
// universal /api/skincare/me/profile validator (legal_terms column).
...
1. Services
${vendorName} provides skincare services as described on this profile, by appointment.
...
export default function SkincareTermsPage() {
  ...
  const load = useCallback(async () => {
    ...
    if (!data?.session?.user) { router.replace('/login?next=/dashboard/skincare/terms'); return }
    setLoading(true)
    try {
