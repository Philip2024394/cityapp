...
import {
  TUTORING_BANNER_LIBRARY,
  TUTORING_BANNER_CATEGORIES,
} from '@/lib/tutoring/banners'
import {
  type TutoringProvider,
  type TutoringSpecialty,
} from '@/lib/tutoring/types'

// WYSIWYG profile editor — tutor sees a live preview of
...
// Tutoring brand defaults to sky blue — sport / active energy. Reads as
...
type TutoringHeroEffect = 'none' | 'shimmer' | 'dance' | 'underline'

type TutoringHeroText = {
  line1?:         string
  line2?:         string
  tagline?:       string
  color?:         string
  ...
  effect?:        TutoringHeroEffect
}

type TutoringServicePhotoEntry = {
  url:               string
  name?:             string | null
...
export default function TutoringEditPage() {
  ...
  const reload = useCallback(async () => {
    ...
    if (!data?.session?.user) { router.replace('/login?next=/dashboard/tutoring/edit'); return }
    setLoading(true)
    try {
    ...
    return (
      <Shell>
        <div className="px-4 pt-20 max-w-md mx-auto text-center text-ink">
          <h1 className="text-[20px] font-black mb-2">Not a tutor yet</h1>
          <Link href="/tutoring/signup" className="rounded-full bg-brand text-bg px-6 py-3 text-[13px] font-extrabold inline-block">Register</Link>
        </div>
      </Shell>
    )
  }
  ...
  const effect: TutoringHeroEffect = (['none','shimmer','dance','underline'].includes(rawEffect) ? rawEffect : 'none') as TutoringHeroEffect
    ...
    <Shell>
      <div className="max-w-2xl mx-auto pt-3 pb-32 px-4">
        ...
        {/* Inline banner controls — sit RIGHT under the preview so the
            tutor can see what they're editing without opening
        ...
        <p className="text-[12px] text-black/55 mt-4 leading-snug">
          ...
          {' '}<Link href="/dashboard/tutoring" className="text-yellow-700 hover:underline font-bold">full dashboard form</Link>{' '}
          for the rest.
        </p>
      </div>
    </Shell>
...
function BannerInlineControls({
  ...
  const [draftEffect,       setDraftEffect]       = useState<TutoringHeroEffect>(((['none','shimmer','dance','underline'].includes(ht.effect ?? 'none')) ? (ht.effect ?? 'none') : 'none') as TutoringHeroEffect)
  const [savedFlash,   setSavedFlash]   = useState(false)
  // "Follow theme" mode: true when the tutor hasn't
  // explicitly overridden the hero text color. In that mode, picking
  // a new theme color auto-recolors the Tutor word. The
  ...
  const EFFECTS: Array<{ id: TutoringHeroEffect; label: string; desc: string }> = [
    ...
    <div className="mt-4 space-y-5">
      ...
      <Section title="Banner image" icon={<ImageIcon size={16} strokeWidth={2.5} />}>
        <BannerLibraryPicker
          ...
          library={TUTORING_BANNER_LIBRARY}
          categories={TUTORING_BANNER_CATEGORIES}
      ...
      {/* Effect */}
      <Section title="Text effect (Tutor word)" icon={<Sparkles size={16} strokeWidth={2.5} />}>
      ...
      {/* Running marquee text — the yellow ribbon that scrolls under the
          ...
          (tutoring API cap, mig 0089-equiv). */}
      ...
      <Section title="QRIS payment" icon={<QrCodeIcon228 size={16} strokeWidth={2.5} />}>
        <QrisUploaderCard apiBase="/api/tutoring" accentHex={theme} />
      </Section>
      ...
      <Section title="More" icon={<MoreHorizontal size={16} strokeWidth={2.5} />}>
        <div className="grid grid-cols-2 gap-2">
          <Link
            href="/dashboard/tutoring/services"
      ...
      {/* Explicit Save button — auto-save runs in the background but this
          gives tutors a clear "I'm done" confirmation.
...
function PromoTextEditor({
  ...
  const display = (draft.trim() || 'Nilai naik. Konsep paham. Tugas selesai. Gratis trial 30 menit. Paket UTBK 12x hemat 15%. English online Rp 150k/sesi. Mengaji anak Rp 80k. Reservasi via WhatsApp.') + ' ✦'
  ...
  return (
    <div className="space-y-2">
      ...
      <textarea
        ...
        maxLength={280}
        rows={3}
        placeholder="Write your promo message — appears as scrolling text below your Belajar Bareng gallery."
      ...
      <p className="text-[12px] text-black/60 leading-snug">
        Use this for promos — gratis trial 30 menit, paket UTBK 12x diskon, English online, mengaji anak.
        Tap a suggestion to drop it in.
      </p>
      <div className="flex flex-wrap gap-1.5 pt-1">
        {[
          'Gratis trial 30 menit — coba 1 sesi sebelum komit, matematika / fisika / english / UTBK',
          'Paket UTBK SBMPTN 12x Rp 2.2jt — Rp 183k/sesi (hemat 15%), simulasi mingguan, target masuk PTN',
          'English Conversation Online Rp 150k/sesi — 60 menit via Zoom, business / casual / IELTS prep',
          'Mengaji Anak SD-SMP Rp 80k/pertemuan — 45 menit, datang ke rumah, tartil + tajwid + hafalan juz 30',
          'Matematika SMA datang ke rumah Rp 120k — 90 menit, kurikulum Merdeka / Cambridge, drill soal + konsep',
        ].map((s) => (
          <button
            key={s}
            type="button"
