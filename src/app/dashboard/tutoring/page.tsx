...
import {
  ...
  type TutoringProvider, type TutoringAvailability, type TutoringSpecialty,
...
export default function TutoringDashboardPage() {
  const [p, setP] = useState<TutoringProvider | null>(null)
  ...
  const reload = useCallback(async () => {
    ...
    try {
      ...
      const j = await r.json() as { provider: TutoringProvider | null }
      setP(j.provider)
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const r = await fetch('/api/tutoring/me/analytics', { cache: 'no-store' })
        if (!r.ok) return
  ...
  async function setAvailability(next: TutoringAvailability) {
    if (!p) return
    const prev = p.availability
    setP({ ...p, availability: next })
    try {
      const r = await fetch('/api/tutoring/me/availability', {
  ...
  if (err === 'not_signed_in') return (
    <Shell><div className="px-4 pt-20 max-w-md mx-auto text-center">
      ...
      <Link href="/login?next=/dashboard/tutoring" className="rounded-full bg-brand text-bg px-6 py-3 text-[13px] font-extrabold inline-block">Sign in</Link>
    </div></Shell>
  )
  if (!p) return (
    <Shell><div className="px-4 pt-20 max-w-md mx-auto text-center">
      <h1 className="text-[20px] font-black mb-2">No tutor profile yet</h1>
      <Link href="/tutoring/signup" className="rounded-full bg-brand text-bg px-6 py-3 text-[13px] font-extrabold inline-block">Register as tutor</Link>
    </div></Shell>
  )
  ...
  return (
    <Shell>
      <div className="px-4 pt-6 pb-24 max-w-3xl mx-auto space-y-4">
        ...
        <ProviderRenewBanner provider={p} upgradeHref="/tutoring/upgrade" />
        ...
        >
          <div className="flex items-center justify-between gap-3">
            <div className="min-w-0">
              ...
              <div className="text-[13px] text-black mt-1">Pick mata pelajaran (matematika / fisika / kimia / english / UTBK / mengaji), set per-pertemuan + paket bundle, and add session snapshots that drive the Belajar Bareng gallery.</div>
            </div>
        ...
        <p className="text-[12px] text-black/60 text-center">
          Public profile: <a href={`/tutoring/${p.slug}`} target="_blank" rel="noopener" className="text-brand hover:underline">/tutoring/{p.slug}</a>
        </p>
      </div>
    </Shell>
  )
...
function ReadOnly({ p }: { p: TutoringProvider }) {
  return (
    <div className="space-y-3 text-[13px]">
      <KV k="Years" v={String(p.years_experience)} />
      <KV k="Bio" v={p.bio} multiline />
      <div>
        <div className="text-[11px] uppercase tracking-wider font-bold text-black/55 mb-1">Mata pelajaran</div>
      ...
      <div className="grid grid-cols-2 gap-2 pt-2">
        {p.hourly_rate_idr != null && <PriceBox label="per pertemuan from" v={p.hourly_rate_idr} />}
...
function EditForm({ p, onSaved }: { p: TutoringProvider; onSaved: () => void }) {
  const pe = p as TutoringProvider & {
  ...
  function toggle(s: TutoringSpecialty) {
    setF((p) => {
      if (p.specialties.includes(s)) return { ...p, specialties: p.specialties.filter((x) => x !== s) }
      if (p.specialties.length >= MAX_TUTORING_SPECIALTIES) return p
    ...
    <form onSubmit={save} className="space-y-3">
      ...
      <div>
        <div className="flex items-baseline justify-between mb-1.5">
          <div className="text-[11px] uppercase tracking-wider font-bold text-black/55">Mata pelajaran</div>
          <div className="text-[11px] text-black/55">{f.specialties.length}/{MAX_TUTORING_SPECIALTIES}</div>
        </div>
        <div className="flex flex-wrap gap-1.5">
      ...
      <div className="grid grid-cols-2 gap-2">
        <PriceInput label="per pertemuan from" v={f.hourly_rate} set={(v) => upd('hourly_rate', v)} />
      ...
      <label className="flex items-center gap-2 text-[12px] text-black/85 cursor-pointer">
        ...
        Datang ke rumah (home-visit available)
      </label>
