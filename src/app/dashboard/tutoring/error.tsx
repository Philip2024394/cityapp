...
// /tutoring dashboard page bubbles up to the global layout and can render the
// whole marketplace blank. With this boundary, only the /tutoring segment
// renders the fallback — beautician, tattoo, etc. keep running.
//
...
export default function DashboardTutoringError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    if (process.env.NODE_ENV !== 'production') {
      console.error('[dashboard-tutoring] segment error:', error)
    }
  }, [error])
    ...
    <main className="min-h-[100dvh] bg-white text-[#0A0A0A] flex items-center justify-center px-4">
      <div className="max-w-md w-full text-center py-12">
        ...
        <h1 className="text-[20px] sm:text-[22px] font-black tracking-tight leading-tight mb-2">
          Something went wrong loading Tutoring dashboard
        </h1>
