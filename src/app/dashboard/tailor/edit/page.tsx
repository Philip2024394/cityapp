...
import {
  TAILOR_BANNER_LIBRARY,
  TAILOR_BANNER_CATEGORIES,
} from '@/lib/tailor/banners'
import {
  type TailorProvider,
  type TailorSpecialty,
} from '@/lib/tailor/types'

// WYSIWYG profile editor — tailor sees a live preview of
...
// Tailor brand defaults to rich violet — bridal / couture signage trends
// violet-purple for luxe + custom-craft. Reads as bespoke / premium /
// hand-finished vs mover's logistics-teal. The CTA accent
// (button_text_color) defaults to white for high contrast on violet.
const DEFAULT_THEME = '#9333EA'
const DEFAULT_HERO = {
  line1:   'Penjahit Custom',
  line2:   'Penjahit Custom',
  tagline: 'Kebaya · Jas · Vermak · Seragam',
}
const DEFAULT_HERO_IMAGE = 'https://images.unsplash.com/photo-1558769132-cb1aea458c5e?w=1200&auto=format&fit=crop'

type TailorHeroEffect = 'none' | 'shimmer' | 'dance' | 'underline'
...
type TailorHeroText = {
  ...
  effect?:        TailorHeroEffect
}

type TailorServicePhotoEntry = {
  url:               string
  name?:             string | null
...
export default function TailorEditPage() {
  ...
  const reload = useCallback(async () => {
    ...
    if (!data?.session?.user) { router.replace('/login?next=/dashboard/tailor/edit'); return }
    setLoading(true)
    try {
    ...
    return (
      <Shell>
        <div className="px-4 pt-20 max-w-md mx-auto text-center text-ink">
          <h1 className="text-[20px] font-black mb-2">Not a tailor business yet</h1>
          <Link href="/tailor/signup" className="rounded-full bg-brand text-bg px-6 py-3 text-[13px] font-extrabold inline-block">Register</Link>
        </div>
      </Shell>
    )
  }
  ...
  const effect: TailorHeroEffect = (['none','shimmer','dance','underline'].includes(rawEffect) ? rawEffect : 'none') as TailorHeroEffect
    ...
    <Shell>
      <div className="max-w-2xl mx-auto pt-3 pb-32 px-4">
        ...
        {/* Inline banner controls — sit RIGHT under the preview so the
            tailor can see what they're editing without opening
        ...
        <p className="text-[12px] text-black/55 mt-4 leading-snug">
          ...
          {' '}<Link href="/dashboard/tailor" className="text-yellow-700 hover:underline font-bold">full dashboard form</Link>{' '}
          for the rest.
        </p>
      </div>
    </Shell>
...
function BannerInlineControls({
  ...
  const [draftEffect,       setDraftEffect]       = useState<TailorHeroEffect>(((['none','shimmer','dance','underline'].includes(ht.effect ?? 'none')) ? (ht.effect ?? 'none') : 'none') as TailorHeroEffect)
  const [savedFlash,   setSavedFlash]   = useState(false)
  // "Follow theme" mode: true when the tailor hasn't
  // explicitly overridden the hero text color. In that mode, picking
  // a new theme color auto-recolors the Tailor word. The
  // moment the tailor picks a custom color from the swatches below,
  ...
  const EFFECTS: Array<{ id: TailorHeroEffect; label: string; desc: string }> = [
  ...
  return (
    <div className="mt-4 space-y-5">
      ...
      {/* Theme color — drives the entire page's accent (buttons, badges,
          star ratings, "Penjahit Custom" overlay default, etc.). Lives
          FIRST so tailors make the brand decision before fine-tuning.
          Picking a theme color ALSO clears any prior hero-text color
          override so the "Penjahit Custom" word follows the new theme.
      ...
      <Section title="Banner image" icon={<ImageIcon size={16} strokeWidth={2.5} />}>
        <BannerLibraryPicker
          ...
          library={TAILOR_BANNER_LIBRARY}
          categories={TAILOR_BANNER_CATEGORIES}
      ...
      <Section title="Banner text" icon={<Type size={16} strokeWidth={2.5} />}>
        <div className="space-y-3">
          ...
          <FieldWithColor
            ...
            placeholder="Kebaya · Jas · Vermak · Seragam"
      ...
      {/* Effect */}
      <Section title="Text effect (Penjahit Custom word)" icon={<Sparkles size={16} strokeWidth={2.5} />}>
      ...
      {/* Running marquee text — the yellow ribbon that scrolls under the
          ...
          (tailor API cap, mig 0089-equiv). */}
      <Section title="Running text (marquee under armada snapshots)" icon={<Megaphone size={16} strokeWidth={2.5} />}>
        <PromoTextEditor
      ...
      <Section title="QRIS payment" icon={<QrCodeIcon228 size={16} strokeWidth={2.5} />}>
        <QrisUploaderCard apiBase="/api/tailor" accentHex={theme} />
      </Section>
      ...
      <Section title="More" icon={<MoreHorizontal size={16} strokeWidth={2.5} />}>
        <div className="grid grid-cols-2 gap-2">
          <Link
            href="/dashboard/tailor/services"
      ...
      {/* Explicit Save button — auto-save runs in the background but this
          gives tailors a clear "I'm done" confirmation.
...
function PromoTextEditor({
  ...
  const display = (draft.trim() || 'Setiap jahitan, sesuai badanmu. Vermak mulai Rp 25k. Jas pria custom Rp 800k tanpa bahan. Kebaya bridal premium full payet dengan 3x fitting. Free konsultasi design via WhatsApp.') + ' ✦'
  ...
  return (
    <div className="space-y-2">
      ...
      <textarea
        ...
        maxLength={280}
        rows={3}
        placeholder="Write your promo message — appears as scrolling text below your Galeri Jahitan gallery."
      ...
      <p className="text-[12px] text-black/60 leading-snug">
        Use these for promos — free konsultasi, paket seragam diskon, bridal lead-time, vermak express.
        Tap a suggestion to drop it in.
      </p>
      <div className="flex flex-wrap gap-1.5 pt-1">
        {[
          'Free Konsultasi Design via WhatsApp — kirim referensi, kami balas dengan estimasi harga + lead time',
          'Paket Seragam Kantor Diskon — Rp 200k/pcs untuk min 12 pcs, kemeja kerah + logo bordir include',
          'Kebaya Bridal Premium — full payet hand-made, brokat import, 3x fitting, lead time 4-6 minggu',
          'Vermak Express 3 Hari — pendekan celana, ganti resleting, sesuaikan ukuran, antar-jemput dalam kota',
          'Bahan dari Customer atau Stock Kami — pilih sendiri kain, atau pakai brokat / wool / poly cotton dari stok',
        ].map((s) => (
          <button
            key={s}
            type="button"
