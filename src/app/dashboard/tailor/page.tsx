...
import {
  ...
  type TailorProvider, type TailorAvailability, type TailorSpecialty,
...
export default function TailorDashboardPage() {
  const [p, setP] = useState<TailorProvider | null>(null)
  ...
  const reload = useCallback(async () => {
    ...
    try {
      ...
      const j = await r.json() as { provider: TailorProvider | null }
      setP(j.provider)
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const r = await fetch('/api/tailor/me/analytics', { cache: 'no-store' })
        if (!r.ok) return
  ...
  async function setAvailability(next: TailorAvailability) {
    if (!p) return
    const prev = p.availability
    setP({ ...p, availability: next })
    try {
      const r = await fetch('/api/tailor/me/availability', {
  ...
  if (err === 'not_signed_in') return (
    <Shell><div className="px-4 pt-20 max-w-md mx-auto text-center">
      ...
      <Link href="/login?next=/dashboard/tailor" className="rounded-full bg-brand text-bg px-6 py-3 text-[13px] font-extrabold inline-block">Sign in</Link>
    </div></Shell>
  )
  if (!p) return (
    <Shell><div className="px-4 pt-20 max-w-md mx-auto text-center">
      <h1 className="text-[20px] font-black mb-2">No tailor profile yet</h1>
      <Link href="/tailor/signup" className="rounded-full bg-brand text-bg px-6 py-3 text-[13px] font-extrabold inline-block">Register tailor</Link>
    </div></Shell>
  )
  ...
  return (
    <Shell>
      <div className="px-4 pt-6 pb-24 max-w-3xl mx-auto space-y-4">
        ...
        <ProviderRenewBanner provider={p} upgradeHref="/tailor/upgrade" />
        ...
        >
          <div className="flex items-center justify-between gap-3">
            <div className="min-w-0">
              <div className="text-[14px] font-extrabold uppercase tracking-wider text-brand">Katalog Jahitan →</div>
              <div className="text-[13px] text-black mt-1">Pick garment specialties (Kebaya / Jas / Kemeja / Gaun / Seragam / Vermak / Streetwear / Muslim wear), set anchor price + paket bridal/seragam rate, and add Galeri Jahitan photos that customers browse.</div>
            </div>
        ...
        <p className="text-[12px] text-black/60 text-center">
          Public profile: <a href={`/tailor/${p.slug}`} target="_blank" rel="noopener" className="text-brand hover:underline">/tailor/{p.slug}</a>
        </p>
      </div>
    </Shell>
  )
...
function ReadOnly({ p }: { p: TailorProvider }) {
  return (
...
function EditForm({ p, onSaved }: { p: TailorProvider; onSaved: () => void }) {
  const pe = p as TailorProvider & {
  ...
  function toggle(s: TailorSpecialty) {
    setF((p) => {
      if (p.specialties.includes(s)) return { ...p, specialties: p.specialties.filter((x) => x !== s) }
      if (p.specialties.length >= MAX_TAILOR_SPECIALTIES) return p
    ...
    <form onSubmit={save} className="space-y-3">
      ...
      <div>
        <div className="flex items-baseline justify-between mb-1.5">
          ...
          <div className="text-[11px] text-black/55">{f.specialties.length}/{MAX_TAILOR_SPECIALTIES}</div>
        </div>
        <div className="flex flex-wrap gap-1.5">
      ...
      <label className="flex items-center gap-2 text-[12px] text-black/85 cursor-pointer">
        ...
        Datang ke Rumah (home-visit for measurement)
      </label>
