'use client'
// ============================================================================
// /dashboard/car/edit — Page design (banner picker)
// ----------------------------------------------------------------------------
// Drivers pick the hero backdrop shown on their public /car/[slug]
...
// this page scoped to what the customer actually sees on /car/[slug].
// ============================================================================
...
export default function CarEditPage() {
  ...
  if (state.kind === 'unauth')      return <FullPageMessage cta={{ href: '/login?next=/dashboard/car/edit', label: 'Sign in' }}>Sign in to edit your page design.</FullPageMessage>
...
function EditShell({ row, onSaved }: { row: DriverRow; onSaved: () => void }) {
  ...
  const profileHref = row.slug ? `/car/${row.slug}` : null
