'use client'
// ============================================================================
// /dashboard/car/services — Services & rates (Phase 1B real implementation)
// ----------------------------------------------------------------------------
...
import { carZoneForCity, CAR_BATAS_BAWAH_PER_KM, CAR_MIN_FEE_BY_ZONE } from '@/lib/pricing/zones'
...
export default function CarServicesPage() {
  ...
  const reload = useCallback(async () => {
    ...
    const { data, error } = await supabase
      .from('drivers')
      .select(
        'user_id, vehicle_type, vehicle_make, vehicle_model, city, service_offerings, ' +
  ...
  if (state.kind === 'unauth')      return <FullMsg cta={{ href: '/login?next=/dashboard/car/services', label: 'Sign in' }}>Sign in to edit your services.</FullMsg>
  if (state.kind === 'no_driver')   return <FullMsg cta={{ href: '/signup?role=driver&vehicle=car', label: 'Create driver profile' }}>No driver profile yet.</FullMsg>
...
function PricingCard({
  ...
  // The numbers come from PM 118/2018 + KP 348/2019 (car batas bawah per
  // zone). The platform shows them as a REFERENCE; the driver self-
  // publishes their actual rate. This keeps CityDrivers outside Permenhub
  // PM 12/2019's aplikator scope.
  const market = getMarketDefault('car', row.city)
  const zone = carZoneForCity(row.city)
  const bawahPerKm = CAR_BATAS_BAWAH_PER_KM[zone]
  const bawahMinFee = CAR_MIN_FEE_BY_ZONE[zone]
  ...
  return (
    <Card
      ...
      hint={`YOUR published rates for point-to-point trips. CityDrivers shows them as-is — we never compute fares. Zone ${zone} reference (PM 118/2018): Rp ${formatIdr(bawahPerKm)}/km.`}
    ...
    >
      <div className="grid grid-cols-2 gap-2">
        <div className="space-y-1">
          <IdrInput
            label="Price per km"
            placeholder={market ? `Zone ${zone}: Rp ${formatIdr(market.price_per_km)}` : '3.500'}
            value={perKm}
            onChange={setPerKm}
        ...
        <div className="space-y-1">
          <IdrInput
            label="Minimum fee"
            placeholder={market ? `Zone ${zone}: Rp ${formatIdr(market.min_fee)}` : '20.000'}
            value={minFee}
            onChange={setMinFee}
      ...
      {(perKmBelowBawah || minFeeBelowBawah) && (
        ...
        >
          <strong className="font-extrabold">Below the Zone {zone} minimum (PM 118/2018).</strong>{' '}
          ...
          You can publish below this and customers will still see your profile, but most drivers earn more by sticking to the zone reference.
        </div>
      )}
    </Card>
  )
