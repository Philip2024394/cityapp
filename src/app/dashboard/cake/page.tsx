...
import {
  ...
  type CakeProvider, type CakeAvailability, type CakeSpecialty,
} from '@/lib/cake/types'
import {
  CAKE_BANNER_LIBRARY,
...
export default function CakeDashboardPage() {
  const [p, setP] = useState<CakeProvider | null>(null)
  ...
  const reload = useCallback(async () => {
    ...
    try {
      ...
      const j = await r.json() as { provider: CakeProvider | null }
      setP(j.provider)
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const r = await fetch('/api/cake/me/analytics', { cache: 'no-store' })
        if (!r.ok) return
  ...
  async function setAvailability(next: CakeAvailability) {
    if (!p) return
    const prev = p.availability
    setP({ ...p, availability: next })
    try {
      const r = await fetch('/api/cake/me/availability', {
  ...
  if (err === 'not_signed_in') return (
    <Shell><div className="px-4 pt-20 max-w-md mx-auto text-center">
      ...
      <Link href="/login?next=/dashboard/cake" className="rounded-full bg-brand text-bg px-6 py-3 text-[13px] font-extrabold inline-block">Sign in</Link>
    </div></Shell>
  )
  if (!p) return (
    <Shell><div className="px-4 pt-20 max-w-md mx-auto text-center">
      <h1 className="text-[20px] font-black mb-2">No cake & bakery business profile yet</h1>
      <Link href="/cake/signup" className="rounded-full bg-brand text-bg px-6 py-3 text-[13px] font-extrabold inline-block">Register as cake & bakery business</Link>
    </div></Shell>
  )
  ...
  return (
    <Shell>
      <div className="px-4 pt-6 pb-24 max-w-3xl mx-auto space-y-4">
        ...
        <ProviderRenewBanner provider={p} upgradeHref="/cake/upgrade" />
        ...
        {/* Quick-link to the dedicated Cake catalog & portfolio editor. */}
        <Link
        ...
        >
          <div className="flex items-center justify-between gap-3">
            <div className="min-w-0">
              <div className="text-[14px] font-extrabold uppercase tracking-wider text-brand">Cake catalog &amp; portfolio →</div>
              <div className="text-[13px] text-black mt-1">Pick kategori (birthday / Korean bento / dessert table), set per-cake + paket bundle, and add cake photos that drive the gallery.</div>
            </div>
        ...
        <p className="text-[12px] text-black/60 text-center">
          Public profile: <a href={`/cake/${p.slug}`} target="_blank" rel="noopener" className="text-brand hover:underline">/cake/{p.slug}</a>
        </p>
      </div>
    </Shell>
  )
...
function ReadOnly({ p }: { p: CakeProvider }) {
  return (
    <div className="space-y-3 text-[13px]">
      <KV k="Years" v={String(p.years_experience)} />
      <KV k="Bio" v={p.bio} multiline />
      <div>
        <div className="text-[11px] uppercase tracking-wider font-bold text-black/55 mb-1">Kategori kue</div>
      ...
      <div className="grid grid-cols-2 gap-2 pt-2">
        {p.hourly_rate_idr != null && <PriceBox label="Per-cake from" v={p.hourly_rate_idr} />}
...
function EditForm({ p, onSaved }: { p: CakeProvider; onSaved: () => void }) {
  const pe = p as CakeProvider & {
  ...
  function toggle(s: CakeSpecialty) {
    setF((p) => {
      if (p.specialties.includes(s)) return { ...p, specialties: p.specialties.filter((x) => x !== s) }
      if (p.specialties.length >= MAX_CAKE_SPECIALTIES) return p
    ...
    <form onSubmit={save} className="space-y-3">
      ...
      <div>
        <div className="flex items-baseline justify-between mb-1.5">
          <div className="text-[11px] uppercase tracking-wider font-bold text-black/55">Kategori kue</div>
          <div className="text-[11px] text-black/55">{f.specialties.length}/{MAX_CAKE_SPECIALTIES}</div>
        </div>
        <div className="flex flex-wrap gap-1.5">
      ...
      <div className="grid grid-cols-2 gap-2">
        <PriceInput label="Per-cake from" v={f.hourly_rate} set={(v) => upd('hourly_rate', v)} />
