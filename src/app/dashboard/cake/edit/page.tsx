...
import {
  CAKE_BANNER_LIBRARY,
  CAKE_BANNER_CATEGORIES,
} from '@/lib/cake/banners'
import {
  type CakeProvider,
  type CakeSpecialty,
} from '@/lib/cake/types'

// WYSIWYG profile editor — cake & bakery business sees a live preview of
...
// Cake brand defaults to soft pink — bakery aesthetic. Pink imagery
// is appetising for sweets, photographs well on a feed, and matches
// the pastel buttercream + sprinkle palette that dominates Indonesian
// cake studio Instagram. The CTA accent (button_text_color) defaults
// to white for high contrast on pink.
const DEFAULT_THEME = '#F472B6'
const DEFAULT_HERO = {
  line1:   'Kue & Bakery',
  line2:   'Cake & bakery business',
  tagline: 'Birthday · Wedding · Korean Bento · Dessert Table',
}
const DEFAULT_HERO_IMAGE = 'https://images.unsplash.com/photo-1535254973040-607b474cb50d?w=1200&auto=format&fit=crop'

type CakeHeroEffect = 'none' | 'shimmer' | 'dance' | 'underline'
...
type CakeHeroText = {
  ...
  effect?:        CakeHeroEffect
}

type CakeServicePhotoEntry = {
  url:               string
  name?:             string | null
...
export default function CakeEditPage() {
  ...
  const reload = useCallback(async () => {
    ...
    if (!data?.session?.user) { router.replace('/login?next=/dashboard/cake/edit'); return }
    setLoading(true)
    try {
    ...
    return (
      <Shell>
        <div className="px-4 pt-20 max-w-md mx-auto text-center text-ink">
          <h1 className="text-[20px] font-black mb-2">Not a cake & bakery business yet</h1>
          <Link href="/cake/signup" className="rounded-full bg-brand text-bg px-6 py-3 text-[13px] font-extrabold inline-block">Register</Link>
        </div>
      </Shell>
    )
  }
  ...
  const effect: CakeHeroEffect = (['none','shimmer','dance','underline'].includes(rawEffect) ? rawEffect : 'none') as CakeHeroEffect
    ...
    <Shell>
      <div className="max-w-2xl mx-auto pt-3 pb-32 px-4">
        ...
        {/* Inline banner controls — sit RIGHT under the preview so the
            cake & bakery business can see what they're editing without opening a
        ...
        <p className="text-[12px] text-black/55 mt-4 leading-snug">
          ...
          {' '}<Link href="/dashboard/cake" className="text-yellow-700 hover:underline font-bold">full dashboard form</Link>{' '}
          for the rest.
        </p>
      </div>
    </Shell>
...
function BannerInlineControls({
  ...
  const [draftEffect,       setDraftEffect]       = useState<CakeHeroEffect>(((['none','shimmer','dance','underline'].includes(ht.effect ?? 'none')) ? (ht.effect ?? 'none') : 'none') as CakeHeroEffect)
  const [savedFlash,   setSavedFlash]   = useState(false)
  // "Follow theme" mode: true when the cake & bakery business hasn't explicitly
  // overridden the hero text color. In that mode, picking a new theme
  // color auto-recolors the Cake business word. The moment the cake &
  // bakery business picks a custom color from the swatches below, this
  ...
  const EFFECTS: Array<{ id: CakeHeroEffect; label: string; desc: string }> = [
  ...
  return (
    <div className="mt-4 space-y-5">
      ...
      {/* Theme color — drives the entire page's accent (buttons, badges,
          star ratings, "Cake business" overlay default, etc.). Lives FIRST
          so cake & bakery businesses make the brand decision before fine-tuning.
          Picking a theme color ALSO clears any prior hero-text color
          override so the "Cake business" word follows the new theme.
      ...
      <Section title="Banner image" icon={<ImageIcon size={16} strokeWidth={2.5} />}>
        <BannerLibraryPicker
          ...
          library={CAKE_BANNER_LIBRARY}
          categories={CAKE_BANNER_CATEGORIES}
      ...
      <Section title="Banner text" icon={<Type size={16} strokeWidth={2.5} />}>
        <div className="space-y-3">
          ...
          <FieldWithColor
            ...
            placeholder="Birthday · Wedding · Korean Bento · Dessert Table"
      ...
      {/* Effect */}
      <Section title="Text effect (Cake business word)" icon={<Sparkles size={16} strokeWidth={2.5} />}>
      ...
      {/* Running marquee text — the yellow ribbon that scrolls under the
          ...
          (cake API cap, mig 0089-equiv). */}
      <Section title="Running text (marquee under cake gallery)" icon={<Megaphone size={16} strokeWidth={2.5} />}>
        <PromoTextEditor
      ...
      <Section title="QRIS payment" icon={<QrCodeIcon228 size={16} strokeWidth={2.5} />}>
        <QrisUploaderCard apiBase="/api/cake" accentHex={theme} />
      </Section>
      ...
      {/* Explicit Save button — auto-save runs in the background but this
          gives cake & bakery businesses a clear "I'm done" confirmation.
...
function PromoTextEditor({
  ...
  const display = (draft.trim() || 'Kue ulang tahun custom yang bikin foto-foto cantik — pesan H-3 untuk birthday, H-2 untuk Korean bento. Konsultasi via WhatsApp.') + ' ✦'
  ...
  return (
    <div className="space-y-2">
      ...
      <textarea
        ...
        maxLength={280}
        rows={3}
        placeholder="Write your promo message — appears as scrolling text below your cake gallery."
      ...
      <p className="text-[12px] text-black/60 leading-snug">
        Use this for promos — Korean bento flash sale, hampers Lebaran, custom 3D launch.
        Tap a suggestion to drop it in.
      </p>
      <div className="flex flex-wrap gap-1.5 pt-1">
        {[
          'Korean Bento mini Rp 95k — viral, lead time 2 hari, custom message',
          'Hampers Lebaran cookies Rp 95k/box — minimum 5 box, halal sertifikat',
          'Custom 3D figurine cake — design sendiri, lead time 7 hari',
          'Dessert Table 30-pax Rp 1.5jt — 5 jenis mini dessert, siap acara',
          'Birthday 1kg Rp 350k — hiasan custom sesuai tema, lead time 3 hari',
        ].map((s) => (
          <button
            key={s}
            type="button"
