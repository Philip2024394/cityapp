...
// /dashboard/beautician/orders — the vendor's cart-order inbox. Lists every
// vendor_orders row owned by this beautician, with filter chips, expand-to-
// see-detail rows, and Accept / Fulfill / Cancel actions that POST to
// /api/beautician/me/orders/[id].
//
// Stays close to /dashboard/beautician/promos for visual parity — same
...
export default function BeauticianOrdersPage() {
  ...
  const reload = useCallback(async (isRefresh = false) => {
    ...
    try {
      const [pRes, oRes] = await Promise.all([
        ...
        fetch('/api/beautician/me/orders?status=all', { cache: 'no-store' }),
      ])
    ...
    return (
      <Shell>
        <div className="px-4 pt-20 max-w-md mx-auto text-center">
          ...
          <Link href="/login?next=/dashboard/beautician/orders" className="rounded-full bg-pink-500 text-white px-6 py-3 text-[14px] font-extrabold inline-block">Sign in</Link>
        </div>
      </Shell>
    )
  }
...
function OrderRow({
  ...
  async function setStatus(next: 'accepted' | 'fulfilled' | 'cancelled', kind: 'accept' | 'fulfill' | 'cancel') {
    ...
    try {
      const r = await fetch(`/api/beautician/me/orders/${order.id}`, {
        method:  'POST',
