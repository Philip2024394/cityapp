...
import {
  MOVER_BANNER_LIBRARY,
  MOVER_BANNER_CATEGORIES,
} from '@/lib/mover/banners'
import {
  type MoverProvider,
  type MoverSpecialty,
} from '@/lib/mover/types'

// WYSIWYG profile editor — mover sees a live preview of
...
// Mover brand defaults to deep teal — moving / logistics signage trends
// teal-green for trustworthy + sturdy. Reads as reliable / professional
// / heavy-duty vs pet's playful orange. The CTA accent
// (button_text_color) defaults to white for high contrast on teal.
const DEFAULT_THEME = '#0F766E'
const DEFAULT_HERO = {
  line1:   'Jasa Pindahan',
  line2:   'Jasa Pindahan',
  tagline: 'Grandmax · Pickup · Box · Antar Kota',
}
const DEFAULT_HERO_IMAGE = 'https://images.unsplash.com/photo-1600320254374-ce2d293c324e?w=1200&auto=format&fit=crop'

type MoverHeroEffect = 'none' | 'shimmer' | 'dance' | 'underline'
...
type MoverHeroText = {
  ...
  effect?:        MoverHeroEffect
}

type MoverServicePhotoEntry = {
  url:               string
  name?:             string | null
...
export default function MoverEditPage() {
  ...
  const reload = useCallback(async () => {
    ...
    if (!data?.session?.user) { router.replace('/login?next=/dashboard/mover/edit'); return }
    setLoading(true)
    try {
      const r = await fetch('/api/mover/me', { cache: 'no-store' })
      if (r.ok) {
  ...
  async function save(patch: Partial<FullProvider>) {
    const r = await fetch('/api/mover/me/profile', {
      method: 'POST',
    ...
    return (
      <Shell>
        <div className="px-4 pt-20 max-w-md mx-auto text-center text-ink">
          <h1 className="text-[20px] font-black mb-2">Not a mover business yet</h1>
          <Link href="/mover/signup" className="rounded-full bg-brand text-bg px-6 py-3 text-[13px] font-extrabold inline-block">Register</Link>
        </div>
      </Shell>
    )
  }
  ...
  const effect: MoverHeroEffect = (['none','shimmer','dance','underline'].includes(rawEffect) ? rawEffect : 'none') as MoverHeroEffect
    ...
    <Shell>
      <div className="max-w-2xl mx-auto pt-3 pb-32 px-4">
        ...
        {/* Inline banner controls — sit RIGHT under the preview so the
            mover can see what they're editing without opening
        ...
        <p className="text-[12px] text-black/55 mt-4 leading-snug">
          ...
          {' '}<Link href="/dashboard/mover" className="text-yellow-700 hover:underline font-bold">full dashboard form</Link>{' '}
          for the rest.
        </p>
      </div>
    </Shell>
...
function BannerInlineControls({
  ...
  const [draftEffect,       setDraftEffect]       = useState<MoverHeroEffect>(((['none','shimmer','dance','underline'].includes(ht.effect ?? 'none')) ? (ht.effect ?? 'none') : 'none') as MoverHeroEffect)
  const [savedFlash,   setSavedFlash]   = useState(false)
  // "Follow theme" mode: true when the mover hasn't
  // explicitly overridden the hero text color. In that mode, picking
  // a new theme color auto-recolors the Mover word. The
  // moment the mover picks a custom color from the swatches below,
  ...
  const EFFECTS: Array<{ id: MoverHeroEffect; label: string; desc: string }> = [
  ...
  return (
    <div className="mt-4 space-y-5">
      ...
      {/* Theme color — drives the entire page's accent (buttons, badges,
          star ratings, "Jasa Pindahan" overlay default, etc.). Lives
          FIRST so movers make the brand decision before fine-tuning.
          Picking a theme color ALSO clears any prior hero-text color
          override so the "Jasa Pindahan" word follows the new theme.
      ...
      <Section title="Banner image" icon={<ImageIcon size={16} strokeWidth={2.5} />}>
        <BannerLibraryPicker
          ...
          library={MOVER_BANNER_LIBRARY}
          categories={MOVER_BANNER_CATEGORIES}
      ...
      <Section title="Banner text" icon={<Type size={16} strokeWidth={2.5} />}>
        <div className="space-y-3">
          ...
          <FieldWithColor
            ...
            placeholder="Grandmax · Pickup · Box · Antar Kota"
      ...
      {/* Effect */}
      <Section title="Text effect (Jasa Pindahan word)" icon={<Sparkles size={16} strokeWidth={2.5} />}>
      ...
      <Section title="QRIS payment" icon={<QrCodeIcon228 size={16} strokeWidth={2.5} />}>
        <QrisUploaderCard apiBase="/api/mover" accentHex={theme} />
      </Section>
      ...
      {/* Explicit Save button — auto-save runs in the background but this
          gives movers a clear "I'm done" confirmation.
...
function PromoTextEditor({
  ...
  const display = (draft.trim() || 'Pindah tanpa drama. Kami yang angkat. Gratis survey rumah penuh. Packing material include + asuransi all-risk. Antar provinsi cross-Java available. Reservasi via WhatsApp.') + ' ✦'
  ...
  return (
    <div className="space-y-2">
      ...
      <textarea
        ...
        maxLength={280}
        rows={3}
        placeholder="Write your promo message — appears as scrolling text below your Armada & Pindahan gallery."
      ...
      <p className="text-[12px] text-black/60 leading-snug">
        Use these for promos — gratis survey, packing material include, asuransi all-risk, antar kota / antar provinsi available.
        Tap a suggestion to drop it in.
      </p>
      <div className="flex flex-wrap gap-1.5 pt-1">
        {[
          'Gratis Survey Pindahan Rumah Penuh — kami datang ke lokasi sebelum quote, tanpa biaya',
          'Packing Material INCLUDE — kardus + bubble wrap + wrapping film + plester, tidak perlu beli sendiri',
          'Asuransi All-Risk Barang — perlindungan 1-2% nilai barang selama perjalanan',
          'Antar Provinsi Cross-Java Available — Yogya · Solo · Semarang · Surabaya · Jakarta · Bandung',
          'Grandmax Dalam Kota Mulai Rp 350k — driver + 1 helper, 10km pertama, cocok kasur/lemari/kulkas single',
        ].map((s) => (
          <button
            key={s}
            type="button"
