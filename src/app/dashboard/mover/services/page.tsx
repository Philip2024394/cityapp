...
import {
  ALL_SPECIALTIES, SPECIALTY_LABELS, MAX_MOVER_SPECIALTIES,
  type MoverProvider, type MoverSpecialty,
} from '@/lib/mover/types'

// Mirrors /dashboard/pet/services. Single page for everything the
// mover sells:
//   1. Specialties — max 3 across the vehicle + service axes (grandmax /
//      pickup / box / cdd / wing / tronton / home_small / home_full /
//      office_relocation / single_furniture / heavy_lifting /
//      packing_service / insurance).
//   2. Pricing — anchor "starting from" price (cheapest dalam-kota
//      base, e.g. Rp 350k Grandmax) + optional paket rumah-penuh
//      all-in rate. Mover pricing has THREE axes: vehicle tier
//      (Grandmax cheap → Tronton premium), distance (dalam kota base
//      + km after first 10km), AND crew size (driver-only / +1 helper
//      / +2 crew). Floor is Rp 180k (single furniture pickup-drop);
//      full-house Box CDD ~Rp 2.5jt; antar provinsi CDD survey first.
//   3. Armada & Pindahan (service_photos) with per-photo metadata
//      (name, description, vehicle + service price, optional
//      before/after pindahan pair) — drives the Armada & Pindahan +
//      View Details popup on /mover/[slug].
...
export default function MoverServicesPage() {
    ...
    return (
      <Shell>
        <div className="px-4 pt-20 max-w-md mx-auto text-center">
          ...
          <Link href="/login?next=/dashboard/mover/services" className="rounded-full bg-yellow-500 text-black px-6 py-3 text-[14px] font-extrabold inline-block">Sign in</Link>
        </div>
      </Shell>
    )
  }
  if (!provider) {
    return (
      <Shell>
        <div className="px-4 pt-20 max-w-md mx-auto text-center">
          <h1 className="text-[20px] font-black text-[#0A0A0A] mb-2">Not a mover business yet</h1>
          <Link href="/mover/signup" className="rounded-full bg-yellow-500 text-black px-6 py-3 text-[14px] font-extrabold inline-block">Sign up</Link>
        </div>
      </Shell>
    )
  }
    ...
    <Shell>
      <div className="px-4 pt-3 pb-32 max-w-lg mx-auto">
        <header className="mb-5 flex items-center justify-between gap-3">
          <div>
            <Link href="/dashboard/mover" className="text-[12px] font-bold text-gray-500 hover:text-gray-900 inline-block">← Dashboard</Link>
            <h1 className="text-[24px] font-black text-[#0A0A0A] leading-tight">Armada &amp; Layanan</h1>
            <p className="text-[13px] text-gray-600 mt-1">Pick vehicle + services, set anchor price + paket rumah-penuh rate, and add Armada & Pindahan photos that owners browse.</p>
          </div>
        </header>
...
function ServicesForm({ provider, onSaved }: { provider: FullProvider; onSaved: () => void }) {
  ...
  function toggleSpecialty(sid: MoverSpecialty) {
    const on = f.specialties.includes(sid)
    if (!on && f.specialties.length >= MAX_MOVER_SPECIALTIES) return
  ...
  return (
    <form onSubmit={save} className="space-y-4">
      {/* 1. Specialties */}
      <Card title="Armada & Layanan" hint={`Pick up to ${MAX_MOVER_SPECIALTIES} vehicle + services — these become the chips on your profile (${f.specialties.length}/${MAX_MOVER_SPECIALTIES}).`}>
        <div className="flex flex-wrap gap-1.5">
          {ALL_SPECIALTIES.map((sid) => {
            const on    = f.specialties.includes(sid)
            const atCap = !on && f.specialties.length >= MAX_MOVER_SPECIALTIES
            return (
              <button
                key={sid}
                type="button"
      ...
      {/* 2. Pricing */}
      <Card title="Rates" hint="Shown on the public profile. Type thousands — e.g. 350 = Rp 350k Grandmax dalam kota base, 2500 = Rp 2.5jt paket Box CDD rumah penuh, 180 = Rp 180k single-furniture floor (max 9999).">
        <div className="grid grid-cols-2 gap-2">
          {([
            { k: 'hourly_rate', label: 'Mulai dari (anchor)' },
            { k: 'day_rate',    label: 'Paket rumah penuh'    },
        ...
        {/* Antar Provinsi toggle. Stored on has_own_tools for column
            reuse; the mover surface label (interstate cross-Java /
            cross-island readiness) is what matters to the buyer. */}
        <label className="flex items-center gap-2 text-[13px] text-gray-700 cursor-pointer mt-2">
          ...
          />
          Antar Provinsi (siap interstate cross-Java / cross-island)
        </label>
      </Card>

      {/* 3. Portfolio */}
      <Card title="Armada & Pindahan" hint="Up to 12 photos of armada (Grandmax/Pickup/Box/CDD/Wing), dokumentasi pindahan rumah penuh, kantor relocation, packing material in action — with name, vehicle + service price, optional before/after pindahan pair. These power the Armada & Pindahan + View Details popup on your profile — owners pilih mover by visual.">
        {provider.user_id && (
