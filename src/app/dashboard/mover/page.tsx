...
import {
  ...
  type MoverProvider, type MoverAvailability, type MoverSpecialty,
...
export default function MoverDashboardPage() {
  const [p, setP] = useState<MoverProvider | null>(null)
  ...
  const reload = useCallback(async () => {
    ...
    try {
      ...
      const j = await r.json() as { provider: MoverProvider | null }
      setP(j.provider)
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const r = await fetch('/api/mover/me/analytics', { cache: 'no-store' })
        if (!r.ok) return
  ...
  async function setAvailability(next: MoverAvailability) {
    if (!p) return
    const prev = p.availability
    setP({ ...p, availability: next })
    try {
      const r = await fetch('/api/mover/me/availability', {
  ...
  if (err === 'not_signed_in') return (
    <Shell><div className="px-4 pt-20 max-w-md mx-auto text-center">
      ...
      <Link href="/login?next=/dashboard/mover" className="rounded-full bg-brand text-bg px-6 py-3 text-[13px] font-extrabold inline-block">Sign in</Link>
    </div></Shell>
  )
  if (!p) return (
    <Shell><div className="px-4 pt-20 max-w-md mx-auto text-center">
      <h1 className="text-[20px] font-black mb-2">No mover profile yet</h1>
      <Link href="/mover/signup" className="rounded-full bg-brand text-bg px-6 py-3 text-[13px] font-extrabold inline-block">Register mover</Link>
    </div></Shell>
  )
  ...
  return (
    <Shell>
      <div className="px-4 pt-6 pb-24 max-w-3xl mx-auto space-y-4">
        ...
        <ProviderRenewBanner provider={p} upgradeHref="/mover/upgrade" />
        ...
        {/* Quick-link to the dedicated Armada catalog & portfolio editor. */}
        <Link
        ...
        >
          <div className="flex items-center justify-between gap-3">
            <div className="min-w-0">
              <div className="text-[14px] font-extrabold uppercase tracking-wider text-brand">Armada &amp; Layanan →</div>
              <div className="text-[13px] text-black mt-1">Pick vehicle + services (Grandmax / Pickup / Box CDD / Wing / rumah penuh / kantor / single furniture), set anchor price + paket rumah-penuh rate, and add Armada & Pindahan photos that owners browse.</div>
            </div>
        ...
        <p className="text-[12px] text-black/60 text-center">
          Public profile: <a href={`/mover/${p.slug}`} target="_blank" rel="noopener" className="text-brand hover:underline">/mover/{p.slug}</a>
        </p>
      </div>
    </Shell>
  )
...
function ReadOnly({ p }: { p: MoverProvider }) {
  return (
    <div className="space-y-3 text-[13px]">
      <KV k="Years" v={String(p.years_experience)} />
      <KV k="Bio" v={p.bio} multiline />
      <div>
        <div className="text-[11px] uppercase tracking-wider font-bold text-black/55 mb-1">Specialties</div>
      ...
      <div className="grid grid-cols-2 gap-2 pt-2">
        {p.hourly_rate_idr != null && <PriceBox label="mulai dari" v={p.hourly_rate_idr} />}
        {p.day_rate_idr    != null && <PriceBox label="paket rumah penuh" v={p.day_rate_idr}    />}
      </div>
      <KV k="Antar Provinsi" v={p.has_own_tools ? 'Yes' : 'No'} />
...
function EditForm({ p, onSaved }: { p: MoverProvider; onSaved: () => void }) {
  const pe = p as MoverProvider & {
  ...
  function toggle(s: MoverSpecialty) {
    setF((p) => {
      if (p.specialties.includes(s)) return { ...p, specialties: p.specialties.filter((x) => x !== s) }
      if (p.specialties.length >= MAX_MOVER_SPECIALTIES) return p
    ...
    <form onSubmit={save} className="space-y-3">
      ...
      <div>
        <div className="flex items-baseline justify-between mb-1.5">
          <div className="text-[11px] uppercase tracking-wider font-bold text-black/55">Specialties</div>
          <div className="text-[11px] text-black/55">{f.specialties.length}/{MAX_MOVER_SPECIALTIES}</div>
        </div>
        <div className="flex flex-wrap gap-1.5">
      ...
      <div className="grid grid-cols-2 gap-2">
        <PriceInput label="mulai dari" v={f.hourly_rate} set={(v) => upd('hourly_rate', v)} />
        <PriceInput label="paket rumah penuh"        v={f.day_rate}    set={(v) => upd('day_rate', v)} />
      </div>
      <label className="flex items-center gap-2 text-[12px] text-black/85 cursor-pointer">
        ...
        Antar Provinsi (interstate available)
      </label>
