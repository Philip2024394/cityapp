...
import {
  PET_BANNER_LIBRARY,
  PET_BANNER_CATEGORIES,
} from '@/lib/pet/banners'
import {
  type PetProvider,
  type PetSpecialty,
} from '@/lib/pet/types'
...
// Pet brand defaults to sky blue — sport / active energy. Reads as
...
const DEFAULT_HERO = {
  line1:   'Pet Care',
  line2:   'Pet Care',
  tagline: 'Grooming · Hotel · Sitting · Training',
}
const DEFAULT_HERO_IMAGE = 'https://images.unsplash.com/photo-1587300003388-59208cc962cb?w=1200&auto=format&fit=crop'

type PetHeroEffect = 'none' | 'shimmer' | 'dance' | 'underline'
...
type PetHeroText = {
  ...
  effect?:        PetHeroEffect
}

type PetServicePhotoEntry = {
  url:               string
  name?:             string | null
...
export default function PetEditPage() {
  ...
  const reload = useCallback(async () => {
    ...
    if (!data?.session?.user) { router.replace('/login?next=/dashboard/pet/edit'); return }
    setLoading(true)
    try {
    ...
    return (
      <Shell>
        <div className="px-4 pt-20 max-w-md mx-auto text-center text-ink">
          <h1 className="text-[20px] font-black mb-2">Not a pet care business yet</h1>
          <Link href="/pet/signup" className="rounded-full bg-brand text-bg px-6 py-3 text-[13px] font-extrabold inline-block">Register</Link>
        </div>
      </Shell>
    )
  }
  ...
  const effect: PetHeroEffect = (['none','shimmer','dance','underline'].includes(rawEffect) ? rawEffect : 'none') as PetHeroEffect
    ...
    <Shell>
      <div className="max-w-2xl mx-auto pt-3 pb-32 px-4">
        ...
        <p className="text-[12px] text-black/55 mt-4 leading-snug">
          ...
          {' '}<Link href="/dashboard/pet" className="text-yellow-700 hover:underline font-bold">full dashboard form</Link>{' '}
          for the rest.
        </p>
      </div>
    </Shell>
...
function BannerInlineControls({
  ...
  const [draftEffect,       setDraftEffect]       = useState<PetHeroEffect>(((['none','shimmer','dance','underline'].includes(ht.effect ?? 'none')) ? (ht.effect ?? 'none') : 'none') as PetHeroEffect)
  ...
  const EFFECTS: Array<{ id: PetHeroEffect; label: string; desc: string }> = [
    ...
    <div className="mt-4 space-y-5">
      ...
      <Section title="Banner image" icon={<ImageIcon size={16} strokeWidth={2.5} />}>
        <BannerLibraryPicker
          ...
          library={PET_BANNER_LIBRARY}
          categories={PET_BANNER_CATEGORIES}
      ...
      {/* Effect */}
      <Section title="Text effect (Pet Care word)" icon={<Sparkles size={16} strokeWidth={2.5} />}>
      ...
      <Section title="QRIS payment" icon={<QrCodeIcon228 size={16} strokeWidth={2.5} />}>
        <QrisUploaderCard apiBase="/api/pet" accentHex={theme} />
      </Section>
...
function PromoTextEditor({
  ...
  const display = (draft.trim() || 'Anabul-mu pantas perawatan terbaik. Home grooming antar-jemput. Pet hotel weekend ber-AC + foto update tiap hari. Reminder vaksin GRATIS. Free trial pet sitting 1 jam. Reservasi via WhatsApp.') + ' ✦'
  ...
  return (
    <div className="space-y-2">
      ...
      <textarea
        ...
        maxLength={280}
        rows={3}
        placeholder="Write your promo message — appears as scrolling text below your Pet Gallery."
      ...
      <p className="text-[12px] text-black/60 leading-snug">
        Use this for promos — home grooming antar-jemput, pet hotel weekend, vaccine reminder, free trial pet sitting.
        Tap a suggestion to drop it in.
      </p>
      <div className="flex flex-wrap gap-1.5 pt-1">
        {[
          'Home Grooming Antar-Jemput GRATIS dalam kota — kucing Rp 80k, anjing kecil Rp 100k, handuk steril per pet',
          'Pet Hotel Weekend Rp 120k/malam — ber-AC, 3x makan, 2x bermain, foto update WA tiap hari',
          'Reminder Vaksin GRATIS — daftar nomor WA, kami ingatkan jadwal vaksin tahunan + cek kesehatan',
          'Free Trial Pet Sitting 1 Jam — coba dulu sebelum booking penuh, untuk pemilik yang mau traveling',
          'Full Grooming Premium Anjing Sedang-Besar Rp 200k — bath + cut sesuai breed + ear cleaning + nail trim + parfum',
        ].map((s) => (
          <button
            key={s}
            type="button"
