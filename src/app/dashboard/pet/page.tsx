...
import {
  ...
  type PetProvider, type PetAvailability, type PetSpecialty,
} from '@/lib/pet/types'
import {
  PET_BANNER_LIBRARY,
...
export default function PetDashboardPage() {
  const [p, setP] = useState<PetProvider | null>(null)
  ...
  const reload = useCallback(async () => {
    ...
    try {
      ...
      const j = await r.json() as { provider: PetProvider | null }
      setP(j.provider)
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const r = await fetch('/api/pet/me/analytics', { cache: 'no-store' })
        if (!r.ok) return
  ...
  async function setAvailability(next: PetAvailability) {
    if (!p) return
    const prev = p.availability
    setP({ ...p, availability: next })
    try {
      const r = await fetch('/api/pet/me/availability', {
  ...
  if (err === 'not_signed_in') return (
    <Shell><div className="px-4 pt-20 max-w-md mx-auto text-center">
      ...
      <Link href="/login?next=/dashboard/pet" className="rounded-full bg-brand text-bg px-6 py-3 text-[13px] font-extrabold inline-block">Sign in</Link>
    </div></Shell>
  )
  if (!p) return (
    <Shell><div className="px-4 pt-20 max-w-md mx-auto text-center">
      <h1 className="text-[20px] font-black mb-2">No pet care profile yet</h1>
      <Link href="/pet/signup" className="rounded-full bg-brand text-bg px-6 py-3 text-[13px] font-extrabold inline-block">Register pet care</Link>
    </div></Shell>
  )
  ...
  return (
    <Shell>
      <div className="px-4 pt-6 pb-24 max-w-3xl mx-auto space-y-4">
        ...
        <ProviderRenewBanner provider={p} upgradeHref="/pet/upgrade" />
        ...
        >
          <div className="flex items-center justify-between gap-3">
            <div className="min-w-0">
              <div className="text-[14px] font-extrabold uppercase tracking-wider text-brand">Service catalog &amp; portfolio →</div>
              <div className="text-[13px] text-black mt-1">Pick species + services (kucing / anjing / grooming bath / full grooming / pet hotel / sitting), set anchor price + pet-hotel per-malam rate, and add Pet Gallery photos that owners browse.</div>
            </div>
        ...
        <p className="text-[12px] text-black/60 text-center">
          Public profile: <a href={`/pet/${p.slug}`} target="_blank" rel="noopener" className="text-brand hover:underline">/pet/{p.slug}</a>
        </p>
      </div>
    </Shell>
  )
...
function ReadOnly({ p }: { p: PetProvider }) {
  return (
    <div className="space-y-3 text-[13px]">
      ...
      <div className="grid grid-cols-2 gap-2 pt-2">
        ...
        {p.day_rate_idr    != null && <PriceBox label="per malam / hari"        v={p.day_rate_idr}    />}
      </div>
...
function EditForm({ p, onSaved }: { p: PetProvider; onSaved: () => void }) {
  const pe = p as PetProvider & {
  ...
  function toggle(s: PetSpecialty) {
    setF((p) => {
      if (p.specialties.includes(s)) return { ...p, specialties: p.specialties.filter((x) => x !== s) }
      if (p.specialties.length >= MAX_PET_SPECIALTIES) return p
    ...
    <form onSubmit={save} className="space-y-3">
      ...
      <div>
        <div className="flex items-baseline justify-between mb-1.5">
          ...
          <div className="text-[11px] text-black/55">{f.specialties.length}/{MAX_PET_SPECIALTIES}</div>
        </div>
        <div className="flex flex-wrap gap-1.5">
      ...
      <div className="grid grid-cols-2 gap-2">
        ...
        <PriceInput label="per malam / hari"        v={f.day_rate}    set={(v) => upd('day_rate', v)} />
      </div>
