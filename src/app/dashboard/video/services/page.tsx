...
import {
  ...
  type VideoProvider, type VideoSpecialty,
} from '@/lib/video/types'

// Mirrors /dashboard/photo/services. Single page for everything the
// videographer sells:
//   1. Genres — max 3 they cover (wedding cinematic / music video /
//      brand commercial / social reel / drone aerial / etc.)
//   2. Pricing  — package starter price + optional full-day rate. The
//      videography industry quotes per-package (or per full-day), not
//      per-hour like most trades. Floor is higher than photo because
//      post-production hours dominate.
//   3. Portfolio photos with per-photo metadata (name, description,
//      package price, optional sample-shot pair) — drives the carousel +
//      View Details popup on /video/[slug].
...
export default function VideoServicesPage() {
    ...
    return (
      <Shell>
        <div className="px-4 pt-20 max-w-md mx-auto text-center">
          ...
          <Link href="/login?next=/dashboard/video/services" className="rounded-full bg-yellow-500 text-black px-6 py-3 text-[14px] font-extrabold inline-block">Sign in</Link>
        </div>
      </Shell>
    )
  }
  if (!provider) {
    return (
      <Shell>
        <div className="px-4 pt-20 max-w-md mx-auto text-center">
          <h1 className="text-[20px] font-black text-[#0A0A0A] mb-2">Not a videographer yet</h1>
          <Link href="/video/signup" className="rounded-full bg-yellow-500 text-black px-6 py-3 text-[14px] font-extrabold inline-block">Sign up</Link>
        </div>
      </Shell>
    )
  }
    ...
    <Shell>
      <div className="px-4 pt-3 pb-32 max-w-lg mx-auto">
        <header className="mb-5 flex items-center justify-between gap-3">
          <div>
            <Link href="/dashboard/video" className="text-[12px] font-bold text-gray-500 hover:text-gray-900 inline-block">← Dashboard</Link>
            <h1 className="text-[24px] font-black text-[#0A0A0A] leading-tight">Packages &amp; portfolio</h1>
            <p className="text-[13px] text-gray-600 mt-1">Pick your genres, set package + full-day rates, and add showreel + portfolio.</p>
          </div>
        </header>
...
function ServicesForm({ provider, onSaved }: { provider: FullProvider; onSaved: () => void }) {
  ...
  function toggleSpecialty(sid: VideoSpecialty) {
    const on = f.specialties.includes(sid)
    if (!on && f.specialties.length >= MAX_VIDEO_SPECIALTIES) return
  ...
  return (
    <form onSubmit={save} className="space-y-4">
      {/* 1. Genres */}
      <Card title="Packages I offer" hint={`Pick up to ${MAX_VIDEO_SPECIALTIES} genres — these become the Genres chips on your profile (${f.specialties.length}/${MAX_VIDEO_SPECIALTIES}).`}>
        <div className="flex flex-wrap gap-1.5">
          {ALL_SPECIALTIES.map((sid) => {
            const on    = f.specialties.includes(sid)
            const atCap = !on && f.specialties.length >= MAX_VIDEO_SPECIALTIES
            return (
              <button
                key={sid}
                type="button"
      ...
      {/* 2. Pricing */}
      <Card title="Rates" hint="Shown on the public profile. Type thousands — e.g. 2500 = Rp 2.5jt social reel, 6500 = Rp 6.5jt wedding cinematic highlight (max 9999).">
        <div className="grid grid-cols-2 gap-2">
          {([
            { k: 'hourly_rate', label: 'Package from' },
            { k: 'day_rate',    label: 'Full-day shoot' },
        ...
        {/* Travels-for-shoot toggle — replaces photo's travels-to-client.
            Stored on has_own_tools for column reuse; the video surface
        ...
        <label className="flex items-center gap-2 text-[13px] text-gray-700 cursor-pointer mt-2">
          ...
          />
          Travels for the shoot (on-location, not studio-only)
        </label>
      </Card>

      {/* 3. Portfolio */}
      <Card title="Showreel + portfolio gallery" hint="Up to 12 finished cover frames per genre with name, description, starting price, and optional sample-shot pair. These power the carousel + View Details popup on your profile.">
        {provider.user_id && (
