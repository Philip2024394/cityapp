...
import {
  ALL_SPECIALTIES, SPECIALTY_LABELS, MAX_VIDEO_SPECIALTIES,
  type VideoProvider, type VideoAvailability, type VideoSpecialty,
...
export default function VideoDashboardPage() {
  const [p, setP] = useState<VideoProvider | null>(null)
  ...
  const reload = useCallback(async () => {
    ...
    try {
      ...
      const j = await r.json() as { provider: VideoProvider | null }
      setP(j.provider)
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const r = await fetch('/api/video/me/analytics', { cache: 'no-store' })
        if (!r.ok) return
  ...
  async function setAvailability(next: VideoAvailability) {
    if (!p) return
    const prev = p.availability
    setP({ ...p, availability: next })
    try {
      const r = await fetch('/api/video/me/availability', {
  ...
  if (err === 'not_signed_in') return (
    <Shell><div className="px-4 pt-20 max-w-md mx-auto text-center">
      ...
      <Link href="/login?next=/dashboard/video" className="rounded-full bg-brand text-bg px-6 py-3 text-[13px] font-extrabold inline-block">Sign in</Link>
    </div></Shell>
  )
  if (!p) return (
    <Shell><div className="px-4 pt-20 max-w-md mx-auto text-center">
      <h1 className="text-[20px] font-black mb-2">No videographer profile yet</h1>
      <Link href="/video/signup" className="rounded-full bg-brand text-bg px-6 py-3 text-[13px] font-extrabold inline-block">Register as videographer</Link>
    </div></Shell>
  )
  ...
  return (
    <Shell>
      <div className="px-4 pt-6 pb-24 max-w-3xl mx-auto space-y-4">
        ...
        <ProviderRenewBanner provider={p} upgradeHref="/video/upgrade" />
        ...
        >
          <div className="flex items-center justify-between gap-3">
            <div className="min-w-0">
              ...
              <div className="text-[13px] text-black mt-1">Pick genres, set package + full-day rates, and add portfolio reels that drive the carousel.</div>
            </div>
        ...
        <p className="text-[12px] text-black/60 text-center">
          Public profile: <a href={`/video/${p.slug}`} target="_blank" rel="noopener" className="text-brand hover:underline">/video/{p.slug}</a>
        </p>
      </div>
    </Shell>
  )
...
function ReadOnly({ p }: { p: VideoProvider }) {
  return (
    <div className="space-y-3 text-[13px]">
      ...
      <KV k="Travels for the shoot" v={p.has_own_tools ? 'Yes' : 'No'} />
...
function EditForm({ p, onSaved }: { p: VideoProvider; onSaved: () => void }) {
  const pe = p as VideoProvider & {
  ...
  function toggle(s: VideoSpecialty) {
    setF((p) => {
      if (p.specialties.includes(s)) return { ...p, specialties: p.specialties.filter((x) => x !== s) }
      if (p.specialties.length >= MAX_VIDEO_SPECIALTIES) return p
    ...
    <form onSubmit={save} className="space-y-3">
      ...
      <div>
        <div className="flex items-baseline justify-between mb-1.5">
          ...
          <div className="text-[11px] text-black/55">{f.specialties.length}/{MAX_VIDEO_SPECIALTIES}</div>
        </div>
        <div className="flex flex-wrap gap-1.5">
      ...
      <label className="flex items-center gap-2 text-[12px] text-black/85 cursor-pointer">
        ...
        Travels for the shoot (on-location)
      </label>
