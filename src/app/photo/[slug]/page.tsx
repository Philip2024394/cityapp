...
// Default theme accent — used when the photographer hasn't picked their
// own theme_color (mig 0087). Photographers choose their accent from the
...
// charcoal slate so the imagery pops against the chrome.
const DEFAULT_THEME = '#1F2937'
...
// /photo/[slug] — universal profile flagship build. Mirrors the
// /barber/[slug] page byte-for-byte at the JSX layer; only the data
// source swaps from barber_providers to photo_providers and a few
...
export default function PhotoProviderPage() {
  ...
  const [activeService, setActiveService] = useState<PhotoSpecialty | null>(null)
  ...
  // available when the photographer has opted into a physical studio).
  ...
  useEffect(() => {
    ...
    const url = `/api/photo/${encodeURIComponent(slug)}/public${
      draftPassword ? `?p=${encodeURIComponent(draftPassword)}` : ''
    }`
    fetch(url, { cache: 'no-store' })
      .then((r) => r.ok ? r.json() : null)
      .then((j: { provider?: PhotoProviderPublic; locked?: boolean } | null) => {
  ...
  useProfileViewTracker({ providerType: 'photo', providerId: p?.id })

  // Resolved accent color for every accent surface on this page.
  // p.theme_color (mig 0087) wins; fall back to global default slate.
  ...
  useEffect(() => {
    ...
    fetch(`/api/reviews?provider_type=photo&provider_id=${encodeURIComponent(p.id)}`, { cache: 'no-store' })
      .then((r) => r.ok ? r.json() : null)
    ...
    return (
      <Shell>
        <div className="px-4 pt-20 max-w-md mx-auto text-center">
          <h1 className="text-[20px] font-black mb-2">Photographer not found</h1>
          <Link href="/photo" className="rounded-full bg-brand text-bg px-6 py-3 text-[13px] font-extrabold inline-block">Back to marketplace</Link>
        </div>
      </Shell>
    )
  }
    ...
    return (
      <Shell>
        <DraftPasswordGate
          ...
          fetchUrl={(pw) => `/api/photo/${encodeURIComponent(slug)}/public?p=${encodeURIComponent(pw)}`}
          onUnlocked={(pw, prov) => {
            setDraftPassword(pw)
            setLocked(false)
            setP(prov as PhotoProviderPublic)
          }}
        />
      </Shell>
    )
  ...
  return (
    <Shell>
      ...
      <div className="relative pb-2">
        ...
        >
          <img
            src={bannerSrc(p.cover_image_url) || DEFAULT_PHOTO_HERO}
            alt=""
          ...
          {(() => {
            ...
            const line2   = ht.line2   || 'Photographer'
            const tagline = ht.tagline || 'Wedding · Prewedding · Product · Editorial'
            ...
            return (
              <div className={`absolute left-4 z-10 select-none leading-none cr-hero-${effect}`} style={{ top: 31 }}>
                ...
                {/* Service locations the photographer travels to — Home and
                    Hotel are the everyday on-location pairs for indie
                    photogs (family shoots at home, prewedding at hotel
                    venues). A missing or null service_locations array
                    still hides the whole row to avoid implying locations
                    the photographer doesn't actually serve. */}
                {(() => {
        ...
        <div className="px-4 relative z-20" style={{ marginTop: 12 }}>
          ...
          >
            ...
            {/* Top Rated Seller badge — neutral gray pill so the chip
                ...
                photographer shows their own accent. */}
            <div
      ...
      <div className="px-4 pb-6 max-w-2xl mx-auto space-y-3 pt-3">
        ...
        {(() => {
          const offered = (p.specialties ?? []) as PhotoSpecialty[]
        ...
        {/* Portfolio carousel — rich cards (image + name + 2-line desc
            ...
            Photographers sell on the finished work, not before/after,
        ...
        <RunningMarquee
          text={p.promo_text || 'Professional photography that earns its place on your wall — turnaround in 7 days, deposit 30%. Konsultasi paket via WhatsApp.'}
        />
      ...
      {/* Right-edge "back" bar — tall vertical strip flush against the
          ...
          vertical "BACK" text below. Diverts back to /photo. */}
      <Link
        href="/photo"
        aria-label="Back to Kita2u photo"
      ...
      {/* Contact / booking popup — opened by both the bottom Contact
          ...
          request server-side first (so it shows up on the photographer's
          calendar) and then opens WhatsApp with a matching pre-filled
          message. Skips busy_dates the photographer has marked. */}
      {contactOpen && p.whatsapp_e164 && (
        <ContactBookingPopup
          ...
          providerId={p.id}
          intentVertical="photo"
          intentSource="photo_profile"
          themeColor={theme}
          ...
          busyDates={(p.busy_dates ?? []) as string[]}
          bookEndpoint={`/api/photo/${p.slug}/book`}
      ...
      <PoweredByKita2u defaultVertical="photo" />
    </Shell>
  )
}
...
}: { sid: PhotoSpecialty; active: boolean; onClick: () => void; theme: string }) {
  return (
    <button
      type="button"
      onClick={onClick}
...
// never reads empty.
function formatStartFromPrice(p: PhotoProviderPublic): string {
  ...
  if (candidates.length === 0) return 'Rp 800k'
  return formatPriceIdr(Math.min(...candidates)) ?? 'Rp 800k'
}

// Normalises one service_photos entry — photo uses a flat array of
...
// Combines photographer photos into the carousel feed. When a genre
...
function buildPortfolioPhotos(
  p: PhotoProviderPublic,
  active: PhotoSpecialty | null,
): PortfolioPhoto[] {
  ...
  // Partial-match against the genre label — works when the photographer
