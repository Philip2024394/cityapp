...
import {
  ...
  type PhotoProviderPublic, type PhotoSpecialty,
} from '@/lib/photo/types'

// ============================================================================
// /photo — Marketplace ported from the shared barber/handyman shell.
...
// Mirrors /barber + /handyman + /tattoo exactly so all verticals feel like
// one product.
// ============================================================================
...
function effectiveAvailability(p: PhotoProviderPublic): 'online' | 'busy' | 'offline' {
...
type ChipDef = { id: 'all' | PhotoSpecialty; label: string }
...
const PRIMARY_IDS = new Set<PhotoSpecialty>(['wedding','prewedding','family','product','food'])
const SECONDARY_IDS: PhotoSpecialty[] = ALL_SPECIALTIES.filter((s) => !PRIMARY_IDS.has(s))

export default function PhotoMarketplacePage() {
  return (
...
function MarketplaceInner() {
  ...
  const t       = useTranslations('verticals.photo')
  const tCommon = useTranslations('verticals.common')
  const haptic    = useHaptic()
  const search    = useSearchParams()
  const cityLabel = search?.get('city')?.trim() || 'Yogyakarta'

  const [providers, setProviders] = useState<PhotoProviderPublic[]>([])
  const [query, setQuery]         = useState('')
  const [specialty, setSpecialty] = useState<PhotoSpecialty | null>(null)
  ...
  const load = useCallback(async () => {
    ...
    const r = await fetch(`/api/photo/marketplace${qs.toString() ? `?${qs}` : ''}`, { cache: 'no-store' })
    const j = await r.json() as { providers: PhotoProviderPublic[] }
          ...
          <div className="flex flex-col p-4 sm:p-5">
            ...
            <div className="mt-3 relative shrink-0">
              <input
                ...
                placeholder="Cari fotografer — nama, studio, genre…"
            ...
            <div
              ...
              role="tablist"
              aria-label="Filter photographers by genre"
            >
                ...
                <div className="py-10 text-center">
                  <p className="text-[13px] font-bold text-[#71717A] leading-snug">
                    {query.trim()
                      ...
                      : 'No photographers in this genre yet. Try a different filter.'}
                  </p>
                </div>
              )}
            ...
            <p className="mt-3 text-center text-[12px] text-[#52525B] font-bold leading-snug px-2 shrink-0">
              Capture the moments that matter — book direct via WhatsApp.
              Kita2u takes no commission, you pay your photographer directly.
            </p>
          </div>
        </div>
      </div>
...
// ─────────────────────────────────────────────────────────────────────────────
// PhotoRowCard — landscape card, 112px tall to match /barber + /handyman.
...
function PhotoRowCard({
  ...
  // the photography industry) then full-day rate then years of experience.
...
// ─────────────────────────────────────────────────────────────────────────────
// Shell — wordmark header matching /barber + /handyman + /beautician + /places + /tattoo.
// ─────────────────────────────────────────────────────────────────────────────
