...
import { Star, Award, Menu, Home, Hotel, Building2, Store, Share2, Link2, MessageCircle, X, ChevronLeft, ChevronDown, BadgeCheck, MapPin, Bike, Calendar, Mail, FileText, ShieldCheck, HelpCircle, type LucideIcon } from 'lucide-react'
...
// /facial/[slug] — universal profile flagship build. Visual-first
// category (portfolio matters more than text); the kit's ProfileGallery
// is the centerpiece. 3-pack pricing (60/90/120 min) renders only
// the packages the facial provider actually offers.

// Vertical default for the hero. Used when the facial provider hasn't set
// their own cover_image_url yet — keeps the page on-brand instead of
// showing the generic yellow gradient on every new signup.
const DEFAULT_FACIAL_HERO =
  'https://ik.imagekit.io/nepgaxllc/ChatGPT%20Image%20May%2025,%202026,%2006_53_11%20AM.png'

export default function FacialProviderPage() {
  const params = useParams<{ slug: string }>()
  const slug = String(params?.slug || '').toLowerCase()
  const [p, setP] = useState<FacialProviderPublic | null>(null)
  ...
  const [activeService, setActiveService] = useState<FacialServiceOffered | null>(null)
  ...
  // Vendor cart — keyed on the provider id so each facial pro keeps its
  ...
  const cart = useVendorCart(`facial:${p?.id ?? '_loading'}`)
  ...
  useEffect(() => {
    ...
    const url = `/api/facial/${encodeURIComponent(slug)}/public${
      draftPassword ? `?p=${encodeURIComponent(draftPassword)}` : ''
    }`
    fetch(url, { cache: 'no-store' })
      .then((r) => r.ok ? r.json() : null)
      .then((j: { provider?: FacialProviderPublic; locked?: boolean } | null) => {
  ...
  // Facial isn't in the useProfileViewTracker union yet — cast through
  // so view tracking still pings /api/profile-view. Safe at runtime since
  // the hook only forwards the string to the API.
  useProfileViewTracker({ providerType: 'facial' as 'beautician', providerId: p?.id })
  ...
  useEffect(() => {
    ...
    fetch(`/api/reviews?provider_type=facial&provider_id=${encodeURIComponent(p.id)}`, { cache: 'no-store' })
      .then((r) => r.ok ? r.json() : null)
    ...
    return (
      <Shell>
        <div className="px-4 pt-20 max-w-md mx-auto text-center">
          <h1 className="text-[20px] font-black mb-2">Facial provider not found</h1>
          <Link href="/facial" className="rounded-full bg-brand text-bg px-6 py-3 text-[13px] font-extrabold inline-block">Back to marketplace</Link>
        </div>
      </Shell>
    )
  }
    ...
    return (
      <Shell>
        <DraftPasswordGate
          ...
          fetchUrl={(pw) => `/api/facial/${encodeURIComponent(slug)}/public?p=${encodeURIComponent(pw)}`}
          onUnlocked={(pw, prov) => {
            setDraftPassword(pw)
            setLocked(false)
            setP(prov as FacialProviderPublic)
          }}
        />
      </Shell>
    )
  ...
  const waText = [
    ...
    `Saya tertarik untuk booking session facial.`,
  ...
  // Midtrans on /dashboard/facial/payments. 'none' keeps the profile
  ...
  return (
    <Shell>
      ...
      <div className="relative pb-2">
        ...
        >
          <img
            src={bannerSrc(p.cover_image_url) || DEFAULT_FACIAL_HERO}
            alt=""
          ...
          {(() => {
            const ht = (p.hero_text || {}) as {
              line1?: string; line2?: string; tagline?: string
              color?: string; line1_color?: string; tagline_color?: string
              effect?: string
            }
            const line1   = ht.line1   || 'Professional'
            const line2   = ht.line2   || 'Facial'
            ...
            return (
              <div className={`absolute left-4 z-10 select-none leading-none cr-hero-${effect}`} style={{ top: 31 }}>
                ...
                {/* Service locations the facial provider travels to —
                ...
                {(() => {
                  ...
                  // "Facial Spa" so it's obvious this is in-salon only.
                  if (hasSpa) {
                    const label = locs.size === 0 ? 'Facial Spa' : 'Spa'
      ...
      <div className="px-4 pb-6 max-w-2xl mx-auto space-y-3 pt-3">
        ...
        {showVisitUs ? (
          ...
          <section className="space-y-3">
            ...
            {/* Legal footer — Terms / Privacy links. Tapping opens an
                ...
                corresponding page in /dashboard/facial/{terms,privacy}. */}
        ...
        {(() => {
          const offered = (p.services_offered ?? []) as FacialServiceOffered[]
          ...
          return (
            <section className="space-y-2">
              <div className="flex items-center justify-between gap-2">
                <h2 className="text-[13px] font-extrabold uppercase tracking-wider text-black">
                  {activeService
                    ? `${FACIAL_SERVICE_LABELS[activeService]} — Portfolio`
                    : 'Portfolio'}
                </h2>
                <PortfolioViewToggle
                  view={portfolioView}
        ...
        <RunningMarquee
          text={p.promo_text || 'Message me this week — exclusive promo on professional facial service delivered straight to your home, hotel or villa, in the comfort of your stay.'}
        />
      ...
      {/* Contact / booking popup — opened by both the bottom Contact
          ...
          request server-side first (so it shows up on the facial
          provider's calendar) and then opens WhatsApp with a matching
          pre-filled message. Skips busy_dates the provider has marked. */}
      {contactOpen && p.whatsapp_e164 && (
        <ContactBookingPopup
          ...
          serviceOptions={(p.services_offered ?? []).map((sid) => ({
            ...
            label: FACIAL_SERVICE_LABELS[sid] ?? sid,
          }))}
          presetService={contactServiceName}
          busyDates={(p.busy_dates ?? []) as string[]}
          bookEndpoint={`/api/facial/${p.slug}/book`}
...
}: { sid: FacialServiceOffered; active: boolean; onClick: () => void; theme: string }) {
  return (
    <button
      type="button"
      onClick={onClick}
    ...
    >
      ...
      />
      {FACIAL_SERVICE_LABELS[sid] ?? sid}
    </button>
  )
}

// Cheapest price across all per-photo entries (mig 0074 rich shape).
// Falls back to the 60/90/120 min columns when no photo
// prices are set so the CTA never reads empty.
function formatStartFromPrice(p: FacialProviderPublic): string {
...
function buildPortfolioPhotos(
  p: FacialProviderPublic,
  active: FacialServiceOffered | null,
): BeauticianServicePhoto[] {
  const sp = p.service_photos ?? {}
  // Only show photos for categories the provider actually offers.
  // Old rows can carry leftover keys; those should not surface in the
  // public carousel.
  const offered = new Set<FacialServiceOffered>(p.services_offered ?? [])
  if (active) {
  ...
  for (const cat of FACIAL_SERVICES_OFFERED) {
...
// Silence unused var for an icon imported but only used in legal modal
void Calendar
