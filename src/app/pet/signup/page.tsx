...
import { ALL_SPECIALTIES, SPECIALTY_LABELS, MAX_PET_SPECIALTIES, type PetSpecialty } from '@/lib/pet/types'
    ...
    supabase.auth.getSession().then(async ({ data }) => {
      ...
      try {
        ...
        if (r.ok) {
          ...
          if (j.provider) { setAuth({ status: 'alreadyProvider' }); setTimeout(() => router.replace('/dashboard/pet'), 400); return }
        }
      } catch { /* fall through */ }
  ...
  return (
    <div className="px-4 pt-12 max-w-md mx-auto">
      <div className="text-center mb-8">
        <div className="inline-block bg-brand text-bg text-[11px] font-extrabold uppercase tracking-[0.15em] px-3 py-1 rounded-full mb-4">Pet Care</div>
        <h1 className="text-[26px] font-black leading-tight mb-3">List your pet care business</h1>
        <p className="text-[13px] text-ink/70 leading-relaxed">Create an account first — already have one? Login with the same.</p>
      </div>
      <div className="space-y-3">
        <Link href="/signup?intent=pet&next=/pet/signup" className="block w-full rounded-2xl bg-brand text-bg px-6 py-4 text-center text-[14px] font-extrabold uppercase tracking-wider hover:brightness-105">
          Create new account
        </Link>
        <Link href="/login?next=/pet/signup" className="block w-full rounded-2xl bg-white/[0.06] border border-ink/15 text-ink px-6 py-4 text-center text-[14px] font-extrabold uppercase tracking-wider hover:bg-white/10">
...
function Form({ userId }: { userId: string }) {
  ...
  function toggleSpecialty(s: PetSpecialty) {
    setF((p) => {
      if (p.specialties.includes(s)) {
        return { ...p, specialties: p.specialties.filter((x) => x !== s) }
      }
      if (p.specialties.length >= MAX_PET_SPECIALTIES) return p // cap at 3
  ...
  async function submit(e: React.FormEvent) {
    e.preventDefault()
    if (f.specialties.length === 0) { setErr(`Pick min 1, max ${MAX_PET_SPECIALTIES} species + service.`); return }
    const h = f.hourly_rate.trim()
    const d = f.day_rate.trim()
    if (h === '' && d === '') { setErr('Enter at least one rate — anchor price or per-malam/hari.'); return }
    setSubmitting(true); setErr(null)
    try {
      const r = await fetch('/api/pet/signup', {
      ...
      setDone(true); setTimeout(() => router.push('/dashboard/pet'), 1200)
  ...
  return (
    <div className="px-4 pt-6 pb-20 max-w-md mx-auto">
      ...
      <h1 className="text-[26px] font-black leading-tight mb-2">Pet care signup</h1>
      <p className="text-[13px] text-ink/70 mb-6">Pick max 3 species + services. Set anchor price (cheapest small-pet bath) dan/atau pet-hotel per-malam rate. Rp 38,000/month after a 7-day trial.</p>

      <form onSubmit={submit} className="space-y-4">
        <Field label="Business name *">
          <input required maxLength={80} value={f.display_name} onChange={(e) => upd('display_name', e.target.value)} placeholder="Bella Pet Care" className={inputCls} />
        </Field>
        ...
        <Field label="Bio (max 300) *">
          <textarea required maxLength={300} rows={3} value={f.bio} onChange={(e) => upd('bio', e.target.value)} placeholder="Layanan andalan (grooming kucing / anjing semua ukuran, full grooming, pet hotel ber-AC, pet sitting datang ke rumah, training), sertifikat groomer, antar-jemput dalam kota, vaksin wajib untuk boarding, peralatan steril, pengalaman tahun." className={inputCls + ' resize-none'} />
        </Field>

        <div className="rounded-2xl bg-black/85 border border-white/10 p-4">
          <div className="flex items-baseline justify-between mb-1">
            <div className="text-[13px] font-extrabold uppercase tracking-wider text-ink">Species + services *</div>
            <div className="text-[11px] text-ink/55">{f.specialties.length}/{MAX_PET_SPECIALTIES}</div>
          </div>
          <p className="text-[11px] text-ink/55 mb-3">Pick max {MAX_PET_SPECIALTIES} (species + service combinations).</p>
          <div className="flex flex-wrap gap-1.5">
            {ALL_SPECIALTIES.map((s) => {
              const on = f.specialties.includes(s)
              const atLimit = !on && f.specialties.length >= MAX_PET_SPECIALTIES
              return (
                <button
                  key={s} type="button"
                  disabled={atLimit}
        ...
        <div className="rounded-2xl bg-black/85 border border-white/10 p-4">
          ...
          <p className="text-[11px] text-ink/55 mb-3">Enter at least one — anchor price (cheapest small-pet bath, e.g. cat S Rp 80k) atau pet hotel per-malam / pet sitting per-hari.</p>
          <div className="grid grid-cols-2 gap-2">
            <PriceField label="Mulai dari (anchor)" v={f.hourly_rate} set={(v) => upd('hourly_rate', v)} />
            <PriceField label="Per malam / hari"    v={f.day_rate}    set={(v) => upd('day_rate', v)} />
          </div>
          <label className="flex items-center gap-2 mt-3 text-[12px] text-ink/85 cursor-pointer">
            ...
            <span>Datang ke rumah (antar-jemput grooming + pet sitting di rumah pemilik)</span>
          </label>
        </div>
        ...
        <Field label="Service area / catatan jangkauan"><input value={f.service_area_notes} onChange={(e) => upd('service_area_notes', e.target.value)} placeholder="Yogya · Sleman · Bantul · antar-jemput dalam kota · pet hotel ber-AC" className={inputCls} /></Field>
        ...
        <button type="submit" disabled={submitting} className="w-full rounded-full bg-brand text-bg px-6 py-3.5 text-[14px] font-extrabold uppercase tracking-wider disabled:opacity-60">
          {submitting ? 'Registering…' : 'Create pet care profile'}
        </button>
      </form>
    </div>
  )
...
function humaniseError(code?: string): string {
  switch (code) {
    ...
    case 'at_least_one_specialty':return `Pick min 1, max ${MAX_PET_SPECIALTIES} species + service.`
    case 'at_least_one_price':    return 'Enter at least one rate — anchor price or per-malam/hari.'
    case 'slug_collision':        return 'Name already taken — try a variation.'
    case 'already_registered':    return 'You already have a pet care profile.'
