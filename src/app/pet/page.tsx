...
import {
  ...
  type PetProviderPublic, type PetSpecialty,
} from '@/lib/pet/types'

// ============================================================================
// /pet — Marketplace ported from the shared florist/cake/catering shell.
...
function effectiveAvailability(p: PetProviderPublic): 'online' | 'busy' | 'offline' {
...
type ChipDef = { id: 'all' | PetSpecialty; label: string }
const PRIMARY_CHIPS: ChipDef[] = [
  { id: 'all',             label: 'All'        },
  { id: 'cat',             label: 'Kucing'     },
  { id: 'dog',             label: 'Anjing'     },
  { id: 'grooming_bath',   label: 'Mandi'      },
  { id: 'full_grooming',   label: 'Full Groom' },
  { id: 'pet_hotel',       label: 'Pet Hotel'  },
  { id: 'pet_sitting',     label: 'Sitting'    },
]
const PRIMARY_IDS = new Set<PetSpecialty>(['cat','dog','grooming_bath','full_grooming','pet_hotel','pet_sitting'])
const SECONDARY_IDS: PetSpecialty[] = ALL_SPECIALTIES.filter((s) => !PRIMARY_IDS.has(s))
...
function MarketplaceInner() {
  ...
  const t       = useTranslations('verticals.pet')
  const tCommon = useTranslations('verticals.common')
  const haptic    = useHaptic()
  const search    = useSearchParams()
  const cityLabel = search?.get('city')?.trim() || 'Yogyakarta'

  const [providers, setProviders] = useState<PetProviderPublic[]>([])
  const [query, setQuery]         = useState('')
  const [specialty, setSpecialty] = useState<PetSpecialty | null>(null)
  ...
  const load = useCallback(async () => {
    ...
    const r = await fetch(`/api/pet/marketplace${qs.toString() ? `?${qs}` : ''}`, { cache: 'no-store' })
    const j = await r.json() as { providers: PetProviderPublic[] }
          ...
          <div className="flex flex-col p-4 sm:p-5">
            ...
            <div className="mt-3 relative shrink-0">
              <input
                ...
                placeholder="Cari pet groomer — nama, kota, layanan…"
            ...
            <div
              ...
              role="tablist"
              aria-label="Filter pet care by service"
            >
                ...
                <div className="py-10 text-center">
                  <p className="text-[13px] font-bold text-[#71717A] leading-snug">
                    {query.trim()
                      ...
                      : 'No pet groomers in this category yet. Try a different filter.'}
                  </p>
                </div>
              )}
            ...
            <p className="mt-3 text-center text-[12px] text-[#52525B] font-bold leading-snug px-2 shrink-0">
              Your furry family member deserves the spa treatment. Book direct via WhatsApp.
              Kita2u takes no commission, you pay your groomer directly.
            </p>
          </div>
        </div>
      </div>
...
// ─────────────────────────────────────────────────────────────────────────────
// PetRowCard — landscape card, 112px tall to match /florist + /cake.
...
function PetRowCard({
  ...
  if (p.day_rate_idr != null && pills.length < 2) {
    pills.push({ key: 'day', label: `Rp ${p.day_rate_idr.toLocaleString('id-ID')}/malam` })
  }
