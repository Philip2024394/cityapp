...
// Default theme accent — used when the groomer hasn't picked
// their own theme_color (mig 0087). Pet businesses choose their accent
...
// warm orange (#F97316) so the playful / animal-friendly imagery pops
// with cheerful, pet-care-tuned signage (distinct from tutoring's
// academic blue and florist's botanical green).
...
// /pet/[slug] — universal profile flagship build. Mirrors the
// /florist/[slug] page byte-for-byte at the JSX layer; only the data
// source swaps from florist_providers to pet_providers and a few
...
// Vertical default for the hero. Used when the groomer hasn't
...
const DEFAULT_PET_HERO =
  'https://images.unsplash.com/photo-1587300003388-59208cc962cb?w=1200&auto=format&fit=crop'

export default function PetProviderPage() {
  const params = useParams<{ slug: string }>()
  const slug = String(params?.slug || '').toLowerCase()
  const [p, setP] = useState<PetProviderPublic | null>(null)
  ...
  const [activeService, setActiveService] = useState<PetSpecialty | null>(null)
  ...
  useEffect(() => {
    ...
    const url = `/api/pet/${encodeURIComponent(slug)}/public${
      draftPassword ? `?p=${encodeURIComponent(draftPassword)}` : ''
    }`
    fetch(url, { cache: 'no-store' })
      .then((r) => r.ok ? r.json() : null)
      .then((j: { provider?: PetProviderPublic; locked?: boolean } | null) => {
  ...
  useProfileViewTracker({ providerType: 'pet', providerId: p?.id })
  ...
  useEffect(() => {
    ...
    fetch(`/api/reviews?provider_type=pet&provider_id=${encodeURIComponent(p.id)}`, { cache: 'no-store' })
      .then((r) => r.ok ? r.json() : null)
    ...
    return (
      <Shell>
        <div className="px-4 pt-20 max-w-md mx-auto text-center">
          <h1 className="text-[20px] font-black mb-2">Pet care not found</h1>
          <Link href="/pet" className="rounded-full bg-brand text-bg px-6 py-3 text-[13px] font-extrabold inline-block">Back to marketplace</Link>
        </div>
      </Shell>
    )
  }
    ...
    return (
      <Shell>
        <DraftPasswordGate
          ...
          fetchUrl={(pw) => `/api/pet/${encodeURIComponent(slug)}/public?p=${encodeURIComponent(pw)}`}
          onUnlocked={(pw, prov) => {
            setDraftPassword(pw)
            setLocked(false)
            setP(prov as PetProviderPublic)
          }}
        />
      </Shell>
    )
  ...
  const profileUrl = `${siteOrigin}/pet/${p.slug}`
  ...
  return (
    <Shell>
      ...
      <div className="relative pb-2">
        ...
        >
          <img
            src={bannerSrc(p.cover_image_url) || DEFAULT_PET_HERO}
            alt=""
          ...
          {(() => {
            ...
            const line1   = ht.line1   || 'Pet Care'
            const line2   = ht.line2   || 'Grooming · Hotel · Sitting · Training'
      ...
      <div className="px-4 pb-6 max-w-2xl mx-auto space-y-3 pt-3">
        ...
        {(() => {
          const offered = (p.specialties ?? []) as PetSpecialty[]
          ...
          return (
            <section className="space-y-2">
              <div className="flex items-center justify-between gap-2">
                <h2 className="text-[13px] font-extrabold uppercase tracking-wider text-black">
                  {activeService
                    ? `${SPECIALTY_LABELS[activeService]} — Pet Gallery`
                    : 'Pet Gallery'}
                </h2>
                <PortfolioViewToggle
                  view={portfolioView}
        ...
        <RunningMarquee
          text={p.promo_text || 'Anabul-mu pantas perawatan terbaik. Mandi kucing kecil Rp 80k. Full grooming anjing besar premium. Pet hotel ber-AC + foto update WA tiap hari. Pet sitting datang ke rumah. Reservasi via WhatsApp.'}
        />
      ...
      {/* Right-edge "back" bar — tall vertical strip flush against the
          ...
          vertical "BACK" text below. Diverts back to /pet. */}
      <Link
        href="/pet"
        aria-label="Back to Kita2u pet care"
      ...
      {contactOpen && p.whatsapp_e164 && (
        <ContactBookingPopup
          ...
          busyDates={(p.busy_dates ?? []) as string[]}
          bookEndpoint={`/api/pet/${p.slug}/book`}
      ...
      <PoweredByKita2u defaultVertical="pet" />
    </Shell>
  )
}
...
}: { sid: PetSpecialty; active: boolean; onClick: () => void; theme: string }) {
  return (
    <button
      type="button"
      onClick={onClick}
...
// Cheapest price across service-anchor + pet-hotel rate + per-photo
// prices. Falls back to a sensible default when nothing is set so the
// CTA never reads empty. Pet floor is small-cat bath at ~Rp 80k
// (Persian dewasa / anak — cheapest baseline; full grooming dog L
// avg Rp 200k, XL anjing besar Rp 250-350k).
function formatStartFromPrice(p: PetProviderPublic): string {
...
// Normalises one service_photos entry — pet uses a flat array of
