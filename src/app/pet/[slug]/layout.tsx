...
// Server-side SEO + lapsed redirect for /pet/[slug].
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  if (!p) return { title: 'Pet care not found', robots: { index: false, follow: false } }
  const pkg = p.hourly_rate_idr ?? null
  const title       = `${p.display_name} · Pet Care ${p.city || 'Indonesia'}`
  const description = (p.bio?.trim())
    || `Book ${p.display_name} (pet care) in ${p.city || 'Indonesia'} via WhatsApp.${pkg ? ` Mulai dari Rp ${pkg.toLocaleString('id-ID')}.` : ''}`
  const canonical   = `${SITE_URL}/pet/${p.slug}`
...
export default async function PetProviderLayout({
  children, params,
  ...
  if (p) {
    ...
    if (lapsed) {
      const qs = new URLSearchParams({ from: 'lapsed_pet', slug: p.slug })
      redirect(`/pet?${qs.toString()}`)
    }
  }
  const jsonLd = p
    ? {
        '@context': 'https://schema.org', '@type': 'LocalBusiness',
        '@id': `${SITE_URL}/pet/${p.slug}`,
        name: p.display_name, description: p.bio || undefined,
        url: `${SITE_URL}/pet/${p.slug}`,
