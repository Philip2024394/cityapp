...
//
// All copy comes from messages/{id,en}.json under the `driversBike`
// namespace. LocaleSwitcher (rendered from the customer landing) writes
// the NEXT_LOCALE cookie that swaps this page's locale at the next
// server render.
...
export async function generateMetadata() {
  const t = await getTranslations('driversBike')
  return {
    title:       t('metaTitle'),
...
export default async function DriversLandingPage() {
  const t = await getTranslations('driversBike')
  return (
    <main
...
// 5-way Bike/Car/Bus/Truck/Jeep mini selector. Active chip renders as
// plain text (no link); inactive chips are pill links. Labels come from
// the parent — same pill style on every /drivers/* sibling.
