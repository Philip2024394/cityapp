...
import {
  ...
  type CateringProviderPublic, type CateringSpecialty,
} from '@/lib/catering/types'

// ============================================================================
// /catering — Marketplace ported from the shared photo/barber/handyman shell.
...
function effectiveAvailability(p: CateringProviderPublic): 'online' | 'busy' | 'offline' {
...
type ChipDef = { id: 'all' | CateringSpecialty; label: string }
...
const PRIMARY_IDS = new Set<CateringSpecialty>(['nasi_box','prasmanan','tumpeng','snack_box','halal'])
const SECONDARY_IDS: CateringSpecialty[] = ALL_SPECIALTIES.filter((s) => !PRIMARY_IDS.has(s))

export default function CateringMarketplacePage() {
  return (
...
function MarketplaceInner() {
  ...
  const t       = useTranslations('verticals.catering')
  const tCommon = useTranslations('verticals.common')
  const haptic    = useHaptic()
  const search    = useSearchParams()
  const cityLabel = search?.get('city')?.trim() || 'Yogyakarta'

  const [providers, setProviders] = useState<CateringProviderPublic[]>([])
  const [query, setQuery]         = useState('')
  const [specialty, setSpecialty] = useState<CateringSpecialty | null>(null)
  ...
  const load = useCallback(async () => {
    ...
    const r = await fetch(`/api/catering/marketplace${qs.toString() ? `?${qs}` : ''}`, { cache: 'no-store' })
    const j = await r.json() as { providers: CateringProviderPublic[] }
          ...
          <div className="flex flex-col p-4 sm:p-5">
            ...
            <div className="mt-3 relative shrink-0">
              <input
                ...
                placeholder="Cari catering — nama, kota, menu…"
            ...
            <div
              ...
              role="tablist"
              aria-label="Filter catering businesses by cuisine"
            >
                ...
                <div className="py-10 text-center">
                  <p className="text-[13px] font-bold text-[#71717A] leading-snug">
                    {query.trim()
                      ...
                      : 'No catering businesses in this category yet. Try a different filter.'}
                  </p>
                </div>
              )}
            ...
            <p className="mt-3 text-center text-[12px] text-[#52525B] font-bold leading-snug px-2 shrink-0">
              ...
              Kita2u takes no commission, you pay your catering business directly.
            </p>
          </div>
        </div>
      </div>
...
// ─────────────────────────────────────────────────────────────────────────────
// CateringRowCard — landscape card, 112px tall to match /photo + /barber + /handyman.
...
// ─────────────────────────────────────────────────────────────────────────────
function CateringRowCard({
  provider: p,
  onOpen,
}: {
  provider: CateringProviderPublic
  onOpen: () => void
}) {
  ...
  // the catering industry) then full-day rate then years of experience.
