...
import {
  ALL_SPECIALTIES,
  SPECIALTY_LABELS,
  type CateringProviderPublic,
  type CateringSpecialty,
...
// Default theme accent — used when the catering business hasn't picked their
// own theme_color (mig 0087). Catering businesses choose their accent from the
...
// /catering/[slug] — universal profile flagship build. Mirrors the
// /photo/[slug] page byte-for-byte at the JSX layer; only the data
// source swaps from photo_providers to catering_providers and a few
...
export default function CateringProviderPage() {
  ...
  const [p, setP] = useState<CateringProviderPublic | null>(null)
  ...
  const [activeService, setActiveService] = useState<CateringSpecialty | null>(null)
  ...
  // available when the catering business has opted into a physical studio).
  ...
  useEffect(() => {
    ...
    const url = `/api/catering/${encodeURIComponent(slug)}/public${
      draftPassword ? `?p=${encodeURIComponent(draftPassword)}` : ''
    }`
    fetch(url, { cache: 'no-store' })
      .then((r) => r.ok ? r.json() : null)
      .then((j: { provider?: CateringProviderPublic; locked?: boolean } | null) => {
  ...
  useProfileViewTracker({ providerType: 'catering', providerId: p?.id })
  ...
  useEffect(() => {
    ...
    fetch(`/api/reviews?provider_type=catering&provider_id=${encodeURIComponent(p.id)}`, { cache: 'no-store' })
      .then((r) => r.ok ? r.json() : null)
    ...
    return (
      <Shell>
        <div className="px-4 pt-20 max-w-md mx-auto text-center">
          <h1 className="text-[20px] font-black mb-2">Catering business not found</h1>
          <Link href="/catering" className="rounded-full bg-brand text-bg px-6 py-3 text-[13px] font-extrabold inline-block">Back to marketplace</Link>
        </div>
      </Shell>
    )
  }
    ...
    return (
      <Shell>
        <DraftPasswordGate
          ...
          fetchUrl={(pw) => `/api/catering/${encodeURIComponent(slug)}/public?p=${encodeURIComponent(pw)}`}
          onUnlocked={(pw, prov) => {
            setDraftPassword(pw)
            setLocked(false)
            setP(prov as CateringProviderPublic)
          }}
        />
      </Shell>
    )
  ...
  const profileUrl = `${siteOrigin}/catering/${p.slug}`

  // WhatsApp prefill text for the under-carousel contact button.
  const waText = [
    `Halo ${p.display_name}, saya menemukan profile Anda di Kita2u.`,
    `Halo, saya tertarik pesan catering — boleh konsultasi menu dan harga per-pax?`,
  ...
  return (
    <Shell>
      ...
      <div className="relative pb-2">
        ...
        >
          <img
            src={bannerSrc(p.cover_image_url) || DEFAULT_CATERING_HERO}
            alt=""
          ...
          {(() => {
            ...
            const line1   = ht.line1   || 'Catering Rumahan'
            const line2   = ht.line2   || 'Catering business'
            const tagline = ht.tagline || 'Padang · Jawa · Snack Box · Buffet'
            ...
            return (
              <div className={`absolute left-4 z-10 select-none leading-none cr-hero-${effect}`} style={{ top: 31 }}>
                ...
                {/* Service locations the catering business delivers to — Home
                    and Hotel are the everyday delivery pairs for indie
                    catering businesses (family arisan / syukuran at home,
                    wedding buffet at hotel venues). A missing or null
                    service_locations array still hides the whole row to
                    avoid implying locations the caterer doesn't serve. */}
                {(() => {
        ...
        <div className="px-4 relative z-20" style={{ marginTop: 12 }}>
          ...
          >
            ...
            {/* Top Rated Seller badge — neutral gray pill so the chip
                ...
                catering business shows their own accent. */}
            <div
      ...
      <div className="px-4 pb-6 max-w-2xl mx-auto space-y-3 pt-3">
        ...
        {(() => {
          const offered = (p.specialties ?? []) as CateringSpecialty[]
        ...
        {/* Portfolio carousel — rich cards (image + name + 2-line desc
            ...
            Catering businesses sell on finished motion proof, not before/after,
        ...
        <RunningMarquee
          text={p.promo_text || 'Makanan rumahan yang bikin tamumu nambah — pesan H-3, uang muka 50%. Konsultasi menu via WhatsApp.'}
        />
      ...
      {/* Right-edge "back" bar — tall vertical strip flush against the
          ...
          vertical "BACK" text below. Diverts back to /catering. */}
      <Link
        href="/catering"
        aria-label="Back to Kita2u catering"
      ...
      {/* Contact / booking popup — opened by both the bottom Contact
          ...
          request server-side first (so it shows up on the catering business's
          calendar) and then opens WhatsApp with a matching pre-filled
          message. Skips busy_dates the catering business has marked. */}
      {contactOpen && p.whatsapp_e164 && (
        <ContactBookingPopup
          ...
          providerId={p.id}
          intentVertical="catering"
          intentSource="catering_profile"
          themeColor={theme}
          ...
          busyDates={(p.busy_dates ?? []) as string[]}
          bookEndpoint={`/api/catering/${p.slug}/book`}
      ...
      <PoweredByKita2u defaultVertical="catering" />
    </Shell>
  )
}
...
}: { sid: CateringSpecialty; active: boolean; onClick: () => void; theme: string }) {
  return (
    <button
      type="button"
      onClick={onClick}
...
// Cheapest price across per-pax + paket bundle + per-photo prices.
// Falls back to a sensible default when nothing is set so the CTA
// never reads empty. Catering floor is per-pax snack box ~Rp 22k
// baseline (much lower per-unit than video).
function formatStartFromPrice(p: CateringProviderPublic): string {
  ...
  if (candidates.length === 0) return 'Rp 22k'
  return formatPriceIdr(Math.min(...candidates)) ?? 'Rp 22k'
}
...
// Combines catering business photos into the carousel feed. When a genre
...
function buildPortfolioPhotos(
  p: CateringProviderPublic,
  active: CateringSpecialty | null,
): PortfolioPhoto[] {
  ...
  // Partial-match against the genre label — works when the catering business
