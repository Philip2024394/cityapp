...
// Server-side SEO + lapsed redirect for /catering/[slug].
// Mirrors /photo/[slug] / /barber/[slug] / /tattoo/[slug] / etc.
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  if (!p) return { title: 'Catering business not found', robots: { index: false, follow: false } }
  const pkg = p.hourly_rate_idr ?? null
  const title       = `${p.display_name} · Catering business ${p.city || 'Indonesia'}`
  const description = (p.bio?.trim())
    || `Book ${p.display_name} (catering business) in ${p.city || 'Indonesia'} via WhatsApp.${pkg ? ` Packages from Rp ${pkg.toLocaleString('id-ID')}.` : ''}`
  const canonical   = `${SITE_URL}/catering/${p.slug}`
...
export default async function CateringProviderLayout({
  children, params,
  ...
  if (p) {
    ...
    if (lapsed) {
      const qs = new URLSearchParams({ from: 'lapsed_catering', slug: p.slug })
      redirect(`/catering?${qs.toString()}`)
    }
  }
  const jsonLd = p
    ? {
        '@context': 'https://schema.org', '@type': 'LocalBusiness',
        '@id': `${SITE_URL}/catering/${p.slug}`,
        name: p.display_name, description: p.bio || undefined,
        url: `${SITE_URL}/catering/${p.slug}`,
