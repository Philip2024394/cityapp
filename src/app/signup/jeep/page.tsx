'use client'
// ============================================================================
// /signup/jeep — Multi-step sign-up flow for new JEEP drivers.
// ----------------------------------------------------------------------------
...
// vehicle_type='jeep'. After signup the driver lands on /dashboard/jeep where
// the QRIS pay modal handles the Rp 38.000/month subscription that flips
// their listing live in the public /jeep marketplace.
//
// Steps:
...
//
// Submit calls POST /api/signup/jeep which writes the profile + drivers row
...
const CITY_DATALIST_ID = 'signup-jeep-cities'
...
// Page
// ============================================================================
export default function SignupJeepPage() {
  ...
  useEffect(() => {
    ...
    const d = loadDraft<JeepSignupDraft>(SIGNUP_DRAFT_KEYS.jeep)
    if (d) {
  ...
  useEffect(() => {
    if (!draftHydrated) return
    saveDraft<JeepSignupDraft>(SIGNUP_DRAFT_KEYS.jeep, {
      step,
  ...
  async function signupWithPassword(e?: React.FormEvent) {
    ...
    const { error: err } = await supabase.auth.signUp({
      phone: cleaned,
      password,
      options: { data: { role: 'driver', vehicle_type: 'jeep' } },
    })
  ...
  // ──────────────────────────────────────────────────────────────────────
  // Final submit — POST /api/signup/jeep
  // ──────────────────────────────────────────────────────────────────────
  async function submit() {
    ...
    try {
      ...
      const r = await fetch('/api/signup/jeep', {
        method: 'POST',
      ...
      clearDraft(SIGNUP_DRAFT_KEYS.jeep)
      router.push(json.redirectTo || '/dashboard/jeep')
      router.refresh()
    } catch (e) {
  ...
  return (
    <main className="relative min-h-[100dvh] text-black">
      ...
      <div className="max-w-md mx-auto px-4 pt-8 pb-24">
        {/* Landing-style brand — same logo + "CityDrivers" wordmark used
            on the public /drivers landing pages. Replaces the white
            sticky CityDriversBrandStrip + AppNav header container
            (founder direction Jun 2026 — keep the surface clean so the
            hero backdrop + form card carry the whole top of the page). */}
        <Link
          href="/cityriders"
        ...
        <div className="rounded-2xl bg-white border border-gray-200 shadow-sm p-5 space-y-4">
          ...
          {step === 1 && (
            <>
              <Header
                title="Sign up as a Jeep driver"
                sub="List your jeep. Customers find you. You agree the fare and trip directly."
              />
          ...
          {step === 2 && (
            <>
              <Header
                ...
                sub="This is what customers see when they browse the /jeep marketplace."
              />
              <Field label="Business name" hint="Customers see this in search and on your booking page.">
                <input
                  className={inputCls}
                  placeholder="Budi Suzuki Jimny"
                  value={businessName}
              ...
              <Field label="Short bio" hint={`${bio.length}/280 characters`}>
                <textarea
                  ...
                  placeholder="8 years driving Bromo — sunrise tours, volcano runs, off-road adventures."
                  value={bio}
                  maxLength={280}
          ...
          {step === 3 && (
            <>
              ...
              <div className="grid grid-cols-2 gap-2">
                <Field label="Make">
                  <input className={inputCls} placeholder="Suzuki" value={vMake} onChange={(e) => setVMake(e.target.value)} />
                </Field>
                <Field label="Model">
                  <input className={inputCls} placeholder="Jimny" value={vModel} onChange={(e) => setVModel(e.target.value)} />
                </Field>
          ...
          {step === 6 && (
            <>
              ...
              <div className="rounded-xl bg-yellow-50 border border-yellow-200 px-3 py-2 text-[12px] text-yellow-900 leading-snug">
                Next step after sign-up: pay your <strong>Rp 38.000/month</strong> dashboard subscription via QRIS to flip your listing live in the public /jeep marketplace.
              </div>
