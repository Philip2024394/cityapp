'use client'
// ============================================================================
// /signup/bus — Multi-step sign-up flow for new MINIBUS / BUS drivers.
// ----------------------------------------------------------------------------
...
// vehicle_type='minibus'. After signup the driver lands on /dashboard/bus
// where the QRIS pay modal handles the Rp 38.000/month subscription that
// flips their listing live in the public /bus marketplace.
//
// Steps:
...
//   6. Review + submit      (read-only summary, SIM A Umum / B1 Umum + STNK
//                            confirmation, submit)
//
// Submit calls POST /api/signup/bus which writes the profile + drivers row
...
// Minibus seat counts — typical Yogya fleet (Avanza/APV → 7, Hiace → 14/16).
const SEAT_OPTIONS = [4, 7, 8, 14, 16] as const
...
const CITY_DATALIST_ID = 'signup-bus-cities'
...
// Page
// ============================================================================
export default function SignupBusPage() {
  ...
  // Default seats = 14 — the most common Hiace minibus configuration.
  const [vSeats, setVSeats] = useState<number>(14)
  const [vPhotos, setVPhotos] = useState<string[]>([''])

  // ── Step 4: Pricing ─────────────────────────────────────────────────────
  // Minibus defaults are higher than car: Rp 6,000/km and Rp 60,000 min fee.
  ...
  useEffect(() => {
    ...
    const d = loadDraft<BusSignupDraft>(SIGNUP_DRAFT_KEYS.bus)
    if (d) {
  ...
  useEffect(() => {
    if (!draftHydrated) return
    saveDraft<BusSignupDraft>(SIGNUP_DRAFT_KEYS.bus, {
      step,
  ...
  // ──────────────────────────────────────────────────────────────────────
  // Final submit — POST /api/signup/bus
  // ──────────────────────────────────────────────────────────────────────
  async function submit() {
    ...
    try {
      ...
      const r = await fetch('/api/signup/bus', {
        method: 'POST',
      ...
      clearDraft(SIGNUP_DRAFT_KEYS.bus)
      router.push(json.redirectTo || '/dashboard/bus')
      router.refresh()
    } catch (e) {
  ...
  return (
    <main className="relative min-h-[100dvh] text-black">
      ...
      <CityDriversBrandStrip subtitle="Sign up — Minibus driver" />
      <AppNav />
      <div className="max-w-md mx-auto px-4 pt-6 pb-24">
        ...
        <div className="rounded-2xl bg-white border border-gray-200 shadow-sm p-5 space-y-4">
          ...
          {step === 1 && (
            <>
              <Header
                title="Sign up as a Minibus / Bus Driver"
                sub="List your minibus. Customers find you. You agree the fare and trip directly."
              />
          ...
          {step === 2 && (
            <>
              <Header
                ...
                sub="This is what customers see when they browse the /bus marketplace."
              />
              <Field label="Business name" hint="Customers see this in search and on your booking page.">
                <input
                  className={inputCls}
                  placeholder="Budi Hiace Yogya"
                  value={businessName}
              ...
              <Field label="Short bio" hint={`${bio.length}/280 characters · Tip: include your charter/daily rate (Rp 600K–1M typical Yogya) in your bio so customers know what to expect.`}>
                <textarea
                  rows={3}
                  className={inputCls + ' resize-none'}
                  placeholder="10 years driving Hiace — airport charters, wedding shuttles, tour groups around Yogya."
                  value={bio}
                  maxLength={280}
          ...
          {step === 3 && (
            <>
              <Header
                title="Your minibus"
                sub="Hiace, Avanza, Innova, APV — for tour groups, airport charters, weddings."
              />
              <div className="grid grid-cols-2 gap-2">
                <Field label="Make">
                ...
                <Field label="Model">
                  <input className={inputCls} placeholder="Hiace Commuter" value={vModel} onChange={(e) => setVModel(e.target.value)} />
                </Field>
                ...
                <Field label="Seats" hint="Typical minibus: 7 (Avanza/APV) · 14 / 16 (Hiace)">
          ...
          {step === 4 && (
            <>
              ...
              <p className="text-[12px] text-black/65 bg-yellow-50 border border-yellow-200 rounded-lg px-3 py-2 leading-snug">
                Tip: include your charter/daily rate (Rp 600K–1M typical Yogya) in your bio so customers know what to expect.
              </p>
              {error && <ErrorBanner>{error}</ErrorBanner>}
          ...
          {step === 6 && (
            <>
              ...
              <div className="rounded-xl bg-yellow-50 border border-yellow-200 px-3 py-2 text-[12px] text-yellow-900 leading-snug">
                Next step after sign-up: pay your <strong>Rp 38.000/month</strong> dashboard subscription via QRIS to flip your listing live in the public /bus marketplace.
              </div>
...
function Compliance() {
  return (
    <p className="text-[12px] text-black/60 leading-snug">
      By continuing you confirm you are 18+ and have a valid <strong className="text-black">SIM A Umum or B1 Umum</strong> and <strong className="text-black">STNK</strong>.
      You agree to the{' '}
