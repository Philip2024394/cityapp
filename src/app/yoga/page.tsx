...
import {
  ...
  type YogaProviderPublic, type YogaSpecialty,
} from '@/lib/yoga/types'

// ============================================================================
// /yoga — Marketplace ported from the shared florist/cake/catering shell.
...
function effectiveAvailability(p: YogaProviderPublic): 'online' | 'busy' | 'offline' {
...
type ChipDef = { id: 'all' | YogaSpecialty; label: string }
const PRIMARY_CHIPS: ChipDef[] = [
  ...
  { id: 'beginner_friendly',  label: 'Beginner'   },
]
const PRIMARY_IDS = new Set<YogaSpecialty>(['vinyasa','yin','hatha','prenatal','beginner_friendly'])
const SECONDARY_IDS: YogaSpecialty[] = ALL_SPECIALTIES.filter((s) => !PRIMARY_IDS.has(s))

export default function YogaMarketplacePage() {
  return (
...
function MarketplaceInner() {
  ...
  const t       = useTranslations('verticals.yoga')
  const tCommon = useTranslations('verticals.common')
  const haptic    = useHaptic()
  const search    = useSearchParams()
  const cityLabel = search?.get('city')?.trim() || 'Yogyakarta'

  const [providers, setProviders] = useState<YogaProviderPublic[]>([])
  const [query, setQuery]         = useState('')
  const [specialty, setSpecialty] = useState<YogaSpecialty | null>(null)
  ...
  const load = useCallback(async () => {
    ...
    const r = await fetch(`/api/yoga/marketplace${qs.toString() ? `?${qs}` : ''}`, { cache: 'no-store' })
    const j = await r.json() as { providers: YogaProviderPublic[] }
          ...
          <div className="flex flex-col p-4 sm:p-5">
            ...
            <div className="mt-3 relative shrink-0">
              <input
                ...
                placeholder="Cari yoga teacher — nama, kota, style…"
            ...
            <div
              ...
              role="tablist"
              aria-label="Filter yoga teachers by style"
            >
                ...
                <div className="py-10 text-center">
                  <p className="text-[13px] font-bold text-[#71717A] leading-snug">
                    {query.trim()
                      ...
                      : 'No yoga teachers in this style yet. Try a different filter.'}
                  </p>
                </div>
              )}
            ...
            <p className="mt-3 text-center text-[12px] text-[#52525B] font-bold leading-snug px-2 shrink-0">
              Find your breath. Bring your mat — or we&apos;ll lend you one. Book direct via WhatsApp.
              Kita2u takes no commission, you pay your teacher directly.
            </p>
          </div>
        </div>
      </div>
...
// ─────────────────────────────────────────────────────────────────────────────
// YogaRowCard — landscape card, 112px tall to match /florist + /cake.
...
// ─────────────────────────────────────────────────────────────────────────────
function YogaRowCard({
  provider: p,
  onOpen,
}: {
  provider: YogaProviderPublic
  onOpen: () => void
}) {
