...
// Server-side SEO + lapsed redirect for /yoga/[slug].
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  if (!p) return { title: 'Yoga teacher not found', robots: { index: false, follow: false } }
  const pkg = p.hourly_rate_idr ?? null
  const title       = `${p.display_name} · Yoga teacher ${p.city || 'Indonesia'}`
  const description = (p.bio?.trim())
    || `Book ${p.display_name} (yoga teacher) in ${p.city || 'Indonesia'} via WhatsApp.${pkg ? ` Sessions from Rp ${pkg.toLocaleString('id-ID')}.` : ''}`
  const canonical   = `${SITE_URL}/yoga/${p.slug}`
...
export default async function YogaProviderLayout({
  children, params,
  ...
  if (p) {
    ...
    if (lapsed) {
      const qs = new URLSearchParams({ from: 'lapsed_yoga', slug: p.slug })
      redirect(`/yoga?${qs.toString()}`)
    }
  }
  const jsonLd = p
    ? {
        '@context': 'https://schema.org', '@type': 'LocalBusiness',
        '@id': `${SITE_URL}/yoga/${p.slug}`,
        name: p.display_name, description: p.bio || undefined,
        url: `${SITE_URL}/yoga/${p.slug}`,
