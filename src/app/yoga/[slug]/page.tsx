...
// Default theme accent — used when the yoga teacher hasn't picked
...
// /yoga/[slug] — universal profile flagship build. Mirrors the
// /florist/[slug] page byte-for-byte at the JSX layer; only the data
// source swaps from florist_providers to yoga_providers and a few
...
// Vertical default for the hero. Used when the yoga teacher hasn't
...
const DEFAULT_YOGA_HERO =
  'https://images.unsplash.com/photo-1571019613454-1cb2f99b2d8b?w=1200&auto=format&fit=crop'

export default function YogaProviderPage() {
  const params = useParams<{ slug: string }>()
  const slug = String(params?.slug || '').toLowerCase()
  const [p, setP] = useState<YogaProviderPublic | null>(null)
  ...
  const [activeService, setActiveService] = useState<YogaSpecialty | null>(null)
  ...
  // available when the yoga teacher has opted into a physical studio).
  ...
  useEffect(() => {
    ...
    const url = `/api/yoga/${encodeURIComponent(slug)}/public${
      draftPassword ? `?p=${encodeURIComponent(draftPassword)}` : ''
    }`
    fetch(url, { cache: 'no-store' })
      .then((r) => r.ok ? r.json() : null)
      .then((j: { provider?: YogaProviderPublic; locked?: boolean } | null) => {
  ...
  useProfileViewTracker({ providerType: 'yoga', providerId: p?.id })
  ...
  useEffect(() => {
    ...
    fetch(`/api/reviews?provider_type=yoga&provider_id=${encodeURIComponent(p.id)}`, { cache: 'no-store' })
      .then((r) => r.ok ? r.json() : null)
    ...
    return (
      <Shell>
        <div className="px-4 pt-20 max-w-md mx-auto text-center">
          <h1 className="text-[20px] font-black mb-2">Yoga teacher not found</h1>
          <Link href="/yoga" className="rounded-full bg-brand text-bg px-6 py-3 text-[13px] font-extrabold inline-block">Back to marketplace</Link>
        </div>
      </Shell>
    )
  }
    ...
    return (
      <Shell>
        <DraftPasswordGate
          ...
          fetchUrl={(pw) => `/api/yoga/${encodeURIComponent(slug)}/public?p=${encodeURIComponent(pw)}`}
          onUnlocked={(pw, prov) => {
            setDraftPassword(pw)
            setLocked(false)
            setP(prov as YogaProviderPublic)
          }}
        />
      </Shell>
    )
  ...
  const profileUrl = `${siteOrigin}/yoga/${p.slug}`
  ...
  return (
    <Shell>
      ...
      <div className="relative pb-2">
        ...
        >
          <img
            src={bannerSrc(p.cover_image_url) || DEFAULT_YOGA_HERO}
            alt=""
          ...
          {(() => {
            ...
            const line1   = ht.line1   || 'Yoga Teacher'
            const line2   = ht.line2   || 'Yoga Teacher'
            const tagline = ht.tagline || 'Vinyasa · Yin · Hatha · Prenatal'
            ...
            return (
              <div className={`absolute left-4 z-10 select-none leading-none cr-hero-${effect}`} style={{ top: 31 }}>
                ...
                {/* Service locations the trainer delivers to — Home and
                    ...
                    yoga teachers (home gym for clients, hotel gym
      ...
      <div className="px-4 pb-6 max-w-2xl mx-auto space-y-3 pt-3">
        ...
        {(() => {
          const offered = (p.specialties ?? []) as YogaSpecialty[]
        ...
        {/* Class Vibes — rich cards (image + name + 2-line desc
            + start price + View Details). Cards swipe left-to-right.
            Yoga teachers sell on studio atmosphere and pose demos
            (clients pick the teacher whose energy fits them), so the
            header reads "Class Vibes". */}
        {(() => {
          ...
          return (
            <section className="space-y-2">
              <div className="flex items-center justify-between gap-2">
                <h2 className="text-[13px] font-extrabold uppercase tracking-wider text-black">
                  {activeService
                    ? `${SPECIALTY_LABELS[activeService]} — Class Vibes`
                    : 'Class Vibes'}
                </h2>
                <PortfolioViewToggle
                  view={portfolioView}
        ...
        <RunningMarquee
          text={p.promo_text || 'Mat-mu sudah menunggu. Ikutilah. Trial class Rp 50k, 10-pack diskon 17%, monthly unlimited + 1 private session bonus. Mat & props disediakan. Reservasi via WhatsApp.'}
        />
      ...
      {/* Right-edge "back" bar — tall vertical strip flush against the
          ...
          vertical "BACK" text below. Diverts back to /yoga. */}
      <Link
        href="/yoga"
        aria-label="Back to Kita2u yoga teachers"
      ...
      {/* Contact / booking popup — opened by both the bottom Contact
          ...
          pre-filled message. Skips busy_dates the yoga teacher
          has marked. */}
      {contactOpen && p.whatsapp_e164 && (
        <ContactBookingPopup
          ...
          busyDates={(p.busy_dates ?? []) as string[]}
          bookEndpoint={`/api/yoga/${p.slug}/book`}
      ...
      <PoweredByKita2u defaultVertical="yoga" />
    </Shell>
  )
}
...
}: { sid: YogaSpecialty; active: boolean; onClick: () => void; theme: string }) {
  return (
    <button
      type="button"
      onClick={onClick}
...
// never reads empty. Yoga floor is drop-in single group class
// ~Rp 120k (cheaper than PT because group format spreads teacher
// time across 8+ students; indie yoga studios in Indonesia don't
// sell group drop-ins below Rp 100k in 2026).
function formatStartFromPrice(p: YogaProviderPublic): string {
  ...
  if (candidates.length === 0) return 'Rp 120k'
  return formatPriceIdr(Math.min(...candidates)) ?? 'Rp 120k'
}

// Normalises one service_photos entry — yoga uses a flat array of
...
// Combines yoga teacher photos into the carousel feed. When a type
...
function buildPortfolioPhotos(
  p: YogaProviderPublic,
  active: YogaSpecialty | null,
): PortfolioPhoto[] {
