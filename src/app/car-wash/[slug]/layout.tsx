...
// Server-side SEO + lapsed redirect for /car-wash/[slug].
// Mirrors /tailor/[slug] / /pet/[slug] / /florist/[slug] / /cake/[slug] / etc.
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  if (!p) return { title: 'Car wash not found', robots: { index: false, follow: false } }
  const pkg = p.hourly_rate_idr ?? null
  const title       = `${p.display_name} · Car Wash ${p.city || 'Indonesia'}`
  const description = (p.bio?.trim())
    || `Book ${p.display_name} (car wash) in ${p.city || 'Indonesia'} via WhatsApp.${pkg ? ` Mulai dari Rp ${pkg.toLocaleString('id-ID')}.` : ''}`
  const canonical   = `${SITE_URL}/car-wash/${p.slug}`
...
export default async function CarwashProviderLayout({
  children, params,
  ...
  if (p) {
    ...
    if (lapsed) {
      const qs = new URLSearchParams({ from: 'lapsed_carwash', slug: p.slug })
      redirect(`/car-wash?${qs.toString()}`)
    }
  }
  const jsonLd = p
    ? {
        '@context': 'https://schema.org', '@type': 'LocalBusiness',
        '@id': `${SITE_URL}/car-wash/${p.slug}`,
        name: p.display_name, description: p.bio || undefined,
        url: `${SITE_URL}/car-wash/${p.slug}`,
