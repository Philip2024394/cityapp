...
import {
  ALL_SPECIALTIES,
  SPECIALTY_LABELS,
  type CarwashProviderPublic,
  type CarwashSpecialty,
...
// Default theme accent — used when the car wash hasn't picked their own
// theme_color (mig 0087). Car wash businesses choose their accent from the
// dashboard color palette; the chosen hex flows through every accent
// surface on this page via the `theme` constant below. Default clean
// water blue (#0284C7) so the fresh / hydrophobic / professional energy
// reads, distinct from tailor's bridal violet and pet's playful orange.
const DEFAULT_THEME = '#0284C7'
...
// /car-wash/[slug] — universal profile flagship build. Mirrors the
// /tailor/[slug] page byte-for-byte at the JSX layer; only the data
// source swaps from tailor_providers to carwash_providers and a few
// vertical-specific adaptations apply (vehicle-tier chips, anchor +
// paket pricing block, flat service_photos array with partial-name
// filter).

// Vertical default for the hero. Used when the car wash hasn't set their
// own cover_image_url yet — keeps the page on-brand instead of showing
// the generic yellow gradient on every new signup.
const DEFAULT_CARWASH_HERO =
  'https://images.unsplash.com/photo-1607860108855-64acf2078ed9?w=1200&auto=format&fit=crop'

export default function CarwashProviderPage() {
  const params = useParams<{ slug: string }>()
  const slug = String(params?.slug || '').toLowerCase()
  const [p, setP] = useState<CarwashProviderPublic | null>(null)
  ...
  // null = show photos from ALL vehicle/service types (combined). When a
  // specialty is set, the portfolio carousel filters to just that type.
  const [activeService, setActiveService] = useState<CarwashSpecialty | null>(null)
  ...
  // available when the car wash has opted into a physical pool / depo).
  ...
  useEffect(() => {
    ...
    const url = `/api/carwash/${encodeURIComponent(slug)}/public${
      draftPassword ? `?p=${encodeURIComponent(draftPassword)}` : ''
    }`
    fetch(url, { cache: 'no-store' })
      .then((r) => r.ok ? r.json() : null)
      .then((j: { provider?: CarwashProviderPublic; locked?: boolean } | null) => {
  ...
  useProfileViewTracker({ providerType: 'carwash', providerId: p?.id })

  // Resolved accent color for every accent surface on this page.
  // p.theme_color (mig 0087) wins; fall back to global default clean blue.
  ...
  useEffect(() => {
    ...
    fetch(`/api/reviews?provider_type=carwash&provider_id=${encodeURIComponent(p.id)}`, { cache: 'no-store' })
      .then((r) => r.ok ? r.json() : null)
    ...
    return (
      <Shell>
        <div className="px-4 pt-20 max-w-md mx-auto text-center">
          <h1 className="text-[20px] font-black mb-2">Car wash not found</h1>
          <Link href="/car-wash" className="rounded-full bg-brand text-bg px-6 py-3 text-[13px] font-extrabold inline-block">Back to marketplace</Link>
        </div>
      </Shell>
    )
  }
    ...
    return (
      <Shell>
        <DraftPasswordGate
          ...
          fetchUrl={(pw) => `/api/carwash/${encodeURIComponent(slug)}/public?p=${encodeURIComponent(pw)}`}
          onUnlocked={(pw, prov) => {
            setDraftPassword(pw)
            setLocked(false)
            setP(prov as CarwashProviderPublic)
          }}
        />
      </Shell>
    )
  ...
  const profileUrl = `${siteOrigin}/car-wash/${p.slug}`

  // WhatsApp prefill text for the under-carousel contact button.
  const waText = [
    `Halo ${p.display_name}, saya menemukan profile Anda di Kita2u.`,
    `Halo, saya tertarik jasa cuci kendaraan — boleh minta quote dan slot booking?`,
  ...
  return (
    <Shell>
      ...
      <div className="relative pb-2">
        ...
        >
          <img
            src={bannerSrc(p.cover_image_url) || DEFAULT_CARWASH_HERO}
            alt=""
          ...
          {(() => {
            ...
            const line1   = ht.line1   || 'Cuci Mobil & Motor'
            const line2   = ht.line2   || 'Body · Detailing · Poles · Coating'
            const tagline = ht.tagline || 'Mobil bersih, hari menyenangkan.'
            ...
            return (
              <div className={`absolute left-4 z-10 select-none leading-none cr-hero-${effect}`} style={{ top: 31 }}>
                ...
                {/* Service locations the car wash delivers to — Lokasi
                    Cuci and Home are the everyday job locations for
                    indie car washes (lokasi cuci / panggilan ke rumah
                    / hotel valet). A missing or null service_locations
                    array still hides the whole row to avoid implying
                    locations the car wash doesn't serve. */}
                {(() => {
        ...
        <div className="px-4 relative z-20" style={{ marginTop: 12 }}>
          ...
          >
            ...
            {/* Top Rated Seller badge — neutral gray pill so the chip
                ...
                car wash shows their own accent. */}
            <div
      ...
      <div className="px-4 pb-6 max-w-2xl mx-auto space-y-3 pt-3">
        ...
        {/* Vehicle Types Provided — only renders specialties that
            actually have at least one "live" carousel entry (image +
            matching name + description). Empty placeholder specialties
            don't deserve a public badge. */}
        {(() => {
          const offered = (p.specialties ?? []) as CarwashSpecialty[]
          if (offered.length === 0) return null
          const sp = (p.service_photos ?? []) as Array<{
            url?: string; name?: string; description?: string
          }>
          // A type is "live" when at least one service_photos entry has
          // a non-empty url + description AND its name partial-matches
          // the type's label (case-insensitive). Same filter shape the
          // carousel uses below, kept in sync so the chip the user
          // picks always has photos to show.
          ...
          return (
            <section className="space-y-2" style={{ marginTop: 15 }}>
              <h2 className="text-[13px] font-extrabold uppercase tracking-wider text-black">
                Vehicles & Services Covered
              </h2>
              <div className="flex flex-wrap items-center gap-1.5">
                {/* "All" reset chip — clears any active type filter so
                    the portfolio carousel shows photos from every type
                ...
                {hasMore && (
                  <button
                    ...
                    aria-label={showMoreServices ? 'Hide other types' : 'Show other types'}
        ...
        {/* Hasil Cucian — rich cards (image + name + 2-line desc
            + start price + View Details). Cards swipe left-to-right.
            Car washes sell on vehicle-tier specialisation + finish quality
            (clients pick the car wash whose past work matches their
            vehicle), so the header reads "Hasil Cucian" (Wash Results). */}
        {(() => {
          ...
          return (
            <section className="space-y-2">
              <div className="flex items-center justify-between gap-2">
                <h2 className="text-[13px] font-extrabold uppercase tracking-wider text-black">
                  {activeService
                    ? `${SPECIALTY_LABELS[activeService]} — Hasil Cucian`
                    : 'Hasil Cucian'}
                </h2>
                <PortfolioViewToggle
                  view={portfolioView}
              ...
              <p className="text-[11px] text-gray-500 italic -mt-1">
                Please contact for custom service atau kategori belum tercantum
              </p>
              <PortfolioCarousel
                photos={photos}
        ...
        <RunningMarquee
          text={p.promo_text || 'Mobil bersih, hari menyenangkan — motor body Rp 15k express 15 menit. Mobil sedang body+dalam Rp 50k. Detailing premium SUV Rp 350k 3-4 jam. Coating ceramic 9H garansi 1 tahun. Member card 10x cuci diskon. Panggilan ke rumah dalam kota Yogya.'}
        />
      ...
      {/* Right-edge "back" bar — tall vertical strip flush against the
          ...
          vertical "BACK" text below. Diverts back to /car-wash. */}
      <Link
        href="/car-wash"
        aria-label="Back to Kita2u car wash"
      ...
      {/* Move Snapshot "View Details" popup — full image + sample
          work thumbs (if uploaded) + description + start price + Contact. */}
      {detailPhoto && (
        <PortfolioDetailPopup
          photo={detailPhoto}
          themeColor={theme}
      ...
      {/* Contact / booking popup — opened by both the bottom Contact
          ...
          request server-side first (so it shows up on the car wash's
          calendar) and then opens WhatsApp with a matching pre-filled
          message. Skips busy_dates the car wash has marked. */}
      {contactOpen && p.whatsapp_e164 && (
        <ContactBookingPopup
          ...
          providerId={p.id}
          intentVertical="carwash"
          intentSource="carwash_profile"
          themeColor={theme}
          ...
          busyDates={(p.busy_dates ?? []) as string[]}
          bookEndpoint={`/api/carwash/${p.slug}/book`}
      ...
      <PoweredByKita2u defaultVertical="carwash" />
    </Shell>
  )
}
...
}: { sid: CarwashSpecialty; active: boolean; onClick: () => void; theme: string }) {
  return (
    <button
      type="button"
      onClick={onClick}
...
// Cheapest price across anchor + paket + per-photo prices. Falls back
// to a sensible default when nothing is set so the CTA never reads
// empty. Car wash floor is motor body-only at ~Rp 15k.
function formatStartFromPrice(p: CarwashProviderPublic): string {
  ...
  if (candidates.length === 0) return 'Rp 15k'
  return formatPriceIdr(Math.min(...candidates)) ?? 'Rp 15k'
}

// Normalises one service_photos entry — car wash uses a flat array of
...
// Combines car wash photos into the carousel feed. When a type filter is
// active, partial-matches the photo's `name` against the type label
// (case-insensitive) so the chip filter still works on the flat-array
// shape (mig 0089). Falls back to the legacy gallery_image_urls
// (treated as headless URLs) when service_photos is empty.
function buildPortfolioPhotos(
  p: CarwashProviderPublic,
  active: CarwashSpecialty | null,
): PortfolioPhoto[] {
  ...
  // Partial-match against the type label — works when the car wash named
  // their photos sensibly. If the filter yields nothing, fall back to
  // all photos so the carousel doesn't collapse to empty when chip
