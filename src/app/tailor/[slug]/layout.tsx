...
// Server-side SEO + lapsed redirect for /tailor/[slug].
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  if (!p) return { title: 'Tailor not found', robots: { index: false, follow: false } }
  const pkg = p.hourly_rate_idr ?? null
  const title       = `${p.display_name} · Tailor ${p.city || 'Indonesia'}`
  const description = (p.bio?.trim())
    || `Book ${p.display_name} (tailor) in ${p.city || 'Indonesia'} via WhatsApp.${pkg ? ` Mulai dari Rp ${pkg.toLocaleString('id-ID')}.` : ''}`
  const canonical   = `${SITE_URL}/tailor/${p.slug}`
...
export default async function TailorProviderLayout({
  children, params,
  ...
  if (p) {
    ...
    if (lapsed) {
      const qs = new URLSearchParams({ from: 'lapsed_tailor', slug: p.slug })
      redirect(`/tailor?${qs.toString()}`)
    }
  }
  const jsonLd = p
    ? {
        '@context': 'https://schema.org', '@type': 'LocalBusiness',
        '@id': `${SITE_URL}/tailor/${p.slug}`,
        name: p.display_name, description: p.bio || undefined,
        url: `${SITE_URL}/tailor/${p.slug}`,
