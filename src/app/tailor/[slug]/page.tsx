...
import {
  ALL_SPECIALTIES,
  SPECIALTY_LABELS,
  type TailorProviderPublic,
  type TailorSpecialty,
...
// Default theme accent — used when the tailor hasn't picked their own
// theme_color (mig 0087). Tailor businesses choose their accent from the
// dashboard color palette; the chosen hex flows through every accent
// surface on this page via the `theme` constant below. Default deep
// teal (#0F766E) so the trustworthy / sturdy / logistics-signage energy
// reads, distinct from pet's playful orange and florist's botanical
// green.
...
// /tailor/[slug] — universal profile flagship build. Mirrors the
// /pet/[slug] page byte-for-byte at the JSX layer; only the data
// source swaps from pet_providers to tailor_providers and a few
// vertical-specific adaptations apply (vehicle-tier chips, per-trip
// + paket pricing block, flat service_photos array with partial-name
// filter).

// Vertical default for the hero. Used when the tailor hasn't set their
// own cover_image_url yet — keeps the page on-brand instead of showing
// the generic yellow gradient on every new signup.
const DEFAULT_TAILOR_HERO =
  'https://images.unsplash.com/photo-1600320254374-ce2d293c324e?w=1200&auto=format&fit=crop'

export default function TailorProviderPage() {
  const params = useParams<{ slug: string }>()
  const slug = String(params?.slug || '').toLowerCase()
  const [p, setP] = useState<TailorProviderPublic | null>(null)
  ...
  const [activeService, setActiveService] = useState<TailorSpecialty | null>(null)
  ...
  // available when the tailor has opted into a physical pool / depo).
  ...
  useEffect(() => {
    ...
    const url = `/api/tailor/${encodeURIComponent(slug)}/public${
      draftPassword ? `?p=${encodeURIComponent(draftPassword)}` : ''
    }`
    fetch(url, { cache: 'no-store' })
      .then((r) => r.ok ? r.json() : null)
      .then((j: { provider?: TailorProviderPublic; locked?: boolean } | null) => {
  ...
  useProfileViewTracker({ providerType: 'tailor', providerId: p?.id })

  // Resolved accent color for every accent surface on this page.
  // p.theme_color (mig 0087) wins; fall back to global default deep teal.
  ...
  useEffect(() => {
    ...
    fetch(`/api/reviews?provider_type=tailor&provider_id=${encodeURIComponent(p.id)}`, { cache: 'no-store' })
      .then((r) => r.ok ? r.json() : null)
    ...
    return (
      <Shell>
        <div className="px-4 pt-20 max-w-md mx-auto text-center">
          <h1 className="text-[20px] font-black mb-2">Tailor not found</h1>
          <Link href="/tailor" className="rounded-full bg-brand text-bg px-6 py-3 text-[13px] font-extrabold inline-block">Back to marketplace</Link>
        </div>
      </Shell>
    )
  }
    ...
    return (
      <Shell>
        <DraftPasswordGate
          ...
          fetchUrl={(pw) => `/api/tailor/${encodeURIComponent(slug)}/public?p=${encodeURIComponent(pw)}`}
          onUnlocked={(pw, prov) => {
            setDraftPassword(pw)
            setLocked(false)
            setP(prov as TailorProviderPublic)
          }}
        />
      </Shell>
    )
  ...
  return (
    <Shell>
      ...
      <div className="relative pb-2">
        ...
        >
          <img
            src={bannerSrc(p.cover_image_url) || DEFAULT_TAILOR_HERO}
            alt=""
          ...
          {(() => {
            ...
            const line1   = ht.line1   || 'Penjahit Custom'
            const line2   = ht.line2   || 'Kebaya · Jas · Vermak · Seragam'
            const tagline = ht.tagline || 'Setiap jahitan, sesuai badanmu.'
            ...
            return (
              <div className={`absolute left-4 z-10 select-none leading-none cr-hero-${effect}`} style={{ top: 31 }}>
                ...
                {/* Service locations the tailor delivers to — Studio
                    and Home are the everyday job locations for indie
                    tailors (studio fitting / home-visit measurement).
                    A missing or null service_locations array still
                    hides the whole row to avoid implying locations the
                    tailor doesn't serve. */}
                {(() => {
      ...
      <div className="px-4 pb-6 max-w-2xl mx-auto space-y-3 pt-3">
        ...
        {(() => {
          const offered = (p.specialties ?? []) as TailorSpecialty[]
        ...
        {/* Galeri Jahitan — rich cards (image + name + 2-line desc
            + start price + View Details). Cards swipe left-to-right.
            Tailors sell on garment-type specialisation + craftsmanship
            (clients pick the tailor whose past work fits their dream
            garment), so the header reads "Galeri Jahitan". */}
        {(() => {
          ...
          return (
            <section className="space-y-2">
              <div className="flex items-center justify-between gap-2">
                <h2 className="text-[13px] font-extrabold uppercase tracking-wider text-black">
                  {activeService
                    ? `${SPECIALTY_LABELS[activeService]} — Galeri Jahitan`
                    : 'Galeri Jahitan'}
                </h2>
                <PortfolioViewToggle
                  view={portfolioView}
              ...
              <p className="text-[11px] text-gray-500 italic -mt-1">
                Please contact for custom route atau kategori belum tercantum
              </p>
              <PortfolioCarousel
                photos={photos}
        ...
        <RunningMarquee
          text={p.promo_text || 'Setiap jahitan, sesuai badanmu — vermak mulai Rp 25k. Jas pria custom Rp 800k tanpa bahan. Kebaya bridal premium full payet dengan 3x fitting. Seragam kantor paket Rp 200k/pcs min 12 pcs. Free konsultasi design via WhatsApp.'}
        />
      ...
      {/* Right-edge "back" bar — tall vertical strip flush against the
          ...
          vertical "BACK" text below. Diverts back to /tailor. */}
      <Link
        href="/tailor"
        aria-label="Back to Kita2u tailor"
      ...
      {/* Contact / booking popup — opened by both the bottom Contact
          ...
          request server-side first (so it shows up on the tailor's
          calendar) and then opens WhatsApp with a matching pre-filled
          message. Skips busy_dates the tailor has marked. */}
      {contactOpen && p.whatsapp_e164 && (
        <ContactBookingPopup
          ...
          providerId={p.id}
          intentVertical="tailor"
          intentSource="tailor_profile"
          themeColor={theme}
          ...
          busyDates={(p.busy_dates ?? []) as string[]}
          bookEndpoint={`/api/tailor/${p.slug}/book`}
      ...
      <PoweredByKita2u defaultVertical="tailor" />
    </Shell>
  )
}
...
}: { sid: TailorSpecialty; active: boolean; onClick: () => void; theme: string }) {
  return (
    <button
      type="button"
      onClick={onClick}
...
// empty. Tailor floor is vermak / alteration at ~Rp 25k (pendekan
// celana / ganti resleting / sesuaikan ukuran).
function formatStartFromPrice(p: TailorProviderPublic): string {
  ...
  if (candidates.length === 0) return 'Rp 25k'
  return formatPriceIdr(Math.min(...candidates)) ?? 'Rp 25k'
}

// Normalises one service_photos entry — tailor uses a flat array of
...
// Combines tailor photos into the carousel feed. When a type filter is
...
function buildPortfolioPhotos(
  p: TailorProviderPublic,
  active: TailorSpecialty | null,
): PortfolioPhoto[] {
  ...
  // Partial-match against the type label — works when the tailor named
