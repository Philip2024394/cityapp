...
import {
  ...
  type TailorProviderPublic, type TailorSpecialty,
} from '@/lib/tailor/types'

// ============================================================================
// /tailor — Marketplace ported from the shared florist/cake/catering shell.
...
function effectiveAvailability(p: TailorProviderPublic): 'online' | 'busy' | 'offline' {
...
type ChipDef = { id: 'all' | TailorSpecialty; label: string }
const PRIMARY_CHIPS: ChipDef[] = [
  { id: 'all',     label: 'All'     },
  { id: 'kebaya',  label: 'Kebaya'  },
  { id: 'jas',     label: 'Jas'     },
  { id: 'kemeja',  label: 'Kemeja'  },
  { id: 'gaun',    label: 'Gaun'    },
  { id: 'seragam', label: 'Seragam' },
  { id: 'vermak',  label: 'Vermak'  },
]
const PRIMARY_IDS = new Set<TailorSpecialty>(['kebaya','jas','kemeja','gaun','seragam','vermak'])
const SECONDARY_IDS: TailorSpecialty[] = ALL_SPECIALTIES.filter((s) => !PRIMARY_IDS.has(s))

export default function TailorMarketplacePage() {
  return (
...
function MarketplaceInner() {
  ...
  const t       = useTranslations('verticals.tailor')
  const tCommon = useTranslations('verticals.common')
  const haptic    = useHaptic()
  const search    = useSearchParams()
  const cityLabel = search?.get('city')?.trim() || 'Yogyakarta'

  const [providers, setProviders] = useState<TailorProviderPublic[]>([])
  const [query, setQuery]         = useState('')
  const [specialty, setSpecialty] = useState<TailorSpecialty | null>(null)
  ...
  const load = useCallback(async () => {
    ...
    const r = await fetch(`/api/tailor/marketplace${qs.toString() ? `?${qs}` : ''}`, { cache: 'no-store' })
    const j = await r.json() as { providers: TailorProviderPublic[] }
          ...
          <div className="flex flex-col p-4 sm:p-5">
            ...
            <div className="mt-3 relative shrink-0">
              <input
                ...
                placeholder="Cari penjahit — nama, kota, jenis jahitan…"
            ...
            <div
              ...
              role="tablist"
              aria-label="Filter tailor by garment"
            >
                ...
                <div className="py-10 text-center">
                  <p className="text-[13px] font-bold text-[#71717A] leading-snug">
                    {query.trim()
                      ...
                      : 'No tailors in this category yet. Try a different filter.'}
                  </p>
                </div>
              )}
            ...
            <p className="mt-3 text-center text-[12px] text-[#52525B] font-bold leading-snug px-2 shrink-0">
              Setiap jahitan, sesuai badanmu. Book direct via WhatsApp.
              Kita2u takes no commission, you pay your tailor directly.
            </p>
          </div>
        </div>
      </div>
...
// ─────────────────────────────────────────────────────────────────────────────
// TailorRowCard — landscape card, 112px tall to match /florist + /cake + /pet.
...
// ─────────────────────────────────────────────────────────────────────────────
function TailorRowCard({
  provider: p,
  onOpen,
}: {
  provider: TailorProviderPublic
  onOpen: () => void
}) {
  ...
  // the tailor industry) then full-house package then years of experience.
