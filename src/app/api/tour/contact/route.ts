...
// POST /api/tour/contact
// Public form submission from /tour/[slug]. Body shape:
...
export async function POST(req: Request) {
  ...
  // Resolve provider + opt-in check. tour_guide_listings uses `name`
  // instead of `display_name`, and `status='approved'` as the public gate
  // (mig 0037 tgl_public_read_approved policy).
  const { data: provider, error: lookupErr } = await admin
    .from('tour_guide_listings')
    .select('id, name, contact_form_enabled, contact_email, theme_color')
    .eq('slug', slug)
    .eq('status', 'approved')
    .maybeSingle()
  if (lookupErr) {
    console.error('[tour/contact] lookup failed', { code: lookupErr.code, message: lookupErr.message })
  ...
  if (insertErr) {
    console.error('[tour/contact] insert failed', { code: insertErr.code, message: insertErr.message })
  ...
  void sendResendEmail({
    ...
    console.error('[tour/contact] resend failed', e instanceof Error ? e.message : e)
  })
...
async function sendResendEmail({
  ...
  if (!apiKey) {
    console.warn('[tour/contact] RESEND_API_KEY missing — message saved but no email sent')
    return
  }
