...
import { ALL_SPECIALTIES, MAX_TATTOO_SPECIALTIES } from '@/lib/tattoo/types'
...
type Body = {
  ...
  languages?:          string[]
  // mig 0087 + 0089 + 0091 — profile-parity fields tattoo gained for
  // the polished /tattoo/[slug] page. All optional.
...
export async function POST(req: Request) {
  ...
  const update: TableUpdate<'tattoo_providers'> = {}
  ...
  if (Array.isArray(body.specialties)) {
    const valid = body.specialties
      ...
      .slice(0, MAX_TATTOO_SPECIALTIES)
  ...
  const { error } = await admin.from('tattoo_providers').update(update).eq('user_id', user.id)
