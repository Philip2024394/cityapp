...
import { slugify } from '@/lib/tattoo/slug'
import { ALL_SPECIALTIES, MAX_TATTOO_SPECIALTIES, type TattooSpecialty } from '@/lib/tattoo/types'
...
export async function POST(req: Request) {
  ...
  const specialties = (body.specialties ?? [])
    ...
    .slice(0, MAX_TATTOO_SPECIALTIES) as TattooSpecialty[]
  ...
  for (let i = 2; i <= 9; i++) {
    const { data: existing } = await admin
      .from('tattoo_providers').select('id').eq('slug', slug).maybeSingle()
    if (!existing) break
    slug = `${base}-${i}`
  ...
  if (error) {
    console.error('[tattoo/signup] insert failed', { code: error.code, message: error.message })
