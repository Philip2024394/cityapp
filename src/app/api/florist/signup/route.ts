...
import { slugify } from '@/lib/florist/slug'
import { ALL_SPECIALTIES, MAX_FLORIST_SPECIALTIES, type FloristSpecialty } from '@/lib/florist/types'
...
export async function POST(req: Request) {
  ...
  const specialties = (body.specialties ?? [])
    ...
    .slice(0, MAX_FLORIST_SPECIALTIES) as FloristSpecialty[]
  if (specialties.length === 0) {
    return NextResponse.json({ error: 'at_least_one_specialty' }, { status: 400 })
  }

  // Pricing — per-arrangement starter rate and/or premium bundle. CHECK enforces
  // at least one is set.
  ...
  for (let i = 2; i <= 9; i++) {
    const { data: existing } = await admin
      .from('florist_providers').select('id').eq('slug', slug).maybeSingle()
    if (!existing) break
    slug = `${base}-${i}`
  ...
  if (error) {
    console.error('[florist/signup] insert failed', { code: error.code, message: error.message })
