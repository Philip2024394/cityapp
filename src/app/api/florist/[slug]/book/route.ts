...
// POST /api/florist/[slug]/book
// Customer submits a booking request for a florist. She sees it
// in her dashboard and confirms / declines. Platform never holds money
// — the actual handshake (DP, design brief, pickup) continues on
// WhatsApp; this row just records the request so the florist knows who
// and when.
//
// Validation + shape mirrors catering's /api/catering/[slug]/book.
...
export async function POST(req: Request, ctx: { params: Promise<{ slug: string }> }) {
  ...
  // busy_dates is a date[] on florist_providers (mig 0089-equiv).
  ...
  // Same 8-requests-per-day abuse cap as catering.
  const since = new Date(Date.now() - dayMs).toISOString()
  const { count: recent } = await admin
    .from('florist_bookings')
  ...
  if (error) {
    console.error('[florist/book] insert failed', { code: error.code, message: error.message })
