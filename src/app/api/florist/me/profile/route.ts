...
import { ALL_SPECIALTIES, MAX_FLORIST_SPECIALTIES } from '@/lib/florist/types'
...
type Body = {
  ...
  languages?:          string[]
  // mig 0087 + 0089 + 0091 — profile-parity fields florist gained for
  // the polished /florist/[slug] page. All optional.
...
export async function POST(req: Request) {
  ...
  const update: TableUpdate<'florist_providers'> = {}
  ...
  if (Array.isArray(body.specialties)) {
    const valid = body.specialties
      ...
      .slice(0, MAX_FLORIST_SPECIALTIES)
  ...
  const { error } = await admin.from('florist_providers').update(update).eq('user_id', user.id)
