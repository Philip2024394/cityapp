...
import { ALL_SPECIALTIES, MAX_PET_SPECIALTIES } from '@/lib/pet/types'
...
type Body = {
  ...
  languages?:          string[]
  // mig 0087 + 0089 + 0091 — profile-parity fields pet gained for
  // the polished /pet/[slug] page. All optional.
...
export async function POST(req: Request) {
  ...
  const update: TableUpdate<'pet_providers'> = {}
  ...
  const { error } = await admin.from('pet_providers').update(update).eq('user_id', user.id)
