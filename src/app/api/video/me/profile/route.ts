...
import { ALL_SPECIALTIES, MAX_VIDEO_SPECIALTIES } from '@/lib/video/types'
...
type Body = {
  ...
  // the polished /video/[slug] page. All optional.
...
export async function POST(req: Request) {
  ...
  const update: TableUpdate<'video_providers'> = {}
  ...
  const { error } = await admin.from('video_providers').update(update).eq('user_id', user.id)
