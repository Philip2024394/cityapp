...
import { slugify } from '@/lib/video/slug'
import { ALL_SPECIALTIES, MAX_VIDEO_SPECIALTIES, type VideoSpecialty } from '@/lib/video/types'
...
export async function POST(req: Request) {
  ...
  const specialties = (body.specialties ?? [])
    ...
    .slice(0, MAX_VIDEO_SPECIALTIES) as VideoSpecialty[]
  ...
  for (let i = 2; i <= 9; i++) {
    const { data: existing } = await admin
      .from('video_providers').select('id').eq('slug', slug).maybeSingle()
    if (!existing) break
    slug = `${base}-${i}`
  ...
  if (error) {
    console.error('[video/signup] insert failed', { code: error.code, message: error.message })
