...
// POST /api/video/[slug]/book
// Customer submits a booking request for a videographer. She sees it in
...
// WhatsApp; this row just records the request so the videographer knows
// who and when.
//
...
export async function POST(req: Request, ctx: { params: Promise<{ slug: string }> }) {
  ...
  // busy_dates is a date[] on video_providers (mig 0089-equiv).
  ...
  if (error) {
    console.error('[video/book] insert failed', { code: error.code, message: error.message })
