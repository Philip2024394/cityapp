...
import { slugify } from '@/lib/handyman/slug'
import { ALL_SPECIALTIES, MAX_HANDYMAN_SPECIALTIES, type HandymanSpecialty } from '@/lib/handyman/types'
...
export async function POST(req: Request) {
  ...
  const specialties = (body.specialties ?? [])
    ...
    .slice(0, MAX_HANDYMAN_SPECIALTIES) as HandymanSpecialty[]
  if (specialties.length === 0) {
    return NextResponse.json({ error: 'at_least_one_specialty' }, { status: 400 })
  }

  // Pricing — hour and/or day (day = 8 hours). Visit fee removed per
  // UX rule (mig 0062 dropped the column). At least one must be set.
  ...
  for (let i = 2; i <= 9; i++) {
    const { data: existing } = await admin
      .from('handyman_providers').select('id').eq('slug', slug).maybeSingle()
    if (!existing) break
    slug = `${base}-${i}`
  ...
  if (error) {
    console.error('[handyman/signup] insert failed', { code: error.code, message: error.message })
