...
import { ALL_SPECIALTIES, MAX_PHOTO_SPECIALTIES } from '@/lib/photo/types'
...
type Body = {
  ...
  languages?:          string[]
  // mig 0087 + 0089 + 0091 — profile-parity fields photo gained for
  // the polished /photo/[slug] page. All optional.
...
export async function POST(req: Request) {
  ...
  const update: TableUpdate<'photo_providers'> = {}
  ...
  const { error } = await admin.from('photo_providers').update(update).eq('user_id', user.id)
