...
// POST /api/photo/[slug]/book
// Customer submits a booking request for a photographer. She sees it in
...
// WhatsApp; this row just records the request so the photographer knows
// who and when.
//
// Validation + shape mirrors barber's /api/barber/[slug]/book.
...
export async function POST(req: Request, ctx: { params: Promise<{ slug: string }> }) {
  ...
  // busy_dates is a date[] on photo_providers (mig 0089).
  ...
  // Same 8-requests-per-day abuse cap as barber.
  ...
  if (error) {
    console.error('[photo/book] insert failed', { code: error.code, message: error.message })
