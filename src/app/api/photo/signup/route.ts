...
import { slugify } from '@/lib/photo/slug'
import { ALL_SPECIALTIES, MAX_PHOTO_SPECIALTIES, type PhotoSpecialty } from '@/lib/photo/types'
...
export async function POST(req: Request) {
  ...
  const specialties = (body.specialties ?? [])
    ...
    .slice(0, MAX_PHOTO_SPECIALTIES) as PhotoSpecialty[]
  ...
  for (let i = 2; i <= 9; i++) {
    const { data: existing } = await admin
      .from('photo_providers').select('id').eq('slug', slug).maybeSingle()
    if (!existing) break
    slug = `${base}-${i}`
  ...
  if (error) {
    console.error('[photo/signup] insert failed', { code: error.code, message: error.message })
