...
import { ALL_SPECIALTIES, MAX_PARCEL_SPECIALTIES } from '@/lib/parcel/types'
...
type Body = {
  ...
  languages?:          string[]
  // mig 0087 + 0089 + 0091 — profile-parity fields parcel gained for
  // the polished /parcel/[slug] page. All optional.
...
export async function POST(req: Request) {
  ...
  const update: TableUpdate<'parcel_providers'> = {}
  ...
  if (Array.isArray(body.specialties)) {
    const valid = body.specialties
      ...
      .slice(0, MAX_PARCEL_SPECIALTIES)
  ...
  const { error } = await admin.from('parcel_providers').update(update).eq('user_id', user.id)
