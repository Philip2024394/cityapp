...
import { slugify } from '@/lib/parcel/slug'
import { ALL_SPECIALTIES, MAX_PARCEL_SPECIALTIES, type ParcelSpecialty } from '@/lib/parcel/types'
...
export async function POST(req: Request) {
  ...
  const specialties = (body.specialties ?? [])
    ...
    .slice(0, MAX_PARCEL_SPECIALTIES) as ParcelSpecialty[]
  ...
  for (let i = 2; i <= 9; i++) {
    const { data: existing } = await admin
      .from('parcel_providers').select('id').eq('slug', slug).maybeSingle()
    if (!existing) break
    slug = `${base}-${i}`
  ...
  if (error) {
    console.error('[parcel/signup] insert failed', { code: error.code, message: error.message })
