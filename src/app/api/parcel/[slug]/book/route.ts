...
// POST /api/parcel/[slug]/book
// Customer submits a parcel pickup/delivery request for a kurir. The
// owner sees it in the dashboard and confirms / declines. Platform
// never holds money — the actual handshake (DP, pickup window,
// drop-off, COD) continues on WhatsApp; this row just records the
// request so the kurir knows who and when.
//
// Validation + shape mirrors carwash's /api/carwash/[slug]/book.
...
export async function POST(req: Request, ctx: { params: Promise<{ slug: string }> }) {
  ...
  // busy_dates is a date[] on parcel_providers (mig 0089-equiv).
  ...
  // Same 8-requests-per-day abuse cap as carwash/tailor/pet/florist.
  ...
  if (error) {
    console.error('[parcel/book] insert failed', { code: error.code, message: error.message })
