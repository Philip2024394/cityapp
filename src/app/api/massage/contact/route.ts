...
// POST /api/massage/contact
// Public form submission from /massage/[slug]. Body shape:
...
export async function POST(req: Request) {
  ...
  const { data: provider, error: lookupErr } = await admin
    ...
    .maybeSingle()
  if (lookupErr) {
    console.error('[massage/contact] lookup failed', { code: lookupErr.code, message: lookupErr.message })
  ...
  if (insertErr) {
    console.error('[massage/contact] insert failed', { code: insertErr.code, message: insertErr.message })
  ...
  void sendResendEmail({
    ...
    console.error('[massage/contact] resend failed', e instanceof Error ? e.message : e)
  })
...
async function sendResendEmail({
  ...
  if (!apiKey) {
    console.warn('[massage/contact] RESEND_API_KEY missing — message saved but no email sent')
    return
  }
