...
import { slugify } from '@/lib/tutoring/slug'
import { ALL_SPECIALTIES, MAX_TUTORING_SPECIALTIES, type TutoringSpecialty } from '@/lib/tutoring/types'
...
export async function POST(req: Request) {
  ...
  const specialties = (body.specialties ?? [])
    ...
    .slice(0, MAX_TUTORING_SPECIALTIES) as TutoringSpecialty[]
  if (specialties.length === 0) {
    return NextResponse.json({ error: 'at_least_one_specialty' }, { status: 400 })
  }

  // Pricing — per-session drop-in and/or monthly coaching bundle.
  ...
  for (let i = 2; i <= 9; i++) {
    const { data: existing } = await admin
      .from('tutoring_providers').select('id').eq('slug', slug).maybeSingle()
    if (!existing) break
    slug = `${base}-${i}`
  ...
  if (error) {
    console.error('[tutoring/signup] insert failed', { code: error.code, message: error.message })
