...
import { ALL_SPECIALTIES, MAX_TUTORING_SPECIALTIES } from '@/lib/tutoring/types'
...
type Body = {
  ...
  languages?:          string[]
  // mig 0087 + 0089 + 0091 — profile-parity fields tutoring gained for
  // the polished /tutoring/[slug] page. All optional.
...
export async function POST(req: Request) {
  ...
  const update: TableUpdate<'tutoring_providers'> = {}
  ...
  if (Array.isArray(body.specialties)) {
    const valid = body.specialties
      ...
      .slice(0, MAX_TUTORING_SPECIALTIES)
  ...
  const { error } = await admin.from('tutoring_providers').update(update).eq('user_id', user.id)
