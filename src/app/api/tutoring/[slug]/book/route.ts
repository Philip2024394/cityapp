...
// POST /api/tutoring/[slug]/book
// Customer submits a session booking request for a tutor.
// He/she sees it in their dashboard and confirms / declines. Platform
// never holds money — the actual handshake (DP, intake form, session
// plan) continues on WhatsApp; this row just records the request so
// the trainer knows who and when.
//
// Validation + shape mirrors florist's /api/florist/[slug]/book.
...
export async function POST(req: Request, ctx: { params: Promise<{ slug: string }> }) {
  ...
  // busy_dates is a date[] on tutoring_providers (mig 0089-equiv).
  ...
  // Same 8-requests-per-day abuse cap as florist.
  ...
  if (error) {
    console.error('[tutoring/book] insert failed', { code: error.code, message: error.message })
