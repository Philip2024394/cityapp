...
import { ALL_SPECIALTIES, MAX_FITNESS_SPECIALTIES } from '@/lib/fitness/types'
...
type Body = {
  ...
  languages?:          string[]
  // mig 0087 + 0089 + 0091 — profile-parity fields fitness gained for
  // the polished /fitness/[slug] page. All optional.
...
export async function POST(req: Request) {
  ...
  const update: TableUpdate<'fitness_providers'> = {}
  ...
  if (Array.isArray(body.specialties)) {
    const valid = body.specialties
      ...
      .slice(0, MAX_FITNESS_SPECIALTIES)
  ...
  const { error } = await admin.from('fitness_providers').update(update).eq('user_id', user.id)
