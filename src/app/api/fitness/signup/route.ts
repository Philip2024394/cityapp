...
import { ALL_SPECIALTIES, MAX_FITNESS_SPECIALTIES, type FitnessSpecialty } from '@/lib/fitness/types'
...
export async function POST(req: Request) {
  ...
  const specialties = (body.specialties ?? [])
    ...
    .slice(0, MAX_FITNESS_SPECIALTIES) as FitnessSpecialty[]
  ...
  for (let i = 2; i <= 9; i++) {
    const { data: existing } = await admin
      .from('fitness_providers').select('id').eq('slug', slug).maybeSingle()
    if (!existing) break
    slug = `${base}-${i}`
  ...
  if (error) {
    console.error('[fitness/signup] insert failed', { code: error.code, message: error.message })
