...
// POST /api/tailor/[slug]/book
// Customer submits a tailoring job request for a tailor.
// He/she sees it in their dashboard and confirms / declines. Platform
// never holds money — the actual handshake (DP, measurement, fitting)
// continues on WhatsApp; this row just records the request so the tailor
// knows who and when.
//
...
export async function POST(req: Request, ctx: { params: Promise<{ slug: string }> }) {
  ...
  // busy_dates is a date[] on tailor_providers (mig 0089-equiv).
  ...
  if (error) {
    console.error('[tailor/book] insert failed', { code: error.code, message: error.message })
