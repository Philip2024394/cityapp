...
import { slugify } from '@/lib/tailor/slug'
import { ALL_SPECIALTIES, MAX_TAILOR_SPECIALTIES, type TailorSpecialty } from '@/lib/tailor/types'
...
export async function POST(req: Request) {
  ...
  const specialties = (body.specialties ?? [])
    ...
    .slice(0, MAX_TAILOR_SPECIALTIES) as TailorSpecialty[]
  ...
  for (let i = 2; i <= 9; i++) {
    const { data: existing } = await admin
      .from('tailor_providers').select('id').eq('slug', slug).maybeSingle()
    if (!existing) break
    slug = `${base}-${i}`
  ...
  if (error) {
    console.error('[tailor/signup] insert failed', { code: error.code, message: error.message })
