...
import { ALL_SPECIALTIES, MAX_TAILOR_SPECIALTIES } from '@/lib/tailor/types'
...
type Body = {
  ...
  languages?:          string[]
  // mig 0087 + 0089 + 0091 — profile-parity fields tailor gained for
  // the polished /tailor/[slug] page. All optional.
...
export async function POST(req: Request) {
  ...
  const update: TableUpdate<'tailor_providers'> = {}
  ...
  if (Array.isArray(body.specialties)) {
    const valid = body.specialties
      ...
      .slice(0, MAX_TAILOR_SPECIALTIES)
  ...
  const { error } = await admin.from('tailor_providers').update(update).eq('user_id', user.id)
