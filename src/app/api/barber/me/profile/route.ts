...
import { ALL_SPECIALTIES, MAX_BARBER_SPECIALTIES } from '@/lib/barber/types'
...
type Body = {
  ...
  languages?:          string[]
  // mig 0087 + 0089 + 0091 — profile-parity fields barber gained for
  // the polished /barber/[slug] page. All optional.
...
export async function POST(req: Request) {
  ...
  const update: TableUpdate<'barber_providers'> = {}
  ...
  if (Array.isArray(body.specialties)) {
    const valid = body.specialties
      ...
      .slice(0, MAX_BARBER_SPECIALTIES)
  ...
  const { error } = await admin.from('barber_providers').update(update).eq('user_id', user.id)
