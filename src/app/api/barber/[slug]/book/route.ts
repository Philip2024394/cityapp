...
// POST /api/barber/[slug]/book
// Customer submits a booking request for a barber. He sees it in his
...
// request so the barber knows who and when.
//
// Validation + shape mirrors beautician's /api/beautician/[slug]/book
// and tattoo's /api/tattoo/[slug]/book.
...
export async function POST(req: Request, ctx: { params: Promise<{ slug: string }> }) {
  ...
  // busy_dates is a date[] on barber_providers (mig 0089).
  ...
  // Same 8-requests-per-day abuse cap as beautician / tattoo.
  ...
  if (error) {
    console.error('[barber/book] insert failed', { code: error.code, message: error.message })
