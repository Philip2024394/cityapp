...
import { ALL_SPECIALTIES, MAX_CATERING_SPECIALTIES } from '@/lib/catering/types'
...
type Body = {
  ...
  languages?:          string[]
  // mig 0087 + 0089 + 0091 — profile-parity fields video gained for
  // the polished /catering/[slug] page. All optional.
...
export async function POST(req: Request) {
  ...
  const update: TableUpdate<'catering_providers'> = {}
  ...
  if (Array.isArray(body.specialties)) {
    const valid = body.specialties
      ...
      .slice(0, MAX_CATERING_SPECIALTIES)
  ...
  const { error } = await admin.from('catering_providers').update(update).eq('user_id', user.id)
