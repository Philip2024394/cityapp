...
// POST /api/catering/[slug]/book
// Customer submits a booking request for a catering business. She sees it in
// her dashboard and confirms / declines. Platform never holds money —
// the actual handshake (deposit, contract, location) continues on
// WhatsApp; this row just records the request so the catering business knows
// who and when.
//
// Validation + shape mirrors photo's /api/photo/[slug]/book.
...
export async function POST(req: Request, ctx: { params: Promise<{ slug: string }> }) {
  ...
  // busy_dates is a date[] on catering_providers (mig 0089-equiv).
  ...
  // Same 8-requests-per-day abuse cap as photo.
  ...
  if (error) {
    console.error('[catering/book] insert failed', { code: error.code, message: error.message })
