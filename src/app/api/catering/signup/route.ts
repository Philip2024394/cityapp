...
import { slugify } from '@/lib/catering/slug'
import { ALL_SPECIALTIES, MAX_CATERING_SPECIALTIES, type CateringSpecialty } from '@/lib/catering/types'
...
export async function POST(req: Request) {
  ...
  const specialties = (body.specialties ?? [])
    ...
    .slice(0, MAX_CATERING_SPECIALTIES) as CateringSpecialty[]
  if (specialties.length === 0) {
    return NextResponse.json({ error: 'at_least_one_specialty' }, { status: 400 })
  }

  // Pricing — per-package starter rate and/or full-day. CHECK enforces
  // at least one is set.
  ...
  for (let i = 2; i <= 9; i++) {
    const { data: existing } = await admin
      .from('catering_providers').select('id').eq('slug', slug).maybeSingle()
    if (!existing) break
    slug = `${base}-${i}`
  ...
  if (error) {
    console.error('[catering/signup] insert failed', { code: error.code, message: error.message })
