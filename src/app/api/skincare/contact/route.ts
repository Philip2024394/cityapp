...
// POST /api/skincare/contact
// Public form submission from /skincare/[slug]. Body shape:
...
export async function POST(req: Request) {
  ...
  const { data: provider, error: lookupErr } = await admin
    ...
    .maybeSingle()
  if (lookupErr) {
    console.error('[skincare/contact] lookup failed', { code: lookupErr.code, message: lookupErr.message })
  ...
  if (insertErr) {
    console.error('[skincare/contact] insert failed', { code: insertErr.code, message: insertErr.message })
  ...
  void sendResendEmail({
    ...
    console.error('[skincare/contact] resend failed', e instanceof Error ? e.message : e)
  })
...
async function sendResendEmail({
  ...
  if (!apiKey) {
    console.warn('[skincare/contact] RESEND_API_KEY missing — message saved but no email sent')
    return
  }
