...
// POST /api/beautician/me/orders/[id]
// ----------------------------------------------------------------------------
...
//
// Ownership enforced by joining vendor_id → beautician_providers.user_id.
...
export async function POST(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const { data, error } = await admin
    ...
    .eq('id', id)
    .eq('vendor_type', 'beautician')
    .eq('vendor_id', bp.id)
    .select('*')
    .maybeSingle()
  if (error) {
    console.error('[me/orders/:id] update failed', { code: error.code, message: error.message })
