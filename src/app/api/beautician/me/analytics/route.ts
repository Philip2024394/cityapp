...
// GET /api/beautician/me/analytics
...
//
// We deliberately do NOT delete rows older than 28 days from the table;
// we only refuse to QUERY past the user's plan window. When a Free user
// upgrades, the full year of historical data is already there waiting.
//
// Schema: provider_profile_views (id, provider_type, provider_id,
// anon_session_id, source, viewed_at, utm_*) — confirmed via
// information_schema query 2026-06-09.
...
export async function GET() {
  ...
  // Bucket by day (UTC date string YYYY-MM-DD) then backfill every day
  // in the window so the chart renders a continuous time axis with 0s
  // for empty days. Without this the chart compresses gaps and lies
  // about traffic shape.
