...
import { ALL_SPECIALTIES, MAX_MOVER_SPECIALTIES } from '@/lib/mover/types'
...
type Body = {
  ...
  languages?:          string[]
  // mig 0087 + 0089 + 0091 — profile-parity fields mover gained for
  // the polished /mover/[slug] page. All optional.
...
export async function POST(req: Request) {
  ...
  const update: TableUpdate<'mover_providers'> = {}
  ...
  const { error } = await admin.from('mover_providers').update(update).eq('user_id', user.id)
