...
import { slugify } from '@/lib/mover/slug'
import { ALL_SPECIALTIES, MAX_MOVER_SPECIALTIES, type MoverSpecialty } from '@/lib/mover/types'
...
export async function POST(req: Request) {
  ...
  const specialties = (body.specialties ?? [])
    ...
    .slice(0, MAX_MOVER_SPECIALTIES) as MoverSpecialty[]
  ...
  for (let i = 2; i <= 9; i++) {
    const { data: existing } = await admin
      .from('mover_providers').select('id').eq('slug', slug).maybeSingle()
    if (!existing) break
    slug = `${base}-${i}`
  ...
  if (error) {
    console.error('[mover/signup] insert failed', { code: error.code, message: error.message })
