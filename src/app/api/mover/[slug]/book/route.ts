...
// POST /api/mover/[slug]/book
// Customer submits a moving job request for a mover.
// He/she sees it in their dashboard and confirms / declines. Platform
// never holds money — the actual handshake (DP, survey, fixed quote)
// continues on WhatsApp; this row just records the request so the mover
// knows who and when.
//
// Validation + shape mirrors pet's /api/pet/[slug]/book.
...
export async function POST(req: Request, ctx: { params: Promise<{ slug: string }> }) {
  ...
  // busy_dates is a date[] on mover_providers (mig 0089-equiv).
  ...
  // Same 8-requests-per-day abuse cap as pet/florist.
  ...
  if (error) {
    console.error('[mover/book] insert failed', { code: error.code, message: error.message })
