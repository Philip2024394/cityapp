...
type Body = {
  ...
  languages?:          string[]
  // mig 0123 services offered catalog
  services_offered?:   string[]
  // mig 0087 per-profile theme accent color (hex #RRGGBB)
  theme_color?: string | null
  // mig 0105 hero text customisation (mirrors beautician 0081)
  hero_text?: {
    line1?:         string
    line2?:         string
    tagline?:       string
  ...
  // mig 0105 running marquee promo text
  promo_text?: string | null
  // mig 0105 per-service photo gallery — rich shape
...
export async function POST(req: Request) {
  ...
  const update: TableUpdate<'home_clean_providers'> = {}
  ...
  const { error } = await admin.from('home_clean_providers').update(update).eq('user_id', user.id)
