...
// POST /api/signup/car
// ----------------------------------------------------------------------------
// Creates the profile (role=driver) + drivers row for a new CAR driver that
// has already passed the phone-OTP step on /signup/car. Mirrors the bike
// flow's `/api/onboarding/driver` but is scoped to car-specific fields:
//
//   • vehicle_type='car'
//   • services=['person'] (car drivers do passenger rides)
...
export async function POST(req: Request) {
  ...
  let body: SignupCarPayload
  try {
    body = (await req.json()) as SignupCarPayload
  } catch {
  ...
  // anti-abuse threshold (Rp 2,000/km for cars). NOT regulatory
  // enforcement — that lives client-side as a warn banner. This just
  // catches typos / test data / accidental zeros.
  if (!Number.isFinite(body.price_per_km) || body.price_per_km < antiSpamPerKm('car') || body.price_per_km > 50000) {
    return NextResponse.json({ error: `Price per km must be Rp ${antiSpamPerKm('car').toLocaleString('id-ID')}–50.000 (driver-self-published)` }, { status: 400 })
  }
  ...
  // 4. Upsert drivers row. vehicle_type='car' is the discriminator that
  //    keys the marketplace + dashboard routing. paid_until stays NULL —
  //    the driver lands on /dashboard/car next and pays via the QRIS modal
  //    to flip their listing into the public /car marketplace.
  const driverRes = await admin.from('drivers').upsert({
    ...
    // Service — car drivers only do passenger rides in Phase 1
    services: ['person'],
    ...
    // QRIS payment flow on /dashboard/car.
    paid_until: null,
    status: 'active',
  ...
  return NextResponse.json({ ok: true, redirectTo: '/dashboard/car', slug })
}
