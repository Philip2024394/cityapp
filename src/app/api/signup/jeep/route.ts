...
// ============================================================================
// POST /api/signup/jeep
// ----------------------------------------------------------------------------
// Creates the profile (role=driver) + drivers row for a new JEEP driver that
// has already passed the phone-OTP step on /signup/jeep. Mirrors the bike
// flow's `/api/onboarding/driver` but is scoped to jeep-specific fields:
//
//   • vehicle_type='jeep'
//   • services=['person'] (jeep drivers do passenger rides)
...
//     subscription via /dashboard/jeep before their listing goes live in
//     the public /jeep marketplace.
//
...
export async function POST(req: Request) {
  ...
  let body: SignupJeepPayload
  try {
    body = (await req.json()) as SignupJeepPayload
  } catch {
    return NextResponse.json({ error: 'Invalid JSON body' }, { status: 400 })
  }

  // ──────────────────────────────────────────────────────────────────────────
  // Server-side validation. Mirror /api/onboarding/driver shape.
  // ──────────────────────────────────────────────────────────────────────────
  ...
  const ALLOWED_SEATS = new Set([4, 5, 6, 7, 8, 14])
  if (!Number.isFinite(body.vehicle_seats) || !ALLOWED_SEATS.has(body.vehicle_seats)) {
    return NextResponse.json({ error: 'Seats must be one of 4/5/6/7/8/14' }, { status: 400 })
  }
  // Anti-spam floor (Option B): reject values below the platform's
  // anti-abuse threshold. NOT regulatory enforcement.
  if (!Number.isFinite(body.price_per_km) || body.price_per_km < antiSpamPerKm('jeep') || body.price_per_km > 50000) {
    return NextResponse.json({ error: `Price per km must be Rp ${antiSpamPerKm('jeep').toLocaleString('id-ID')}–50.000 (driver-self-published)` }, { status: 400 })
  }
  ...
  // 4. Upsert drivers row. vehicle_type='jeep' is the discriminator that
  //    keys the marketplace + dashboard routing. paid_until stays NULL —
  //    the driver lands on /dashboard/jeep next and pays via the QRIS modal
  //    to flip their listing into the public /jeep marketplace.
  const driverRes = await admin.from('drivers').upsert({
    ...
    // Service — jeep drivers only do passenger rides in Phase 1
    services: ['person'],
    ...
    // QRIS payment flow on /dashboard/jeep.
    paid_until: null,
    status: 'active',
  ...
  return NextResponse.json({ ok: true, redirectTo: '/dashboard/jeep', slug })
}
