...
// POST /api/carwash/[slug]/book
// Customer submits a car-wash job request for a car wash. The owner
// sees it in the dashboard and confirms / declines. Platform never
// holds money — the actual handshake (DP, drop-off, valet pickup)
// continues on WhatsApp; this row just records the request so the
// car wash knows who and when.
//
// Validation + shape mirrors tailor's /api/tailor/[slug]/book.
...
export async function POST(req: Request, ctx: { params: Promise<{ slug: string }> }) {
  ...
  const { data: pp, error: ppErr } = await admin
    .from('carwash_providers')
    .select('id, status, busy_dates')
    .eq('slug', slug)
    .maybeSingle()
  if (ppErr) return NextResponse.json({ error: 'fetch_failed' }, { status: 500 })
  if (!pp || pp.status !== 'active') return NextResponse.json({ error: 'not_found' }, { status: 404 })

  // busy_dates is a date[] on carwash_providers (mig 0089-equiv).
  if (Array.isArray(pp.busy_dates) && pp.busy_dates.includes(date)) {
    return NextResponse.json({ error: 'date_unavailable' }, { status: 409 })
  }

  const ipHash = hashIp(req)

  // Same 8-requests-per-day abuse cap as tailor/pet/florist.
  ...
  if (error) {
    console.error('[carwash/book] insert failed', { code: error.code, message: error.message })
