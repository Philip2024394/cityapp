...
import { ALL_SPECIALTIES, MAX_CARWASH_SPECIALTIES } from '@/lib/carwash/types'
...
type Body = {
  ...
  languages?:          string[]
  // mig 0087 + 0089 + 0091 — profile-parity fields carwash gained for
  // the polished /car-wash/[slug] page. All optional.
...
export async function POST(req: Request) {
  ...
  const update: TableUpdate<'carwash_providers'> = {}
  ...
  if (Array.isArray(body.specialties)) {
    const valid = body.specialties
      ...
      .slice(0, MAX_CARWASH_SPECIALTIES)
  ...
  const { error } = await admin.from('carwash_providers').update(update).eq('user_id', user.id)
