...
import { slugify } from '@/lib/carwash/slug'
import { ALL_SPECIALTIES, MAX_CARWASH_SPECIALTIES, type CarwashSpecialty } from '@/lib/carwash/types'
...
export async function POST(req: Request) {
  ...
  const specialties = (body.specialties ?? [])
    ...
    .slice(0, MAX_CARWASH_SPECIALTIES) as CarwashSpecialty[]
  if (specialties.length === 0) {
    return NextResponse.json({ error: 'at_least_one_specialty' }, { status: 400 })
  }

  // Pricing — anchor "starting from" price + optional all-in package.
  ...
  for (let i = 2; i <= 9; i++) {
    const { data: existing } = await admin
      .from('carwash_providers').select('id').eq('slug', slug).maybeSingle()
    if (!existing) break
    slug = `${base}-${i}`
  ...
  if (error) {
    console.error('[carwash/signup] insert failed', { code: error.code, message: error.message })
