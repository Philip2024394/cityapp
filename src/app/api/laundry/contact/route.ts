...
// POST /api/laundry/contact
// Public form submission from /laundry/[slug]. Body shape:
...
export async function POST(req: Request) {
  ...
  const { data: provider, error: lookupErr } = await admin
    ...
    .maybeSingle()
  if (lookupErr) {
    console.error('[laundry/contact] lookup failed', { code: lookupErr.code, message: lookupErr.message })
  ...
  if (insertErr) {
    console.error('[laundry/contact] insert failed', { code: insertErr.code, message: insertErr.message })
  ...
  void sendResendEmail({
    ...
    console.error('[laundry/contact] resend failed', e instanceof Error ? e.message : e)
  })
...
async function sendResendEmail({
  ...
  if (!apiKey) {
    console.warn('[laundry/contact] RESEND_API_KEY missing — message saved but no email sent')
    return
  }
