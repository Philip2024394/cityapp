...
import { ALL_SPECIALTIES, MAX_CAKE_SPECIALTIES } from '@/lib/cake/types'
...
type Body = {
  ...
  languages?:          string[]
  // mig 0087 + 0089 + 0091 — profile-parity fields cake gained for
  // the polished /cake/[slug] page. All optional.
...
export async function POST(req: Request) {
  ...
  const update: TableUpdate<'cake_providers'> = {}
  ...
  const { error } = await admin.from('cake_providers').update(update).eq('user_id', user.id)
