...
import { slugify } from '@/lib/cake/slug'
import { ALL_SPECIALTIES, MAX_CAKE_SPECIALTIES, type CakeSpecialty } from '@/lib/cake/types'
...
export async function POST(req: Request) {
  ...
  const specialties = (body.specialties ?? [])
    ...
    .slice(0, MAX_CAKE_SPECIALTIES) as CakeSpecialty[]
  if (specialties.length === 0) {
    return NextResponse.json({ error: 'at_least_one_specialty' }, { status: 400 })
  }

  // Pricing — per-cake starter rate and/or paket bundle. CHECK enforces
  // at least one is set.
  ...
  for (let i = 2; i <= 9; i++) {
    const { data: existing } = await admin
      .from('cake_providers').select('id').eq('slug', slug).maybeSingle()
    if (!existing) break
    slug = `${base}-${i}`
  ...
  if (error) {
    console.error('[cake/signup] insert failed', { code: error.code, message: error.message })
