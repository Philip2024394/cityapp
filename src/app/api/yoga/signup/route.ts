...
import { slugify } from '@/lib/yoga/slug'
import { ALL_SPECIALTIES, MAX_YOGA_SPECIALTIES, type YogaSpecialty } from '@/lib/yoga/types'
...
export async function POST(req: Request) {
  ...
  const specialties = (body.specialties ?? [])
    ...
    .slice(0, MAX_YOGA_SPECIALTIES) as YogaSpecialty[]
  ...
  for (let i = 2; i <= 9; i++) {
    const { data: existing } = await admin
      .from('yoga_providers').select('id').eq('slug', slug).maybeSingle()
    if (!existing) break
    slug = `${base}-${i}`
  ...
  if (error) {
    console.error('[yoga/signup] insert failed', { code: error.code, message: error.message })
