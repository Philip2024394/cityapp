...
import { ALL_SPECIALTIES, MAX_YOGA_SPECIALTIES } from '@/lib/yoga/types'
...
type Body = {
  ...
  languages?:          string[]
  // mig 0087 + 0089 + 0091 — profile-parity fields yoga gained for
  // the polished /yoga/[slug] page. All optional.
...
export async function POST(req: Request) {
  ...
  const update: TableUpdate<'yoga_providers'> = {}
  ...
  const { error } = await admin.from('yoga_providers').update(update).eq('user_id', user.id)
