...
import {
  SKINCARE_SERVICES_OFFERED,
  SKINCARE_SERVICE_LABELS,
  type SkincareProviderPublic,
...
// /skincare/[slug] — universal profile flagship build. Visual-first
// category (portfolio matters more than text); the kit's ProfileGallery
// is the centerpiece. 3-pack pricing (60/90/120min) renders only
// the packages the provider actually offers.

// Vertical default for the hero. Used when the provider hasn't set
// their own cover_image_url yet — keeps the page on-brand instead of
// showing the generic yellow gradient on every new signup.
const DEFAULT_SKINCARE_HERO =
  'https://ik.imagekit.io/nepgaxllc/ChatGPT%20Image%20May%2025,%202026,%2006_53_11%20AM.png'

export default function SkincareProviderPage() {
  const params = useParams<{ slug: string }>()
  const slug = String(params?.slug || '').toLowerCase()
  const [p, setP] = useState<SkincareProviderPublic | null>(null)
  ...
  const [activeService, setActiveService] = useState<SkincareServiceOffered | null>(null)
  ...
  // available when the provider has opted into a physical location).
  ...
  // Vendor cart — keyed on the provider id so each provider keeps its
  ...
  const cart = useVendorCart(`skincare:${p?.id ?? '_loading'}`)
  ...
  useEffect(() => {
    ...
    fetch(`/api/skincare/${encodeURIComponent(slug)}/public`, { cache: 'no-store' })
      .then((r) => r.ok ? r.json() : null)
      .then((j: { provider?: SkincareProviderPublic } | null) => {
        if (j?.provider) setP(j.provider); else setNotFound(true)
      })
      .catch(() => setNotFound(true))
  }, [slug])

  // Cast: useProfileViewTracker hasn't been widened to include 'skincare'
  // yet. Tracker behaviour is structural — the row is keyed by
  // provider_type so it can still record skincare views once the hook
  // adds the new union member.
  useProfileViewTracker({ providerType: 'skincare' as 'beautician', providerId: p?.id })
  ...
  useEffect(() => {
    ...
    fetch(`/api/reviews?provider_type=skincare&provider_id=${encodeURIComponent(p.id)}`, { cache: 'no-store' })
      .then((r) => r.ok ? r.json() : null)
    ...
    return (
      <Shell>
        <div className="px-4 pt-20 max-w-md mx-auto text-center">
          <h1 className="text-[20px] font-black mb-2">Skincare provider not found</h1>
          <Link href="/skincare" className="rounded-full bg-brand text-bg px-6 py-3 text-[13px] font-extrabold inline-block">Back to marketplace</Link>
        </div>
      </Shell>
    )
  }
  ...
  return (
    <Shell>
      ...
      <div className="relative pb-2">
        ...
        >
          <img
            src={bannerSrc(p.cover_image_url) || DEFAULT_SKINCARE_HERO}
            alt=""
          ...
          {(() => {
            ...
            const line1   = (ht as { line1?: string }).line1   || 'Professional'
            const line2   = (ht as { line2?: string }).line2   || 'Skincare'
            const tagline = (ht as { tagline?: string }).tagline || 'Healthy skin, expert care, every day'
            const line2Color   = (ht as { color?: string }).color         || theme
            const line1Color   = (ht as { line1_color?: string }).line1_color   || '#000000'
            const taglineColor = (ht as { tagline_color?: string }).tagline_color || '#000000'
            // Map legacy 'dance' / 'flyin' values to 'none' so old saved
            // data doesn't break the new effect set.
            const rawEffect = (ht as { effect?: string }).effect || 'none'
            ...
            return (
              <div className={`absolute left-4 z-10 select-none leading-none cr-hero-${effect}`} style={{ top: 31 }}>
                ...
                {/* Service locations the provider travels to —
                ...
                {(() => {
                  ...
                  // "Skincare Studio" so it's obvious this is in-clinic only.
                  if (hasSpa) {
                    const label = locs.size === 0 ? 'Skincare Studio' : 'Studio'
        ...
        <div className="px-4 relative z-20" style={{ marginTop: 12 }}>
          ...
          >
            ...
            <AvatarFrame
              ...
              alt={p.display_name}
              size={64}
              style={(p as unknown as { avatar_frame_style?: 'none' | 'gradient' | 'pulse' | 'rainbow' }).avatar_frame_style ?? 'none'}
              themeColor={theme}
      ...
      <div className="px-4 pb-6 max-w-2xl mx-auto space-y-3 pt-3">
        ...
        {showVisitUs ? (
          ...
          <section className="space-y-3">
            ...
            {/* Legal footer — Terms / Privacy links. Tapping opens an
                ...
                corresponding page in /dashboard/skincare/{terms,privacy}. */}
        ...
        {(() => {
          const offered = (p.services_offered ?? []) as SkincareServiceOffered[]
          ...
          return (
            <section className="space-y-2">
              <div className="flex items-center justify-between gap-2">
                <h2 className="text-[13px] font-extrabold uppercase tracking-wider text-black">
                  {activeService
                    ? `${SKINCARE_SERVICE_LABELS[activeService]} — Portfolio`
                    : 'Portfolio'}
                </h2>
                <PortfolioViewToggle
                  view={portfolioView}
        ...
        <RunningMarquee
          text={p.promo_text || 'Message me this week — exclusive promo on professional skincare service delivered straight to your home, hotel or villa, in the comfort of your stay.'}
        />
            ...
            <>
              ...
              <button
                ...
                className={`inline-flex items-center gap-1.5 justify-center px-5 py-3 rounded-xl text-white font-extrabold text-[13px] shadow-md active:scale-[0.97] transition shrink-0 ${
                  (p as unknown as { cta_button_effect?: string }).cta_button_effect === 'pulse' ? 'cr-cta-pulse'
                : (p as unknown as { cta_button_effect?: string }).cta_button_effect === 'glow'  ? 'cr-cta-glow'
                : (p as unknown as { cta_button_effect?: string }).cta_button_effect === 'shake' ? 'cr-cta-shake'
                : ''
                }`}
                style={{ background: theme }}
              >
      ...
      {detailPhoto && (() => {
        ...
        return (
          <PortfolioDetailPopup
            photo={detailPhoto as unknown as PortfolioPhoto}
            themeColor={theme}
      ...
      {/* Contact / booking popup. NOTE: the /book endpoint is not cloned
          for skincare yet (per spec); the popup will submit to the
          skincare path and fall back to the WhatsApp open flow if the
          server-side booking insert fails. */}
      {contactOpen && p.whatsapp_e164 && (
        <ContactBookingPopup
          ...
          serviceOptions={(p.services_offered ?? []).map((sid) => ({
            ...
            label: SKINCARE_SERVICE_LABELS[sid] ?? sid,
          }))}
          presetService={contactServiceName}
          busyDates={(p.busy_dates ?? []) as string[]}
          bookEndpoint={`/api/skincare/${p.slug}/book`}
...
}: { sid: SkincareServiceOffered; active: boolean; onClick: () => void; theme: string }) {
  return (
    <button
      type="button"
      onClick={onClick}
    ...
    >
      ...
      />
      {SKINCARE_SERVICE_LABELS[sid] ?? sid}
    </button>
  )
}

// Cheapest price across all per-photo entries (mig 0074 rich shape).
// Falls back to the 60/90/120min columns when no photo prices are set.
function formatStartFromPrice(p: SkincareProviderPublic): string {
...
// Combines per-service photos into the carousel feed.
function buildPortfolioPhotos(
  p: SkincareProviderPublic,
  active: SkincareServiceOffered | null,
): BeauticianServicePhoto[] {
  const sp = p.service_photos ?? {}
  const offered = new Set<SkincareServiceOffered>(p.services_offered ?? [])
  if (active) {
  ...
  for (const cat of SKINCARE_SERVICES_OFFERED) {
