...
//
// This page surfaces drivers whose `vehicle_type = 'car'` from BOTH the
...
//
// Phase 1: no real car drivers exist yet (the /dashboard/car onboarding
...
function realToDriver(r: RealRow): CarDriver {
  return {
    id: r.user_id,
    slug: r.slug,
...
function mockToDriver(r: MockRow): CarDriver {
  return {
    id: r.id,
    slug: r.slug,
    business_name: r.business_name,
...
export default async function CarMarketplacePage({
  searchParams,
}: {
  ...
  // empty on first load before real car drivers exist).
  ...
  // is a no-op (no real car drivers yet), but wiring it now means the
  // gate is live the moment the /dashboard/car onboarding ships.
  const today = new Date().toISOString().slice(0, 10)

  // Real car drivers — active status, vehicle_type=car, subscription window
  ...
  // Mock car drivers — seeded by migration 0093. Auto-hide trigger on
  ...
  const merged: CarDriver[] = [...reals, ...mocks].sort((a, b) => {
        ...
        <header className="mb-4 flex items-start justify-between gap-3">
          <div className="min-w-0 flex-1">
            <h1 className="text-[24px] sm:text-[28px] font-extrabold tracking-tight leading-tight text-black">
              {cityLabel} <span className="gradient-text">Car Drivers</span>
            </h1>
        ...
        {list.length === 0 ? (
          <div className="rounded-2xl p-8 text-center border border-black/10 bg-white shadow-sm space-y-3">
            ...
            <div className="text-[14px] font-extrabold text-black">
              No car drivers listed yet
            </div>
            <p className="text-[13px] text-black/60">
              Real car driver dashboards are launching soon. Check back shortly.
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {list.map((d) => (
              <CarDriverCard key={d.slug} driver={d} />
            ))}
          </div>
        )}
...
// Adapter from CarDriver → UniversalProviderCard props. Kept inline (not
// extracted into a shared lib) because the field mapping is specific to
// the /car vertical and changing one card shouldn't ripple to handyman /
// laundry / etc.
function CarDriverCard({ driver: d }: { driver: CarDriver }) {
  // Vehicle line — e.g. "Toyota Avanza 2021 · Silver". Collapses to the
  ...
  // Specialty pill = seat count (the most car-specific quick read). Falls
  // back to "Car" when seats aren't set so the pill never disappears
  // entirely.
