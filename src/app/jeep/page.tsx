...
import { HOURLY_DEFAULTS_JEEP } from '@/lib/pricing/hourlyHire'
import { getAdminSupabase } from '@/lib/supabase/admin'
import { haversineKm } from '@/lib/geo/haversine'

// ============================================================================
// /jeep — Jeep driver marketplace (Phase 1)
// ----------------------------------------------------------------------------
...
//
// This page surfaces drivers whose `vehicle_type = 'jeep'` from BOTH the
...
//
// Phase 1: no real jeep drivers exist yet (the /dashboard/jeep onboarding
// dashboard ships in a follow-up). The 4 Yogya demo mocks seeded in
...
export const metadata = {
  title: 'Jeep Drivers · CityDrivers',
  description:
    'Browse independent jeep drivers in Indonesia. Tours, off-road, sunrise charters — ' +
...
// immediately without a redeploy. Aligns with /tour and /rent.
...
// founder's primary launch market.
...
function realToDriver(r: RealRow): JeepDriver {
  return {
    id: r.user_id,
    slug: r.slug,
...
function mockToDriver(r: MockRow): JeepDriver {
  return {
    id: r.id,
    slug: r.slug,
    business_name: r.business_name,
...
export default async function JeepMarketplacePage({
  searchParams,
}: {
  ...
  // Empty / absent params skip the geo check entirely (Phase 1 has 4 mocks
  // in Yogya; no point pruning them by distance when they're our only stock).
  ...
  // empty on first load before real jeep drivers exist).
  ...
  // is a no-op (no real jeep drivers yet), but wiring it now means the
  // gate is live the moment the /dashboard/jeep onboarding ships.
  const today = new Date().toISOString().slice(0, 10)

  // Real jeep drivers — active status, vehicle_type=jeep, subscription window
  // (paid_until NULL = grace period / never gated, paid_until >= today =
  // active subscription).
  ...
  // Mock jeep drivers — seeded by migration 0093. Auto-hide trigger on
  // drivers insert keeps the mix honest as real drivers join.
  ...
  const merged: JeepDriver[] = [...reals, ...mocks].sort((a, b) => {
      ...
      <div className="max-w-3xl mx-auto px-4 pt-3 pb-24">
        <header className="mb-4 flex items-start justify-between gap-3">
          <div className="min-w-0 flex-1">
            <h1 className="text-[24px] sm:text-[28px] font-extrabold tracking-tight leading-tight text-black">
              {cityLabel} <span className="gradient-text">Jeep Drivers</span>
            </h1>
            <p className="mt-1 text-[13px] text-black/60 leading-snug">
              Independent drivers — published rates, agree fare directly via WhatsApp.
            </p>
          </div>
        ...
        {list.length === 0 ? (
          <div className="rounded-2xl p-8 text-center border border-black/10 bg-white shadow-sm space-y-3">
            ...
            >
              <CarIcon className="w-6 h-6 text-bg" strokeWidth={2.5} />
            </div>
            <div className="text-[14px] font-extrabold text-black">
              No jeep drivers listed yet
            </div>
            <p className="text-[13px] text-black/60">
              Real jeep driver dashboards are launching soon. Check back shortly.
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {list.map((d) => (
              <JeepDriverCard key={d.slug} driver={d} />
            ))}
          </div>
        )}
...
// Adapter from JeepDriver → UniversalProviderCard props. Kept inline (not
// extracted into a shared lib) because the field mapping is specific to
// the /jeep vertical and changing one card shouldn't ripple to handyman /
// laundry / etc.
function JeepDriverCard({ driver: d }: { driver: JeepDriver }) {
  // Vehicle line — e.g. "Suzuki Jimny 2021 · Silver". Collapses to the
  ...
  // Specialty pill = seat count (the most jeep-specific quick read). Falls
  // back to "Jeep" when seats aren't set so the pill never disappears
  // entirely.
  const specialtyLabel = d.vehicle_seats
    ? `${d.vehicle_seats} seats`
    : 'Jeep'

  // Bottom row — JEEP-specific pricing model: hourly tiers (3h / 6h / 8h)
  // instead of per-km. Jeeps charter as PACKAGES, not per-km rides. We
  // surface the 3h tier as the "From Rp X" anchor and the 8h tier as the
  // companion full-day price. Min fee + price_per_km are intentionally
  // hidden — they don't apply to the jeep charter model.
  const jeepHourly = HOURLY_DEFAULTS_JEEP
  const bottomItems: UniversalProviderCardBottomItem[] = [
    {
      key:   'tier3',
      icon:  'dollar',
      label: `From Rp ${jeepHourly.tier_3h.toLocaleString('id-ID')}/3h`,
    },
    {
      key:   'tier8',
      label: `Rp ${jeepHourly.tier_8h.toLocaleString('id-ID')}/8h full day`,
    },
  ]
  ...
  // Cover + profile image — both keyed off the driver's chosen jeep
  // colour so the card visibly varies between Yellow / Black / Blue / etc.
  const jeepImg = getJeepImageUrl(d.vehicle_color)
  ...
  return (
    <UniversalProviderCard
      ...
      bio={d.bio?.replace(/\s*\n\s*/g, ' ') ?? null}
      coverImageUrl={jeepImg}
      profileImageUrl={jeepImg}
