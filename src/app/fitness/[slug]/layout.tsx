...
// Server-side SEO + lapsed redirect for /fitness/[slug].
// Mirrors /florist/[slug] / /cake/[slug] / /catering/[slug] / etc.
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  if (!p) return { title: 'Personal trainer not found', robots: { index: false, follow: false } }
  const pkg = p.hourly_rate_idr ?? null
  const title       = `${p.display_name} · Personal trainer ${p.city || 'Indonesia'}`
  const description = (p.bio?.trim())
    || `Book ${p.display_name} (personal trainer) in ${p.city || 'Indonesia'} via WhatsApp.${pkg ? ` Sessions from Rp ${pkg.toLocaleString('id-ID')}.` : ''}`
  const canonical   = `${SITE_URL}/fitness/${p.slug}`
...
export default async function FitnessProviderLayout({
  children, params,
  ...
  if (p) {
    ...
    if (lapsed) {
      const qs = new URLSearchParams({ from: 'lapsed_fitness', slug: p.slug })
      redirect(`/fitness?${qs.toString()}`)
    }
  }
  const jsonLd = p
    ? {
        '@context': 'https://schema.org', '@type': 'LocalBusiness',
        '@id': `${SITE_URL}/fitness/${p.slug}`,
        name: p.display_name, description: p.bio || undefined,
        url: `${SITE_URL}/fitness/${p.slug}`,
