...
import {
  ALL_SPECIALTIES,
  SPECIALTY_LABELS,
  type FitnessProviderPublic,
  type FitnessSpecialty,
...
// Default theme accent — used when the personal trainer hasn't picked
// their own theme_color (mig 0087). Trainers choose their accent from
// the dashboard color palette; the chosen hex flows through every
// accent surface on this page via the `theme` constant below. Default
// sky blue (#0EA5E9) so the action / sweat imagery pops with
// sport-tuned signage (distinct from florist's botanical green).
...
// /fitness/[slug] — universal profile flagship build. Mirrors the
// /florist/[slug] page byte-for-byte at the JSX layer; only the data
// source swaps from florist_providers to fitness_providers and a few
// vertical-specific adaptations apply (training-type chips, per-session
// + monthly bundle pricing block, flat service_photos array with
// partial-name filter).

// Vertical default for the hero. Used when the personal trainer hasn't
// set their own cover_image_url yet — keeps the page on-brand instead of
// showing the generic yellow gradient on every new signup.
const DEFAULT_FITNESS_HERO =
  'https://images.unsplash.com/photo-1571019613454-1cb2f99b2d8b?w=1200&auto=format&fit=crop'

export default function FitnessProviderPage() {
  const params = useParams<{ slug: string }>()
  const slug = String(params?.slug || '').toLowerCase()
  const [p, setP] = useState<FitnessProviderPublic | null>(null)
  ...
  // null = show photos from ALL training types (combined). When a
  // specialty is set, the portfolio carousel filters to just that type.
  const [activeService, setActiveService] = useState<FitnessSpecialty | null>(null)
  ...
  // available when the personal trainer has opted into a physical studio).
  ...
  useEffect(() => {
    ...
    const url = `/api/fitness/${encodeURIComponent(slug)}/public${
      draftPassword ? `?p=${encodeURIComponent(draftPassword)}` : ''
    }`
    fetch(url, { cache: 'no-store' })
      .then((r) => r.ok ? r.json() : null)
      .then((j: { provider?: FitnessProviderPublic; locked?: boolean } | null) => {
  ...
  useProfileViewTracker({ providerType: 'fitness', providerId: p?.id })
  ...
  useEffect(() => {
    ...
    fetch(`/api/reviews?provider_type=fitness&provider_id=${encodeURIComponent(p.id)}`, { cache: 'no-store' })
      .then((r) => r.ok ? r.json() : null)
    ...
    return (
      <Shell>
        <div className="px-4 pt-20 max-w-md mx-auto text-center">
          <h1 className="text-[20px] font-black mb-2">Personal trainer not found</h1>
          <Link href="/fitness" className="rounded-full bg-brand text-bg px-6 py-3 text-[13px] font-extrabold inline-block">Back to marketplace</Link>
        </div>
      </Shell>
    )
  }
    ...
    return (
      <Shell>
        <DraftPasswordGate
          ...
          fetchUrl={(pw) => `/api/fitness/${encodeURIComponent(slug)}/public?p=${encodeURIComponent(pw)}`}
          onUnlocked={(pw, prov) => {
            setDraftPassword(pw)
            setLocked(false)
            setP(prov as FitnessProviderPublic)
          }}
        />
      </Shell>
    )
  ...
  const profileUrl = `${siteOrigin}/fitness/${p.slug}`

  // WhatsApp prefill text for the under-carousel contact button.
  const waText = [
    `Halo ${p.display_name}, saya menemukan profile Anda di Kita2u.`,
    `Halo, saya tertarik personal training — boleh konsultasi goal dan harga paket?`,
  ...
  return (
    <Shell>
      ...
      <div className="relative pb-2">
        ...
        >
          <img
            src={bannerSrc(p.cover_image_url) || DEFAULT_FITNESS_HERO}
            alt=""
          ...
          {(() => {
            ...
            const line1   = ht.line1   || 'Personal Trainer'
            const line2   = ht.line2   || 'Personal Trainer'
            const tagline = ht.tagline || 'Strength · HIIT · Fat Loss · Muscle Gain'
            ...
            return (
              <div className={`absolute left-4 z-10 select-none leading-none cr-hero-${effect}`} style={{ top: 31 }}>
                ...
                {/* Service locations the trainer delivers to — Home and
                    Hotel are the everyday session locations for indie
                    personal trainers (home gym for clients, hotel gym
                    for travel-friendly bookings). A missing or null
                    service_locations array still hides the whole row to
                    avoid implying locations the trainer doesn't serve. */}
                {(() => {
        ...
        <div className="px-4 relative z-20" style={{ marginTop: 12 }}>
          ...
          >
            ...
            {/* Top Rated Seller badge — neutral gray pill so the chip
                ...
                trainer shows their own accent. */}
            <div
      ...
      <div className="px-4 pb-6 max-w-2xl mx-auto space-y-3 pt-3">
        ...
        {/* Training Types Provided — only renders specialties that
        ...
        {(() => {
          const offered = (p.specialties ?? []) as FitnessSpecialty[]
        ...
        {/* Training Snapshots — rich cards (image + name + 2-line desc
            + start price + View Details). Cards swipe left-to-right.
            Personal trainers sell on action shots and before/after
            transformations (clients pick the coach whose style fits
            them), so the header reads "Training Snapshots". */}
        {(() => {
          ...
          return (
            <section className="space-y-2">
              <div className="flex items-center justify-between gap-2">
                <h2 className="text-[13px] font-extrabold uppercase tracking-wider text-black">
                  {activeService
                    ? `${SPECIALTY_LABELS[activeService]} — Training Snapshots`
                    : 'Training Snapshots'}
                </h2>
                <PortfolioViewToggle
                  view={portfolioView}
              ...
              <p className="text-[11px] text-gray-500 italic -mt-1">
                Please contact for custom program atau kategori belum tercantum
              </p>
              <PortfolioCarousel
                photos={photos}
        ...
        <RunningMarquee
          text={p.promo_text || 'Hasilnya datang dari konsistensi — kami yang jaga. 10-pack diskon 10%, free body assessment di sesi pertama, monthly coaching include meal plan + WA 24/7. Konsultasi via WhatsApp.'}
        />
      ...
      {/* Right-edge "back" bar — tall vertical strip flush against the
          ...
          vertical "BACK" text below. Diverts back to /fitness. */}
      <Link
        href="/fitness"
        aria-label="Back to Kita2u personal trainers"
      ...
      {/* Training Snapshot "View Details" popup — full image + sample
      ...
      {/* Contact / booking popup — opened by both the bottom Contact
          ...
          request server-side first (so it shows up on the personal
          trainer's calendar) and then opens WhatsApp with a matching
          pre-filled message. Skips busy_dates the personal trainer
          has marked. */}
      {contactOpen && p.whatsapp_e164 && (
        <ContactBookingPopup
          ...
          providerId={p.id}
          intentVertical="fitness"
          intentSource="fitness_profile"
          themeColor={theme}
          ...
          busyDates={(p.busy_dates ?? []) as string[]}
          bookEndpoint={`/api/fitness/${p.slug}/book`}
      ...
      <PoweredByKita2u defaultVertical="fitness" />
    </Shell>
  )
}
...
}: { sid: FitnessSpecialty; active: boolean; onClick: () => void; theme: string }) {
  return (
    <button
      type="button"
      onClick={onClick}
...
// Cheapest price across drop-in + monthly bundle + per-photo prices.
// Falls back to a sensible default when nothing is set so the CTA
// never reads empty. Fitness floor is drop-in single session ~Rp 200k
// baseline (per-session; even the cheapest indie PT in Indonesia
// doesn't sell 60-minute slots below Rp 200k in 2026).
function formatStartFromPrice(p: FitnessProviderPublic): string {
  ...
  if (candidates.length === 0) return 'Rp 200k'
  return formatPriceIdr(Math.min(...candidates)) ?? 'Rp 200k'
}

// Normalises one service_photos entry — fitness uses a flat array of
...
// Combines personal trainer photos into the carousel feed. When a type
// filter is active, partial-matches the photo's `name` against the
// type label (case-insensitive) so the chip filter still works on
...
function buildPortfolioPhotos(
  p: FitnessProviderPublic,
  active: FitnessSpecialty | null,
): PortfolioPhoto[] {
  ...
  // Partial-match against the type label — works when the personal
  // trainer named their photos sensibly. If the filter yields nothing,
  // fall back to all photos so the carousel doesn't collapse to empty
  // when chip data and photo data diverge.
