...
import {
  ...
  type FitnessProviderPublic, type FitnessSpecialty,
} from '@/lib/fitness/types'

// ============================================================================
// /fitness — Marketplace ported from the shared florist/cake/catering shell.
...
// Mirrors /florist + /cake + /catering + /photo + /barber + /handyman + /tattoo
...
function effectiveAvailability(p: FitnessProviderPublic): 'online' | 'busy' | 'offline' {
...
type ChipDef = { id: 'all' | FitnessSpecialty; label: string }
...
const PRIMARY_IDS = new Set<FitnessSpecialty>(['strength','hiit','weight_loss','muscle_gain','yoga'])
const SECONDARY_IDS: FitnessSpecialty[] = ALL_SPECIALTIES.filter((s) => !PRIMARY_IDS.has(s))

export default function FitnessMarketplacePage() {
  return (
...
function MarketplaceInner() {
  ...
  const t       = useTranslations('verticals.fitness')
  const tCommon = useTranslations('verticals.common')
  const haptic    = useHaptic()
  const search    = useSearchParams()
  const cityLabel = search?.get('city')?.trim() || 'Yogyakarta'

  const [providers, setProviders] = useState<FitnessProviderPublic[]>([])
  const [query, setQuery]         = useState('')
  const [specialty, setSpecialty] = useState<FitnessSpecialty | null>(null)
  ...
  const load = useCallback(async () => {
    ...
    const r = await fetch(`/api/fitness/marketplace${qs.toString() ? `?${qs}` : ''}`, { cache: 'no-store' })
    const j = await r.json() as { providers: FitnessProviderPublic[] }
          ...
          <div className="flex flex-col p-4 sm:p-5">
            ...
            <div className="mt-3 relative shrink-0">
              <input
                ...
                placeholder="Cari coach — nama, kota, training type…"
            ...
            <div
              ...
              role="tablist"
              aria-label="Filter personal trainers by category"
            >
                ...
                <div className="py-10 text-center">
                  <p className="text-[13px] font-bold text-[#71717A] leading-snug">
                    {query.trim()
                      ...
                      : 'No personal trainers in this category yet. Try a different filter.'}
                  </p>
                </div>
              )}
            ...
            <p className="mt-3 text-center text-[12px] text-[#52525B] font-bold leading-snug px-2 shrink-0">
              Results that stick — because the coach keeps showing up. Book direct via WhatsApp.
              Kita2u takes no commission, you pay your coach directly.
            </p>
          </div>
        </div>
      </div>
...
// ─────────────────────────────────────────────────────────────────────────────
// FitnessRowCard — landscape card, 112px tall to match /florist + /cake.
// 88px portrait/headshot left, name + primary category + today's hours middle,
// online dot + credibility pills (drop-in price / monthly bundle / years) below,
// rating + chevron right.
// ─────────────────────────────────────────────────────────────────────────────
function FitnessRowCard({
  provider: p,
  onOpen,
}: {
  provider: FitnessProviderPublic
  onOpen: () => void
}) {
  ...
  // Credibility pills — favour drop-in starting price (most
  // useful for the PT industry) then monthly coaching rate then
  // years of experience.
  const pills: Array<{ key: string; label: string }> = []
  if (p.hourly_rate_idr != null) {
    pills.push({ key: 'pkg', label: `Rp ${p.hourly_rate_idr.toLocaleString('id-ID')}/sesi` })
  }
  if (p.day_rate_idr != null && pills.length < 2) {
    pills.push({ key: 'day', label: `Rp ${p.day_rate_idr.toLocaleString('id-ID')}/bulan` })
  }
...
// ─────────────────────────────────────────────────────────────────────────────
// Shell — wordmark header matching /florist + /cake + /catering.
// ─────────────────────────────────────────────────────────────────────────────
