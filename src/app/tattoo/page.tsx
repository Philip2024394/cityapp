...
import {
  ...
  type TattooProviderPublic, type TattooSpecialty,
} from '@/lib/tattoo/types'

// ============================================================================
// /tattoo — Marketplace ported from the shared handyman shell.
...
// Mirrors /handyman exactly so all verticals feel like one product.
// ============================================================================
...
function effectiveAvailability(p: TattooProviderPublic): 'online' | 'busy' | 'offline' {
...
type ChipDef = { id: 'all' | TattooSpecialty; label: string }
...
const PRIMARY_IDS = new Set<TattooSpecialty>(['fineline','blackwork','traditional','realism','cover_up'])
const SECONDARY_IDS: TattooSpecialty[] = ALL_SPECIALTIES.filter((s) => !PRIMARY_IDS.has(s))

export default function TattooMarketplacePage() {
  return (
...
function MarketplaceInner() {
  ...
  const t       = useTranslations('verticals.tattoo')
  const tCommon = useTranslations('verticals.common')
  const haptic    = useHaptic()
  const search    = useSearchParams()
  const cityLabel = search?.get('city')?.trim() || 'Yogyakarta'

  const [providers, setProviders] = useState<TattooProviderPublic[]>([])
  const [query, setQuery]         = useState('')
  const [specialty, setSpecialty] = useState<TattooSpecialty | null>(null)
  ...
  const load = useCallback(async () => {
    ...
    const r = await fetch(`/api/tattoo/marketplace${qs.toString() ? `?${qs}` : ''}`, { cache: 'no-store' })
    const j = await r.json() as { providers: TattooProviderPublic[] }
          ...
          <div className="flex flex-col p-4 sm:p-5">
            ...
            <div className="mt-3 relative shrink-0">
              <input
                ...
                placeholder="Search tattoo artists — name, studio, style…"
            ...
            <div
              ...
              role="tablist"
              aria-label="Filter tattoo artists by style"
            >
                ...
                <div className="py-10 text-center">
                  <p className="text-[13px] font-bold text-[#71717A] leading-snug">
                    {query.trim()
                      ...
                      : 'No tattoo artists in this style yet. Try a different filter.'}
                  </p>
                </div>
              )}
            ...
            <p className="mt-3 text-center text-[12px] text-[#52525B] font-bold leading-snug px-2 shrink-0">
              Book direct via WhatsApp · Consultation + deposit handled
              by the artist — Kita2u takes no commission.
            </p>
          </div>
        </div>
      </div>
...
// ─────────────────────────────────────────────────────────────────────────────
// TattooRowCard — landscape card, 112px tall to match /handyman.
// 88px artist photo left, name + primary style + today's hours middle,
// online dot + credibility pills (years / hourly rate / day rate) below,
// rating + chevron right.
// ─────────────────────────────────────────────────────────────────────────────
function TattooRowCard({
  provider: p,
  onOpen,
}: {
  provider: TattooProviderPublic
  onOpen: () => void
}) {
  ...
  // Credibility pills — favour hourly rate (most common quoted unit
  // in tattoo industry) then day-session then years of experience.
...
// ─────────────────────────────────────────────────────────────────────────────
// Shell — wordmark header matching /handyman + /beautician + /places.
// ─────────────────────────────────────────────────────────────────────────────
