...
// Server-side SEO + lapsed redirect for /tattoo/[slug].
// Mirrors /r/[slug] / /massage/[slug] / /beautician/[slug] / /handyman/[slug].
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  if (!p) return { title: 'Tattoo artist not found', robots: { index: false, follow: false } }
  const hourly = p.hourly_rate_idr ?? null
  const title       = `${p.display_name} · Tattoo Artist ${p.city || 'Indonesia'}`
  const description = (p.bio?.trim())
    || `Book ${p.display_name} (tattoo artist) in ${p.city || 'Indonesia'} via WhatsApp.${hourly ? ` From Rp ${hourly.toLocaleString('id-ID')}/hour.` : ''}`
  const canonical   = `${SITE_URL}/tattoo/${p.slug}`
...
export default async function TattooProviderLayout({
  children, params,
  ...
  if (p) {
    ...
    if (lapsed) {
      const qs = new URLSearchParams({ from: 'lapsed_tattoo', slug: p.slug })
      redirect(`/tattoo?${qs.toString()}`)
    }
  }
  const jsonLd = p
    ? {
        '@context': 'https://schema.org', '@type': 'LocalBusiness',
        '@id': `${SITE_URL}/tattoo/${p.slug}`,
        name: p.display_name, description: p.bio || undefined,
        url: `${SITE_URL}/tattoo/${p.slug}`,
