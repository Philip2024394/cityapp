...
// Default theme accent — used when the tattoo artist hasn't picked their own
// theme_color (mig 0087). Tattoo Artist choose their accent from the dashboard
...
// /tattoo/[slug] — universal profile flagship build. Mirrors the
// /beautician/[slug] page byte-for-byte at the JSX layer; only the data
// source swaps from beautician_providers to tattoo_providers and a
...
export default function TattooProviderPage() {
  ...
  const [activeService, setActiveService] = useState<TattooSpecialty | null>(null)
  ...
  // available when the tattoo artist has opted into a physical location).
  ...
  useEffect(() => {
    ...
    const url = `/api/tattoo/${encodeURIComponent(slug)}/public${
      draftPassword ? `?p=${encodeURIComponent(draftPassword)}` : ''
    }`
    fetch(url, { cache: 'no-store' })
      .then((r) => r.ok ? r.json() : null)
      .then((j: { provider?: TattooProviderPublic; locked?: boolean } | null) => {
  ...
  useProfileViewTracker({ providerType: 'tattoo', providerId: p?.id })
  ...
  useEffect(() => {
    ...
    fetch(`/api/reviews?provider_type=tattoo&provider_id=${encodeURIComponent(p.id)}`, { cache: 'no-store' })
      .then((r) => r.ok ? r.json() : null)
    ...
    return (
      <Shell>
        <div className="px-4 pt-20 max-w-md mx-auto text-center">
          <h1 className="text-[20px] font-black mb-2">Tattoo artist not found</h1>
          <Link href="/tattoo" className="rounded-full bg-brand text-bg px-6 py-3 text-[13px] font-extrabold inline-block">Back to marketplace</Link>
        </div>
      </Shell>
    )
  }
    ...
    return (
      <Shell>
        <DraftPasswordGate
          ...
          fetchUrl={(pw) => `/api/tattoo/${encodeURIComponent(slug)}/public?p=${encodeURIComponent(pw)}`}
          onUnlocked={(pw, prov) => {
            setDraftPassword(pw)
            setLocked(false)
            setP(prov as TattooProviderPublic)
          }}
        />
      </Shell>
    )
  ...
  return (
    <Shell>
      ...
      <div className="relative pb-2">
        ...
        >
          <img
            src={bannerSrc(p.cover_image_url) || DEFAULT_TATTOO_HERO}
            alt=""
          ...
          {(() => {
            ...
            const line2   = ht.line2   || 'Tattoo Artist'
            const tagline = ht.tagline || 'Custom designs · clean studio · book a consult'
            ...
            return (
              <div className={`absolute left-4 z-10 select-none leading-none cr-hero-${effect}`} style={{ top: 31 }}>
                ...
                {/* Service locations the tattoo artist travels to — for the
                    tattoo vertical only Home and Hotel are sensible
                    (some artists do home / villa visits — studio still
                    ...
                    tattoo artist doesn't actually serve. */}
                {(() => {
        ...
        <div className="px-4 relative z-20" style={{ marginTop: 12 }}>
          ...
          >
            ...
            {/* Top Rated Seller badge — neutral gray pill so the chip
                ...
                text tint the active profile theme color so each tattoo artist
      ...
      <div className="px-4 pb-6 max-w-2xl mx-auto space-y-3 pt-3">
        ...
        {(() => {
          const offered = (p.specialties ?? []) as TattooSpecialty[]
        ...
        <RunningMarquee
          text={p.promo_text || 'Message me this week — clean studio, custom design, single-use needles. Book your consult on WhatsApp.'}
        />
      ...
      {/* Right-edge "back" bar — tall vertical strip flush against the
          ...
          vertical "BACK" text below. Diverts back to /tattoo. */}
      <Link
        href="/tattoo"
        aria-label="Back to Kita2u tattoo artist"
      ...
      {/* Contact / booking popup — opened by both the bottom Contact
          ...
          request server-side first (so it shows up on the tattoo artist's
          calendar) and then opens WhatsApp with a matching pre-filled
          message. Skips busy_dates the tattoo artist has marked. */}
      {contactOpen && p.whatsapp_e164 && (
        <ContactBookingPopup
          ...
          providerId={p.id}
          intentVertical="tattoo"
          intentSource="tattoo_profile"
          themeColor={theme}
          ...
          busyDates={(p.busy_dates ?? []) as string[]}
          bookEndpoint={`/api/tattoo/${p.slug}/book`}
      ...
      <PoweredByKita2u defaultVertical="tattoo" />
    </Shell>
  )
}
...
}: { sid: TattooSpecialty; active: boolean; onClick: () => void; theme: string }) {
  return (
    <button
      type="button"
      onClick={onClick}
...
// reads empty.
function formatStartFromPrice(p: TattooProviderPublic): string {
...
// Normalises one service_photos entry — tattoo uses a flat array of
...
// Combines tattoo artist photos into the carousel feed. When a specialty
...
function buildPortfolioPhotos(
  p: TattooProviderPublic,
  active: TattooSpecialty | null,
): PortfolioPhoto[] {
  ...
  // Partial-match against the specialty label — works when the tattoo artist
