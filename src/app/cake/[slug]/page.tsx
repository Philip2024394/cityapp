...
// Default theme accent — used when the cake & bakery business hasn't picked their
// own theme_color (mig 0087). Bakery businesses choose their accent from the
// dashboard color palette; the chosen hex flows through every accent
// surface on this page via the `theme` constant below. Default soft
// pink (#F472B6) so the pastel buttercream / sprinkle imagery pops with
// bakery-tuned signage.
const DEFAULT_THEME = '#F472B6'
...
// /cake/[slug] — universal profile flagship build. Mirrors the
// /catering/[slug] page byte-for-byte at the JSX layer; only the data
// source swaps from catering_providers to cake_providers and a few
// vertical-specific adaptations apply (genre chips, per-cake + paket
// bundle pricing block, flat service_photos array with partial-name
// filter).

// Vertical default for the hero. Used when the cake & bakery business hasn't set
// their own cover_image_url yet — keeps the page on-brand instead of
// showing the generic yellow gradient on every new signup.
const DEFAULT_CAKE_HERO =
  'https://images.unsplash.com/photo-1535254973040-607b474cb50d?w=1200&auto=format&fit=crop'

export default function CakeProviderPage() {
  const params = useParams<{ slug: string }>()
  const slug = String(params?.slug || '').toLowerCase()
  const [p, setP] = useState<CakeProviderPublic | null>(null)
  ...
  const [activeService, setActiveService] = useState<CakeSpecialty | null>(null)
  ...
  // available when the cake & bakery business has opted into a physical studio).
  ...
  useEffect(() => {
    ...
    const url = `/api/cake/${encodeURIComponent(slug)}/public${
      draftPassword ? `?p=${encodeURIComponent(draftPassword)}` : ''
    }`
    fetch(url, { cache: 'no-store' })
      .then((r) => r.ok ? r.json() : null)
      .then((j: { provider?: CakeProviderPublic; locked?: boolean } | null) => {
  ...
  useProfileViewTracker({ providerType: 'cake', providerId: p?.id })

  // Resolved accent color for every accent surface on this page.
  // p.theme_color (mig 0087) wins; fall back to global default soft pink.
  ...
  useEffect(() => {
    ...
    fetch(`/api/reviews?provider_type=cake&provider_id=${encodeURIComponent(p.id)}`, { cache: 'no-store' })
      .then((r) => r.ok ? r.json() : null)
    ...
    return (
      <Shell>
        <div className="px-4 pt-20 max-w-md mx-auto text-center">
          <h1 className="text-[20px] font-black mb-2">Cake & bakery business not found</h1>
          <Link href="/cake" className="rounded-full bg-brand text-bg px-6 py-3 text-[13px] font-extrabold inline-block">Back to marketplace</Link>
        </div>
      </Shell>
    )
  }
    ...
    return (
      <Shell>
        <DraftPasswordGate
          ...
          fetchUrl={(pw) => `/api/cake/${encodeURIComponent(slug)}/public?p=${encodeURIComponent(pw)}`}
          onUnlocked={(pw, prov) => {
            setDraftPassword(pw)
            setLocked(false)
            setP(prov as CakeProviderPublic)
          }}
        />
      </Shell>
    )
  ...
  return (
    <Shell>
      ...
      <div className="relative pb-2">
        ...
        >
          <img
            src={bannerSrc(p.cover_image_url) || DEFAULT_CAKE_HERO}
            alt=""
          ...
          {(() => {
            ...
            const line1   = ht.line1   || 'Kue & Bakery'
            const line2   = ht.line2   || 'Cake & bakery business'
            const tagline = ht.tagline || 'Birthday · Wedding · Korean Bento · Dessert Table'
            ...
            return (
              <div className={`absolute left-4 z-10 select-none leading-none cr-hero-${effect}`} style={{ top: 31 }}>
                ...
                {/* Service locations the bakery delivers to — Home and
                    Hotel are the everyday delivery pairs for indie cake
        ...
        <div className="px-4 relative z-20" style={{ marginTop: 12 }}>
          ...
          >
            ...
            {/* Top Rated Seller badge — neutral gray pill so the chip
                ...
                cake & bakery business shows their own accent. */}
            <div
      ...
      <div className="px-4 pb-6 max-w-2xl mx-auto space-y-3 pt-3">
        ...
        {(() => {
          const offered = (p.specialties ?? []) as CakeSpecialty[]
        ...
        {/* Portfolio carousel — rich cards (image + name + 2-line desc
            ...
            Cake studios sell on finished cake photos (clients pick by
            picture), so the header reads "Cake Gallery". */}
        {(() => {
          ...
          return (
            <section className="space-y-2">
              <div className="flex items-center justify-between gap-2">
                <h2 className="text-[13px] font-extrabold uppercase tracking-wider text-black">
                  {activeService
                    ? `${SPECIALTY_LABELS[activeService]} — Cake Gallery`
                    : 'Cake Gallery'}
                </h2>
                <PortfolioViewToggle
                  view={portfolioView}
              ...
              <p className="text-[11px] text-gray-500 italic -mt-1">
                Please contact for custom design or kategori kue belum tercantum
              </p>
              <PortfolioCarousel
                photos={photos}
        ...
        <RunningMarquee
          text={p.promo_text || 'Kue ulang tahun custom yang bikin foto-foto cantik — pesan H-3 untuk birthday, H-2 untuk Korean bento. Konsultasi via WhatsApp.'}
        />
      ...
      {/* Right-edge "back" bar — tall vertical strip flush against the
          ...
          vertical "BACK" text below. Diverts back to /cake. */}
      <Link
        href="/cake"
        aria-label="Back to Kita2u cake"
      ...
      {/* Contact / booking popup — opened by both the bottom Contact
          ...
          request server-side first (so it shows up on the cake & bakery
          business's calendar) and then opens WhatsApp with a matching
          pre-filled message. Skips busy_dates the cake & bakery business
          has marked. */}
      {contactOpen && p.whatsapp_e164 && (
        <ContactBookingPopup
          ...
          busyDates={(p.busy_dates ?? []) as string[]}
          bookEndpoint={`/api/cake/${p.slug}/book`}
      ...
      <PoweredByKita2u defaultVertical="cake" />
    </Shell>
  )
}
...
}: { sid: CakeSpecialty; active: boolean; onClick: () => void; theme: string }) {
  return (
    <button
      type="button"
      onClick={onClick}
...
// Cheapest price across per-cake + paket bundle + per-photo prices.
// Falls back to a sensible default when nothing is set so the CTA
// never reads empty. Cake floor is Korean bento mini or cookies-box
// minimum ~Rp 95k baseline (per-unit; cupcakes go below that but
// minimum-order constraint keeps real bookings above Rp 95k).
function formatStartFromPrice(p: CakeProviderPublic): string {
  ...
  if (candidates.length === 0) return 'Rp 95k'
  return formatPriceIdr(Math.min(...candidates)) ?? 'Rp 95k'
}

// Normalises one service_photos entry — cake uses a flat array of
...
// Combines cake & bakery business photos into the carousel feed. When a genre
...
function buildPortfolioPhotos(
  p: CakeProviderPublic,
  active: CakeSpecialty | null,
): PortfolioPhoto[] {
  ...
  // Partial-match against the genre label — works when the bakery
