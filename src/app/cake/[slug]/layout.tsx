...
// Server-side SEO + lapsed redirect for /cake/[slug].
// Mirrors /catering/[slug] / /photo/[slug] / /barber/[slug] / etc.
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  if (!p) return { title: 'Cake & bakery business not found', robots: { index: false, follow: false } }
  const pkg = p.hourly_rate_idr ?? null
  const title       = `${p.display_name} · Cake & bakery business ${p.city || 'Indonesia'}`
  const description = (p.bio?.trim())
    || `Book ${p.display_name} (cake & bakery business) in ${p.city || 'Indonesia'} via WhatsApp.${pkg ? ` Cakes from Rp ${pkg.toLocaleString('id-ID')}.` : ''}`
  const canonical   = `${SITE_URL}/cake/${p.slug}`
...
export default async function CakeProviderLayout({
  children, params,
  ...
  if (p) {
    ...
    if (lapsed) {
      const qs = new URLSearchParams({ from: 'lapsed_cake', slug: p.slug })
      redirect(`/cake?${qs.toString()}`)
    }
  }
  const jsonLd = p
    ? {
        '@context': 'https://schema.org', '@type': 'LocalBusiness',
        '@id': `${SITE_URL}/cake/${p.slug}`,
        name: p.display_name, description: p.bio || undefined,
        url: `${SITE_URL}/cake/${p.slug}`,
