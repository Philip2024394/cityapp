...
import {
  ...
  type CakeProviderPublic, type CakeSpecialty,
} from '@/lib/cake/types'

// ============================================================================
// /cake — Marketplace ported from the shared catering/photo/barber/handyman shell.
...
// Mirrors /catering + /photo + /barber + /handyman + /tattoo exactly so all
// verticals feel like one product.
// ============================================================================
...
function effectiveAvailability(p: CakeProviderPublic): 'online' | 'busy' | 'offline' {
...
type ChipDef = { id: 'all' | CakeSpecialty; label: string }
const PRIMARY_CHIPS: ChipDef[] = [
  { id: 'all',             label: 'All'           },
  { id: 'birthday',        label: 'Birthday'      },
  { id: 'korean_bento',    label: 'Korean Bento'  },
  { id: 'dessert_table',   label: 'Dessert Table' },
  { id: 'cookies',         label: 'Cookies'       },
  { id: 'halal',           label: 'Halal'         },
]
const PRIMARY_IDS = new Set<CakeSpecialty>(['birthday','korean_bento','dessert_table','cookies','halal'])
const SECONDARY_IDS: CakeSpecialty[] = ALL_SPECIALTIES.filter((s) => !PRIMARY_IDS.has(s))

export default function CakeMarketplacePage() {
  return (
...
function MarketplaceInner() {
  ...
  const t       = useTranslations('verticals.cake')
  const tCommon = useTranslations('verticals.common')
  const haptic    = useHaptic()
  const search    = useSearchParams()
  const cityLabel = search?.get('city')?.trim() || 'Yogyakarta'

  const [providers, setProviders] = useState<CakeProviderPublic[]>([])
  const [query, setQuery]         = useState('')
  const [specialty, setSpecialty] = useState<CakeSpecialty | null>(null)
  ...
  const load = useCallback(async () => {
    ...
    const r = await fetch(`/api/cake/marketplace${qs.toString() ? `?${qs}` : ''}`, { cache: 'no-store' })
    const j = await r.json() as { providers: CakeProviderPublic[] }
          ...
          <div className="flex flex-col p-4 sm:p-5">
            ...
            <div className="mt-3 relative shrink-0">
              <input
                ...
                placeholder="Cari cake — nama, kota, jenis kue…"
            ...
            <div
              ...
              role="tablist"
              aria-label="Filter cake & bakery businesses by category"
            >
                ...
                <div className="py-10 text-center">
                  <p className="text-[13px] font-bold text-[#71717A] leading-snug">
                    {query.trim()
                      ...
                      : 'No cake & bakery businesses in this category yet. Try a different filter.'}
                  </p>
                </div>
              )}
            ...
            <p className="mt-3 text-center text-[12px] text-[#52525B] font-bold leading-snug px-2 shrink-0">
              Custom cakes that earn the photo opportunity — book direct via WhatsApp.
              Kita2u takes no commission, you pay your cake & bakery business directly.
            </p>
          </div>
        </div>
      </div>
...
// ─────────────────────────────────────────────────────────────────────────────
// CakeRowCard — landscape card, 112px tall to match /catering + /photo + /barber + /handyman.
// 88px portrait/headshot left, name + primary category + today's hours middle,
// online dot + credibility pills (per-cake price / bundle / years) below,
// rating + chevron right.
// ─────────────────────────────────────────────────────────────────────────────
function CakeRowCard({
  provider: p,
  onOpen,
}: {
  provider: CakeProviderPublic
  onOpen: () => void
}) {
  ...
  // Credibility pills — favour per-cake starting price (most useful for
  // the cake industry) then bundle rate then years of experience.
  const pills: Array<{ key: string; label: string }> = []
  if (p.hourly_rate_idr != null) {
    pills.push({ key: 'pkg', label: `Rp ${p.hourly_rate_idr.toLocaleString('id-ID')}/cake` })
  }
  if (p.day_rate_idr != null && pills.length < 2) {
    pills.push({ key: 'day', label: `Rp ${p.day_rate_idr.toLocaleString('id-ID')}/paket` })
  }
...
// ─────────────────────────────────────────────────────────────────────────────
// Shell — wordmark header matching /catering + /photo + /barber + /handyman + /beautician + /places + /tattoo.
// ─────────────────────────────────────────────────────────────────────────────
