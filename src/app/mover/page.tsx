...
import {
  ...
  type MoverProviderPublic, type MoverSpecialty,
} from '@/lib/mover/types'

// ============================================================================
// /mover — Marketplace ported from the shared florist/cake/catering shell.
...
// Mirrors /florist + /cake + /catering + /photo + /barber + /handyman + /tattoo + /pet
...
function effectiveAvailability(p: MoverProviderPublic): 'online' | 'busy' | 'offline' {
...
type ChipDef = { id: 'all' | MoverSpecialty; label: string }
const PRIMARY_CHIPS: ChipDef[] = [
  { id: 'all',               label: 'All'           },
  { id: 'grandmax',          label: 'Grandmax'      },
  { id: 'pickup',            label: 'Pickup'        },
  { id: 'box',               label: 'Box CDD'       },
  { id: 'home_full',         label: 'Rumah Penuh'   },
  { id: 'office_relocation', label: 'Kantor'        },
  { id: 'single_furniture',  label: 'Single'        },
]
const PRIMARY_IDS = new Set<MoverSpecialty>(['grandmax','pickup','box','home_full','office_relocation','single_furniture'])
const SECONDARY_IDS: MoverSpecialty[] = ALL_SPECIALTIES.filter((s) => !PRIMARY_IDS.has(s))

export default function MoverMarketplacePage() {
  return (
...
function MarketplaceInner() {
  ...
  const t       = useTranslations('verticals.mover')
  const tCommon = useTranslations('verticals.common')
  const haptic    = useHaptic()
  const search    = useSearchParams()
  const cityLabel = search?.get('city')?.trim() || 'Yogyakarta'

  const [providers, setProviders] = useState<MoverProviderPublic[]>([])
  const [query, setQuery]         = useState('')
  const [specialty, setSpecialty] = useState<MoverSpecialty | null>(null)
  ...
  const load = useCallback(async () => {
    ...
    const r = await fetch(`/api/mover/marketplace${qs.toString() ? `?${qs}` : ''}`, { cache: 'no-store' })
    const j = await r.json() as { providers: MoverProviderPublic[] }
          ...
          <div className="flex flex-col p-4 sm:p-5">
            ...
            <div className="mt-3 relative shrink-0">
              <input
                ...
                placeholder="Cari jasa pindahan — nama, kota, armada…"
            ...
            <div
              ...
              role="tablist"
              aria-label="Filter mover by vehicle"
            >
                ...
                <div className="py-10 text-center">
                  <p className="text-[13px] font-bold text-[#71717A] leading-snug">
                    {query.trim()
                      ...
                      : 'No movers in this category yet. Try a different filter.'}
                  </p>
                </div>
              )}
            ...
            <p className="mt-3 text-center text-[12px] text-[#52525B] font-bold leading-snug px-2 shrink-0">
              Pindah tanpa drama. Kami yang angkat. Book direct via WhatsApp.
              Kita2u takes no commission, you pay your mover directly.
            </p>
          </div>
        </div>
      </div>
...
// ─────────────────────────────────────────────────────────────────────────────
// MoverRowCard — landscape card, 112px tall to match /florist + /cake + /pet.
...
function MoverRowCard({
  ...
  // the mover industry) then full-house package then years of experience.
...
// ─────────────────────────────────────────────────────────────────────────────
// Shell — wordmark header matching /florist + /cake + /catering + /pet.
// ─────────────────────────────────────────────────────────────────────────────
