...
import {
  ALL_SPECIALTIES,
  SPECIALTY_LABELS,
  type MoverProviderPublic,
  type MoverSpecialty,
} from '@/lib/mover/types'
...
// Default theme accent — used when the mover hasn't picked their own
// theme_color (mig 0087). Mover businesses choose their accent from the
...
// /mover/[slug] — universal profile flagship build. Mirrors the
// /pet/[slug] page byte-for-byte at the JSX layer; only the data
// source swaps from pet_providers to mover_providers and a few
...
export default function MoverProviderPage() {
  ...
  const [activeService, setActiveService] = useState<MoverSpecialty | null>(null)
  ...
  // available when the mover has opted into a physical pool / depo).
  ...
  useEffect(() => {
    ...
    const url = `/api/mover/${encodeURIComponent(slug)}/public${
      draftPassword ? `?p=${encodeURIComponent(draftPassword)}` : ''
    }`
    fetch(url, { cache: 'no-store' })
      .then((r) => r.ok ? r.json() : null)
      .then((j: { provider?: MoverProviderPublic; locked?: boolean } | null) => {
  ...
  useProfileViewTracker({ providerType: 'mover', providerId: p?.id })
  ...
  useEffect(() => {
    ...
    fetch(`/api/reviews?provider_type=mover&provider_id=${encodeURIComponent(p.id)}`, { cache: 'no-store' })
      .then((r) => r.ok ? r.json() : null)
    ...
    return (
      <Shell>
        <div className="px-4 pt-20 max-w-md mx-auto text-center">
          <h1 className="text-[20px] font-black mb-2">Mover not found</h1>
          <Link href="/mover" className="rounded-full bg-brand text-bg px-6 py-3 text-[13px] font-extrabold inline-block">Back to marketplace</Link>
        </div>
      </Shell>
    )
  }
    ...
    return (
      <Shell>
        <DraftPasswordGate
          ...
          fetchUrl={(pw) => `/api/mover/${encodeURIComponent(slug)}/public?p=${encodeURIComponent(pw)}`}
          onUnlocked={(pw, prov) => {
            setDraftPassword(pw)
            setLocked(false)
            setP(prov as MoverProviderPublic)
          }}
        />
      </Shell>
    )
  ...
  return (
    <Shell>
      ...
      <div className="relative pb-2">
        ...
        >
          <img
            src={bannerSrc(p.cover_image_url) || DEFAULT_MOVER_HERO}
            alt=""
          ...
          {(() => {
            ...
            const line1   = ht.line1   || 'Jasa Pindahan'
            const line2   = ht.line2   || 'Grandmax · Pickup · Box · Antar Kota'
            const tagline = ht.tagline || 'Pindah tanpa drama. Kami yang angkat.'
            ...
            return (
              <div className={`absolute left-4 z-10 select-none leading-none cr-hero-${effect}`} style={{ top: 31 }}>
                ...
                {/* Service locations the mover delivers to — Home and
                    Hotel are the everyday job locations for indie movers
                    (home pindahan rumah, hotel relocation for travel-
                    friendly bookings). A missing or null service_locations
                    array still hides the whole row to avoid implying
                    locations the mover doesn't serve. */}
                {(() => {
      ...
      <div className="px-4 pb-6 max-w-2xl mx-auto space-y-3 pt-3">
        ...
        {(() => {
          const offered = (p.specialties ?? []) as MoverSpecialty[]
        ...
        {/* Armada & Pindahan — rich cards (image + name + 2-line desc
            + start price + View Details). Cards swipe left-to-right.
            Movers sell on vehicle-tier specialisation + jangkauan
            (clients pick the mover whose armada fits their cargo +
            distance), so the header reads "Armada & Pindahan". */}
        {(() => {
          ...
          return (
            <section className="space-y-2">
              <div className="flex items-center justify-between gap-2">
                <h2 className="text-[13px] font-extrabold uppercase tracking-wider text-black">
                  {activeService
                    ? `${SPECIALTY_LABELS[activeService]} — Armada & Pindahan`
                    : 'Armada & Pindahan'}
                </h2>
                <PortfolioViewToggle
                  view={portfolioView}
        ...
        <RunningMarquee
          text={p.promo_text || 'Pindah tanpa drama — kami yang angkat. Grandmax dalam kota Rp 350k. Pickup antar kota dengan packing material bubble wrap. Box CDD pindahan rumah penuh + gratis survey + asuransi all-risk. Reservasi via WhatsApp.'}
        />
      ...
      {/* Right-edge "back" bar — tall vertical strip flush against the
          ...
          vertical "BACK" text below. Diverts back to /mover. */}
      <Link
        href="/mover"
        aria-label="Back to Kita2u mover"
      ...
      {/* Contact / booking popup — opened by both the bottom Contact
          ...
          request server-side first (so it shows up on the mover's
          calendar) and then opens WhatsApp with a matching pre-filled
          message. Skips busy_dates the mover has marked. */}
      {contactOpen && p.whatsapp_e164 && (
        <ContactBookingPopup
          ...
          providerId={p.id}
          intentVertical="mover"
          intentSource="mover_profile"
          themeColor={theme}
          ...
          busyDates={(p.busy_dates ?? []) as string[]}
          bookEndpoint={`/api/mover/${p.slug}/book`}
      ...
      <PoweredByKita2u defaultVertical="mover" />
    </Shell>
  )
}
...
}: { sid: MoverSpecialty; active: boolean; onClick: () => void; theme: string }) {
  return (
    <button
      type="button"
      onClick={onClick}
...
// empty. Mover floor is single-furniture pickup at ~Rp 180k (1 barang
// dalam kota — sofa/kasur/lemari with driver loading help).
function formatStartFromPrice(p: MoverProviderPublic): string {
  ...
  if (candidates.length === 0) return 'Rp 180k'
  return formatPriceIdr(Math.min(...candidates)) ?? 'Rp 180k'
}

// Normalises one service_photos entry — mover uses a flat array of
...
// Combines mover photos into the carousel feed. When a type filter is
...
function buildPortfolioPhotos(
  p: MoverProviderPublic,
  active: MoverSpecialty | null,
): PortfolioPhoto[] {
  ...
  // Partial-match against the type label — works when the mover named
