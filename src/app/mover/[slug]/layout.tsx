...
// Server-side SEO + lapsed redirect for /mover/[slug].
// Mirrors /pet/[slug] / /florist/[slug] / /cake/[slug] / etc.
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  if (!p) return { title: 'Mover not found', robots: { index: false, follow: false } }
  const pkg = p.hourly_rate_idr ?? null
  const title       = `${p.display_name} · Mover ${p.city || 'Indonesia'}`
  const description = (p.bio?.trim())
    || `Book ${p.display_name} (mover) in ${p.city || 'Indonesia'} via WhatsApp.${pkg ? ` Mulai dari Rp ${pkg.toLocaleString('id-ID')}.` : ''}`
  const canonical   = `${SITE_URL}/mover/${p.slug}`
...
export default async function MoverProviderLayout({
  children, params,
  ...
  if (p) {
    ...
    if (lapsed) {
      const qs = new URLSearchParams({ from: 'lapsed_mover', slug: p.slug })
      redirect(`/mover?${qs.toString()}`)
    }
  }
  const jsonLd = p
    ? {
        '@context': 'https://schema.org', '@type': 'LocalBusiness',
        '@id': `${SITE_URL}/mover/${p.slug}`,
        name: p.display_name, description: p.bio || undefined,
        url: `${SITE_URL}/mover/${p.slug}`,
