...
// Default theme accent — used when the barber hasn't picked their own
// theme_color (mig 0087). Barbers choose their accent from the dashboard
// color palette; the chosen hex flows through every accent surface on
// this page via the `theme` constant below. Default blue mirrors the
// classic barber-pole stripe.
const DEFAULT_THEME = '#1E40AF'
...
// /barber/[slug] — universal profile flagship build. Mirrors the
// /tattoo/[slug] page byte-for-byte at the JSX layer; only the data
// source swaps from tattoo_providers to barber_providers and a few
// vertical-specific adaptations apply (specialty chips, per-cut/combo
// pricing block, flat service_photos array with partial-name filter).

// Vertical default for the hero. Used when the barber hasn't set their
// own cover_image_url yet — keeps the page on-brand instead of showing
// the generic yellow gradient on every new signup.
const DEFAULT_BARBER_HERO =
  'https://images.unsplash.com/photo-1503951914875-452162b0f3f1?w=1200&auto=format&fit=crop'

export default function BarberProviderPage() {
  const params = useParams<{ slug: string }>()
  const slug = String(params?.slug || '').toLowerCase()
  const [p, setP] = useState<BarberProviderPublic | null>(null)
  ...
  const [activeService, setActiveService] = useState<BarberSpecialty | null>(null)
  ...
  // available when the barber has opted into a physical location).
  ...
  useEffect(() => {
    ...
    const url = `/api/barber/${encodeURIComponent(slug)}/public${
      draftPassword ? `?p=${encodeURIComponent(draftPassword)}` : ''
    }`
    fetch(url, { cache: 'no-store' })
      .then((r) => r.ok ? r.json() : null)
      .then((j: { provider?: BarberProviderPublic; locked?: boolean } | null) => {
  ...
  useProfileViewTracker({ providerType: 'barber', providerId: p?.id })

  // Resolved accent color for every accent surface on this page.
  // p.theme_color (mig 0087) wins; fall back to global default blue.
  ...
  useEffect(() => {
    ...
    fetch(`/api/reviews?provider_type=barber&provider_id=${encodeURIComponent(p.id)}`, { cache: 'no-store' })
      .then((r) => r.ok ? r.json() : null)
    ...
    return (
      <Shell>
        <div className="px-4 pt-20 max-w-md mx-auto text-center">
          <h1 className="text-[20px] font-black mb-2">Barber not found</h1>
          <Link href="/barber" className="rounded-full bg-brand text-bg px-6 py-3 text-[13px] font-extrabold inline-block">Back to marketplace</Link>
        </div>
      </Shell>
    )
  }
    ...
    return (
      <Shell>
        <DraftPasswordGate
          ...
          fetchUrl={(pw) => `/api/barber/${encodeURIComponent(slug)}/public?p=${encodeURIComponent(pw)}`}
          onUnlocked={(pw, prov) => {
            setDraftPassword(pw)
            setLocked(false)
            setP(prov as BarberProviderPublic)
          }}
        />
      </Shell>
    )
  ...
  return (
    <Shell>
      ...
      <div className="relative pb-2">
        ...
        >
          <img
            src={bannerSrc(p.cover_image_url) || DEFAULT_BARBER_HERO}
            alt=""
          ...
          {(() => {
            ...
            const line2   = ht.line2   || 'Barber'
            const tagline = ht.tagline || 'Classic cut · fade · beard trim · hot towel'
            ...
            return (
              <div className={`absolute left-4 z-10 select-none leading-none cr-hero-${effect}`} style={{ top: 31 }}>
                ...
                {/* Service locations the barber travels to — for the
                    barber vertical only Home and Hotel are sensible
                    (some barbers do home / villa visits — shop still
                    drives the bulk, exposed via the operating hours),
        ...
        <div className="px-4 relative z-20" style={{ marginTop: 12 }}>
          ...
          >
            ...
            {/* Top Rated Seller badge — neutral gray pill so the chip
                ...
                text tint the active profile theme color so each barber
      ...
      <div className="px-4 pb-6 max-w-2xl mx-auto space-y-3 pt-3">
        ...
        {(() => {
          const offered = (p.specialties ?? []) as BarberSpecialty[]
          ...
          return (
            <section className="space-y-2">
              <div className="flex items-center justify-between gap-2">
                <h2 className="text-[13px] font-extrabold uppercase tracking-wider text-black">
                  {activeService
                    ? `${SPECIALTY_LABELS[activeService]} — Before/After`
                    : 'Before/After'}
                </h2>
                <PortfolioViewToggle
                  view={portfolioView}
        ...
        <RunningMarquee
          text={p.promo_text || 'Walk-ins welcome — clean chair, sanitised clippers, hot-towel finish. Book your slot on WhatsApp.'}
        />
      ...
      {/* Right-edge "back" bar — tall vertical strip flush against the
          ...
          vertical "BACK" text below. Diverts back to /barber. */}
      <Link
        href="/barber"
        aria-label="Back to Kita2u barber"
      ...
      {/* Contact / booking popup — opened by both the bottom Contact
          ...
          request server-side first (so it shows up on the barber's
          calendar) and then opens WhatsApp with a matching pre-filled
          message. Skips busy_dates the barber has marked. */}
      {contactOpen && p.whatsapp_e164 && (
        <ContactBookingPopup
          ...
          providerId={p.id}
          intentVertical="barber"
          intentSource="barber_profile"
          themeColor={theme}
          ...
          busyDates={(p.busy_dates ?? []) as string[]}
          bookEndpoint={`/api/barber/${p.slug}/book`}
      ...
      <PoweredByKita2u defaultVertical="barber" />
    </Shell>
  )
}
...
}: { sid: BarberSpecialty; active: boolean; onClick: () => void; theme: string }) {
  return (
    <button
      type="button"
      onClick={onClick}
...
// Cheapest price across per-cut + combo rates + per-photo prices. Falls
// back to a sensible default when nothing is set so the CTA never
// reads empty.
function formatStartFromPrice(p: BarberProviderPublic): string {
...
// Normalises one service_photos entry — barber uses a flat array of
...
// Combines barber photos into the carousel feed. When a specialty
...
function buildPortfolioPhotos(
  p: BarberProviderPublic,
  active: BarberSpecialty | null,
): PortfolioPhoto[] {
  ...
  // Partial-match against the specialty label — works when the barber
