...
// Server-side SEO + lapsed redirect for /barber/[slug].
// Mirrors /r/[slug] / /massage/[slug] / /beautician/[slug] / /handyman/[slug] / /tattoo/[slug].
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  if (!p) return { title: 'Barber not found', robots: { index: false, follow: false } }
  const perCut = p.hourly_rate_idr ?? null
  const title       = `${p.display_name} · Barber ${p.city || 'Indonesia'}`
  const description = (p.bio?.trim())
    || `Book ${p.display_name} (barber) in ${p.city || 'Indonesia'} via WhatsApp.${perCut ? ` From Rp ${perCut.toLocaleString('id-ID')}/cut.` : ''}`
  const canonical   = `${SITE_URL}/barber/${p.slug}`
...
export default async function BarberProviderLayout({
  children, params,
  ...
  if (p) {
    ...
    if (lapsed) {
      const qs = new URLSearchParams({ from: 'lapsed_barber', slug: p.slug })
      redirect(`/barber?${qs.toString()}`)
    }
  }
  const jsonLd = p
    ? {
        '@context': 'https://schema.org', '@type': 'LocalBusiness',
        '@id': `${SITE_URL}/barber/${p.slug}`,
        name: p.display_name, description: p.bio || undefined,
        url: `${SITE_URL}/barber/${p.slug}`,
