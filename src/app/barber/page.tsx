...
import {
  ...
  type BarberProviderPublic, type BarberSpecialty,
} from '@/lib/barber/types'

// ============================================================================
// /barber — Marketplace ported from the shared handyman/tattoo shell.
...
// Mirrors /handyman + /tattoo exactly so all verticals feel like one product.
// ============================================================================
...
function effectiveAvailability(p: BarberProviderPublic): 'online' | 'busy' | 'offline' {
...
type ChipDef = { id: 'all' | BarberSpecialty; label: string }
const PRIMARY_CHIPS: ChipDef[] = [
  { id: 'all',             label: 'All'        },
  { id: 'classic_cut',     label: 'Classic'    },
  { id: 'fade',            label: 'Fade'       },
  { id: 'beard_trim',      label: 'Beard'      },
  { id: 'hot_towel_shave', label: 'Hot Towel'  },
  { id: 'kids_cut',        label: 'Kids'       },
]
const PRIMARY_IDS = new Set<BarberSpecialty>(['classic_cut','fade','beard_trim','hot_towel_shave','kids_cut'])
const SECONDARY_IDS: BarberSpecialty[] = ALL_SPECIALTIES.filter((s) => !PRIMARY_IDS.has(s))
...
function MarketplaceInner() {
  ...
  const t       = useTranslations('verticals.barber')
  const tCommon = useTranslations('verticals.common')
  const haptic    = useHaptic()
  const search    = useSearchParams()
  const cityLabel = search?.get('city')?.trim() || 'Yogyakarta'

  const [providers, setProviders] = useState<BarberProviderPublic[]>([])
  const [query, setQuery]         = useState('')
  const [specialty, setSpecialty] = useState<BarberSpecialty | null>(null)
  ...
  const load = useCallback(async () => {
    ...
    const r = await fetch(`/api/barber/marketplace${qs.toString() ? `?${qs}` : ''}`, { cache: 'no-store' })
    const j = await r.json() as { providers: BarberProviderPublic[] }
          ...
          <div className="flex flex-col p-4 sm:p-5">
            ...
            <div className="mt-3 relative shrink-0">
              <input
                ...
                placeholder="Cari barber — nama, shop, layanan…"
            ...
            <div
              ...
              role="tablist"
              aria-label="Filter barbers by service"
            >
                ...
                <div className="py-10 text-center">
                  <p className="text-[13px] font-bold text-[#71717A] leading-snug">
                    {query.trim()
                      ...
                      : 'No barbers in this service yet. Try a different filter.'}
                  </p>
                </div>
              )}
            ...
            <p className="mt-3 text-center text-[12px] text-[#52525B] font-bold leading-snug px-2 shrink-0">
              Book direct via WhatsApp · Walk-ins welcome — Kita2u takes no
              commission, you pay your barber directly.
            </p>
          </div>
        </div>
      </div>
...
// ─────────────────────────────────────────────────────────────────────────────
// BarberRowCard — landscape card, 112px tall to match /handyman + /tattoo.
// 88px barber photo left, name + primary service + today's hours middle,
// online dot + credibility pills (per-cut / combo / years) below,
// rating + chevron right.
// ─────────────────────────────────────────────────────────────────────────────
function BarberRowCard({
  provider: p,
  onOpen,
}: {
  provider: BarberProviderPublic
  onOpen: () => void
}) {
  const eff      = effectiveAvailability(p)
  const rating   = (p.rating ?? placeholderRating(p.slug)).toFixed(1)
  const hours    = todayHoursLabel(p.operating_hours) ?? 'Walk-ins welcome'

  const primary       = p.specialties?.[0] ?? null
  const primaryLabel  = primary ? SPECIALTY_LABELS[primary] : null

  // Credibility pills — favour per-cut price (most useful for the
  // barber industry) then combo rate then years of experience.
  const pills: Array<{ key: string; label: string }> = []
  if (p.hourly_rate_idr != null) {
    pills.push({ key: 'cut', label: `Rp ${p.hourly_rate_idr.toLocaleString('id-ID')}/cut` })
  }
  if (p.day_rate_idr != null && pills.length < 2) {
    pills.push({ key: 'combo', label: `Rp ${p.day_rate_idr.toLocaleString('id-ID')}/combo` })
  }
...
// ─────────────────────────────────────────────────────────────────────────────
// Shell — wordmark header matching /handyman + /beautician + /places + /tattoo.
// ─────────────────────────────────────────────────────────────────────────────
