...
import { ALL_SPECIALTIES, SPECIALTY_LABELS, MAX_BARBER_SPECIALTIES, type BarberSpecialty } from '@/lib/barber/types'
    ...
    supabase.auth.getSession().then(async ({ data }) => {
      ...
      try {
        ...
        if (r.ok) {
          ...
          if (j.provider) { setAuth({ status: 'alreadyProvider' }); setTimeout(() => router.replace('/dashboard/barber'), 400); return }
        }
      } catch { /* fall through */ }
  ...
  return (
    <div className="px-4 pt-12 max-w-md mx-auto">
      <div className="text-center mb-8">
        <div className="inline-block bg-brand text-bg text-[11px] font-extrabold uppercase tracking-[0.15em] px-3 py-1 rounded-full mb-4">Barber</div>
        <h1 className="text-[26px] font-black leading-tight mb-3">List as a barber</h1>
        <p className="text-[13px] text-ink/70 leading-relaxed">Create an account first — already have one? Login with the same.</p>
      </div>
      <div className="space-y-3">
        <Link href="/signup?intent=barber&next=/barber/signup" className="block w-full rounded-2xl bg-brand text-bg px-6 py-4 text-center text-[14px] font-extrabold uppercase tracking-wider hover:brightness-105">
          Create new account
        </Link>
        <Link href="/login?next=/barber/signup" className="block w-full rounded-2xl bg-white/[0.06] border border-ink/15 text-ink px-6 py-4 text-center text-[14px] font-extrabold uppercase tracking-wider hover:bg-white/10">
...
function Form({ userId }: { userId: string }) {
  ...
  const [f, setF] = useState({
    ...
    years_experience: 0,
    bio: '',
    specialties: [] as BarberSpecialty[],
    hourly_rate: '',
    day_rate: '',
    has_own_tools: true,
    whatsapp_e164: '',
  ...
  function toggleSpecialty(s: BarberSpecialty) {
    setF((p) => {
      if (p.specialties.includes(s)) {
        return { ...p, specialties: p.specialties.filter((x) => x !== s) }
      }
      if (p.specialties.length >= MAX_BARBER_SPECIALTIES) return p // cap at 3
  ...
  async function submit(e: React.FormEvent) {
    e.preventDefault()
    if (f.specialties.length === 0) { setErr(`Pick min 1, max ${MAX_BARBER_SPECIALTIES} services.`); return }
    const h = f.hourly_rate.trim()
    const d = f.day_rate.trim()
    if (h === '' && d === '') { setErr('Enter at least one rate — per cut or combo.'); return }
    ...
    try {
      ...
      setDone(true); setTimeout(() => router.push('/dashboard/barber'), 1200)
  ...
  return (
    <div className="px-4 pt-6 pb-20 max-w-md mx-auto">
      ...
      <h1 className="text-[26px] font-black leading-tight mb-2">Barber signup</h1>
      <p className="text-[13px] text-ink/70 mb-6">Pick max 3 services. Set your per-cut and/or combo rate. Rp 38,000/month after a 7-day trial.</p>

      <form onSubmit={submit} className="space-y-4">
        <Field label="Barber / shop name *">
          <input required maxLength={80} value={f.display_name} onChange={(e) => upd('display_name', e.target.value)} placeholder="Bagas Barber" className={inputCls} />
        </Field>
        ...
        <Field label="Bio (max 300) *">
          <textarea required maxLength={300} rows={3} value={f.bio} onChange={(e) => upd('bio', e.target.value)} placeholder="Your shop + signature cut + chair location + walk-in/appointment policy." className={inputCls + ' resize-none'} />
        </Field>

        <div className="rounded-2xl bg-black/85 border border-white/10 p-4">
          <div className="flex items-baseline justify-between mb-1">
            <div className="text-[13px] font-extrabold uppercase tracking-wider text-ink">Services *</div>
            <div className="text-[11px] text-ink/55">{f.specialties.length}/{MAX_BARBER_SPECIALTIES}</div>
          </div>
          <p className="text-[11px] text-ink/55 mb-3">Pick max {MAX_BARBER_SPECIALTIES} signature services.</p>
          <div className="flex flex-wrap gap-1.5">
            {ALL_SPECIALTIES.map((s) => {
              const on = f.specialties.includes(s)
              const atLimit = !on && f.specialties.length >= MAX_BARBER_SPECIALTIES
              return (
                <button
                  key={s} type="button"
                  disabled={atLimit}
        ...
        <div className="rounded-2xl bg-black/85 border border-white/10 p-4">
          ...
          <p className="text-[11px] text-ink/55 mb-3">Enter at least one — per-cut flat fee or combo (cut + beard + hot towel).</p>
          <div className="grid grid-cols-2 gap-2">
            <PriceField label="Per cut"        v={f.hourly_rate} set={(v) => upd('hourly_rate', v)} />
            <PriceField label="Combo · cut+beard" v={f.day_rate}    set={(v) => upd('day_rate', v)} />
          </div>
          <label className="flex items-center gap-2 mt-3 text-[12px] text-ink/85 cursor-pointer">
            ...
            <span>I have my own kit (clippers, razor, cape, sanitised setup)</span>
          </label>
        </div>
        ...
        <Field label="Shop area / walk-in notes"><input value={f.service_area_notes} onChange={(e) => upd('service_area_notes', e.target.value)} placeholder="Yogya · Sleman · walk-in barbershop" className={inputCls} /></Field>
        ...
        <button type="submit" disabled={submitting} className="w-full rounded-full bg-brand text-bg px-6 py-3.5 text-[14px] font-extrabold uppercase tracking-wider disabled:opacity-60">
          {submitting ? 'Registering…' : 'Create barber profile'}
        </button>
      </form>
    </div>
  )
...
function humaniseError(code?: string): string {
  switch (code) {
    ...
    case 'at_least_one_specialty':return `Pick min 1, max ${MAX_BARBER_SPECIALTIES} services.`
    case 'at_least_one_price':    return 'Enter at least one rate — per cut or combo.'
    case 'slug_collision':        return 'Name already taken — try a variation.'
    case 'already_registered':    return 'You already have a barber profile.'
