...
export default function HomeCleanProviderPage() {
  ...
  useEffect(() => {
    ...
    const url = `/api/home-clean/${encodeURIComponent(slug)}/public${
      draftPassword ? `?p=${encodeURIComponent(draftPassword)}` : ''
    }`
    fetch(url, { cache: 'no-store' })
      .then((r) => r.ok ? r.json() : null)
      .then((j: { provider?: HomeCleanProviderPublic; locked?: boolean } | null) => {
    ...
    return (
      <Shell>
        <DraftPasswordGate
          ...
          fetchUrl={(pw) => `/api/home-clean/${encodeURIComponent(slug)}/public?p=${encodeURIComponent(pw)}`}
          onUnlocked={(pw, prov) => {
            setDraftPassword(pw)
            setLocked(false)
            setP(prov as HomeCleanProviderPublic)
          }}
        />
      </Shell>
    )
  ...
  return (
    <Shell>
          ...
          <>
            ...
            {Boolean((p as unknown as { contact_form_enabled?: boolean }).contact_form_enabled
              ...
              <ContactFormPanel
                displayName={p.display_name}
                themeColor={theme}
                endpoint="/api/home-clean/contact"
                providerSlug={p.slug}
              />
            )}
            {!p.has_physical_location
      ...
      <Link
        href="/home-clean"
        aria-label="Back to Kita2u cleaners"
