...
// Server-side SEO + lapsed redirect for /home-clean/[slug].
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  if (!p) return { title: 'Cleaner not found', robots: { index: false, follow: false } }
  const hourly = p.hourly_rate_idr ?? null
  const title       = `${p.display_name} · Cleaner ${p.city || 'Indonesia'}`
  const description = (p.bio?.trim())
    || `Pesan ${p.display_name} (cleaner) di ${p.city || 'Indonesia'} via WhatsApp.${hourly ? ` Mulai Rp ${hourly.toLocaleString('id-ID')}/jam.` : ''}`
  const canonical   = `${SITE_URL}/home-clean/${p.slug}`
...
export default async function HomeCleanProviderLayout({
  children, params,
  ...
  if (p) {
    ...
    if (lapsed) {
      const qs = new URLSearchParams({ from: 'lapsed_home_clean', slug: p.slug })
      redirect(`/home-clean?${qs.toString()}`)
    }
  }
  const jsonLd = p
    ? {
        '@context': 'https://schema.org', '@type': 'LocalBusiness',
        '@id': `${SITE_URL}/home-clean/${p.slug}`,
        name: p.display_name, description: p.bio || undefined,
        url: `${SITE_URL}/home-clean/${p.slug}`,
