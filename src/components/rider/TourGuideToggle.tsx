...
export default function TourGuideToggle() {
  ...
  return (
    <div className="space-y-2.5">
      ...
      {enabled && (
        <div
          className="rounded-3xl bg-gray-100 border border-gray-200 shadow-sm p-3 space-y-3"
          style={{ borderColor: 'rgba(250,204,21,0.25)', background: 'rgba(250,204,21,0.04)' }}
        >
          {/* Day rate */}
          <div>
            <label className="text-[11px] uppercase tracking-wider font-extrabold text-gray-600">
              Day rate (8 hours, including fuel)
            </label>
            <div className="mt-1.5 flex items-center gap-2">
              <span className="text-[14px] font-extrabold text-gray-600">Rp</span>
              <input
                type="number"
                inputMode="numeric"
                min={RATE_MIN}
                ...
                onBlur={commitRate}
                className="flex-1 min-w-0 rounded-xl px-3 py-2.5 text-[14px] font-extrabold text-[#0F172A] focus:outline-none focus:ring-2 focus:ring-brand/60 bg-gray-50 border border-gray-200"
                style={{
                  minHeight: 44,
                }}
              />
              ...
              {TOUR_LANGUAGES.map((lang) => {
                ...
                return (
                  <button
                    ...
                    style={{
                      background: active ? 'linear-gradient(135deg, #FACC15, #EAB308)' : '#F3F4F6',
                      color: active ? '#0F172A' : '#4B5563',
                      border: `1px solid ${active ? 'rgba(0,0,0,0.40)' : '#E5E7EB'}`,
                      minHeight: 32,
          ...
          <Link
            href="/places?tour=1"
            target="_blank"
            className="inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg font-extrabold text-[12px] text-[#0F172A] hover:text-brand transition bg-gray-50 border border-gray-200"
            style={{ minHeight: 36 }}
          >
