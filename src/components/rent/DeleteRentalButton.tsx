...
// ============================================================================
// DeleteRentalButton — owner-side delete with confirm step.
// Calls DELETE /api/rentals/[id] (owner-scoped RLS) and refreshes the
// server-rendered list afterwards.
// ============================================================================

export default function DeleteRentalButton({
  rentalId, label,
}: { rentalId: string; label: string }) {
  ...
  if (phase === 'deleting') {
    return (
      <span className="mt-1 inline-flex items-center gap-1 text-[12px] font-extrabold uppercase tracking-wider text-muted">
  ...
  if (phase === 'error') {
    return (
      <span className="mt-1 inline-flex items-center gap-1 text-[12px] font-extrabold text-red-400">
        {err}
  ...
  return (
    <button
      ...
      className="mt-1 inline-flex items-center gap-1 text-[12px] font-extrabold uppercase tracking-wider text-red-400 hover:text-red-300"
    >
