...
// the same chip. Renders only when the user owns >1 video page OR they
...
const ACTIVE_SLUG_KEY = 'video.activeSlug'

export default function VideoPageSwitcher() {
  ...
  const loadPages = useCallback(async () => {
    try {
      const [pagesRes, acctRes] = await Promise.all([
        fetch('/api/video/me/pages', { cache: 'no-store' }),
...
function AddPageModal({
  ...
  async function submit() {
    ...
    setSaving(true)
    try {
      const r = await fetch('/api/video/me/pages', {
        method:  'POST',
