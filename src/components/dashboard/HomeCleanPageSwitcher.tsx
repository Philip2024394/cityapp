...
// the same chip. Renders only when the user owns >1 home-clean page OR they
...
const ACTIVE_SLUG_KEY = 'home-clean.activeSlug'

export default function HomeCleanPageSwitcher() {
  ...
  const loadPages = useCallback(async () => {
    try {
      const [pagesRes, acctRes] = await Promise.all([
        fetch('/api/home-clean/me/pages', { cache: 'no-store' }),
      ...
      {adding && (
        <AddPageModal
          ...
          onCreated={(slug) => {
            try { window.localStorage.setItem(ACTIVE_SLUG_KEY, slug) } catch { /* private mode */ }
            const url = new URL(window.location.href)
            url.pathname = '/dashboard/home-clean'
            url.searchParams.set('slug', slug)
            window.location.assign(url.toString())
          }}
        />
...
function AddPageModal({
  ...
  async function submit() {
    ...
    setSaving(true)
    try {
      const r = await fetch('/api/home-clean/me/pages', {
        method:  'POST',
