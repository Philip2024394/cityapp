...
// the same chip. Renders only when the user owns >1 tattoo page OR they
...
const ACTIVE_SLUG_KEY = 'tattoo.activeSlug'

export default function TattooPageSwitcher() {
  ...
  const loadPages = useCallback(async () => {
    try {
      const [pagesRes, acctRes] = await Promise.all([
        fetch('/api/tattoo/me/pages', { cache: 'no-store' }),
...
function AddPageModal({
  ...
  async function submit() {
    ...
    setSaving(true)
    try {
      const r = await fetch('/api/tattoo/me/pages', {
        method:  'POST',
