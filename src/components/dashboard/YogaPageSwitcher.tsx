...
// the same chip. Renders only when the user owns >1 yoga page OR they
...
const ACTIVE_SLUG_KEY = 'yoga.activeSlug'

export default function YogaPageSwitcher() {
  ...
  const loadPages = useCallback(async () => {
    try {
      const [pagesRes, acctRes] = await Promise.all([
        fetch('/api/yoga/me/pages', { cache: 'no-store' }),
...
function AddPageModal({
  ...
  async function submit() {
    ...
    setSaving(true)
    try {
      const r = await fetch('/api/yoga/me/pages', {
        method:  'POST',
