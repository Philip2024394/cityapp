...
// the same chip. Renders only when the user owns >1 cake page OR they
...
const ACTIVE_SLUG_KEY = 'cake.activeSlug'

export default function CakePageSwitcher() {
  ...
  const loadPages = useCallback(async () => {
    try {
      const [pagesRes, acctRes] = await Promise.all([
        fetch('/api/cake/me/pages', { cache: 'no-store' }),
...
function AddPageModal({
  ...
  async function submit() {
    ...
    setSaving(true)
    try {
      const r = await fetch('/api/cake/me/pages', {
        method:  'POST',
