...
// the same chip. Renders only when the user owns >1 facial page OR they
...
const ACTIVE_SLUG_KEY = 'facial.activeSlug'

export default function FacialPageSwitcher() {
  ...
  const loadPages = useCallback(async () => {
    try {
      const [pagesRes, acctRes] = await Promise.all([
        fetch('/api/facial/me/pages', { cache: 'no-store' }),
...
function AddPageModal({
  ...
  async function submit() {
    ...
    setSaving(true)
    try {
      const r = await fetch('/api/facial/me/pages', {
        method:  'POST',
