...
// ============================================================================
// BeauticianPageSwitcher
// ----------------------------------------------------------------------------
// Studio-tier (Rp 149K/mo) multi-location / agency sub-account switcher.
// Renders ONLY when the signed-in user owns more than one beautician page,
// OR they're on the Studio plan (so the upgrade affordance — "+ Add a new
// page" — is discoverable even when they only have one page so far).
//
// Mounts in /dashboard/beautician/layout.tsx so every beautician sub-page
// gets the chip.
//
// Active page selection is driven by ?slug=<x>:
//   • The chip + dropdown list the user's pages (oldest-first).
//   • Tapping a page sets ?slug=<x>, navigates, and persists to localStorage.
//   • The dashboard reads ?slug ?? localStorage to pick which row to fetch.
// ============================================================================
...
export default function BeauticianPageSwitcher() {
  ...
  // Read URL ?slug= once on mount; fall back to localStorage. We can't
  // useSearchParams() because this component lives in a layout that's
  // rendered as part of an RSC tree — reading window directly stays
  // client-safe and avoids forcing the parent into Suspense gymnastics.
  useEffect(() => {
  ...
  const loadPages = useCallback(async () => {
    try {
      const [pagesRes, acctRes] = await Promise.all([
        fetch('/api/beautician/me/pages', { cache: 'no-store' }),
    ...
    } catch { /* silent — the dashboard renders without the switcher */ }
  ...
  // Mount-gate: hide entirely for the 99% single-page case so the chip
  // doesn't clutter Free/Pro dashboards. Studio sees it always (so they
  // can add pages from day 1) and anyone with >1 page sees it (so they
  // can switch).
      ...
      {adding && (
        <AddPageModal
          ...
          onCreated={(slug) => {
            try { window.localStorage.setItem(ACTIVE_SLUG_KEY, slug) } catch { /* private mode */ }
            const url = new URL(window.location.href)
            url.pathname = '/dashboard/beautician'
            url.searchParams.set('slug', slug)
            window.location.assign(url.toString())
          }}
        />
...
// AddPageModal
// ----------------------------------------------------------------------------
// Tiny modal — just a page-name input. On submit, POSTs to /api/.../pages.
// Renders an upgrade CTA when the API returns 409 page_cap_reached so
// Free/Pro users (who shouldn't see the trigger button but might via API
// race / future deep-link) get a real upgrade path instead of a dead end.
// ============================================================================
...
function AddPageModal({
  ...
  async function submit() {
    ...
    setSaving(true)
    try {
      const r = await fetch('/api/beautician/me/pages', {
        method:  'POST',
  ...
  // Cap-reached upgrade CTA — shown when the API returns 409, or when a
  // Free/Pro user somehow lands here despite the gate. Founder direction
  // 2026-06-09: any wall must show the next step (Studio plan link), never
  // a dead end.
        ...
        {showUpgrade ? (
          ...
          <>
            ...
            <input
              ...
              autoFocus
              placeholder="e.g. Ayu Bali, Ayu Jakarta, …"
