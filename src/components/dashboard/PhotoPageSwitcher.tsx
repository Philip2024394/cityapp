...
// the same chip. Renders only when the user owns >1 photo page OR they
...
const ACTIVE_SLUG_KEY = 'photo.activeSlug'

export default function PhotoPageSwitcher() {
  ...
  const loadPages = useCallback(async () => {
    try {
      const [pagesRes, acctRes] = await Promise.all([
        fetch('/api/photo/me/pages', { cache: 'no-store' }),
...
function AddPageModal({
  ...
  async function submit() {
    ...
    setSaving(true)
    try {
      const r = await fetch('/api/photo/me/pages', {
        method:  'POST',
