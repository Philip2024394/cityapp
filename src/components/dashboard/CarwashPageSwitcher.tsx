...
// the same chip. Renders only when the user owns >1 carwash page OR they
...
const ACTIVE_SLUG_KEY = 'carwash.activeSlug'

export default function CarwashPageSwitcher() {
  ...
  const loadPages = useCallback(async () => {
    try {
      const [pagesRes, acctRes] = await Promise.all([
        fetch('/api/carwash/me/pages', { cache: 'no-store' }),
      ...
      {adding && (
        <AddPageModal
          ...
          onCreated={(slug) => {
            try { window.localStorage.setItem(ACTIVE_SLUG_KEY, slug) } catch { /* private mode */ }
            const url = new URL(window.location.href)
            url.pathname = '/dashboard/car-wash'
            url.searchParams.set('slug', slug)
            window.location.assign(url.toString())
          }}
        />
...
function AddPageModal({
  ...
  async function submit() {
    ...
    setSaving(true)
    try {
      const r = await fetch('/api/carwash/me/pages', {
        method:  'POST',
