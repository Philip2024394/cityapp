...
// the same chip. Renders only when the user owns >1 fitness page OR they
...
const ACTIVE_SLUG_KEY = 'fitness.activeSlug'

export default function FitnessPageSwitcher() {
  ...
  const loadPages = useCallback(async () => {
    try {
      const [pagesRes, acctRes] = await Promise.all([
        fetch('/api/fitness/me/pages', { cache: 'no-store' }),
...
function AddPageModal({
  ...
  async function submit() {
    ...
    setSaving(true)
    try {
      const r = await fetch('/api/fitness/me/pages', {
        method:  'POST',
