...
// the same chip. Renders only when the user owns >1 parcel page OR they
...
const ACTIVE_SLUG_KEY = 'parcel.activeSlug'

export default function ParcelPageSwitcher() {
  ...
  const loadPages = useCallback(async () => {
    try {
      const [pagesRes, acctRes] = await Promise.all([
        fetch('/api/parcel/me/pages', { cache: 'no-store' }),
...
function AddPageModal({
  ...
  async function submit() {
    ...
    setSaving(true)
    try {
      const r = await fetch('/api/parcel/me/pages', {
        method:  'POST',
