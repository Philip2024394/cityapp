...
// the same chip. Renders only when the user owns >1 tailor page OR they
...
const ACTIVE_SLUG_KEY = 'tailor.activeSlug'

export default function TailorPageSwitcher() {
  ...
  const loadPages = useCallback(async () => {
    try {
      const [pagesRes, acctRes] = await Promise.all([
        fetch('/api/tailor/me/pages', { cache: 'no-store' }),
...
function AddPageModal({
  ...
  async function submit() {
    ...
    setSaving(true)
    try {
      const r = await fetch('/api/tailor/me/pages', {
        method:  'POST',
