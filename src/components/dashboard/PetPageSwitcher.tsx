...
// the same chip. Renders only when the user owns >1 pet page OR they
...
const ACTIVE_SLUG_KEY = 'pet.activeSlug'

export default function PetPageSwitcher() {
  ...
  const loadPages = useCallback(async () => {
    try {
      const [pagesRes, acctRes] = await Promise.all([
        fetch('/api/pet/me/pages', { cache: 'no-store' }),
...
function AddPageModal({
  ...
  async function submit() {
    ...
    setSaving(true)
    try {
      const r = await fetch('/api/pet/me/pages', {
        method:  'POST',
