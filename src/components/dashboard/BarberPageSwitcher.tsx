...
// the same chip. Renders only when the user owns >1 barber page OR they
...
const ACTIVE_SLUG_KEY = 'barber.activeSlug'

export default function BarberPageSwitcher() {
  ...
  const loadPages = useCallback(async () => {
    try {
      const [pagesRes, acctRes] = await Promise.all([
        fetch('/api/barber/me/pages', { cache: 'no-store' }),
...
function AddPageModal({
  ...
  async function submit() {
    ...
    setSaving(true)
    try {
      const r = await fetch('/api/barber/me/pages', {
        method:  'POST',
