...
// the same chip. Renders only when the user owns >1 mover page OR they
...
const ACTIVE_SLUG_KEY = 'mover.activeSlug'

export default function MoverPageSwitcher() {
  ...
  const loadPages = useCallback(async () => {
    try {
      const [pagesRes, acctRes] = await Promise.all([
        fetch('/api/mover/me/pages', { cache: 'no-store' }),
...
function AddPageModal({
  ...
  async function submit() {
    ...
    setSaving(true)
    try {
      const r = await fetch('/api/mover/me/pages', {
        method:  'POST',
